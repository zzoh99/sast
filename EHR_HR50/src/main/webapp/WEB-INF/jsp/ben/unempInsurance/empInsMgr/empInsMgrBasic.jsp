...
$(function() {
	...
	initdata.Cols = [
		...
		{Header:"<sht:txt mid='mon1V3' mdef='보수월액'/>",		Type:"Int",			Hidden:0,		Width:100,			Align:"Right",	ColMerge:0,	SaveName:"rewardTotMon",KeyField:1,	Format:"Integer",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
...
function doAction1(sAction) {
	switch (sAction) {
		case "Search":
			...
			// 기본사항조회
			var basicInfo = ajaxCall("${ctx}/EmpInsMgr.do?cmd=getEmpInsMgrBasicMap", $("#sheet1Form").serialize(), false);
			...
			// 변동내역 조회
			sheet1.DoSearch("${ctx}/EmpInsMgr.do?cmd=getEmpInsMgrChangeList", $("#sheet1Form").serialize());
			break;
		...
		case "SaveBasicInfo":
			...
			if (confirm("저장하시겠습니까?")) {
				...
				var result = ajaxCall("${ctx}/EmpInsMgr.do?cmd=saveEmpInsMgrBasic", $("#sheet1Form").serialize(), false);
		...
		case "Save":
			...
			sheet1.DoSave("${ctx}/EmpInsMgr.do?cmd=saveEmpInsMgrChange", $("#sheet1Form").serialize());
			break;
...
function sheet1_OnChange(Row, Col, Value) {
	 try{
		...
		if(sSaveName == "rewardTotMon"){
			...
			sheet1.SetCellValue(Row, "empMon", empMon) ;
...
<div class="wrapper">
	...
	<form id="sheet1Form" name="sheet1Form" >
		...
		<tr>
			...
			<td> <input type="text" id="acqYmd" name="acqYmd" class="date2" value="" />
