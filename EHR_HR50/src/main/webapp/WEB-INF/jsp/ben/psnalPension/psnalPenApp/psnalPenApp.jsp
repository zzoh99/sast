...
<!DOCTYPE html> <html class="hidden"><head> <title>개인연금신청</title>
...
var pGubun = "";
	...
	function init_sheet(){
		...
		initdata1.Cols = [
			...
			{Header:"삭제",		Type:"${sDelTy}",	Hidden:1,    Width:"${sDelWdt}",	Align:"Center",	SaveName:"sDelete",	Sort:0 },
			{Header:"상태",		Type:"${sSttTy}",	Hidden:1,    Width:"${sSttWdt}",	Align:"Center",	SaveName:"sStatus",	Sort:0 },
			{Header:"삭제",		Type:"Html",		Hidden:0,	 Width:45,              Align:"Center",	SaveName:"btnDel",  Sort:0 },
			...
			{Header:"가입상품",	Type:"Combo",	Hidden:0,	Width:80,	Align:"Center",	SaveName:"pensCd",		Edit:0 },
			{Header:"회사지원금",	Type:"Int",		Hidden:0,	Width:80,	Align:"Right",	SaveName:"compMon",		Format:"#,###\\원",	Edit:0 },
			{Header:"개인부담금",	Type:"Int",		Hidden:0,	Width:80,	Align:"Right",	SaveName:"psnlMon",		Format:"#,###\\원",	Edit:0 },
			{Header:"계",		Type:"Int",		Hidden:0,	Width:80,	Align:"Right",	SaveName:"totMon",		Format:"#,###\\원",	Edit:0 },
			{Header:"적용시작년월",	Type:"Date",	Hidden:0,   Width:80,	Align:"Center", SaveName:"payYm",		Format:"Ym",		Edit:0 },
		...
		sheet1.SetColProperty("pensCd", 		{ComboText:"|"+codeLists["B65110"][0], ComboCode:"|"+codeLists["B65110"][1]} );
		//==============================================================================================================================
	...
	function doAction1(sAction) {
		switch (sAction) {
			case "Search":
				// 개인연금가입현황 조회
				var data = ajaxCall( "${ctx}/PsnalPenApp.do?cmd=getPsnalPenAppMap", $("#sheet1Form").serialize(),false);
				if ( data != null && data.DATA != null ){ 
					if( data.DATA.pensNm != "" ){
						$("#td01").html(data.DATA.pensNm);
						$("#td02").html(makeComma(data.DATA.compMon));
						$("#td03").html(makeComma(data.DATA.psnlMon));
						$("#td04").html(makeComma(data.DATA.totMon));
					}else{
						$("#td01").html("");
						$("#td02").html("");
						$("#td03").html("");
					...
					if( data.DATA.compMon == "" && data.DATA.appYn != "Y" ){
						$("#appYn").val("N");
						$("#td01").html("지원 대상자가 아닙니다.");
					}else{
						$("#appYn").val("Y");
					}
				}
				var sXml = sheet1.GetSearchData("${ctx}/PsnalPenApp.do?cmd=getPsnalPenAppList", $("#sheet1Form").serialize() );
	        ...
	        case "Save": //임시저장의 경우 삭제처리.      
	        	...
	        	sheet1.DoSave( "${ctx}/PsnalPenApp.do?cmd=deletePsnalPenApp", $("#sheet1Form").serialize(), -1, 0); 
	        	break;
...
<div class="wrapper">
	...
	<form id="sheet1Form" name="sheet1Form" >
		...
		<input type="hidden" id="appYn"		  name="appYn" 		 value="Y"/>
	</form>
	
	<div class="outer">
		<div class="sheet_title">
			<ul>
				<li class="txt">개인연금 가입현황</li>
				<li class="btn">&nbsp;</li>
			</ul>
		</div>
		<table class="table">
