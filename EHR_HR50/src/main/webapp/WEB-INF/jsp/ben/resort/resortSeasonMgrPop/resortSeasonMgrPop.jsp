...
<script type="text/javascript">
	...
	$(function() {
		...
		//리조트 선택시
		$('#sheet2Form').on('change', 'select[name="companyCd"]',function() {
			//리조트 회사 > 이름 콤보
			var param = "";
		...
		//지점 선택시
		$('#sheet2Form').on('change', 'select[name="resortNm"]',function() {
			var param = "";
		...
		//사용기간 선택시
		$('#sheet2Form').on('change', 'select[name="resortSeq"]',function() {
	...
	function init_sheet2(){
		...
		var statusCdList = convCode(codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList", "B49520"), "");
 		sheet2.SetColProperty("statusCd", {ComboText:"|예약완료", ComboCode:"|Y"} ); //리조트명
	}
	...
	function doAction1(sAction) {
		switch (sAction) {
			case "Search":
				sheet2.DoSearch( "${ctx}/ResortSeasonMgrPop.do?cmd=getResortSeasonMgrPopAprList", $("#sheet2Form").serialize() );
				break;
			case "Save":
				
				if (confirm("예약상태를 변경 하시겠습니까?")) {
					//아이비시트에서 예약완료 값 가져오기
					for(var forRow=1; forRow <= sheet2.LastRow(); forRow++){
						if (sheet2.GetCellValue(forRow, "statusCd") == 'Y') {
							$("#applSeq").val(sheet2.GetCellValue(forRow, "applSeq"));
							break; 
						}
					}
					
					var rtn = ajaxCall( "${ctx}/ResortSeasonMgrPop.do?cmd=saveResortSeasonMgrPop", $("#sheet2Form").serialize(),false);
	...
	function sheet2_OnChange(Row, Col, Value) {
		 try{
			// sheet2.GetSelectRow() => 최초 이벤트 발생 Row
			if ( Row > 0 && sheet2.ColSaveName(Col) == "statusCd" && sheet2.GetCellValue(Row, "statusCd") == 'Y' && Row == sheet2.GetSelectRow() ) {
				for(var forRow=1; forRow <= sheet2.LastRow(); forRow++){
					if (forRow == Row) { continue; }
					sheet2.SetCellValue(forRow, "statusCd", "");
				}
			}
	...
	function getResortSeasonMgrPopRs() {
		...
		var rvData = ajaxCall( "${ctx}/ResortSeasonMgrPop.do?cmd=getResortSeasonMgrPopRs", $("#sheet2Form").serialize(),false);
