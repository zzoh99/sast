...
$(function() {
	...
	if(allFlag) {
		benefitBizCdList = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList",url,false).codeList, ("${ssnLocaleCd}" != "en_US" ? "전체" : "All"));	
	} else {
		benefitBizCdList = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList",url,false).codeList, "");
	}
...
function doAction1(sAction) {
	switch (sAction) {
		case "Search":
			sheet1.DoSearch("${ctx}/WelfareHisResultMgr.do?cmd=getWelfareHisResult2MgrList", $("#sheet1Form").serialize());
			break;
