...
var schStdData       = null;
	...
	$(function() {
		...
		// 학자금 기준 데이터 조회
		var stdList = ajaxCall( "${ctx}/SchApp.do?cmd=getSchAppDetStdDataList", "&searchApplSabun=" + searchApplSabun, false);
		if( stdList != null && stdList != undefined && stdList.DATA != null && stdList.DATA != undefined && stdList.DATA.length > 0 ) {
			if( schStdData == null ) schStdData = {};
			var item = null;
			for(row in stdList.DATA) {
				item = stdList.DATA[row];
				if( schStdData[item.schCd] == null || schStdData[item.schCd] == undefined ) {
					schStdData[item.schCd] = item;
				}
			}
		}
		...
		var schTypeCdList = convCode(ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList", "queryId=getSchAppDetTypeList", false).codeList, " ");//학자금구분(B60050)
		...
		if(authPg == "A") {
			...
			// 학자금지원구분 선택 이벤트
			$("#schSupTypeCd").change(function() {
				// 가족 콤보 목록 조회
				var param = "&searchApplSabun="+searchApplSabun + "&schTypeCd="+$("#schTypeCd").val() + "&schSupTypeCd="+$("#schSupTypeCd").val();
				var famList = convCodeCols(ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList", "queryId=getSchAppDetFamList"+param, false).codeList
				            , "famNm,famCd,famYmd,sexType,famCdNm"
				            , " ");
	...
	function init_sheet(){
		...
		initdata1.Cols = [
			...
			{Header:"<sht:txt mid='schTypeCd' mdef='학자금구분'/>",		Type:"Combo",		Hidden:1,	Width:60,	Align:"Center",	ColMerge:0,	SaveName:"schTypeCd",		Edit:0 },
			{Header:"<sht:txt mid='schSupTypeCd' mdef='학자금지원구분'/>",	Type:"Combo",		Hidden:1,	Width:60,	Align:"Center",	ColMerge:0,	SaveName:"schSupTypeCd",	Edit:0 },
			{Header:"<sht:txt mid='famCdV4' mdef='가족구분'/>",		Type:"Combo",		Hidden:1,	Width:60,	Align:"Center",	ColMerge:0,	SaveName:"famCd",			Edit:0 },
			{Header:"<sht:txt mid='empname' mdef='대상자명'/>",		Type:"Text",		Hidden:1,	Width:60,	Align:"Center",	ColMerge:0,	SaveName:"famNm",			Edit:0 },
			{Header:"<sht:txt mid='birYmd' mdef='생년월일'/>",		Type:"Date",		Hidden:1,	Width:70,	Align:"Center",	ColMerge:0,	SaveName:"famYmd",			Format:"Ymd", 	Edit:0 },
			{Header:"<sht:txt mid='appYear' mdef='신청년도'/>",		Type:"Text",		Hidden:0,	Width:70,	Align:"Center",	ColMerge:0,	SaveName:"appYear",			Edit:0 },
			{Header:"<sht:txt mid='divCd' mdef='신청분기'/>",		Type:"Text",		Hidden:0,	Width:70,	Align:"Center",	ColMerge:0,	SaveName:"divCd",			Format:"###\\분기", 	Edit:0 },
			{Header:"<sht:txt mid='acaSchNm' mdef='학교명'/>",			Type:"Text",		Hidden:0,	Width:150,	Align:"Left",	ColMerge:0,	SaveName:"schName",			Edit:0 },
			{Header:"<sht:txt mid='applMon' mdef='신청금액'/>",		Type:"Int",			Hidden:0,	Width:80,	Align:"Center",	ColMerge:0,	SaveName:"applMon",			Format:"#,###\\원",	Edit:0 },
			{Header:"<sht:txt mid='payMonV1' mdef='지급금액'/>",		Type:"AutoSum",		Hidden:0,	Width:80,	Align:"Center",	ColMerge:0,	SaveName:"payMon",			Format:"#,###\\원",	Edit:0 },
			{Header:"<sht:txt mid='payYmV2' 			mdef='급여년월'/>",		Type:"Date",		Hidden:0,	Width:80,	Align:"Center",	ColMerge:0,	SaveName:"payYm",			Format:"Ym", 	Edit:0 },
		...
		sheet1.SetDataAlternateBackColor(sheet1.GetDataBackColor()); //홀짝 배경색 같게
 		...
 		//==============================================================================================================================
		//공통코드 한번에 조회
		var grpCds = "R10010,B60050,B60051,B60030";
	...
	function doAction(sAction) {
		switch (sAction) {
			case "Search":
				// 입력 폼 값 셋팅
				var data = ajaxCall( "${ctx}/SchApp.do?cmd=getSchAppDetMap", $("#searchForm").serialize(),false);
				...
				if ( data != null && data.DATA != null ){ 
					...
					if(authPg == "A") {
						...
						setSchSupTypeCdCombo(data.DATA.schTypeCd);
						$("#schSupTypeCd").val(data.DATA.schSupTypeCd);
			...
			case "SearchList":
			    ...
			    var params = "searchApplSabun="+$("#searchApplSabun").val()
			               ...
			               + "&famNm="+encodeURIComponent($("#famNm").val())
			               + "&famYmd="+$("#famYmd").val();
			       
				var sXml = sheet1.GetSearchData("${ctx}/SchApp.do?cmd=getSchAppDetList", params);
	...
	function checkList(status) {
		...
		if( ch ){
			...
			//학자금 체크 
			var map = ajaxCall( "${ctx}/SchApp.do?cmd=getSchAppDupChk",params,false);
	...
	function setValue(status) {
		...
		try {
			...
			if( adminRecevYn == "Y" ){
				...
				var rtn = ajaxCall("${ctx}/SchApp.do?cmd=saveSchAppDetAdmin", $("#searchForm").serialize(), false);
		        ...
		        if( !$("#schPayYn").is(":checked") ){
		      	...
		      	//저장
				var data = ajaxCall("${ctx}/SchApp.do?cmd=saveSchAppDet", $("#searchForm").serialize(), false);
	...
	// 학자금지원구분 코드 콤보 셋팅
	function setSchSupTypeCdCombo(schTypeCd) {
		var schSupTypeList = convCode(ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList", "queryId=getSchAppDetSupTypeList" + "&schTypeCd=" + schTypeCd, false).codeList, " ");//학자금지원구분(B60051)
		$("#schSupTypeCd").html(schSupTypeList[2]);
	}
	
	function setSchStdData() {
		// 학자금 기준 데이터 조회
		var schStd = schStdData[ $("#schTypeCd").val() + "_" + $("#schSupTypeCd").val() + "_" + $("#famCd").val() ];
		if( schStd != null && schStd != undefined ) {
			$("#applMonLimit").val(schStd.applMon);
			$("#applMonLimitTxt").html(makeComma(schStd.applMon) + "원");
			$("#evidenceDocTxt").html(schStd.evidenceDoc);
		}
	}
</script>
...
<div class="wrapper">
	...
	<input type="hidden" id="applMonLimit"		name="applMonLimit"	 value=""/>
	...
	<table class="table">
		...
		<tr>
			<th><tit:txt mid='schTypeCd' mdef='학자금구분'/></th>
			<td>
				<select id="schTypeCd" name="schTypeCd" class="${selectCss} ${required} w100" ${disabled}></select>
				<select id="schSupTypeCd" name="schSupTypeCd" class="${selectCss} ${required} w100" ${disabled}></select>
			</td>
		...
		<tr>
			...
			<td>
				<input type="text" id="appYear" name="appYear" class="${textCss} w20p" readonly maxlength="20"/>
			</td> 
			<th><tit:txt mid='divCd' mdef='신청분기'/></th>
			<td>
		...
		<tr>
			<th><tit:txt mid='schLocCd' mdef='국내/국외'/></th>
			<td>
		...
		<div class="sheet_title">
			<ul>
				<li class="txt"><tit:txt mid='schAppHis' mdef='학자금 신청 이력'/></li>
			</ul>
		</div>
		...
		<table class="table payInfo">
			...
			<tr>
				...
				<th><tit:txt mid='payYm' mdef='지급년월'/></th>
