...
$(function() {
	...
	initdata.Cols = [
		...
		{Header:"<sht:txt mid='mon1' mdef='기준소득월액'/>",	Type:"Int",			Hidden:0,					Width:100,			Align:"Right",	ColMerge:0,	SaveName:"rewardTotMon",KeyField:1,	Format:"Integer",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
	...
	$("#retireRemainMon").bind("keyup",function(event){
		makeNumber(this, 'A');
		Num_Comma(this);
	});
...
function doAction1(sAction) {
	switch (sAction) {
		case "Search":
			...
			// 기본사항조회
			var basicInfo = ajaxCall("${ctx}/StaPenMgr.do?cmd=getStaPenMgrBasicMap", $("#sheet1Form").serialize(), false);

			$(':radio[name="retireRemainYn"]').eq(0).attr("checked", false);
			$(':radio[name="retireRemainYn"]').eq(1).attr("checked", false);
			$("#retireRemainMon").val("");
			$("#acqYmd"			).val("");
			$("#lossYmd"		).val("");
			...
			if(basicInfo.Map != null) {
				...
				if (basicInfo.retireRemainYn == "Y") {
					$(':radio[name="retireRemainYn"]').eq(0).attr("checked", "checked");
				} else {
					$(':radio[name="retireRemainYn"]').eq(1).attr("checked", "checked");
				}
				$("#retireRemainMon").val(basicInfo.retireRemainMon);
				$("#acqYmd"		 	).val(basicInfo.acqYmd);
				$("#lossYmd"		).val(basicInfo.lossYmd);
			}

			// 변동내역 조회
			sheet1.DoSearch("${ctx}/StaPenMgr.do?cmd=getStaPenMgrChangeList", $("#sheet1Form").serialize());
			break;
		...
		case "SaveBasicInfo":
			...
			if (confirm("저장하시겠습니까?")) {
				...
				var result = ajaxCall("${ctx}/StaPenMgr.do?cmd=saveStaPenMgrBasic", $("#sheet1Form").serialize(), false);
		...
		case "Save":
			...
			sheet1.DoSave("${ctx}/StaPenMgr.do?cmd=saveStaPenMgrChange", $("#sheet1Form").serialize());
			break;
		...
		case "Copy":
			...
			sheet1.SetCellValue(Row, "selfMon", "");
			break;
...
function sheet1_OnChange(Row, Col, Value) {
	 try{
		...
		if(sSaveName == "rewardTotMon"){
			...
			var params = "&searchYmd="+sheet1.GetCellValue(Row, "sdate") +
						"&searchRewardTotMon="+sheet1.GetCellValue(Row, "rewardTotMon") ;
			...
			sheet1.SetCellValue(Row, "selfMon", selfMon) ;
...
<div class="wrapper">
	...
	<form id="sheet1Form" name="sheet1Form" >
		...
		<tr>
			<th><tit:txt mid='112406' mdef='퇴직전환금'/></th>
			<td> <input type="text" id="retireRemainMon" name="retireRemainMon" class="text right date" value="" style="width:92px" />
				 <input type="hidden" id="sabun" name="sabun" class="text" value=""/> </td>
			<th><tit:txt mid='112062' mdef='퇴직전환금공제여부'/></th>
			<td> <input type="radio" id="retireRemainYn" name="retireRemainYn" value="Y" /> 예
				 <input type="radio" id="retireRemainYn" name="retireRemainYn" value="N" /> 아니오 </td>
		</tr>
		<tr>
