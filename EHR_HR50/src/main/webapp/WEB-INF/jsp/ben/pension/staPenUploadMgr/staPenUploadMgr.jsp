...
<!DOCTYPE html> <html class="hidden"> <head> <title>국민연금등급변경관리</title>
...
$(function() {
	...
	initdata1.Cols = [
		...
		{Header:"<sht:txt mid='mon2' mdef='본인부담금'/>",		Type:"Int",			Hidden:0,					Width:100,			Align:"Right",	ColMerge:0,	SaveName:"mon2",		KeyField:0,	Format:"Integer",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 }
...
function doAction1(sAction) {
	switch (sAction) {
		case "Search":
			...
			sheet1.DoSearch("${ctx}/StaPenUploadMgr.do?cmd=getStaPenUploadMgrList", $("#sheet1Form").serialize());
			break;
		...
		case "Save":
			...
			sheet1.DoSave("${ctx}/StaPenUploadMgr.do?cmd=saveStaPenUploadMgr", $("#sheet1Form").serialize());
			break;

		case "Insert":
			var Row = sheet1.DataInsert(0);
			// 복리후생업무구분코드(B10230)
			sheet1.SetCellValue(Row, "benefitBizCd", "10");
		...
		case "PrcP_BEN_NP_UPD":
			...
			if (confirm("국민연금변동이력에 반영하시겠습니까?")) {
				var prcSdate = $("#sdate").val().replaceAll("-","");
				

				// 반영작업(국민연금변동이력)
				var result = ajaxCall("${ctx}/StaPenUploadMgr.do?cmd=prcP_BEN_NP_UPD", "sdate="+prcSdate, false);

				if (result != null && result["Result"] != null && result["Result"]["Code"] != null) {
					if (result["Result"]["Code"] == "0") {
						alert("<msg:txt mid='alertStaPenUpload4' mdef='국민연금변동이력 반영 되었습니다.'/>");
						// 프로시저 호출 후 재조회
						doAction1("Search");
					...
					alert("<msg:txt mid='alertStaPenUpload5' mdef='국민연금변동이력 반영 오류입니다.'/>");
				}
			}
			break;
		...
		case "DownTemplate":
			// 양식다운로드
			sheet1.Down2Excel({ SheetDesign:1, Merge:1, DownRows:"0", DownCols:"10|15|16|17"});
			break;
	}
}
	...
	if(result) {
		...
		if ( sheet1.RowCount() > 0 ){
			...
			for(var i = sheet1.HeaderRows(); i<sheet1.RowCount()+sheet1.HeaderRows(); i++){
				...
				sheet1.SetCellValue(i, "benefitBizCd", "10");
				sheet1.SetCellValue(i, "mon2", selfMon) ;
...
function sheet1_OnChange(Row, Col, Value) {
	 try{
		...
		if(sSaveName == "mon1"){
			...
			sheet1.SetCellValue(Row, "mon2", selfMon) ;
			...
			<td>
				<div class="inner">
					<div class="sheet_title">
						<ul>
							<li id="txt" class="txt">국민연금등급변경관리</li>
							<li class="btn">
