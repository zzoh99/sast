...
<!DOCTYPE html> <html class="hidden"><head> <title>동호회 재신청</title>
...
var pGubun = "";
	...
	function init_sheet1(){
		...
		initdata1.Cols = [
			...
			{Header:"신청구분|신청구분",	Type:"Combo", Hidden:0,	Width:65, Align:"Center", ColMerge:1, SaveName:"gubunCd",		Format:"",		UpdateEdit:0, InsertEdit:0 },
			...
			{Header:"고문|고문", 			Type:"Text",  Hidden:1, Width:80, Align:"Center", ColMerge:1, SaveName:"sabunBName", 	Format:"", 		UpdateEdit:0, InsertEdit:0 },
		...
		sheet1.SetDataLinkMouse("detail",1);
 		...
 		sheet1.SetColProperty("gubunCd",  		{ComboText:"|"+codeLists["B50510"][0], ComboCode:"|"+codeLists["B50510"][1]} );
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search":
			var sXml = sheet1.GetSearchData("${ctx}/ClubrefApp.do?cmd=getClubrefAppList", $("#sheet1Form").serialize() );
			sheet1.LoadSearchData(sXml );
			break;
		case "Save":
			if( !confirm("삭제하시겠습니까?")) { initDelStatus(sheet1); return;}
			IBS_SaveName(document.sheet1Form,sheet1);
			sheet1.DoSave( "${ctx}/ClubrefApp.do?cmd=deleteClubrefApp", $("#sheet1Form").serialize(), -1, 0);
			break;
		case "Down2Excel":
	...
	function showApplPopup(Row) {
		...
		if( auth == "A" ){
			var data = ajaxCall( "${ctx}/ClubrefApp.do?cmd=getClubrefAppDateChk", $("#searchForm").serialize(),false);
			if ( data != null && data.DATA != null && data.DATA.appDateYn != null && data.DATA.appDateYn == 'N'){
				alert("동호회 (재)등록 신청기간이 아닙니다.");
				return;
			}
		}
		var p = {
