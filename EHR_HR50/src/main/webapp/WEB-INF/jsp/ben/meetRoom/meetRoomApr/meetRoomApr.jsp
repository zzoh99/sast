...
//			{Header:"삭제",		Type:"${sDelTy}",	Hidden:Number("${sDelHdn}"),Width:"${sDelWdt}",	Align:"Center",	ColMerge:0,	SaveName:"sDelete",	Sort:0 },
			...
			{Header:"신청자|직급",	Type:"Text",	Hidden:1, 		Width:80,	Align:"Center",	ColMerge:1,	SaveName:"applInJikgubNm",	KeyField:0,	CalcLogic:"",	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:50 },
      		...
      		{Header:"회의실|회의실",		Type:"Combo",	Hidden:0,	Width:100,	Align:"Center",	ColMerge:1,	SaveName:"meetRoomNo",	KeyField:0,	CalcLogic:"",	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:20 },
		...
		var empGubun = [{"code": "1", "codeNm": "일반직"}, {"code": "2", "codeNm": "생산직"}];
 		...
 		$("#searchEmpGubun").html(empGubun[2]).on("change", function() {
 			doAction1("Search");
 		});
		...
		// 회의실명
		var meetRoomNo = convCode( ajaxCall("${ctx}/MeetRoomApp.do?cmd=getMeetRoomCdList", "useYn=Y", false).DATA, "전체");
		sheet1.SetColProperty("meetRoomNo",	{ComboText:"|"+meetRoomNo[0], ComboCode:"|"+meetRoomNo[1]} );
		$("#searchMeetRoomNo").html(meetRoomNo[2]);

		$("#searchOrgNm, #searchSabunName, #searchFromYmd, #searchToYmd").bind("keyup", function(event){
			if( event.keyCode == 13){ doAction1("Search"); $(this).focus(); }
		});

		$("#searchApplStatusCd, #searchMeetRoomNo").bind("change", function(e) {
			doAction1("Search");
		});
	...
	function doAction1(sAction) {
		...
		case "Search":
			...
			sheet1.DoSearch( "${ctx}/MeetRoomApr.do?cmd=getMeetRoomAprList", $("#sheetForm").serialize() ); break;
		case "Save":
		    if(!dupChk(sheet1,"sabun|applSeq", true, true)){break;}
			IBS_SaveName(document.sheetForm,sheet1);
			sheet1.DoSave( "${ctx}/MeetRoomApr.do?cmd=saveMeetRoomApr", $("#sheetForm").serialize()); break;
        case "Down2Excel":
			...
			<div>
				<table>
					...
					<tr>
						<th>신청일자</th>
						<td colspan="2">
							<input type="text" id="searchFromYmd" name="searchFromYmd" class="date2" value="<%=DateUtil.getCurrentTime("yyyy-MM")%>-01">&nbsp;~&nbsp;
							<input type="text" id="searchToYmd" name="searchToYmd" class="date2" value="<%=DateUtil.getCurrentTime("yyyy-MM-dd")%>">
						</td>
						<th>회의실명</th>
						<td>
							<select id="searchMeetRoomNo" name="searchMeetRoomNo" class="box"></select>
						</td>
						<td>
