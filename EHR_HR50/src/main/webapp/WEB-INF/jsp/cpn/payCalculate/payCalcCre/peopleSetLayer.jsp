...
$(function() {
	...
	createIBSheet3(document.getElementById('sheet1-wrap'), "sheet1", "100%", "100%", "kr");

	const modal = window.top.document.LayerModalUtility.getModal('peopleSetLayer');
	...
	let businessPlaceNm	= modal.parameters.businessPlaceNm || '';
	let closeYn			= modal.parameters.closeYn || '';
...
function peopleCnt(){
	...
	if (peopleInfo != null) {
		const modal = window.top.document.LayerModalUtility.getModal('peopleSetLayer');
		modal.fire('peopleSetTrigger', {
			peopleTotCnt : peopleInfo.peopleTotCnt
			, peopleSubCnt : peopleInfo.peopleSubCnt
			, peopleJCnt : peopleInfo.peopleJCnt
		}).hide();
		...
		, trigger :[
			{
				...
				, callback : function(result){
					...
					sheet1.SetCellValue(Row, "jikchakCd", result.jikchakCd);
					sheet1.SetCellValue(Row, "jikweeCd", result.jikweeCd);
					sheet1.SetCellValue(Row, "jikgubCd", result.jikgubCd);
					sheet1.SetCellValue(Row, "manageCd", result.manageCd);
					sheet1.SetCellValue(Row, "statusCd", result.statusCd);
					sheet1.SetCellValue(Row, "empYmd", result.empYmd);
					sheet1.SetCellValue(Row, "gempYmd", result.gempYmd);
					sheet1.SetCellValue(Row, "resNo", result.resNo);
					sheet1.SetCellValue(Row, "businessPlaceCd",result. businessPlaceCd);
					sheet1.SetCellValue(Row, "ccCd", result.ccCd);
				}
			}
		]
	});
...
<%--					<script type="text/javascript">createIBSheet("sheet1", "100%", "100%", "kr"); </script>--%>
			...
			<div class="popup_button outer">
				<ul>
					<li>
						<a href="javascript:closeCommonLayer('peopleSetLayer');" class="gray large">닫기</a>
					</li>
				</ul>
			</div>
		</div>
