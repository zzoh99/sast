...
<%--	var p = eval("${popUpStatus}");--%>
	$(function(){
		...
		const sheetHeight = $('.modal_body').height() - $('#mySheetForm').height() - $('.sheet_title').outerHeight(true);
	...
	function setElementData(adjCd, adjNm){
		const modal = window.top.document.LayerModalUtility.getModal('viewDedEleLayer');
		modal.fire('dedTrigger', {adjElementCd : adjCd, adjElementNm : adjNm}).hide();
	}
</script>
...
<%--		</div>--%>
		...
		<div class="modal_footer">
			<btn:a href="javascript:closeCommonLayer('viewDedEleLayer');" css="btn outline_gray" mid='110881' mdef="닫기"/>
		</div>
	</div>
</body>
</html>
