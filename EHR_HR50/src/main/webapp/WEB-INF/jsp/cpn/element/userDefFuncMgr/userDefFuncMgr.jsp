...
var gridFirstOnClickUdfCd = "";
	...
	$(function() {
		...
		initdata0.Cols = [
	                  ...
	                  {Header:"<sht:txt mid='udfCd' mdef='함수코드'/>",         Type:"Text",      Hidden:0,  Width:150,    Align:"Left",    ColMerge:1,   SaveName:"udfCd",         KeyField:1,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:150 },
	                  {Header:"<sht:txt mid='udfNm' mdef='함수명'/>",           Type:"Text",      Hidden:0,  Width:200,  Align:"Left",    ColMerge:0,   SaveName:"udfNm",         KeyField:1,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:100 },
	                  {Header:"<sht:txt mid='udfName' mdef='짧은설명'/>",         Type:"Text",      Hidden:0,  Width:100,  Align:"Left",    ColMerge:0,   SaveName:"udfName",       KeyField:1,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:200 },
	                  ...
	                  {Header:"<sht:txt mid='udfGubunCd' mdef='OBJECT구분'/>",       Type:"Combo",     Hidden:0,  Width:80,   Align:"Center",  ColMerge:0,   SaveName:"udfGubunCd",    KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:1 },
		...
		// Combo
        var dataTypeList  = convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","C00025"), "");
        sheet1.SetColProperty("dataType",          {ComboText:"|"+dataTypeList[0], ComboCode:"|"+dataTypeList[1]} );
        ...
        var udfGubunCdList  = convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","S99001"), "");
        sheet1.SetColProperty("udfGubunCd",          {ComboText:"|"+udfGubunCdList[0], ComboCode:"|"+udfGubunCdList[1]} );
        ...
        //Combo
        var udfParamCdList  = convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","S80001"), "");
        sheet2.SetColProperty("udfParamCd",          {ComboText:"|"+udfParamCdList[0], ComboCode:"|"+udfParamCdList[1]} );

        sheet2.SetColProperty("inOutType",          {ComboText:"|IN|OUT", ComboCode:"|IN|OUT"} );

        sheet2.SetColProperty("dataType",          {ComboText:"|"+dataTypeList[0], ComboCode:"|"+dataTypeList[1]} );
	...
	function doActionFirst(sAction) {
		switch (sAction) {
		case "Search": 	 	sheet1.DoSearch( "${ctx}/UserDefFuncMgr.do?cmd=getUserDefFuncMgrFirstList", $("#mySheetForm").serialize() ); break;
		case "Save":
        	if(!dupChk(sheet1,"udfCd", false, true)){break;}
        	IBS_SaveName(document.mySheetForm,sheet1);
			sheet1.DoSave( "${ctx}/UserDefFuncMgr.do?cmd=saveUserDefFuncMgrFirst", $("#mySheetForm").serialize()); break;
    ...
    function doActionSecond(sAction) {
        switch (sAction) {
        case "Search":      sheet2.DoSearch( "${ctx}/UserDefFuncMgr.do?cmd=getUserDefFuncMgrSecondList", $("#mySheetForm").serialize() ); break;
        case "Save":
        	if(!dupChk(sheet2,"seq", false, true)){break;}
        	IBS_SaveName(document.mySheetForm,sheet2);
        	sheet2.DoSave( "${ctx}/UserDefFuncMgr.do?cmd=saveUserDefFuncMgrSecond", $("#mySheetForm").serialize()); break;
        case "Insert":      if(gridFirstOnClickUdfCd == ""){
					            alert("<msg:txt mid='110433' mdef='함수 Master의 대상이 될 항목 링크를 선택 하여주세요.     '/>");
					        }else{
					            var newRow = sheet2.DataInsert(0);

					            sheet2.SetCellValue(newRow, "udfCd", gridFirstOnClickUdfCd);
					        } break;
        case "Copy":
   ...
   function sheet1_OnSelectCell(OldRow, OldCol, NewRow, NewCol){
        try{
            if(OldRow != NewRow){
                ...
                $("#searchUdfCd").val(sheet1.GetCellValue(NewRow,"udfCd"));

                // 그리드 셀 선택시 값 셋팅
                gridFirstOnClickUdfCd = sheet1.GetCellValue(NewRow,"udfCd");
