...
function doAction1(sAction) {
	switch (sAction) {
		case "SearchBasic":
			...
			// 기본사항조회
			var basicInfo = ajaxCall("${ctx}/GetDataMap.do?cmd=getPayCalcCreBasicMap", $("#sheet1Form").serialize(), false);
...
//				$("#empStatus"	).html(basicInfo.empStatus	);
		...
		case "Search":
			...
			sheet1.DoSearch("/GetDataList.do?cmd=getPayCalcCrePeopleSetList", $("#sheet1Form").serialize());
			break;
...
function peopleCnt(){
	...
	var payActionCd = $("#payActionCd").val();
	var businessPlaceCd = $("#businessPlaceCd").val();
	var peopleInfo = ajaxCall("${ctx}/GetDataMap.do?cmd=getPayCalcCrePeopleMap", "payActionCd="+payActionCd+"&businessPlaceCd="+businessPlaceCd, false).DATA;
