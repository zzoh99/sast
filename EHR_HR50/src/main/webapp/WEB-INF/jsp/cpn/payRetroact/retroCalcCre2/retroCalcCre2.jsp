...
<script type="text/javascript">
	...
	$(function() {
		...
		//------------------------------------- 그리드 콤보 -------------------------------------//
		// 사업장(TCPN121)
		var tcpn121Cd = convCode(ajaxCall(
				"${ctx}/CommonCode.do?cmd=getCommonNSCodeList",
				"queryId=getTcpn121List", false).codeList, "");
		sheet1.SetColProperty("businessPlaceCd", {
			ComboText : tcpn121Cd[0],
			ComboCode : tcpn121Cd[1]
		});

		//복리후생업무구분(B10230)
		var benefitBizCdList = convCode(codeList(
				"${ctx}/CommonCode.do?cmd=getCommonCodeList", "B10230"), "");
		sheet1.SetColProperty("benefitBizCd", {
			ComboText : benefitBizCdList[0],
			ComboCode : benefitBizCdList[1]
		});

		// 마감상태코드(S90003)
		var closeSt = convCode(codeList(
				"${ctx}/CommonCode.do?cmd=getCommonCodeList", "S90003"), "");
		sheet1.SetColProperty("closeSt", {
			ComboText : closeSt[0],
			ComboCode : closeSt[1]
		});
		...
		//------------------------------------- 그리드 콤보 -------------------------------------//
		// 직구분코드(H10050)
		var workType = convCode(codeList(
				"${ctx}/CommonCode.do?cmd=getCommonCodeList", "H10050"), "");
		sheet2.SetColProperty("workType", {
			ComboText : "|" + workType[0],
			ComboCode : "|" + workType[1]
		});

		// 직책코드(H20020)
		var jikchakCd = convCode(codeList(
				"${ctx}/CommonCode.do?cmd=getCommonCodeList", "H20020"), "");
		sheet2.SetColProperty("jikchakCd", {
			ComboText : "|" + jikchakCd[0],
			ComboCode : "|" + jikchakCd[1]
		});

		// 직급코드(H20010)
		var jikgubCd = convCode(codeList(
				"${ctx}/CommonCode.do?cmd=getCommonCodeList", "H20010"), "");
		sheet2.SetColProperty("jikgubCd", {
			ComboText : "|" + jikgubCd[0],
			ComboCode : "|" + jikgubCd[1]
		});

		// 직위코드(H20030)
		var jikweeCd = convCode(codeList(
				"${ctx}/CommonCode.do?cmd=getCommonCodeList", "H20030"), "");
		sheet2.SetColProperty("jikweeCd", {
			ComboText : "|" + jikweeCd[0],
			ComboCode : "|" + jikweeCd[1]
		});

		// 계약유형코드(H10030)
		var manageCd = convCode(codeList(
				"${ctx}/CommonCode.do?cmd=getCommonCodeList", "H10030"), "");
		sheet2.SetColProperty("manageCd", {
			ComboText : "|" + manageCd[0],
			ComboCode : "|" + manageCd[1]
		});

		// 급여유형코드(H10110)
		var payType = convCode(codeList(
				"${ctx}/CommonCode.do?cmd=getCommonCodeList", "H10110"), "");
		sheet2.SetColProperty("payType", {
			ComboText : "|" + payType[0],
			ComboCode : "|" + payType[1]
		});

		// 재직상태코드(H10010)
		var statusCd = convCode(codeList(
				"${ctx}/CommonCode.do?cmd=getCommonCodeList", "H10010"), "");
		sheet2.SetColProperty("statusCd", {
			ComboText : "|" + statusCd[0],
			ComboCode : "|" + statusCd[1]
		});
		...
		var tcpn121Cd = convCode(ajaxCall(
				"${ctx}/CommonCode.do?cmd=getCommonNSCodeList",
				"queryId=getTcpn121List", false).codeList, "");
		sheet2.SetColProperty("businessPlaceCd", {
			ComboText : "|" + tcpn121Cd[0],
			ComboCode : "|" + tcpn121Cd[1]
		});

		// 코스트센터(TORG109)
		var torg109Cd = convCode(ajaxCall(
				"${ctx}/CommonCode.do?cmd=getCommonNSCodeList&mapTypeCd=300",
				"queryId=getTorg109List", false).codeList, "");
		sheet2.SetColProperty("ccCd", {
			ComboText : "|" + torg109Cd[0],
			ComboCode : "|" + torg109Cd[1]
		});
		...
		//setSheetAutocompleteEmp( "sheet2", "name" , null , returnFunc );
	...
	function doAction1(sAction) {
		...
		case "SearchBasic":
			// 기본사항조회
			var basicInfo = ajaxCall(
					"${ctx}/RetroCalcCre.do?cmd=getPayCalcCreBasicMap", $(
							"#sheet1Form").serialize(), false);
		...
		case "PrcP_BEN_PAY_DATA_CREATE_ALL":
			...
			if ($("#sheet2Form #empStatus").val() != "10005") {
				...
				if (confirm("모든 복리후생 연계자료의 마감이 취소되고 재생성됩니다. 복리후생 데이터 생성작업을 실행하시겠습니까?")) {
					...
					ajaxCall2("${ctx}/RetroCalcCre.do?cmd=prcP_BEN_PAY_DATA_CREATE_ALL"
		...
		case "SearchPeople":
			// 인원정보 조회
			var peopleInfo = ajaxCall(
					"${ctx}/RetroCalcCre.do?cmd=getPayCalcCrePeopleMap"
					, $("#sheet1Form").serialize(), false);
			$("#peopleTotCnt").html("");
			$("#peopleSubCnt").html("");
			//			$("#peoplePCnt"		).html("");
		...
		case "Search":
			...
			// 마감현황 조회
			sheet1.DoSearch(
					"${ctx}/RetroCalcCre.do?cmd=getPayCalcCreCloseList", $(
		...
		case "PrcP_CPN_CAL_PAY_MAIN":
			 ...
			 */
			if (confirm("[소급계산] 작업을 수행하시겠습니까?")) {
				//작업중인 급여 로그가 있는지 확인
				var ingYn = ajaxCall(
						"/CommonCode.do?cmd=getCommonNSCodeList&searchBizCd=CPN&searchPrgCd=P_CPN_RE_PAY_MAIN&searchPayActionCd="
								+ $("#payActionCd").val(),
						"queryId=getPayCalcMsgYn", false);
				if (ingYn.codeList[0].code != null
						&& ingYn.codeList[0].code == "N") {
					if (confirm("이미 실행중입니다.\nClear 후 진행하시겠습니까?")) {
						//작업중이던 급여 로그를 강제 삭제 후 실행
						ajaxCall(
								"/RetroCalcCre.do?cmd=deleteTSYS904ForPayCalcCre&searchBizCd=CPN&searchPrgCd=P_CPN_RE_PAY_MAIN&searchPayActionCd="
										+ $("#payActionCd").val(),
								"queryId=deleteTSYS904ForPayCalcCre", false);
					} else {
						return;
					}
				}
				...
				$.ajax({
					//url : "${ctx}/PayCalcCreProc.do?cmd=prcP_CPN_CAL_PAY_MAIN",
					url : "${ctx}/RetroCalcCre.do?cmd=prcP_CPN_RE_PAY_MAIN",
					type : "post",
					dataType : "json",
					async : true,
					data : "payActionCd=" + payActionCd + "&businessPlaceCd="
							+ businessPlaceCd,
					success : function(result) {
						if (result != null && result["Result"] != null
								&& result["Result"]["Code"] != null) {
							if (result["Result"]["Code"] == "0") {
								interval = setInterval(chkLayerShow, 1000, "소급계산 되었습니다.", 500, globalWindowPopup, function(){doAction1("SearchPeople"); doAction2("Search");})
							} else if (result["Result"]["Message"] != null
									&& result["Result"]["Message"] != "") {
								alertTimer(result["Result"]["Message"], 1000);
							}
						} else {
							alertTimer("소급계산 오류입니다.", 1000);
						}
						doAction2("Search");
					},
		...
		case "PrcP_CPN_CAL_PAY_CANCEL":
			...
			if (confirm("[소급계산] 작업취소를 수행하시겠습니까?")) {

				progressBar(true, "소급계산 작업취소 중입니다.");

				var payActionCd = $("#payActionCd").val();
				var businessPlaceCd = $("#businessPlaceCd").val();
				// 작업취소
				ajaxCall2("${ctx}/RetroCalcCre.do?cmd=prcP_CPN_RE_PAY_CANCEL"
		...
		case "Close":
			...
			if (confirm("[마감] 작업을 수행하시겠습니까?")) {
				...
				var result = ajaxCall(
						"${ctx}/RetroCalcCre.do?cmd=saveCpnQuery",
						"queryId=saveCpnCloseYn&closeYn=Y&payActionCd="
								+ payActionCd, false);
		...
		case "CancelClose":
			...
			if (confirm("[마감취소] 작업을 수행하시겠습니까?")) {
				...
				var result = ajaxCall(
						"${ctx}/RetroCalcCre.do?cmd=saveCpnQuery",
						"queryId=saveCpnCloseYn&procNm=" + procNm
								+ "&closeYn=N&payActionCd=" + payActionCd,
						false);
				if (result != null && result["Result"] != null
						&& result["Result"]["Code"] != null) {
					if (result["Result"]["Message"] != null
	...
	function getCpnLatestPaymentInfo() {
		...
		var paymentInfo = ajaxCall(
				"${ctx}/RetroCalcCre.do?cmd=getCpnQueryList",
				"queryId=getCpnLatestPaymentInfoMap&procNm=" + procNm
						+ "&runType=RETRO,", false);

		if (paymentInfo.DATA != null && paymentInfo.DATA != ""
				&& typeof paymentInfo.DATA[0] != "undefined") {
	...
	function sabunName() {
		...
		if (sheet2.GetSelectRow() < sheet2.LastRow()) {
			Row = sheet2.FindText("name", $("#sabunName").val(), sheet2
					.GetSelectRow() + 1, 2);
		} else {
			Row = -1;
		}
	...
	// 작업 프로그램 진행현황 팝업
	function openProcessBar(actYn = "1") {
		let layerModal = new window.top.document.LayerModal({
			id : 'processBarLayer',
			url : '/CpnComPopup.do?cmd=viewCpnProcessBarComLayer&authPg=R',
			parameters : {
				prgCd : 'P_CPN_RE_PAY_MAIN',//'P_CPN_CAL_PAY_MAIN',
				payActionCd : $("#payActionCd").val(),
				payActionNm : $("#payActionNm").val(),
				actYn: actYn
			},
	...
	function doAction2(sAction) {
		...
		case "PrcP_CPN_CAL_EMP_INS":
			...
			if ($("#sheet2Form #empStatus").val() != "10005") {
				...
				if (confirm("[작업]을 실행하시겠습니까?")) {
					...
					progressBar(true, "소급대상자 생성중입니다.");

					var payActionCd = $("#sheet2Form #s2_payActionCd").val();
					var businessPlaceCd = $("#sheet2Form #s2_businessPlaceCd").val();

					// 급여대상자 생성
					ajaxCall2("${ctx}/RetroCalcCre.do?cmd=prcP_CPN_CAL_EMP_INS"
		...
		case "Search":
			...
			sheet2.DoSearch("/RetroCalcCre.do?cmd=getPayCalcCrePeopleSetList",
					$("#sheet2Form").serialize());
		...
		case "Save":
			...
			if (sheet2.RowCount() > 0) {
				for (var i = sheet2.HeaderRows(); i < sheet2.RowCount()
						+ sheet2.HeaderRows(); i++) {
					if (sheet2.GetCellValue(i, "payActionCd") == "") {
						sheet2.SetCellValue(i, "payActionCd", $(
								"#sheet2Form #s2_payActionCd").val());
					}
				}
			}

			IBS_SaveName(document.sheet2Form, sheet2);
			sheet2.DoSave("${ctx}/RetroCalcCre.do?cmd=savePayCalcCrePeopleSet",
					$("#sheet2Form").serialize());
			break;
		...
		case "Insert":
			...
			sheet2.SetCellValue(Row, "payActionCd", $(
					"#sheet2Form #s2_payActionCd").val());
		...
		try {
			...
			for (var i = sheet2.HeaderRows(); i < sheet2.RowCount()
				...
				if ($("#sheet2Form #s2_closeYn").val() == "Y") {
					...
					if (sheet2.GetCellValue(i, "status") == "J") {
						sheet2.SetCellEditable(i, "sDelete", 0);
						sheet2.SetCellValue(i, "btnPrt",
								'<a class="basic">계산취소</a>');
	...
	function sheet2_OnClick(Row, Col, Value) {
		try {
			if (sheet2.ColSaveName(Col) == "btnPrt" && Row != 0) {
				if (sheet2.GetCellValue(Row, "status") == "J"
						&& $("#sheet2Form #s2_closeYn").val() != "Y") {
					var sabun = sheet2.GetCellValue(Row, "sabun");
					calcCancel(sabun);
				} else if (sheet2.GetCellValue(Row, "status") == "P"
						&& $("#sheet2Form #s2_closeYn").val() != "Y") {
					var sabun = sheet2.GetCellValue(Row, "sabun");
					calc(sabun);
				}
	...
	function calcCancel(sabun) {
		if (confirm("[급여계산] 작업취소를 수행하시겠습니까?")) {
			...
			var result = ajaxCall(
					"${ctx}/RetroCalcCre.do?cmd=prcP_CPN_RE_PAY_CANCEL",
					"payActionCd=" + payActionCd + "&businessPlaceCd="
							+ businessPlaceCd + "&sabun=" + sabun, false);
	...
	function setSheet1Height() {
		...
		sheet1.SetSheetHeight(topHeight);
	}
</script>
</head>
<body class="hidden">
	<div class="wrapper">
				...
				<table border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td>
							...
							<a href="javascript:payActionSearchPopup();" class="button6">
				...
				<tr>
					<td>
						<div class="outer">
							...
							createIBSheet("sheet1", "50%", "190px", "kr");
						</script>
					</td>
					<td></td>
					<td class="top">
						<div class="inner">
							<div class="sheet_title">
								<ul>
									<li id="txt" class="txt">처리조건</li>
									<li class="btn">
										<a href="javascript:doAction1('SearchPeople')"	class="button authR">조회</a>
									</li>
								</ul>
							</div>
						</div>
						<table border="0" cellpadding="0" cellspacing="0"
							...
							<tr>
								<th>급여계산</th>
								<td colspan="3">
									<a href="javascript:doAction1('PrcP_CPN_CAL_PAY_MAIN')" class="button authA">계산</a>
									<a href="javascript:doAction1('PrcP_CPN_CAL_PAY_CANCEL')" class="basic authA">계산취소</a>
									<a href="javascript:openProcessBar('0')" class="basic authA">진행상태</a>
								</td>
							</tr>
							<!-- 
						<tr>
							<th>급여계산 오류</th>
							<td style="border-right-style: none;"><span id="payErrCnt">0</span><span> 건 </span>
							...
							<td colspan="2" style="border-left: 0pt; border-left-style: none;">
							</td>
						</tr>
						-->
							<tr>
								<th>급여계산 마감여부</th>
								<td colspan="3">
									<input id="payCalcCloseYn" name="payCalcCloseYn" type="checkbox" class="checkbox" disabled />
		...
		<form id="sheet2Form" name="sheet2Form">
			...
			<div class="inner">
				<div class="sheet_title">
					<ul>
						<li class="txt"><span>급여대상자</span></li>
						<li class="btn">
							사번/성명&nbsp;<input type="text" id="sabunName" name="sabunName" class="text" value="" style="ime-mode: active" onKeyUp="check_Enter();" />
