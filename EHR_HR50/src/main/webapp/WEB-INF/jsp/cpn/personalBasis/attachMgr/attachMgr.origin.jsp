...
$(function() {
	...
	initdata.Cols = [
		...
		{Header:"성명",			Type:"Popup",		Hidden:0,					Width:80,			Align:"Center",	ColMerge:0,	SaveName:"name",			KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 },
		...
		{Header:"직군",			Type:"Combo",		Hidden:1,					Width:100,			Align:"Center",	ColMerge:0,	SaveName:"workType",		KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:10 },
		...
		{Header:"청구잔액",			Type:"Int",			Hidden:1,					Width:100,			Align:"Right",	ColMerge:0,	SaveName:"remainAmt",		KeyField:0,	Format:"Integer",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 },
...
function doAction1(sAction) {
	switch (sAction) {
		...
		case "Delete":
			...
			sheet1.DoSave("${ctx}/AttachMgr.do?cmd=saveAttachMgrDtl", $("#sheet1Form").serialize());
			break;
...
function attachMgrDtlPopup(Row, sAction) {
	if(!isPopup()) {return;}
	gPRow = Row;
	pGubun = "viewAttachMgrDtlPopup";

	var w		= 780;
	var h		= 780;
	var url		= "/AttachMgr.do?cmd=viewAttachMgrDtlPopup";
	var args	= new Array();
	...
	if (sAction == "Update" || sAction == "Copy") {
		...
		args["statusCd"]		= sheet1.GetCellValue(Row, "statusCd");
		args["attatchSymd"]		= sheet1.GetCellValue(Row, "attatchSymd");
		args["attatchNo"]		= sheet1.GetCellValue(Row, "attatchNo");
		args["attatchType"]		= sheet1.GetCellValue(Row, "attatchType");
		args["attatchStatus"]	= sheet1.GetCellValue(Row, "attatchStatus");
		args["debtContent"]		= sheet1.GetCellValue(Row, "debtContent");
		args["relationEvent"]	= sheet1.GetCellValue(Row, "relationEvent");
		args["bonder"]			= sheet1.GetCellValue(Row, "bonder");
		args["bondCharger"]		= sheet1.GetCellValue(Row, "bondCharger");
		args["bondHandNo"]		= sheet1.GetCellValue(Row, "bondHandNo");
		args["bondTelNo"]		= sheet1.GetCellValue(Row, "bondTelNo");
		args["bondContent"]		= sheet1.GetCellValue(Row, "bondContent");
		args["bondTelNo"]		= sheet1.GetCellValue(Row, "bondTelNo");
		args["attBankNm"]		= sheet1.GetCellValue(Row, "attBankNm");
		args["attAccountNo"]	= sheet1.GetCellValue(Row, "attAccountNo");
		args["attDepositor"]	= sheet1.GetCellValue(Row, "attDepositor");
		args["attachMon"]		= sheet1.GetCellValue(Row, "attachMon");
		args["attTotMon"]		= sheet1.GetCellValue(Row, "attTotMon");
		args["receiptMon"]		= sheet1.GetCellValue(Row, "receiptMon");
		args["remainAmt"]		= sheet1.GetCellValue(Row, "remainAmt");
		args["courtYmd"]		= sheet1.GetCellValue(Row, "courtYmd");
		args["invalidYmd"]		= sheet1.GetCellValue(Row, "invalidYmd");
		args["elementCd"]		= sheet1.GetCellValue(Row, "elementCd");
		args["note"]			= sheet1.GetCellValue(Row, "note");
		args["jikgubNm"]		= sheet1.GetCellText(Row, "jikgubCd");
		args["jikweeNm"]		= sheet1.GetCellText(Row, "jikweeCd");
		args["jikchakNm"]		= sheet1.GetCellText(Row, "jikchakCd");

		args["statusNm"]		= sheet1.GetCellText(Row, "statusCd");
		args["attatchNoReadonly"]		= sheet1.GetCellValue(Row, "attatchNo");
	}
	args["sAction"]	= sAction;
...
function getReturnValue(returnValue) {
    ...
    }else if(pGubun == "viewAttachMgrDtlPopup"){
    	doAction1("Search");
    }
}
</script>
				...
				<table>
					<tr>
						...
						<td> <input type="hidden" id="orgCd" name="orgCd" class="text" value="" />
														...
														<a onclick="$('#orgCd,#orgNm').val('');" class="button7"><img src="/common/images/icon/icon_undo.png"/></a> </td>
						<th>재직상태</th>
