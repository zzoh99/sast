...
$(function() {
	...
	initdata1.Cols = [
		...
		{Header:"총금액",			Type:"Int",			Hidden:0,					Width:100,			Align:"Right",	ColMerge:0,	SaveName:"mon",				KeyField:1,	CalcLogic:"",	Format:"Integer",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 }
	...
	initdata2.Cols = [
		...
		{Header:"급여일자",			Type:"Popup",		Hidden:0,					Width:130,			Align:"Center",	ColMerge:0,	SaveName:"subPayActionNm",	KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:100 },
		...
		{Header:"금액",			Type:"AutoSum",		Hidden:0,					Width:100,			Align:"Right",	ColMerge:0,	SaveName:"mon",				KeyField:1,	Format:"Integer",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
	...
	initdata3.Cols = [
		...
		{Header:"금액",			Type:"Int",			Hidden:0,					Width:100,			Align:"Right",	ColMerge:0,	SaveName:"mon",				KeyField:1,	Format:"Integer",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
...
function doAction1(sAction) {
	switch (sAction) {
		case "Search":
			...
			sheet1.DoSearch("${ctx}/SepCalcBasicMgr.do?cmd=getSepCalcBasicMgrAverageIncomePayList", $("#sheet1Form").serialize());
			break;
		...
		case "Save":
			...
			sheet1.DoSave("${ctx}/SepCalcBasicMgr.do?cmd=saveSepCalcBasicMgrAverageIncomePay", $("#sheet1Form").serialize());
			break;
		...
		case "Insert":
			...
			sheet1.SetCellValue(Row, "sabun", parent.$("#searchUserId").val());
...
function sheet1_OnLoadExcel() {
	var rowCnt = sheet1.RowCount();
	for (var i=1; i<=rowCnt; i++) {
		sheet1.SetCellValue(i, "sabun",parent.$("#searchUserId").val());
...
function doAction2(sAction) {
	switch (sAction) {
		case "Search":
			...
			// 상여 조회
			sheet2.DoSearch("${ctx}/SepCalcBasicMgr.do?cmd=getSepCalcBasicMgrAverageIncomeBonusList", $("#sheet1Form").serialize());
			break;

		case "Save":
			IBS_SaveName(document.sheet1Form,sheet2);
			sheet2.DoSave("${ctx}/SepCalcBasicMgr.do?cmd=saveSepCalcBasicMgrAverageIncomeBonus", $("#sheet1Form").serialize());
			break;
		...
		case "Insert":
			...
			sheet2.SetCellValue(Row, "sabun", parent.$("#searchUserId").val());
...
function sheet2_OnLoadExcel() {
	...
	for (var i=1; i<=rowCnt; i++) {
		sheet2.SetCellValue(i, "sabun",parent.$("#searchUserId").val());
...
function doAction3(sAction) {
	switch (sAction) {
		case "Search":
			...
			// 연차 조회
			sheet3.DoSearch("${ctx}/SepCalcBasicMgr.do?cmd=getSepCalcBasicMgrAverageIncomeAnnualList", $("#sheet1Form").serialize());

			// 퇴직금계산내역 조회
			var severancePayInfo = ajaxCall("${ctx}/SepCalcBasicMgr.do?cmd=getSepCalcBasicMgrSeverancePayMap", $("#sheet1Form").serialize(), false);
		...
		case "Save":
			...
			sheet3.DoSave("${ctx}/SepCalcBasicMgr.do?cmd=saveSepCalcBasicMgrAverageIncomeAnnual", $("#sheet1Form").serialize());
			break;
		...
		case "Insert":
			...
			sheet3.SetCellValue(Row, "sabun", parent.$("#searchUserId").val());
...
function sheet2_OnPopupClick(Row, Col) {
	try{
		...
		if(colName == "subPayActionNm") {
			// 급여일자검색 팝업
			payActionSearchPopup(Row, Col, "sheet2");
		}
...
function sheet3_OnPopupClick(Row, Col) {
	try{
		...
		if(colName == "subPayActionNm") {
			// 급여일자검색 팝업
			payActionSearchPopup(Row, Col, "sheet3");
		}
	}catch(ex) {alert("OnPopupClick Event Error : " + ex);}
}
...
// 급여일자검색 팝업
function payActionSearchPopup(Row, Col, sheetNm) {
	if(!isPopup()) {return;}
	gPRow = Row;
	...
	args["runType"] = "00001,00002,00003"; // 급여구분(00002.상여 00003.연월차)
	...
	if (result) {
		...
		if (sheetNm == "sheet2") {
			...
			sheet3.SetCellValue(Row, "paymentYmd", paymentYmd);
		}
	}
	*/
}
...
function searchTitleList() {
	...
	titleList = ajaxCall("${ctx}/SepCalcBasicMgr.do?cmd=getSepCalcBasicMgrAverageIncomePayTitleList", $("#sheet1Form").serialize(), false);
	...
	if(titleList != null && titleList.DATA != null) {
		...
		for (var Row=1; Row<=rowCnt; Row++) {
			...
			for(var i=0; i<titleList.DATA.length; i++){
				if (titleList.DATA[i].elementCd != null && titleList.DATA[i].elementCd != "") {
					if (i==0) {
						detailMon = sheet1.GetCellValue(Row, i+10);
					} else {
						detailMon = detailMon + "," + sheet1.GetCellValue(Row, i+10);
					}
				}
			}
	...
	<colgroup>
		...
		<tr>
			<td class="top" colspan="3">
				<div>
					<div class="sheet_title outer">
						<ul>
							...
							<li class="btn">
								<%-- <btn:a href="javascript:doAction1('DownTemplate')"		css="basic authA" mid='down2ExcelV1' mdef="양식다운로드"/>
								<btn:a href="javascript:doAction1('LoadExcel')"		css="basic authA" mid='upload' mdef="업로드"/> --%>
				...
				<script type="text/javascript">createIBSheet("sheet1", "100%", "183px", "${ssnLocaleCd}"); </script>
			</td>
		</tr>
		<tr>
			<td rowspan="2" class="top">
				<div>
					<div class="sheet_title outer">
						<ul>
							...
							<li class="btn">
								<%-- <btn:a href="javascript:doAction2('DownTemplate')"		css="basic authA" mid='down2ExcelV1' mdef="양식다운로드"/>
								<btn:a href="javascript:doAction2('LoadExcel')"		css="basic authA" mid='upload' mdef="업로드"/> --%>
								<btn:a href="javascript:doAction2('Search')"		css="button authR" mid='search' mdef="조회"/>
				...
				<script type="text/javascript">createIBSheet("sheet2", "50%", "278px", "${ssnLocaleCd}"); </script>
			</td>
			<td rowspan="2"></td>
			<td class="top" style="height:100px">
				<div>
					<div class="sheet_title">
						<ul>
							...
							<li class="btn">
								<btn:a href="javascript:doAction3('Search')"		css="button authR" mid='search' mdef="조회"/>
				...
				<script type="text/javascript">createIBSheet("sheet3", "50%", "80px", "${ssnLocaleCd}"); </script>
			</td>
		</tr>
		<tr>
			<td class="top">
