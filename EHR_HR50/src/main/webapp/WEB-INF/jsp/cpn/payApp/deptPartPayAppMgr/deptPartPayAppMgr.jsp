...
<!DOCTYPE html> <html class="hidden"><head> <title>수당지급승인관리</title>
...
//========================================================================================================================
		...
		$("#searchPayYm, #searchApplYmdFrom, #searchApplYmdTo").bind("keyup",function(event){
			if( event.keyCode == 13){ doAction1("Search"); $(this).focus(); }
		});
...
//========================================================================================================================
		...
		initdata1.Cols = [
			...
			{Header:"합산년월|합산년월",		Type:"Date",			Hidden:0,	Width:80,	Align:"Center",	ColMerge:0,	SaveName:"payYm",			KeyField:0,	Format:"Ym",		PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:10 },
			...
			{Header:"지급일자|지급일자",		Type:"Date",			Hidden:0,	Width:120,	Align:"Center",	ColMerge:0,	SaveName:"payPaymentYmd",			KeyField:0,	Format:"Ymd",	PointCount:0,	UpdateEdit:1,	InsertEdit:0,	EditLen:10 },
			{Header:"급여코드|급여코드",		Type:"Combo",			Hidden:0,	Width:120,	Align:"Center",	ColMerge:0,	SaveName:"payCd",			KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:1,	InsertEdit:0,	EditLen:10 },
		...
		// 급여코드
        var payCdList = convCode( ajaxCall("/CommonCode.do?cmd=getCommonNSCodeList","queryId=getCpnPayCdList&searchRunType=00001,00002",false).codeList, "");
	...
	function doAction1(sAction) {
		...
		case "Search":
						...
						+"&searchApplYmdFrom="+$("#searchApplYmdFrom").val()
						+"&searchApplYmdTo="+$("#searchApplYmdTo").val()
						+"&searchBenefitBizCd="+$("#deptPartPayAppBnCd").val()
						+"&searchApplStatusCd="+$("#searchApplStatusCd").val()
						+"&searchPayYm="+$("#searchPayYm").val()
						+"&searchSabunName="+$("#searchSabunName").val()
						+"&searchMagamYn="+$("#searchMagamYn").val()
						+"&searchPayActionCd="+$("#searchPayActionCd").val()
						+"&searchOrgNm="+$("#searchOrgNm").val();

						sheet1.DoSearch( "${ctx}/GetDataList.do?cmd=getDeptPartPayAppMgrList",param );
						
						break;
		case "Save":
						IBS_SaveName(document.sendForm,sheet1);
						sheet1.DoSave( "${ctx}/DeptPartPayAppMgr.do?cmd=saveDeptPartPayAppMgr", $("#sendForm").serialize());						
	...
	function doAction2(sAction) {
		...
		case "Search":
						...
						sheet2.DoSearch( "${ctx}/GetDataList.do?cmd=getDeptPartPayAppMgrDetailList",param );
						break;
		case "Down2Excel":
		...
		try {
			...
			for(var i=2 ; i<sheet1.RowCount()+2; i++){
				...
				if( sheet1.GetCellValue(i,"applStatusCd") != "99" ){
					sheet1.SetCellEditable(i, "magamYn", 0);
					sheet1.SetCellEditable(i, "payPaymentYmd", 0);
					sheet1.SetCellEditable(i, "payCd", 0);
				}				
			}
	...
	<div class="sheet_search outer">
		...
		<table>
			<tr>
				<th>합산년월 </th>
				<td>
					<input id="searchPayYm" name ="searchPayYm" class="date2" type="text"  value="<%=DateUtil.getCurrentTime("yyyy-MM")%>"/>
                ...
                <!-- 
 				...
 				-->
 					<input type="text" id="searchApplYmdFrom" name="searchApplYmdFrom" class="date2" value="" /> ~
                    <input type="text" id="searchApplYmdTo" name="searchApplYmdTo" class="date2" value="" />
                </td>
				<th>수당명</th>
				<td>
					<select id="deptPartPayAppBnCd" name="deptPartPayAppBnCd" onchange="javascript:doAction1('Search');"></select>
				</td>
				<th>확인여부</th>
				<td colspan="2">
