...
<!DOCTYPE html> <html class="hidden"><head> <title><tit:txt mid="202005150000111" mdef="기타지급신청 세부내역" /></title>
...
<script type="text/javascript">
	...
	$(function() {
		...
		var selectText = "<sch:txt mid='select' mdef='선택' />";
  		// 수당코드 리스트
		var benefitBizCdList = convCode(ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList", "queryId=getEtcPayAppBnCdList", false).codeList, selectText);
		...
		if(searchApplSeq != null && searchApplSeq != "") {
			var data = ajaxCall( "${ctx}/GetDataMap.do?cmd=getEtcPayAppDetList", $("#sheetForm").serialize(), false);
			setDataForm(data);
	...
	function doAction1(sAction) {
		switch (sAction) {
			...
			sheet1.DoSearch( "${ctx}/EtcPayAppDet.do?cmd=getEtcPayAppDetailList",param );
			break;
			case "Insert":
	...
	function setValue(){
		...
		try{
			...
			if(authPg == "A" && ( applStatusCd == "" || applStatusCd == "11" )) {
				...
				//alert(" call saveEtcPayAppDet, sheet1.GetSaveString => " + sheet1.GetSaveString());

				var rtn;
				if ( sheet1.GetSaveString() == "" ) {
					rtn = ajaxCall("${ctx}/EtcPayAppDet.do?cmd=saveEtcPayAppDet",$("#sheetForm").serialize(),false);
					...
					sheet1.DoSave("${ctx}/EtcPayAppDet.do?cmd=saveEtcPayAppDet",$("#sheetForm").serialize());
				}
...
<form id="sheetForm" name="sheetForm" >
	...
	<div class="outer">
		...
		<ul>
			<li class="txt"><tit:txt mid="202005150000111" mdef="기타지급신청 세부내역" /></li>
			<li class="btn">
			</li>
		</ul>
		</div>
