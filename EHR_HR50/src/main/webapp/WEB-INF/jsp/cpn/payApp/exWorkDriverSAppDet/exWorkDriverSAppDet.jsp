...
<!DOCTYPE html> <html class="hidden"><head> <title>특근수당(기원)종합신청 세부내역</title>
...
<script type="text/javascript">
	...
	$(function() {
		...
		if(searchApplSeq != null && searchApplSeq != "") { 
			var data = ajaxCall( "${ctx}/GetDataMap.do?cmd=getExWorkDriverSAppDet", $("#sheetForm").serialize(), false);
			setDataForm(data);
	...
	function doAction1(sAction) {
		switch (sAction) {
			...
			sheet1.DoSearch( "${ctx}/GetDataList.do?cmd=getExWorkDriverSAppDetailList",param );
			break;
			case "SearchCalc":
			// 계산금액불러오기
			var param = "workYm="+ $("#workYm").val();
			sheet1.DoSearch( "${ctx}/GetDataList.do?cmd=getExWorkDriverSAppDetCalc",param );
			break;			
			case "Insert":
	...
	function setValue(){
		...
		try{
			...
			if(authPg == "A" && ( applStatusCd == "" || applStatusCd == "11" )) {
				...
				var data = ajaxCall("${ctx}/GetDataMap.do?cmd=getExWorkDriverSAppDetCnt",$("#sheetForm").serialize(),false);
				...
				//alert(" call saveExWorkDriverSAppDet, sheet1.GetSaveString => " + sheet1.GetSaveString());
				
				if ( sheet1.GetSaveString() == "" ) { 
					rtn = ajaxCall("${ctx}/ExWorkDriverSAppDet.do?cmd=saveExWorkDriverSAppDet",$("#sheetForm").serialize(),false);
					...
					sheet1.DoSave("${ctx}/ExWorkDriverSAppDet.do?cmd=saveExWorkDriverSAppDet",$("#sheetForm").serialize());
				}
