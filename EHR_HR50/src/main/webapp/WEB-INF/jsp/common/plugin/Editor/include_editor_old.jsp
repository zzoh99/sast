...
<div class="body">
	...
	<!--
		...
		https://github.com/daumcorp/DaumEditor/issues/306
		--수정사항 daumeditor-7.4.1.zip
		css
		.tx-switchtoggle a {width:48px; background-position:0 -84px; margin:0;}
		.tx-switchtoggle-hovered a {width:48px; background-position:0 -107px; margin:0;}
		.tx-switchtoggle-pushed a {width:48px; background-position:-58px -84px; margin:0;}

		.tx-dictionary a.tx-icon {background-position:-158px -10px;}

		js
		features: { left:250, top:65, width:400, height:350 },
		popPageUrl: "#host#path/pages/trex/image.jsp"
		/* this.form.submit() */

		//
		//selectedMode: "text", // [text,html,source]
...
<script type="text/javascript">
	var config = {
		...
		sidebar: {
			...
			capacity: {
				show: true,
				available: 1048576 * 1 , //1MB.
				maximum: 1048576 * 1
			}
		},

		//save: {onSave: function(editor){}}, //자동sumbmit 방지
		size: {
...
*/
    ...
    $(window).bind('resize', function(e){
	    ...
	    $(window).resize(function(){
	        ...
	        window.resizeEvt = setTimeout(function(){
	        	Editor.getCanvas().setCanvasSize({height:resizeEditor()});
	        	//Editor.getCanvas().setCanvasSize({height:$(document).height()-300});
	        }, 250);
	    });
	});
	...
	function resizeEditor() {
		...
		if(minusHeight !="") {
			var baseHeight = $(document).height()- minusHeight -100;
		}else{
			baseHeight = 250;
		}
...
<script type="text/javascript">
	...
	function setForm(editor) {
		...
		formGenerator.createField(
				tx.textarea({
					'name': "${editor.contentNm}", // 본문 내용을 필드를 생성하여 값을 할당하는 부분
					'style': { 'display': "none" }
				}, content)
		);
