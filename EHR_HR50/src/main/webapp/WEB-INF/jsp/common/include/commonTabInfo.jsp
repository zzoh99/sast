...
<script type="text/javascript">
	...
	function setIframeHeight() {
		var par = cmTabObj.parent().parent();
		let $employeeHeader;
		if (par.find(".employee-header-box").length > 0) {
			$employeeHeader = par.find(".employee-header-box");
		} else {
			$employeeHeader = par.find(".ux_wrapper.header_wrap");
		}

		var tabsHeight = par.height() - $employeeHeader.outerHeight(true) - 20; // 가장 외부의 padding과 margin도 빼줌.
		cmTabObj.css("height", tabsHeight);
		
		//인사기본 내부 탭의 높이
		var wrpHeight = par.height() - $employeeHeader.outerHeight() - cmTabObj.find("#tabBar").outerHeight() - 30; // 마지막 30은 margin 및 padding
	...
	function createCmTabData() {
		...
		if(cmTabData.Message == "") {
			...
			$(cmTabData.DATA).each(function(index) {
				cmTabObj.append("<div id='tabs-"+index+"'></div>");
				...
				// IFRAME 1개만 생성 2020.07.02
			});
			cmTabObj.append("<div class='layout_tabs'><iframe id='iframe-0' name='iframe-0' src='${ctx}/common/hidden.html' frameborder='0' class='tab_iframes'></iframe></div>");
		...
		if(typeof index != 'undefined') {
			if(cmTabIframeIdx == index) {
				...
				$("#cmTabForm").attr("action",obj.prgCd)
								.attr("method","post")
								//.attr("target",cmTabNewIframe.attr("id")) // IFRAME 1개만 생성 2020.07.02
								.attr("target","iframe-0");
				//				.submit();
				
				if( isFirst ){
					// 최초에 Submit이 안되는 현상이 있어서... 더 좋은 해결 방안이 있으면 수정 할 것 2020.07.02
					setTimeout(function(){	$("#cmTabForm").submit(); },100);
					isFirst = false;
				}else{
					$("#cmTabForm").submit();
				}
	...
	function setTabBarBtn() {
		...
		var liTotWidth = 0;
		cmTabObj.find("#tabGroup li").each(function(idx, obj) {
			liTotWidth += $(obj).width() + 3;		// 사이즈가 소수점 자리일 때 사이즈가 안맞는 경우가 생겨, 각 li마다 3정도를 더 해서 사이즈 측정해줌.
		});

		var tabBar = cmTabObj.find("#tabBar");
		// hr50에서 서브탭 bar에 화살표 디자인이 없어 주석 처리 해놓음 2023.11.03 snow2
		// if(tabBar.width() > liTotWidth) {
			cmTabObj.find("a#conTabPrev, a#conTabNext").hide();
			tabBar.removeClass("ma30");
		// } else {
		// 	cmTabObj.find("a#conTabPrev, a#conTabNext").show();
		// 	if(!tabBar.hasClass("ma30"))
		// 		tabBar.addClass("ma30");
		// }
		cmTabObj.find("#tabGroup").css("width", liTotWidth);
	}
	

</script>
	<form id="cmTabForm" name="cmTabForm" method="post">
