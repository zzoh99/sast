...
<script>
    ...
    $(function() {
        ...
        fileInfo    = modal.parameters.fileInfo || '';
        ...
        initIbFileUploadIframe("fileLayerUploadForm", fileSeq, uploadType, tempAuthPg, fileInfo);
        if (tempAuthPg === 'R') {
            $("a#uploadBtn").hide();
            $("a#deleteBtn").hide();
    ...
    function comfirmFileUploadPop(flag) {
		...
		if (flag == 'confirm') {
	        modal.fire('fileMgrTrigger', {
	            ...
	            , fileCheck : fileCheck
                , fileList : getFileUploadContentWindow("fileLayerUploadForm").getFileInfo()
                , uploadType : getFileUploadContentWindow("fileLayerUploadForm").getUploadType()
	        }).hide();	
		} else {
			modal.hide();
		}
