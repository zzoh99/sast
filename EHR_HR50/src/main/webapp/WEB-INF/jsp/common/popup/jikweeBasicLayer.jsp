...
<script type="text/javascript">
	...
	$(function() {
		...
		const modal = window.top.document.LayerModalUtility.getModal('jikweeLayer');
		...
		chooseJikweeCds = modal.parameters.chooseJikweeCds || '';
		...
		initdata.Cols = [
				...
				{Header:"직위코드",		Type:"Text",		Hidden:0,					Width:100,	Align:"Center",	ColMerge:0,	SaveName:"jikweeCd",			KeyField:0,	CalcLogic:"",	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 },
				{Header:"직위명",			Type:"Text",		Hidden:0,					Width:200,	Align:"Left",	ColMerge:0,	SaveName:"jikweeNm",			KeyField:0,	CalcLogic:"",	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:50 },
				{Header:"직위명(전체)",	Type:"Text",		Hidden:1,					Width:200,	Align:"Left",	ColMerge:0,	SaveName:"jikweeFullNm",		KeyField:0,	CalcLogic:"",	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:50 },
				{Header:"직위명(영문)",	Type:"Text",		Hidden:1,					Width:200,	Align:"Left",	ColMerge:0,	SaveName:"jikweeEngNm",			KeyField:0,	CalcLogic:"",	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:50 },
		...
		$("#searchBaseDate,#serachJikweeNm").bind("keyup",function(event){
	...
	function doAction(sAction) {
		switch (sAction) {
		case "Search": 		//조회
			mySheet.DoSearch( "${ctx}/Popup.do?cmd=getJikweeBasicPopupList", $("#mySheetForm").serialize() );
			break;
		}
	}
	...
	function mySheet_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			if(multiSelect == "Y" && chooseJikweeCds != null && chooseJikweeCds != "" && chooseJikweeCds != undefined ) {
				var checkJikweeCds = "|" + chooseJikweeCds.replace(/,/g, "|") + "|";
				for(var Row = 1; Row <= mySheet.RowCount(); Row++) {
					if( checkJikweeCds.indexOf( "|" + mySheet.GetCellValue(Row, "jikweeCd") + "|") > -1 && mySheet.GetCellValue(Row, "schemeUseYn") == "1" ) {
	...
	function mySheet_OnDblClick(Row, Col){
		if(multiSelect == "N") {
			const modal = window.top.document.LayerModalUtility.getModal('jikweeLayer');
			modal.fire('jikweeTrigger', [{
				jikweeCd : mySheet.GetCellValue(Row, "jikweeCd")
				, jikweeNm : mySheet.GetCellValue(Row, "jikweeNm")
			}]).hide();
		}
	}
	...
	function setValue() {
		if(multiSelect == "Y") {
			...
			const modal = window.top.document.LayerModalUtility.getModal('jikweeLayer');
			modal.fire('jikweeTrigger', result).hide();
		}
	}
</script>
</head>
					...
					<div>
						<table>
							<tr>
								...
								<th>직위명</th>
								<td>
									<input id="serachJikweeNm" name ="serachJikweeNm" type="text" class="text" />
								</td>
								<th>현재<br/>사용여부</th>
								<td>
			...
			<table border="0" cellspacing="0" cellpadding="0" class="sheet_main">
				<tr>
					...
					<div class="inner">
						...
						<ul>
							<li id="txt" class="txt">직위 리스트 조회</li>
						</ul>
						</div>
					</div>
		...
		<div class="modal_footer">
			<ul>
				<li>
					...
					<a href="javascript:closeCommonLayer('jikweeLayer');" class="gray large">닫기</a>
				</li>
			</ul>
		</div>
	</div>
