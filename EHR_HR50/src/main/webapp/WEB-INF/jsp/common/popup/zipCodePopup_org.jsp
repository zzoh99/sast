...
<title><tit:txt mid='113057' mdef='주소 조회'/></title>
...
<script type="text/javascript">
	...
	$(function() {
	    ...
	    $('#zipCode').mask('000-000', {reverse: true});
	...
	function doAction(sAction) {
		...
		case "Search": //조회
			...
			if( searchNameCheck($("#searchAddress").val())){
				...
				var param = {"Param":"cmd=getZipCodePopupDoroList&defaultRow=100&"+$("#srchFrm").serialize(),"Sync":"1"};
	...
	function setAddr(){
		...
		if( !$("#notFindAddrStatus").is(":checked") ){
			...
			$("#doroFullAddr").val(sheet1.GetCellValue(sheet1.GetSelectRow(), "doroFullAddr"));
			$("#doroAddr").val(sheet1.GetCellValue(sheet1.GetSelectRow(), "doroAddr"));
			$("#addrNote").val(sheet1.GetCellValue(sheet1.GetSelectRow(), "addrNote"));
			$("#doroFullAddrEng").val(sheet1.GetCellValue(sheet1.GetSelectRow(), "doroAddrEng"));

		}else{
			alert("<msg:txt mid='alertAddrUserKey' mdef='직접입력의 경우, 조회 내역을 사용 할 수 없습니다.\n조회 내역을 사용하시려면 직접입력 체크박스를 해제해주십시오.'/>");
			return;
		}
	}
	...
	function setValue(){
		...
		if( dtlAddr.length < 1 ){
			alert("<msg:txt mid='109512' mdef='상세주소를 입력하여 주십시오.'/>");
		...
		*/
		...
		if( $("#notFindAddrStatus").is(":checked") ){
			...
			if( resZipCode == null || resZipCode == "" || resZipCode.length < 7){
				alert("<msg:txt mid='109983' mdef='우편번호를 입력하여 주십시오.'/>");
				$("#zipCode").focus();
				return;
			}

			if( resDoroFullAddr == null || resDoroFullAddr == "" ){
				alert("<msg:txt mid='110129' mdef='주소를 입력하여 주십시오.'/>");
				$("#dtlAddr").focus();
				return;
			}
...
<body class="bodywrap" onLoad="engCheck();">
    <div class="wrapper">
        <div class="popup_title">
            <ul>
                <li><tit:txt mid='srchAddr' mdef='주소조회'/></li>
                <li class="close"></li>
            </ul>
        </div>
        <div class="popup_main">
            <form id="srchFrm" name="srchFrm">
                ...
                <div class="outer">
                	<table>
                		<tr>
                			<td><span><tit:txt mid='112401' mdef='* 검색방법'/></span><br/>
			...
			<div class="inner">
				...
				<ul>
					<li id="txt" class="txt"><tit:txt mid='srchAddr' mdef='주소조회'/></li>
				</ul>
				</div>
			</div>
			...
			<ul>
				<li>
					<a href="javascript:setValue();" class="pink large"><tit:txt mid='113113' mdef='확인1'/></a>
