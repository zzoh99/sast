...
<c:if test="${ !empty fConfig && fConfig ne '' }">
		...
		// 업로드 컨트롤 초기화
		$("#myUpload").IBUpload({
			uploadServerUrl : "/fileuploadJFileUpload.do?cmd=ibFileUpload&"+$("#uploadForm").serialize(),
			...
			onUploadFinish: function(serverResponseObject , replacedResponseObject){
				if(serverResponseObject.code == "success" && serverResponseObject.data.length){
					...
					let fileInfoArr = getFileInfo();
					// fileInfoArr에 추가
					if(serverResponseObject.data.length > 0) {
						const files = serverResponseObject.data;
						files.forEach(item => {
					...
					setFileInfo(JSON.stringify(fileInfoArr));

					doIBUAction("search");
					$("#myUpload").IBUpload("uploadServerUrl", "/fileuploadJFileUpload.do?cmd=ibFileUpload&"+$("#uploadForm").serialize());
				}
			},
				...
				if (fileList.length > 0 && confirm("선택된 파일을 삭제를 하시겠습니까?")) {
					...
					for (i = fileList.length - 1; i >= 0; i--) { // 반드시 역순으로 돌아야 제거 직후에 자료가 밀리거  당겨지는 일이 없다.
						if ($myUpload.IBUpload('selectedIndex', {index: i}) == true) { // jpg 파일들 중에서 선택된 파일들만 모두 제거
							...
							if(fileList[i].seqNo === 'temp') {
								const index = fileInfo.findIndex(obj => obj.savedName === fileList[i].savedName);
								if (index > -1) {
									fileInfo.splice(index, 1);
								}
							} else {
								params.push(fileList[i]);
								fileInfo = fileInfo.map(obj => {
									if(obj.savedName === fileList[i].savedName) {
										return {...obj, status: "D"}
									} else {
										return obj
									}
					...
					if(params.length > 0) {
						$.ibFileDelete($uploadForm.find("#uploadType").val(), params, function (code, message) {
							if(code == "success") {
					...
					// 변경된 상태 값 수정하여 fileInfo 값 재설정
					setFileInfo(JSON.stringify(fileInfo));
				}
				break;
			...
			case "download":
				...
				for (i = fileList.length - 1; i >= 0; i--) { // 반드시 역순으로 돌아야 제거 직후에 자료가 밀리거나 당겨지는 일이 없다.
					if ($myUpload.IBUpload('selectedIndex', {index: i})) { // jpg 파일들 중에서 선택된 파일들만 모두 제거
						...
						item.sFileNm = fileList[i].savedName;
						item.rFileNm = fileList[i].name;
				...
				if ($uploadForm.find("#fileSeq").val() != null && $uploadForm.find("#fileSeq").val() != "") {
					$.ajax({
						...
						, success: function(result) {
							var searchFile = [];
							var fileInfoArr = [];
							fileInfoArr = getFileInfo();

							if(result.data != null && result.data.length > 0) {
								// fileList 목록을 파일 목록에 추가
								const newFiles = fileInfoArr.filter(fileInfo =>
										!result.data.some(item => item.savedName === fileInfo.savedName)
								);
								searchFile.push(...newFiles);
								...
								$(result.data).each(function(idx,item) {
									...
									for (let i = 0; i < fileInfoArr.length; i++) {
										const fileInfo = fileInfoArr[i];
										if (fileInfo.savedName === item.savedName ) {
											exist = true;
									...
									// 사용자가 삭제 처리한 파일은 파일 목록에서 제거
									if(item.status !== "D")
										searchFile.push(item);

									// 신규 파일 fileInfo 변수에 추가
									if(!exist)
			...
			case "loadView":
				...
				if (selectedFile) {
					...
					if (imageExtensions.includes(fileExt)) {
						var param = "?enterCd=" + selectedFile.enterCd +
									"&reqSeq=" + selectedFile.seqNo +
									"&fileSeq=" + selectedFile.fileSeq +
									"&fileExt=" + fileExt +
									"&sFileNm=" + selectedFile.savedName +
									"&rFileNm=" + selectedFile.name;
						var fileURL = encodeURIComponent("${ctx}/AttachFileView.do" + param);
	...
	function getUploadType() {
		return $("#uploadForm>#uploadType").val();
	}
	...
	function getFileInfo() {
		if($("#uploadForm>#fileInfo").val() !== '')
			return JSON.parse($("#uploadForm>#fileInfo").val())
		else
			return [];
	}

	function setFileInfo(data) {
		$("#uploadForm>#fileInfo").val(data);
	}
</script>
</head>
<body>
	<div id="fileDiv">
		<form id="uploadForm" name="uploadForm">
			...
			<input id="fileInfo" name="fileInfo" type="hidden" value="${param.fileInfo}"/>
		</form>
