...
<%
	...
	/* 20210112 개발용 발급*/
	//String confmKey = "devU01TX0FVVEgyMDIxMDExMTExMzMwMjExMDY1MDE="; // 검색API 승인키
	String domain = "www.juso.go.kr"; // 인터넷망
...
<script type="text/javascript" src="./js/addrlink.js">
...
<link rel="stylesheet" type="text/css" href="/common/css/addrlink.css"/>
<script src="${ctx}/common/js/addrlink.js" type="text/javascript" charset="utf-8"></script>
...
var p = eval("${popUpStatus}");
	...
	function searchUrlJuso(){
		...
		// 인터넷망
		var callUrl = window.location.protocol + "//" + "www.juso.go.kr/addrlink/addrLinkApiJsonp.do";
	...
	function setParent(){
		...
		if(p.window.opener == null || p.window.opener == undefined) {
	 	...
	 	//[표준HR버전 수정] Array에 담아서 등록하는 방식으로 수정
		var arrayList = new Array();
        arrayList["zip"] = rtZipNo;
        arrayList["doroAddr"] = rtAddrPart1+" "+rtAddrPart2;
        arrayList["detailAddr"] = rtAddrDetail;
        arrayList["doroFullAddr"] = doroFullAddr;
        arrayList["resDoroFullAddr"] = resDoroFullAddr;

        arrayList["doroFullAddrEng"] =doroFullAddrEng;
		...
		p.window.opener.getReturnValue(returnValue);
		p.self.close();
	...
	function init(){
		...
		//[표준HR버전 수정] getSystemStdData → getSystemStdValue
		var license = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList&searchStdCd=ZIPCODE_LICENSE", "queryId=getSystemStdValue",false).codeList, "");
