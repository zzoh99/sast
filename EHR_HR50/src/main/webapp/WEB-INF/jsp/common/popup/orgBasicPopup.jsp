...
<script type="text/javascript">
	...
	$(function() {
		...
		modal = window.top.document.LayerModalUtility.getModal('searchOrgBasicPopupLayer');
		...
		if( modal != undefined ) {
			...
			chkVisualYn= modal.parameters.chkVisualYn;

			if (chkVisualYn != null && chkVisualYn != "") {
				$("#chkVisualYn").val(chkVisualYn);
			}
			
			if( modal.parameters.multiSelect != null && modal.parameters.multiSelect != undefined ) {
				multiSelect = modal.parameters.multiSelect;
			}
			if( modal.parameters.chooseOrgCds != null && modal.parameters.chooseOrgCds != undefined ) {
				chooseOrgCds = modal.parameters.chooseOrgCds;
			}
			if( modal.parameters.baseDate != null && modal.parameters.baseDate != undefined ) {
				$("#searchBaseDate").val(modal.parameters.baseDate);
			}
			if( modal.parameters.searchOrgNm != null && modal.parameters.searchOrgNm != undefined ) {
				$("#searchOrgNm").val(modal.parameters.searchOrgNm);
			}
		}
		...
		mySheet.SetSheetHeight(sheetHeigth);

	    $("#searchBaseDate").datepicker2();
	    
	    //$("#searchOrgSchemeUseYn").attr('checked', 'checked');

	    doAction("Search");

	    $(".close").click(function() {
			closeCommonLayer('searchOrgBasicPopupLayer');
	    });
	...
	function mySheet_OnDblClick(Row, Col){
		...
		var rtnModal = window.top.document.LayerModalUtility.getModal('searchOrgBasicPopupLayer');
		rtnModal.fire("searchOrgBasicPopupTrigger", rv).hide();
	}
...
<body class="bodywrap">
				...
				<tr>
                    ...
                    <th><tit:txt mid='104514' mdef='조직명'/></th>
				   	...
				   	<th>현조직도사용여부</th>
						...
						<input id="searchOrgSchemeUseYn" name="searchOrgSchemeUseYn" type="checkbox" class="checkbox"/>
						<input type="hidden" id="searchOrgSchemeUse" name="searchOrgSchemeUse" value="" />
					</td>                       
					<td>
	...
	<div class="modal_footer">
		<a href="javascript:setValue();" id="btn_complete" class="btn filled">선택완료</a>
		<a href="javascript:closeCommonLayer('searchOrgBasicPopupLayer');" class="btn outline_gray">닫기</a>
	</div>
</div>
</body>
</html>
