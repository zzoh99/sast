...
<script type="text/javascript">
	$(function() {
		...
		const modal = window.top.document.LayerModalUtility.getModal('payDayLayer');
		...
		// 급여코드
        var payCdList = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList","queryId=getCpnPayCdList&searchRunType="+$("#multiRunType").val(),false).codeList, ("${ssnLocaleCd}" != "en_US" ? "<tit:txt mid='103895' mdef='전체'/>" : "All"));
        sheet1.SetColProperty("payCd", {ComboText:"|"+payCdList[0], ComboCode:"|"+payCdList[1]} );

        // 조회조건
		var searchPayCdList = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList","queryId=getCpnPayCdList&searchRunType="+$("#multiRunType").val(),false).codeList, ("${ssnLocaleCd}" != "en_US" ? "<tit:txt mid='103895' mdef='전체'/>" : "All"));
	...
	function sheet1_OnDblClick(Row, Col, Value, CellX, CellY, CellW, CellH) {
		...
		modal.fire('payDayTrigger', {
			...
			, payYm : sheet1.GetCellValue(Row, "payYm")
			, paymentYmd : sheet1.GetCellValue(Row, "paymentYmd")
			, paymentYmdHyphen : sheet1.GetCellValue(Row, "paymentYmdHyphen")
			, payNm : sheet1.GetCellValue(Row, "payNm")
			, ordSymd : sheet1.GetCellValue(Row, "ordSymd")
			, ordEymd : sheet1.GetCellValue(Row, "ordEymd")
			, closeYn : sheet1.GetCellValue(Row, "closeYn")
		}).hide();
	}
</script>
</head>
<body class="bodywrap">
	<div class="wrapper modal_layer">
		...
		<div class="modal_footer">
			<btn:a href="javascript:closeCommonLayer('payDayLayer');" css="btn outline_gray" mid='110881' mdef="닫기"/>
		</div>
	</div>
</body>
</html>
