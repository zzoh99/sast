...
<script type="text/javascript">
	...
	var engAddrUseStatus = null
	$(function() {

		const modal = window.top.document.LayerModalUtility.getModal('zipCodeLayer');
		engAddrUseStatus = modal.parameters.engAddrUseStatus
		engCheck();

		createIBSheet3(document.getElementById('zipCodeLayerYSht1-wrap'), "zipCodeLayerYSht1", "100%", "100%","${ssnLocaleCd}");
		...
		]; IBS_InitSheet(zipCodeLayerYSht1, initdata); zipCodeLayerYSht1.SetCountPosition(4); zipCodeLayerYSht1.SetEditable(false);zipCodeLayerYSht1.SetEditableColorDiff(0); //편집불가 상관없이 기본색상 출력

		// sheet 높이 계산
		var sheetHeight = $(".modal_body").height() - $("#form").height() - $(".sheet_title").height() -  $(".outer").height() - 2;
		zipCodeLayerYSht1.SetSheetHeight(sheetHeight);
	...
	function doAction(sAction) {
		...
		case "Search": 	 	var param = {"Param":"cmd=getZipCodePopupDoroList&defaultRow=20&"+$("#form").serialize()};
			zipCodeLayerYSht1.DoSearchPaging( "${ctx}/ZipCodePopup.do", param ); break;
		}
	}

	// 	조회 후 에러 메시지
	function zipCodeLayerYSht1_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			if(Msg != "") alert(Msg);
			sheetResize();
		} catch (ex) {
	...
	function zipCodeLayerYSht1_OnClick(Row, Col){
		try{
			setAddr();
		} catch(ex){
	...
	function setAddr(){
		...
		if( !$("#notFindAddrStatus").is(":checked") ){
			$("#zipCode").val(zipCodeLayerYSht1.GetCellValue(zipCodeLayerYSht1.GetSelectRow(), "zipcode"));
			$("#doroFullAddr").val(zipCodeLayerYSht1.GetCellValue(zipCodeLayerYSht1.GetSelectRow(), "jusoS"));
			$("#doroAddr").val(zipCodeLayerYSht1.GetCellValue(zipCodeLayerYSht1.GetSelectRow(), "juso"));
			var addrNote = "";
			var lawDongName = zipCodeLayerYSht1.GetCellValue(zipCodeLayerYSht1.GetSelectRow(), "lawDongName");
			var sigungubdName = zipCodeLayerYSht1.GetCellValue(zipCodeLayerYSht1.GetSelectRow(), "sigungubdName");
			if(lawDongName != ""){
			...
			$("#doroFullAddrEng").val(zipCodeLayerYSht1.GetCellValue(zipCodeLayerYSht1.GetSelectRow(), "jusoE"));
	...
	function setValue(){
		...
		let detailAddr = resDtlAddr;
		if("${engAddrUseStatus}" == "Y")
			detailAddr = resDtlAddrEng;

		var param = {
				zip: resZipCode,
				doroAddr: resDoroFullAddr,
				doroFullAddr: resDoroFullAddr,
				doroFullAddrEng: resDoroFullAddrEng,
				resDoroFullAddrEng: resDoroFullAddrEng,
				detailAddr: detailAddr
			};
		let modal = window.top.document.LayerModalUtility.getModal('zipCodeLayer');
        modal.fire('zipCodeLayerTrigger', param).hide();
	}
...
<body class="bodywrap" onLoad="engCheck();">
	<div class="wrapper modal_layer">
		<div class="modal_body">
			...
			<div id="zipCodeLayerYSht1-wrap"></div>
		...
		<div class="modal_footer">
			...
			<a href="javascript:closeCommonLayer('zipCodeLayer');" class="btn outline_gray">닫기</a>
		</div>
	</div>
</body>
</html>