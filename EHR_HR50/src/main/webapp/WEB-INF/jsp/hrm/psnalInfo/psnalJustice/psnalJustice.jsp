...
<script type="text/javascript">
	...
	$(function() {
		...
		prizeList = ajaxCall("/CommonCode.do?cmd=getCommonNSCodeList&searchGrcodeCd=H20250", "queryId=getComCodeNoteList", false) ;
		punishList = ajaxCall("/CommonCode.do?cmd=getCommonNSCodeList&searchGrcodeCd=H20270", "queryId=getComCodeNoteList", false) ;
		...
		EMP_INFO_CHANGE_TABLE_SHEET["thrm128"] = sheet1;
		EMP_INFO_CHANGE_TABLE_SHEET["thrm129"] = sheet2;
		
		var initdata1 = {};
		initdata1.Cfg = {SearchMode:smLazyLoad,MergeSheet:0,Page:22,FrozenCol:8,DataRowMerge:0};
	...
	function doAction1(sAction) {
		...
		case "Search":
			...
			sheet1.DoSearch( "${ctx}/PsnalJustice.do?cmd=getPsnalJusticePrizeList", param );
			break;
		case "Save":
			setFileListArr('sheet1');
			IBS_SaveName(document.sheet1Form,sheet1);
			sheet1.DoSave( "${ctx}/PsnalJustice.do?cmd=savePsnalJusticePrize", $("#sheet1Form").serialize());
			break;
		case "Insert":
	...
	function doAction2(sAction) {
		...
		case "Search":
			...
			var param = "sabun="+$("#hdnSabun").val()+"&searchUserEnterCd="+$("#hdnEnterCd").val();
			sheet2.DoSearch( "${ctx}/PsnalJustice.do?cmd=getPsnalJusticePunishList", param );
			break;
		case "Save":
			setFileListArr('sheet2');
			IBS_SaveName(document.sheet1Form,sheet2);
			sheet2.DoSave( "${ctx}/PsnalJustice.do?cmd=savePsnalJusticePunish", $("#sheet1Form").serialize());
			break;
		case "Insert":
	...
	function sheet1_OnChange(Row, Col, Value) {
		try{
			if( sheet1.ColSaveName(Col) == "prizeCd" ) {
				if( prizeList != null ) {
					for( var i = 0; i < prizeList.codeList.length; i++) {
						if( prizeList.codeList[i].code == sheet1.GetCellValue(Row, "prizeCd") ) {
							sheet1.SetCellValue(Row, "sPoint", prizeList.codeList[i].note1) ;
							sheet1.SetCellValue(Row, "sgPoint", prizeList.codeList[i].note2) ;
							break;
						}
					}
				}
	...
	function sheet2_OnChange(Row, Col, Value) {
		try{
			...
			if( sheet2.ColSaveName(Col) == "punishCd" ) {
				if( punishList != null ) {
					for( var i = 0; i < punishList.codeList.length; i++) {
						if( punishList.codeList[i].code == sheet2.GetCellValue(Row, "punishCd") ) {
							sheet2.SetCellValue(Row, "sPoint", punishList.codeList[i].note1) ;
							sheet2.SetCellValue(Row, "sgPoint", punishList.codeList[i].note2) ;
							break;
						}
					}
				}
    ...
    function fileMgrPopup1(Row, Col) {
        ...
        let layerModal = new window.top.document.LayerModal({
              id : 'fileMgrLayer'
            , url : 'fileuploadJFileUpload.do?cmd=viewIbFileMgrLayer&authPg=${authPg}&uploadType=justice'
            , parameters : {
