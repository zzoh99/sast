...
<script type="text/javascript">
	...
	$(function() {
		const modal = window.top.document.LayerModalUtility.getModal('psnalBasicLayer');
		...
		createIBSheet3(document.getElementById('psnalBasicLayerSheet-wrap'), "psnalBasicLayerSheet", "0%", "0%", "${ssnLocaleCd}");
		...
		EMP_INFO_CHANGE_TABLE_SHEET["thrm100"] = psnalBasicLayerSheet;
		...
		]; IBS_InitSheet(psnalBasicLayerSheet, initdata1);psnalBasicLayerSheet.SetEditable(false);psnalBasicLayerSheet.SetVisible(true);psnalBasicLayerSheet.SetCountPosition(4);

		let sheetHeight = $(".modal_body").height() - $(".sheet_title").height() - 2;
		psnalBasicLayerSheet.SetSheetHeight(sheetHeight);
		...
		psnalBasicLayerSheet.SetColProperty("base1Cd", 			{ComboText:"|"+userCd11[0], ComboCode:"|"+userCd11[1]} );
	...
	function doAction1(sAction) {
		...
		case "Search":
			...
			psnalBasicLayerSheet.DoSearch( "${ctx}/PsnalBasic.do?cmd=getPsnalBasicList", param);
			break;
...
<c:if test="${authPg == 'A'}">
		case "Save":
			...
			setSheetData();
			IBS_SaveName(document.infoFrom,psnalBasicLayerSheet);
			psnalBasicLayerSheet.DoSave( "${ctx}/PsnalBasic.do?cmd=updatePsnalBasic", $("#infoFrom").serialize());
			break;
</c:if>
		}
	}
	...
	// 조회 후 에러 메시지
	function psnalBasicLayerSheet_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			if (Msg != "") {
				alert(Msg);
			}
	...
	// 저장 후 메시지
	function psnalBasicLayerSheet_OnSaveEnd(Code, Msg, StCode, StMsg) {
		try {
			if (Msg != "") {
				alert(Msg);
			}
	...
	function getSheetData() {
		...
		var row = psnalBasicLayerSheet.LastRow();
		...
		//기본사항
		$('#name').val(psnalBasicLayerSheet.GetCellValue(row,"name"));
		$('#nameCn').val(psnalBasicLayerSheet.GetCellValue(row,"nameCn"));
		$('#nameUs').val(psnalBasicLayerSheet.GetCellValue(row,"nameUs"));
		$('#gempYmd').val(psnalBasicLayerSheet.GetCellText(row,"gempYmd"));
		$('#empYmd').val(psnalBasicLayerSheet.GetCellText(row,"empYmd"));
		$('#workYymmCnt').val(psnalBasicLayerSheet.GetCellValue(row,"workYymmCnt"));
		$('#totAgreeYymmCnt').text(psnalBasicLayerSheet.GetCellValue(row,"totAgreeYymmCnt"));
		$('#allCareerYymmCnt').text(psnalBasicLayerSheet.GetCellValue(row,"allCareerYymmCnt"));
		$('#stfType').val(psnalBasicLayerSheet.GetCellValue(row,"stfType"));
		$('#stfTypeNm').val(psnalBasicLayerSheet.GetCellValue(row,"stfTypeNm"));
		$('#empType').val(psnalBasicLayerSheet.GetCellValue(row,"empType"));
		$('#empTypeNm').val(psnalBasicLayerSheet.GetCellValue(row,"empTypeNm"));
		$('#traYmd').val(psnalBasicLayerSheet.GetCellText(row,"traYmd"));
		$('#retYmd').val(psnalBasicLayerSheet.GetCellText(row,"retYmd"));
		$('#resignReasonCd').val(psnalBasicLayerSheet.GetCellText(row,"resignReasonCd"));
		$('#resignReasonNm').val(psnalBasicLayerSheet.GetCellText(row,"resignReasonNm"));
		//$('#endWorkDate').text(psnalBasicLayerSheet.GetCellText(row,"endWorkDate"));

		if($("#hdnAuthPg").val() == 'A') {
			$('#resNo1').val(psnalBasicLayerSheet.GetCellValue(row,"resNo").substring(0,6));
			$('#resNo2').val(psnalBasicLayerSheet.GetCellValue(row,"resNo").substring(6,13));
		} else {
			$('#resNo1').val(psnalBasicLayerSheet.GetCellValue(row,"resNo").substring(0,6));
			$('#resNo2').val("*******");
		}

		$('#sexType').val(psnalBasicLayerSheet.GetCellValue(row,"sexType"));
		$('#sexTypeNm').val(psnalBasicLayerSheet.GetCellValue(row,"sexTypeNm"));
		$('#birYmd').val(psnalBasicLayerSheet.GetCellText(row,"birYmd"));
		$('#lunType').val(psnalBasicLayerSheet.GetCellValue(row,"lunType"));
		$('#lunTypeNm').val(psnalBasicLayerSheet.GetCellValue(row,"lunTypeNm"));
		$('#bloodCd').val(psnalBasicLayerSheet.GetCellValue(row,"bloodCd"));
		$('#bloodNm').val(psnalBasicLayerSheet.GetCellValue(row,"bloodNm"));
		$('#wedYn').val(psnalBasicLayerSheet.GetCellValue(row,"wedYn"));
		$('#wedYnNm').val(psnalBasicLayerSheet.GetCellValue(row,"wedYnNm"));
		$('#wedYmd').val(psnalBasicLayerSheet.GetCellText(row,"wedYmd"));
		$('#foreignYn').val(psnalBasicLayerSheet.GetCellValue(row,"foreignYn"));
		$('#nationalCd').val(psnalBasicLayerSheet.GetCellValue(row,"nationalCd"));
		$('#nationalNm').val(psnalBasicLayerSheet.GetCellValue(row,"nationalNm"));
		$('#relCd').val(psnalBasicLayerSheet.GetCellValue(row,"relCd"));
		$('#relNm').val(psnalBasicLayerSheet.GetCellValue(row,"relNm"));
		$('#hobby').val(psnalBasicLayerSheet.GetCellValue(row,"hobby"));
		$('#specialityNote').val(psnalBasicLayerSheet.GetCellValue(row,"specialityNote"));
		$('#base1Nm').val(psnalBasicLayerSheet.GetCellValue(row,"base1Nm"));
		$('#firYmd'			).val(psnalBasicLayerSheet.GetCellText(row,"firYmd"));			// "최초입사일",
		$('#jikgubYmd'		).val(psnalBasicLayerSheet.GetCellText(row,"jikgubYmd"));			// "직급승진일(직급기산일)",
		$('#orgMoveYmd'		).val(psnalBasicLayerSheet.GetCellText(row,"orgMoveYmd"));		// "부서배치일",
		$('#yearYmd'		).val(psnalBasicLayerSheet.GetCellText(row,"yearYmd"));			// "연차기산일",
		$('#rmidYmd'		).val(psnalBasicLayerSheet.GetCellText(row,"rmidYmd"));			// "퇴직금기산일",
		$('#firRmidYmd'		).val(psnalBasicLayerSheet.GetCellText(row,"firRmidYmd"));		// "최초퇴직금기산일",
		$('#jobYmd'			).val(psnalBasicLayerSheet.GetCellText(row,"jobYmd"));			// "직무담당일(조회성)",
		$('#positionYmd'	).val(psnalBasicLayerSheet.GetCellText(row,"positionYmd"));		// "보직임명일(조회성)",
		$('#executiveYmd'	).val(psnalBasicLayerSheet.GetCellText(row,"executiveYmd"));		// "임원선임일",
		$('#conRYmd'		).val(psnalBasicLayerSheet.GetCellText(row,"conRYmd"));			// "계약시작일",
		$('#conEYmd'		).val(psnalBasicLayerSheet.GetCellText(row,"conEYmd"));			// "계약만료일",
		$('#bloodCd'		).val(psnalBasicLayerSheet.GetCellValue(row,"bloodCd"));			// "혈액형(SELECTBOX)
		$('#ht'				).val(psnalBasicLayerSheet.GetCellValue(row,"ht"));				// "신장",
		$('#wt'				).val(psnalBasicLayerSheet.GetCellValue(row,"wt"));				// "체중",
		$('#hobby'			).val(psnalBasicLayerSheet.GetCellValue(row,"hobby"));			// "취미",
		$('#specialityNote'	).val(psnalBasicLayerSheet.GetCellValue(row,"specialityNote"));	// "특기",
		$('#relNm'			).val(psnalBasicLayerSheet.GetCellValue(row,"relNm"));			// "종교",
		$('#eyeL'			).val(psnalBasicLayerSheet.GetCellText(row,"eyeL"));				// "시력 좌",
		$('#eyeR'			).val(psnalBasicLayerSheet.GetCellText(row,"eyeR"));				// "시력 우",

		// JY 추가
		$('#empAcaCd'			).val(psnalBasicLayerSheet.GetCellValue(row,"empAcaCd"));			// "입사시학력",
		$('#finalAcaCd'			).val(psnalBasicLayerSheet.GetCellValue(row,"finalAcaCd"));		// "최종학력",
		$('#finalSchNm'			).val(psnalBasicLayerSheet.GetCellValue(row,"finalSchNm"));		// "최종학교명",
		$('#finalAcamajNm'		).val(psnalBasicLayerSheet.GetCellValue(row,"finalAcamajNm"));	// "최종전공명",
		$('#base1Cd'			).val(psnalBasicLayerSheet.GetCellText(row,"base1Cd"));			// 입사인정경력(직급명)
		$('#ftWorkYmd'			).val(psnalBasicLayerSheet.GetCellText(row,"ftWorkYmd"));			// 정규직전환일
		$('#orgYmd'				).val(formatDate(psnalBasicLayerSheet.GetCellValue(row,"orgYmd"),"-"));		// "현 부서배치일",
		$('#peakYn'				).val(psnalBasicLayerSheet.GetCellValue(row,"peakYn"));			// "임금피크제 여부",
		$('#peakYmd'			).val(psnalBasicLayerSheet.GetCellValue(row,"peakYmd"));			// "임금피크제 적용일",
		$('#bohunYn'			).val(psnalBasicLayerSheet.GetCellValue(row,"bohunYn"));			// "보훈여부"
		$('#jangYn'				).val(psnalBasicLayerSheet.GetCellValue(row,"jangYn"));			// "장애인여부"
		$('#unionYn'			).val(psnalBasicLayerSheet.GetCellValue(row,"unionYn"));			// "노조가입여부(조회성)",
		$('#empAccYn'			).val(psnalBasicLayerSheet.GetCellValue(row,"empAccYn"));			// "사우회가입여부",
		$('#clubYn'				).val(psnalBasicLayerSheet.GetCellValue(row,"clubYn"));			// "동호회가입여부(조회성)",
		$('#gpAddr'				).val(psnalBasicLayerSheet.GetCellValue(row,"gpAddr"));			// "비상연락망(관계)",
		$('#scAddr'				).val(psnalBasicLayerSheet.GetCellValue(row,"scAddr"));			// "비상연락망(연락처)",
		$('#nmAddr'				).val(psnalBasicLayerSheet.GetCellValue(row,"nmAddr"));			// "비상연락망(성명)",
		$('#imAddr'				).val(psnalBasicLayerSheet.GetCellValue(row,"imAddr"));			// "회사이메일",
		$('#base1Cd'			).val(psnalBasicLayerSheet.GetCellValue(row,"base1Cd"));			// "입사인정경력",
		$('#base1CdNm'			).val(psnalBasicLayerSheet.GetCellValue(row,"base1CdNm"));		// "입사인정경력명",
		$('#age'				).val(psnalBasicLayerSheet.GetCellValue(row,"age"));		// "입사인정경력명",
		$('#rk'				).val(psnalBasicLayerSheet.GetCellValue(row,"rk"));						// "나이",
	}
	...
	function setSheetData() {
		...
		//기본사항
		psnalBasicLayerSheet.SetCellValue(row,"name",$('#name').val());
		psnalBasicLayerSheet.SetCellValue(row,"nameCn",$('#nameCn').val());
		psnalBasicLayerSheet.SetCellValue(row,"nameUs",$('#nameUs').val());
		psnalBasicLayerSheet.SetCellValue(row,"gempYmd",formatDate($('#gempYmd').val(),""));
		psnalBasicLayerSheet.SetCellValue(row,"empYmd",formatDate($('#empYmd').val(),""));
		psnalBasicLayerSheet.SetCellValue(row,"stfType", $('#stfType').val());
		psnalBasicLayerSheet.SetCellValue(row,"stfTypeNm", $('#stfTypeNm').val());
		psnalBasicLayerSheet.SetCellValue(row,"empType", $('#empType').val());
		psnalBasicLayerSheet.SetCellValue(row,"empTypeNm", $('#empTypeNm').val());
		psnalBasicLayerSheet.SetCellValue(row,"traYmd",formatDate($('#traYmd').val(),""));
		psnalBasicLayerSheet.SetCellValue(row,"retYmd",formatDate($('#retYmd').val(),""));
		psnalBasicLayerSheet.SetCellValue(row,"resignReasonCd",$('#resignReasonCd').val());
		psnalBasicLayerSheet.SetCellValue(row,"resignReasonNm",$('#resignReasonNm').val());
		psnalBasicLayerSheet.SetCellValue(row,"resNo",$('#resNo1').val()+$('#resNo2').val());
		psnalBasicLayerSheet.SetCellValue(row,"sexType",$('#sexType').val());
		psnalBasicLayerSheet.SetCellValue(row,"birYmd",formatDate($('#birYmd').val(),""));
		psnalBasicLayerSheet.SetCellValue(row,"lunType",$('#lunType').val());
		psnalBasicLayerSheet.SetCellValue(row,"bloodCd", $('#bloodCd').val());
		psnalBasicLayerSheet.SetCellValue(row,"wedYn", $('#wedYn').val());
		psnalBasicLayerSheet.SetCellValue(row,"wedYmd", formatDate($('#wedYmd').val(),""));
		psnalBasicLayerSheet.SetCellValue(row,"foreignYn", $('#foreignYn').val());
		psnalBasicLayerSheet.SetCellValue(row,"nationalCd",$('#nationalCd').val());
		psnalBasicLayerSheet.SetCellValue(row,"nationalNm",$('#nationalNm').val());
		psnalBasicLayerSheet.SetCellValue(row,"relCd", $('#relCd').val());
		psnalBasicLayerSheet.SetCellValue(row,"relNm", $('#relNm').val());
		psnalBasicLayerSheet.SetCellValue(row,"hobby", $('#hobby').val());
		psnalBasicLayerSheet.SetCellValue(row,"specialityNote", $('#specialityNote').val());
		psnalBasicLayerSheet.SetCellValue(row,"base1Cd", $('#base1Cd').val());
		psnalBasicLayerSheet.SetCellValue(row,"base1CdNm", $('#base1CdNm').val());
		psnalBasicLayerSheet.SetCellValue(row,"base1Nm", $('#base1Nm').val());

		psnalBasicLayerSheet.SetCellValue(row,"firYmd", 			formatDate($('#firYmd').val(),""));             //"최초입사일",
		psnalBasicLayerSheet.SetCellValue(row,"jikgubYmd", 		formatDate($('#jikgubYmd').val(),""));          //"직급승진일(직급기산일)",
		psnalBasicLayerSheet.SetCellValue(row,"orgMoveYmd", 		formatDate($('#orgMoveYmd').val(),""));         //"부서배치일",
		psnalBasicLayerSheet.SetCellValue(row,"yearYmd", 			formatDate($('#yearYmd').val(),""));            //"연차기산일",
		psnalBasicLayerSheet.SetCellValue(row,"rmidYmd", 			formatDate($('#rmidYmd').val(),""));            //"퇴직금기산일",
		psnalBasicLayerSheet.SetCellValue(row,"firRmidYmd", 		formatDate($('#firRmidYmd').val(),""));         //"최초퇴직금기산일",
		psnalBasicLayerSheet.SetCellValue(row,"jobYmd", 			formatDate($('#jobYmd').val(),""));             //"직무담당일(조회성)",
		psnalBasicLayerSheet.SetCellValue(row,"positionYmd", 		formatDate($('#positionYmd').val(),""));        //"보직임명일(조회성)",
		psnalBasicLayerSheet.SetCellValue(row,"executiveYmd", 	formatDate($('#executiveYmd').val(),""));       //"임원선임일",
		psnalBasicLayerSheet.SetCellValue(row,"conRYmd", 			formatDate($('#conRYmd').val(),""));            //"계약시작일",
		psnalBasicLayerSheet.SetCellValue(row,"conEYmd", 			formatDate($('#conEYmd').val(),""));            //"계약만료일",
		psnalBasicLayerSheet.SetCellValue(row,"bloodCd", 			$('#bloodCd').val());            //"혈액형(SELECTBOX)
		psnalBasicLayerSheet.SetCellValue(row,"ht", 				$('#ht').val());                 //"신장",
		psnalBasicLayerSheet.SetCellValue(row,"wt", 				$('#wt').val());                 //"체중",
		psnalBasicLayerSheet.SetCellValue(row,"hobby", 			$('#hobby').val());              //"취미",
		psnalBasicLayerSheet.SetCellValue(row,"specialityNote", 	$('#specialityNote').val());     //"특기",
		psnalBasicLayerSheet.SetCellValue(row,"relNm", 			$('#relNm').val());              //"종교",
		psnalBasicLayerSheet.SetCellValue(row,"eyeL", 			$('#eyeL').val());               //"시력 좌",
		psnalBasicLayerSheet.SetCellValue(row,"eyeR", 			$('#eyeR').val());               //"시력 우",

		// JY 추가
		psnalBasicLayerSheet.SetCellValue(row,"empAcaCd", 		$('#empAcaCd').val());             //"입사시학력",
		psnalBasicLayerSheet.SetCellValue(row,"finalAcaCd", 		$('#finalAcaCd').val());           //"최종학력",
		psnalBasicLayerSheet.SetCellValue(row,"finalSchNm", 		$('#finalSchNm').val());           //"최종학교명",
		psnalBasicLayerSheet.SetCellValue(row,"finalAcamajNm", 	$('#finalAcamajNm').val());        //"최종전공명",
		psnalBasicLayerSheet.SetCellValue(row,"base1CdNm", 		$('#base1CdNm').val());     	   //"입사인정경력(직급명)",
		psnalBasicLayerSheet.SetCellValue(row,"ftWorkYmd", 		$('#ftWorkYmd').val());     	   //"정규직전환일",
		psnalBasicLayerSheet.SetCellValue(row,"orgYmd", 			$('#orgYmd').val());     	 	   //"현 부서배치일",
		psnalBasicLayerSheet.SetCellValue(row,"peakYn", 			$('#peakYn').val());     	 	   //"임금피크제 여부",
		psnalBasicLayerSheet.SetCellValue(row,"peakYmd", 			$('#peakYmd').val());     	 	   //"임금피크제 적용일",
		psnalBasicLayerSheet.SetCellValue(row,"bohunYn", 			$('#bohunYn').val());     	 	   //"보훈여부",
		psnalBasicLayerSheet.SetCellValue(row,"jangYn", 			$('#jangYn').val());     	 	   //"장애인여부",
		psnalBasicLayerSheet.SetCellValue(row,"unionYn", 			$('#unionYn').val());              //"노조가입여부(조회성)",
		psnalBasicLayerSheet.SetCellValue(row,"empAccYn", 		$('#empAccYn').val());             //"사우회가입여부",
		psnalBasicLayerSheet.SetCellValue(row,"clubYn", 			$('#clubYn').val());               //"동호회가입여부(조회성)",
		psnalBasicLayerSheet.SetCellValue(row,"gpAddr", 			$('#gpAddr').val());               //"비상연락망(관계)",
		psnalBasicLayerSheet.SetCellValue(row,"scAddr", 			$('#scAddr').val());               //"비상연락망(연락처)",
		psnalBasicLayerSheet.SetCellValue(row,"nmAddr", 			$('#nmAddr').val());               //"비상연락망(성명)",
		psnalBasicLayerSheet.SetCellValue(row,"imAddr", 			$('#imAddr').val());               //"회사이메일",
		psnalBasicLayerSheet.SetCellValue(row,"age", 				$('#age').val());                  //"나이",

		psnalBasicLayerSheet.SetCellValue(row,"rk", $('#rk').val()); //RD 파라메터
	}
...
<c:if test="${authPg == 'A'}">
	...
	function rdPopup(param){
		...
		var sabun = psnalBasicLayerSheet.GetCellValue(psnalBasicLayerSheet.LastRow(),"sabun");
		if( "${ssnEnterCd}" != $("#hdnEnterCd").val() ) {
	...
	function copyStep(){
		...
		var args = new Array();
		args["searchResNo"] = psnalBasicLayerSheet.GetCellValue(1, "resNo");
...
<div class="wrapper modal_layer">
	<div class="modal_body">
				...
				<tr>
					...
					<c:choose>
						<c:when test="${authPg == 'A'}">
							<td><input id="name" name="name" type="text" class="${textCss} required w100p"
									   maxlength="30" ${readonly}></td>
						</c:when>
						<c:otherwise>
							<td><input id="name" name="name" type="text" class="${textCss} w100p"
									   maxlength="30" ${readonly}></td>
						</c:otherwise>
					</c:choose>
					<th><tit:txt mid='104533' mdef='영문성명'/></th>
					<td><input id="nameUs" name="nameUs" type="text" class="${textCss} w100p" maxlength="50" ${readonly}>
					</td>
					<th><tit:txt mid='104040' mdef='한자성명'/></th>
					<td><input id="nameCn" name="nameCn" type="text" class="${textCss} w100p" maxlength="30" ${readonly}>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input id="resNo1" name="resNo1" type="text" class="${textCss} date" maxlength="6" ${readonly}
							   style="width:45px;"> -
						<input id="resNo2" name="resNo2" type="text" class="${textCss} date" maxlength="7" ${readonly}
							   style="width:55px;">
					</td>
					...
					<td>
						...
						<c:choose>
							...
							<c:otherwise>
								...
								&nbsp;만 <input id="age" name="age" type="text" class="${textCss} w20" ${readonly}
											   readonly> 세
							</c:otherwise>
						</c:choose>
					</td>
				...
				<tr>
					...
					<td>
						<c:choose>
							...
							<c:otherwise>
								<input id="stfTypeNm" name="stfTypeNm" type="text" class="text transparent" readonly
									   style="width:30px">
						...
						<c:choose>
							...
							<c:otherwise>
								<input id="empTypeNm" name="empTypeNm" type="text" class="text transparent" readonly
									   style="width:100px">
					...
					<td>
						<c:choose>
							<c:when test="${authPg == 'A'}">
								...
								<input id="base1Nm" name="base1Nm" type="text" class="${dateCss}" ${readonly}
									   style="width:20px;" onKeyup="numberCheck(this);"> 년
							</c:when>
							<c:otherwise>
								<input id="base1CdNm" name="base1CdNm" type="text" class="text transparent" readonly
									   style="width:30px">
								<input id="base1Cd" name="base1Cd" type="hidden" readonly>
								<input id="base1Nm" name="base1Nm" type="text" class="text transparent" readonly
									   style="width:20px;"> 년
							</c:otherwise>
						</c:choose>
					</td>
				...
				<tr>
					...
					<td>
						<c:choose>
							...
							<c:otherwise>
								<input id="resignReasonCd" name="resignReasonCd" type="hidden"
									   class="${textCss}" ${readonly}>
		...
		<div class="hide">
			<%--		<script type="text/javascript"> createIBSheet("psnalBasicLayerSheet", "0", "0", "${ssnLocaleCd}"); </script>--%>
			<div id="psnalBasicLayerSheet-wrap"></div>
		</div>
	</div>
</div>
</body>
