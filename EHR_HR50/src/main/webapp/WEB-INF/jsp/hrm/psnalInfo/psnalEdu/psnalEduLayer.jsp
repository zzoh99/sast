...
<script type="text/javascript">
	...
	$(function() {
		const modal = window.top.document.LayerModalUtility.getModal('psnalEduLayer');
		...
		createIBSheet3(document.getElementById('psnalEduLayer-wrap'), "psnalEduLayer", "100%", "100%", "${ssnLocaleCd}");

		var enterCd = "&enterCd="+$("#hdnEnterCd").val();
		
		var initdata = {};
		initdata.Cfg = {FrozenCol:13,SearchMode:smLazyLoad,Page:22,MergeSheet:msHeaderOnly, AutoFitColWidth:'init|search|resize|rowtransaction'};
        ...
        ]; IBS_InitSheet(psnalEduLayer, initdata);psnalEduLayer.SetEditable("${editable}");psnalEduLayer.SetVisible(true);psnalEduLayer.SetCountPosition(4);psnalEduLayer.SetImageList(0,"${ctx}/common/images/icon/icon_popup.png");

        psnalEduLayer.SetEditableColorDiff(0); //편집불가 배경색 적용안함
		...
		psnalEduLayer.SetColProperty("inOutType", 		{ComboText:"|"+list4[0], ComboCode:"|"+list4[1]} );
		psnalEduLayer.SetColProperty("eduPlace", 		{ComboText:"|"+list5[0], ComboCode:"|"+list5[1]} );
		psnalEduLayer.SetColProperty("mandatoryYn",	{ComboText:"|YES|NO", ComboCode:"|Y|N"} );
		psnalEduLayer.SetColProperty("currencyCd", 	{ComboText:"|"+list7[0], ComboCode:"|"+list7[1]} );
		psnalEduLayer.SetColProperty("laborReturnYn", 	{ComboText:"|YES|NO", ComboCode:"|Y|N"} );
		psnalEduLayer.SetColProperty("eduRewardCd", 	{ComboText:"|"+list8[0], ComboCode:"|"+list8[1]} );
		psnalEduLayer.SetColProperty("eduConfirmType", {ComboText:("${ssnLocaleCd}" != "en_US" ? "수료|미수료" : "Completion|Non-completion"), ComboCode:"1|0"} );
		psnalEduLayer.SetColProperty("eduBranchCd", 	{ComboText:"|"+list1[0], ComboCode:"|"+list1[1]} );
		psnalEduLayer.SetColProperty("eduMethodCd", 	{ComboText:"|"+list3[0], ComboCode:"|"+list3[1]} );
		psnalEduLayer.SetColProperty("eduMBranchCd", 	{ComboText:"|"+list2[0], ComboCode:"|"+list2[1]} );
		psnalEduLayer.SetColProperty("eduAppLevel", 	{ComboText:"|"+list6[0], ComboCode:"|"+list6[1]} );
		psnalEduLayer.SetColProperty("certiYn", 		{ComboText:"YES|NO", ComboCode:"Y|N"} );
		psnalEduLayer.SetColProperty("allowanceYn", 	{ComboText:"YES|NO", ComboCode:"Y|N"} );
		psnalEduLayer.SetColProperty("receiptYn", 		{ComboText:"YES|NO", ComboCode:"Y|N"} );
		psnalEduLayer.SetColProperty("employmentYn", 	{ComboText:"YES|NO", ComboCode:"Y|N"} );

		let sheetHeight = $(".modal_body").height() - $(".sheet_title").height();
		psnalEduLayer.SetSheetHeight(sheetHeight);
	...
	function doAction1(sAction) {
		...
		case "Search":
			...
			+"&searchUserEnterCd="+$("#hdnEnterCd").val();
						/* +"&year="+$("#year").val(); */
			psnalEduLayer.DoSearch( "${ctx}/PsnalEdu.do?cmd=getPsnalEduList", param );
			break;
		case "Down2Excel":
			var downcol = makeHiddenSkipCol(psnalEduLayer);
			var param  = {DownCols:downcol,SheetDesign:1,Merge:1,ExcelFontSize:"9",ExcelRowHeight:"20"};
			psnalEduLayer.Down2Excel(param);
			break;
		}
	}

	// 조회 후 에러 메시지
	function psnalEduLayer_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			if (Msg != "") {
				alert(Msg);
			}
			/*
			$("#txtStdScore").text(psnalEduLayer.GetEtcData("stdScore"));
			$("#txtTotScore").text(psnalEduLayer.GetEtcData("totScore"));
			*/
			sheetResize();
		} catch (ex) {
	...
	// 저장 후 메시지
	function psnalEduLayer_OnSaveEnd(Code, Msg, StCode, StMsg) {
		try {
			if (Msg != "") {
				alert(Msg);
			}
	...
	function goPop(){
		...
		/*
		if(rv!=null){
			psnalEduLayer.SetCellValue(Row,"appOrgNm",(rv["orgNm"]));
			psnalEduLayer.SetCellValue(Row,"appOrgCd",(rv["orgCd"]));
		}
		*/
	}
</script>
...
<%--		<script type="text/javascript"> createIBSheet("psnalEduLayer", "100%", "100%", "${ssnLocaleCd}"); </script>--%>
		<div id="psnalEduLayer-wrap"></div>
	</div>
</div>
</body>
</html>
