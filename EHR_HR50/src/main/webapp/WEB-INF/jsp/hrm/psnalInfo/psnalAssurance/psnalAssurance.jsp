...
<script type="text/javascript">
	...
	$(function() {
		...
		EMP_INFO_CHANGE_TABLE_SHEET["thrm119"] = sheet1;
		//EMP_INFO_CHANGE_TABLE_SHEET["thrm119"] = sheet2;
		...
		var initdata2 = {};
		initdata2.Cfg = {FrozenCol:8,SearchMode:smLazyLoad,Page:22};
	...
	function doAction1(sAction) {
		...
		case "Search":
			...
			sheet1.DoSearch( "${ctx}/PsnalAssurance.do?cmd=getPsnalAssuranceWarrantyList", param );
			break;
		case "Save":
			if(!dupChk(sheet1,"sabun|type|warrantySYmd", true, true)){break;}
			setFileListArr('sheet1');
			IBS_SaveName(document.sheet1Form,sheet1);
			sheet1.DoSave( "${ctx}/PsnalAssurance.do?cmd=savePsnalAssuranceWarranty", $("#sheet1Form").serialize());
			break;
		case "Insert":
	...
	function doAction2(sAction) {
		...
		case "Search":
			...
			sheet2.DoSearch( "${ctx}/PsnalAssurance.do?cmd=getPsnalAssuranceWarrantyUserList", param );
			break;
		case "Save":
			setFileListArr('sheet2');
			if(!dupChk(sheet2,"sabun|type|warrantySYmd", true, true)){break;}
			IBS_SaveName(document.sheet1Form,sheet2);
			sheet2.DoSave( "${ctx}/PsnalAssurance.do?cmd=savePsnalAssuranceWarrantyUser", $("#sheet1Form").serialize());
			break;
		case "Insert":
			...
			sheet2.SetCellValue(row,"type","3");
			break;
		case "Copy":
			sheet2.DataCopy();
			break;
			...
			if(sheet2.ColSaveName(Col) == "zip") {
				...
				/* var postPopup = new daum.Postcode({
					oncomplete : function(data) {
						if(data.userSelectedType == "J" && data.userLanguageType == "K"){
							//지번주소 한글
						 	addr1 = data.jibunAddress;
						}else if(data.userSelectedType == "R" && data.userLanguageType == "K"){
							//도로명주소 한글
							...
							//지번주소 영문
							addr1 = data.jibunAddressEnglish;
						}else if(data.userSelectedType == "R" && data.userLanguageType == "E"){
							//도로명주소 영문
							addr1 = data.roadAddressEnglish;
						}

						sheet2.SetCellValue(Row, "zip", data.zonecode);
						sheet2.SetCellValue(Row, "addr1", addr1);
						sheet2.SetCellValue(Row, "addr2", "");
						//sheet2.SetCellValue(Row, "addrEng", data.addressEnglish);
					}
				}).open(); */
				...
				if(temp != null){
					sheet2.SetCellValue(Row, "zip", temp["zip"]);
					sheet2.SetCellValue(Row, "addr1", temp["resDoroFullAddr"]);
					sheet2.SetCellValue(Row, "addr2", "");
				}
				**/
			}
    ...
    function fileMgrPopup1(Row, Col) {
        ...
        let layerModal = new window.top.document.LayerModal({
              id : 'fileMgrLayer'
            , url : 'fileuploadJFileUpload.do?cmd=viewIbFileMgrLayer&authPg=${authPg}&uploadType=assurance'
            , parameters : {
