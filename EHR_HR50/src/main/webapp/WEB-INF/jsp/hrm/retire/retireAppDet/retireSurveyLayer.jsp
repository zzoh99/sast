...
<script type="text/javascript">
	var retireSurveyLayer = { id: 'retireSurveyLayer' };
	var answerCd = "";
	var authPg = "${authPg}";
	$(function() {
		const modal = window.top.document.LayerModalUtility.getModal(retireSurveyLayer.id);
		var { sabun, reqDate, applSeq } = modal.parameters;
		$("#sabun").val(sabun);
		$("#reqDate").val(reqDate);
		$("#applSeq").val(applSeq);

		createIBSheet3(document.getElementById('srvsheet1_wrap'), "srvsheet1", "100%", "100%", "${ssnLocaleCd}");
		createIBSheet3(document.getElementById('srvsheet2_wrap'), "srvsheet2", "100%", "100%", "${ssnLocaleCd}");
		//만족도
		answerCd = stfConvCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","H91920"), "");
		var initdata1 = {};
		initdata1.Cfg = {SearchMode:smLazyLoad,Page:22,MergeSheet:msPrevColumnMerge,AutoFitColWidth:'init|search|resize|rowtransaction'};
		...
		]; IBS_InitSheet(srvsheet1, initdata1);srvsheet1.SetEditable("${editable}");srvsheet1.SetVisible(true);
		
		var initdata2 = {};
		initdata2.Cfg = {SearchMode:smLazyLoad,Page:22,AutoFitColWidth:'init|search|resize|rowtransaction'};
		...
		]; IBS_InitSheet(srvsheet2, initdata2);srvsheet2.SetEditable("${editable}");srvsheet2.SetVisible(true);

		// var sheetHeight = $(".modal_body").height() - $("#srvsheet2Form").height() - $(".sheet_title").height() - 100;
		srvsheet1.SetSheetHeight("300");
		srvsheet2.SetSheetHeight("100");
	...
	function setValue() {
		...
		var rtn;
      	...
      	//저장
		try{
			saveStr = srvsheet1.GetSaveString(0);
			
			if(saveStr.match("KeyFieldError")) {
				return;
			}
			
			IBS_SaveName(document.srchFrm, srvsheet1);
			rtn = JSON.parse(srvsheet1.GetSaveData("${ctx}/RetireApp.do?cmd=saveRetireSurveyPopList", saveStr + "&"+ $("#srchFrm").serialize()));
			...
			if(rtn.Result.Code < 1) {
				...
				const modal = window.top.document.LayerModalUtility.getModal(retireSurveyLayer.id);
				const p = { surveyYn: 'Y' };
				modal.fire(retireSurveyLayer.id + 'Trigger', p).hide();
			}		
	...
	function checkList() {
		...
		var chkBool = false;
		for(var r = srvsheet1.HeaderRows(); r<srvsheet1.RowCount()+srvsheet1.HeaderRows(); r++){
			if(srvsheet1.GetCellValue(r,"answer") == "") {
				chkBool = true;
				break;
			}
		}
		...
		var chkBool_srvsheet2 = false;
		for(var r = srvsheet2.HeaderRows(); r<srvsheet2.RowCount()+srvsheet1.HeaderRows(); r++){
			if(srvsheet2.GetCellValue(r,"memo") == "") {
				chkBool_srvsheet2 = true;
				break;
			}
		}
		
		if(chkBool_srvsheet2) {
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search":
			srvsheet1.DoSearch( "${ctx}/RetireApp.do?cmd=getRetireSurveyPopList", $("#srchFrm").serialize() );
			break;
		}
	}
	...
	function doAction2(sAction) {
		switch (sAction) {
		case "Search":
			srvsheet2.DoSearch( "${ctx}/RetireApp.do?cmd=getRetireSurveyPopDisList", $("#srchFrm").serialize() );
			break;		
		case "Insert":
			var row = srvsheet1.GetSelectRow();
			//1.같은값이 있을경우 추가 안함
			var chkDup = false;
			for(var i = srvsheet2.HeaderRows(); i<srvsheet2.RowCount()+srvsheet2.HeaderRows(); i++){
				if(srvsheet2.GetCellValue(i,"gubun") == srvsheet1.GetCellValue(row,"gubun") && srvsheet2.GetCellValue(i,"question") == srvsheet1.GetCellValue(row,"question")) {
					chkDup = true;
					break;
				}
			}
			if(!chkDup) {
				var row2 = srvsheet2.DataInsert(0);
				srvsheet2.SetCellValue(row2,"gubun",srvsheet1.GetCellValue(row,"gubun"));
				srvsheet2.SetCellValue(row2,"question",srvsheet1.GetCellValue(row,"question"));
			}
	...
	// Sheet1 조회 후 에러 메시지
	function srvsheet1_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			if (Msg != "") {
				alert(Msg);
			}
			...
			for(var r = srvsheet1.HeaderRows(); r<srvsheet1.RowCount()+srvsheet1.HeaderRows(); r++){
		        ...
		        srvsheet1.InitCellProperty( r,"answer" ,info);
		        if(surveyMemo == "") {
		        	surveyMemo = srvsheet1.GetCellValue(r,"surveyMemo");
		        }
		        
		        if(srvsheet1.GetCellValue(r,"sStatus") == "R") {
					srvsheet1.SetCellValue(r,"sStatus","U");
				}
			}
	...
	// Sheet2 조회 후 에러 메시지
	function srvsheet2_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			if (Msg != "") {
				alert(Msg);
			}
	...
	//답변값 변경시 발생.
	function srvsheet1_OnChange(Row, Col, Value, OldValue, RaiseFlag) {
		try{
			if(srvsheet1.ColSaveName(Col) == "answer" && Row >= srvsheet1.HeaderRows()) {
				if(Value != OldValue) {
					...
					if(Value == "1" || Value == "2") {
						...
						srvsheet2.RowDelete(i,0);
						srvsheet1.SetCellValue(Row,"memo","");
					}
					*/
				}
			}
	...
	//답변값 변경시 발생.
	function srvsheet2_OnChange(Row, Col, Value, OldValue, RaiseFlag) {
		try{
			if(srvsheet2.ColSaveName(Col) == "memo" && Row >= srvsheet2.HeaderRows()) {
				if(Value != OldValue) {
					//srvsheet2의 사유 및 개선사항이 변경될경우 srvsheet1에 저장
					//1.같은값이 있을경우 추가 안함
					for(var i = srvsheet1.HeaderRows(); i<srvsheet1.RowCount()+srvsheet1.HeaderRows(); i++){
						if(srvsheet2.GetCellValue(Row,"gubun") == srvsheet1.GetCellValue(i,"gubun") && srvsheet2.GetCellValue(Row,"question") == srvsheet1.GetCellValue(i,"question")) {
							srvsheet2.SetCellValue(i,"sStatus","U");
							srvsheet1.SetCellValue(i,"memo",srvsheet2.GetCellValue(Row,"memo"));
							break;
						}
					}
				}
...
<div class="wrapper modal_layer">
	...
	<div class="modal_body">
		<div id ="srvsheet1Form">
			...
			<div id="srvsheet1_wrap"></div>
			<!-- <script type="text/javascript"> createIBSheet("srvsheet1", "100%", "30%", "${ssnLocaleCd}"); </script> -->
		</div>
		<div id="srvsheet2Form">
			<div class="inner">
				<div class="sheet_title">
				<ul>
			...
			<div id="srvsheet2_wrap"></div>
			<!-- <script type="text/javascript"> createIBSheet("srvsheet2", "100%", "200px", "${ssnLocaleCd}"); </script> -->
		</div>
		<div class="outer">
			<div class="sheet_title">
			<ul>
	...
	<div class="modal_footer">
		<a href="javascript:setValue();" class="btn filled authA" mid='save'>저장</a>
		<a href="javascript:closeCommonLayer('retireSurveyLayer');" class="btn outline_gray" mid='close'>닫기</a>
	</div>
</div>
