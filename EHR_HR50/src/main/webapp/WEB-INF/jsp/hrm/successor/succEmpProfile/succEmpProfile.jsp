...
<!DOCTYPE html> <html class="hidden"><head> <title>승계자 profile</title>
...
<script type="text/javascript">
	...
	function showEmpList() {		
		try {
            var user = ajaxCall( "${ctx}/SuccPsnalEmp.do?cmd=getSuccPsnalEmpUserList", "searchSabun="+$("#searchSabun").val(), false);
            var searchSabunList = "";
            if ( user != null && user.DATA != null ){ 
                user.DATA.map(( a, i ) => {
                    if(i == 0) {
                        searchSabunList += "'" + a.sabun + "'"
                    }else {
                        searchSabunList += ",'" + a.sabun + "'"
                    }
                });
			...
			var html = "";
			$("#detailList").html("");
			var data = ajaxCall("${ctx}/SuccEmpProfile.do?cmd=getSuccEmpTimelineSrchEmpList", $("#empForm").serialize() + "&searchSabunList=" + searchSabunList , false);
			...
			if( data != null && data != undefined && data.DATA != null && data.DATA != undefined ) {
				for( var i = 0; i < data.DATA.length; i++) {
					item = data.DATA[i];
					var keywordList = ajaxCall( "${ctx}/SuccEmpProfile.do?cmd=getPsnalKeywordList", "&sabun=" + item.sabun , false);
					...
					html += "      </p>";
					html += "      <a href=\'javascript:showRd(" + "\""+item.enterCd+ "\", \""+item.sabun+"\");\' class=\'btn\' style=\'border:1px solid #b3b3b3; float: right; height: 20px; padding: 0px 10px; line-height: 20px;\' >Profile Card</a>";
					html += "    </div>";
                    ...
                    html += "      <li id=\'tdKeywordNm"+i+"\' class=\'full\'>Keyword  : "+keywordText+"</li>";
                    } 
					html += "    </ul>";
					html += "  </div>";
					html += "</div>";
	...
	function showEmpTimeline(sabun) {
		...
		try {
			var data = ajaxCall("${ctx}/SuccEmpProfile.do?cmd=getSuccAppmtTimelineSrchTimelineList", $("#empForm").serialize() + "&searchSabun=" + sabun, false);
			//console.log('data', data);
	...
	function callChart(sabun, chart, chartType) {
		...
		if( chart.id == "myChart1" ) {
			data = ajaxCall("${ctx}/SuccEmpProfile.do?cmd=getSuccAppmtTimelineSrchCpnChartList","searchSabun=" + sabun,false);
		} else if( chart.id == "myChart2" ) {
			data = ajaxCall("${ctx}/SuccEmpProfile.do?cmd=getSuccAppmtTimelineSrchPapChartList","searchSabun=" + sabun,false);
		}
	...
	function drawChartGuage(targetObj, dataset) {
		...
		$(targetObj).css({
			"padding-left" : "calc(100% - " + (width + parseInt(width/2)) + "px)"
		});
	...
	function callCompareBarChar(sabun, targetOrg) {
		...
		if(targetOrg == ".graph_barA") {
			data = ajaxCall("${ctx}/SuccEmpProfile.do?cmd=getSuccAppmtTimelineSrchWorkYearChartList","searchSabun=" + sabun,false);
		}
    ...
    function setEmpPage() {
        ...
        setTimeout(function(){showEmpList();}, 100);
    }
	...
	function showRd(enterCd, sabun) {
		...
		window.top.showRdLayer('/EmpTimelineSrch.do?cmd=getEncryptRd', data);
...
<div class="wrapper">
	...
	<input type="hidden" id="searchSabunList" name="searchSabunList" />
	</form>
