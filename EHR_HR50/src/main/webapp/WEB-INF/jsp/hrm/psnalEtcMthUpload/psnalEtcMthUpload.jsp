...
<!DOCTYPE html> <html class="hidden"><head> <title>기타사항업로드(개월)</title>
...
<script type="text/javascript">
	$(function() {
		...
		initdata1.Cols = [
			...
			{Header:"기준개월",	Type:"Float",		Hidden:0,	Width:100,	Align:"Right",	ColMerge:0,	SaveName:"mth",		KeyField:0,	Format:"",		PointCount:2,	UpdateEdit:1,	InsertEdit:1,	EditLen:5 },
		...
		var userCd1 = convCode( ajaxCall("/CommonCode.do?cmd=getCommonNSCodeList&","queryId=getPsnalEtcMthUploadCodeList",false).codeList, "<tit:txt mid='103895' mdef='전체'/>");
...
// 		$("#toSdate").val("${curSysYyyyMMHyphen}-31") ;
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search":
			sheet1.DoSearch( "${ctx}/PsnalEtcMthUpload.do?cmd=getPsnalEtcMthUploadList",$("#sheet1Form").serialize() );
			break;
		case "Insert":
...
// 			if(!dupChk(sheet1,"sabun|gntCd|applYmd", true, true)){break;}
			...
			sheet1.DoSave( "${ctx}/PsnalEtcMthUpload.do?cmd=savePsnalEtcMthUpload" ,$("#sheet1Form").serialize());
			break;
		case "Down2Excel":
		...
		case "DownTemplate":
			...
			sheet1.Down2Excel({
					...
					, SheetDesign:1,Merge:1,DownRows:0,DownCols:"sabun|divCd|sdate|edate|mth|memo",ExcelRowHeight:"40"});
			break;
		case "LoadExcel":
			/* if($("#searchDivCd").val() == ""){
