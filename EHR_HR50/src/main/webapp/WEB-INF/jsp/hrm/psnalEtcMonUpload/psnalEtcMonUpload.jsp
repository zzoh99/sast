...
<!DOCTYPE html> <html class="hidden"><head> <title>기타사항업로드(금액)</title>
...
<script type="text/javascript">
	$(function() {
		...
		initdata1.Cols = [
			...
			{Header:"<sht:txt mid='basicMon' mdef='금액'/>",	Type:"Int",		Hidden:0,	Width:100,	Align:"Right",	ColMerge:0,	SaveName:"mon",		KeyField:0,	Format:"",		PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:10 },
		...
		var userCd1 = convCode( ajaxCall("/CommonCode.do?cmd=getCommonNSCodeList&","queryId=getPsnalEtcMonUploadCodeList",false).codeList, "<tit:txt mid='103895' mdef='전체'/>");
...
// 		$("#toSdate").val("${curSysYyyyMMHyphen}-31") ;
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search":
			sheet1.DoSearch( "${ctx}/PsnalEtcMonUpload.do?cmd=getPsnalEtcMonUploadList",$("#sheet1Form").serialize() );
			break;
		case "Insert":
...
// 			if(!dupChk(sheet1,"sabun|gntCd|applYmd", true, true)){break;}
			...
			sheet1.DoSave( "${ctx}/PsnalEtcMonUpload.do?cmd=savePsnalEtcMonUpload" ,$("#sheet1Form").serialize());
			break;
		case "Down2Excel":
		...
		case "DownTemplate":
			...
			sheet1.Down2Excel({
					...
					, SheetDesign:1,Merge:1,DownRows:0,DownCols:"sabun|divCd|sdate|edate|mon|memo",ExcelRowHeight:"40"});
			break;
		case "LoadExcel":
/* 			if($("#searchDivCd").val() == ""){
					...
					<table>
						<tr>
							<td id="etcComment">
								...
								<li>- 통상식대 : 평균임금 또는 통상임금 계산 시 활용되며, 금액으로 관리</li>
							</td>
						</tr>
					</table>
				</div>
