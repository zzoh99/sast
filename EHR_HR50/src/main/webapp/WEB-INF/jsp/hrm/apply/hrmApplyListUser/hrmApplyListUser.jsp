	...
	<script>
		$(document).ready(function() {
			...
			function init(){
				...
				const typeList2 = getTypeList2();
				let typeHtml = '<option value="">신청서 종류</option>';
				typeList.forEach(item => {
					if(item.code != 'ALL'){
						typeHtml += '<option value="' + item.code + '">' + item.codeNm + '</option>';
					}
				})
				typeList2.forEach(item => {
					typeHtml += '<option value="' + item.applCd + '">' + item.applNm + '</option>';
				})
				$('#searchEmpTable').html(typeHtml);
			...
			function renderList(page) {
				...
				listData.slice(start, end).forEach(function(item, i) {
                    ...
                    listHtml += '            <span class="list-type">' + item.type + '</span>';
			...
			$(document).on('click', '.empDet', function(e) {
				...
				if(param.empTable){
					const url = '/Popup.do?cmd=viewEmpInfoChangeDetLayer&authPg=A';
					new window.top.document.LayerModal({
						id : 'empInfoChangeDetLayer'
						, url : url
						, parameters : param
						, width : 700
						, height : 520
						, title : param.empTableNm
						, trigger :[
							{
								name : 'empInfoChangeDetTrigger'
			...
			function doAction(){
				console.log(parent.$("#surl").val())
				$("#srchFrm #surl").val(parent.$("#surl").val());
				let param = $("#srchFrm").serialize();
				const data = ajaxCall('PsnalBasicInf.do?cmd=getEmpCommonMgrList', param, false);
				let resultList = [];
				data.DATA.forEach(item => {
					resultList.push({
						title: item.empTableNm,
						date: item.applYmd,
						status: item.applStatusCdNm,
						statusCode: item.applStatusCd,
						name: item.name,
						sabun: item.sabun,
						empTable: item.empTable,
						empTableNm: item.empTableNm,
			...
			function getTypeList() {
				const data = ajaxCall("${ctx}/HrmApplyUser.do?cmd=getTableNameList","",false).DATA;
				return data;
			}

			function getTypeList2() {
				const data = ajaxCall("${ctx}/HrmApplyUser.do?cmd=getTableOtherList","surl=" + encodeURIComponent(parent.$("#surl").val()),false).DATA;
				return data;
			}
		...
		<form id="srchFrm" name="srchFrm" >
			...
			<div class="card d-flex align-center gap-16">
				<div class="select_wrap icon note">
					<select class="custom_select thin" name="searchEmpTable" id="searchEmpTable">
				...
				<button type="button" id="btnModifyPersonalInfo" class="btn ml-auto dark">인사신청</button>
			</div>
		</form>
		<!--// 조회조건 -->
		<!-- 리스트 -->
