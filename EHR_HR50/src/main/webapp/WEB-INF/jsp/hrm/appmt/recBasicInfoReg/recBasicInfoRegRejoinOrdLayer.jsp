...
<!DOCTYPE html> <html class="hidden"> <head> <title>재입사 발령</title>
...
<script type="text/javascript">
	...
	$(function() {
		...
		const modal = window.top.document.LayerModalUtility.getModal('recBasicInfoRegRejoinOrdLayer');
		...
		initdata1.Cols = [
    		{Header:"No",				Type:"${sNoTy}",	Hidden:0,	Width:"${sNoWdt}",	Align:"Center",	ColMerge:0,	SaveName:"sNo" },
   			...
   			{Header:"사진",				Type:"Image",		Hidden:0,  	Width:60, 		Align:"Center", ColMerge:0,		SaveName:"photo",			UpdateEdit:0, ImgMinWidth:50, ImgHeight:60 },
			...
			{Header:"발령상세",			Type:"Combo",		Hidden:1,	Width:80,	Align:"Center",	ColMerge:0,	SaveName:"ordDetailCd",	KeyField:1,	Format:"",		PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:10 },

			{Header:"전출회사",			Type:"Combo",	Hidden:1,	Width:120,	Align:"Center",	ColMerge:0,	SaveName:"ordEnterCd",		KeyField:0,	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:10 },
			...
			{Header:"주민번호",			Type:"Text",		Hidden:0,	Width:110,	Align:"Center",	ColMerge:0,	SaveName:"resNo",		KeyField:0,	Format:"IdNo",		PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:200 },
		...
		$(window).smartresize(sheetResize); sheetInit();sheet1.SetSheetHeight(300);
	...
	function doAction1(sAction) {
		...
		case "Search":
			...
			sheet1.DoSearch( "${ctx}/GetDataList.do?cmd=getRecBasicInfoRegRejoinOrdPopList", $("#sheet1Form").serialize() );
	...
	function passData(){
		...
		if (rowsSerialize=="") {
			...
			//p.self.close();
            const modal = window.top.document.LayerModalUtility.getModal('recBasicInfoRegRejoinOrdLayer');
            modal.fire('recBasicInfoRegRejoinOrdTrigger', rvArray).hide();  
		}
	}
        ...
        <div class="modal_body">
            <form id="sheet1Form" name="sheet1Form" onsubmit="return false;">
                <div class="sheet_search outer">
                    ...
                    <table>
                    <tr>
                    	<th><tit:txt mid="104084" mdef="퇴직일 " /> </th>
						<td> 
							<input type="text" id="searchFromYmd" name ="searchFromYmd" class="date2" value="<%=DateUtil.addMonths(DateUtil.getCurrentTime("yyyy-MM-dd"),-12)%>"/> ~
							<input type="text" id="searchToYmd" name ="searchToYmd" class="date2" value="<%=DateUtil.getCurrentTime("yyyy-MM-dd")%>"/>
						</td>
						<th>사번/성명</th>
                        <td>
                        	<input id="searchSabunName" name="searchSabunName" type="text" class="text required"/>
                        </td>
