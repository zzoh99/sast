...
var ssnSearchType = "${ssnSearchType}";
	...
	$(function() {
		...
		initdata.Cols = [
			...
			{Header:"사진",		Type:"Image",	Hidden:0,   MinWidth:55,   Align:"Center", ColMerge:0, SaveName:"photo",		UpdateEdit:0, ImgMinWidth:50, ImgHeight:60 ,  Cursor:"Pointer"},
		...
		$("#searchOrgNm, #searchSabunName, #searchEndWorkYear, #searchYmd").on("keyup", function(e) {
			if(e.keyCode == 13) {
				doAction("Search");
			}
		});
	...
	function getCommonCodeList() {
		...
		// 급여유형
		var payTypeList = convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","H10110", baseSYmd), "");
		...
		// 직책
		var searchJikchakCd = convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","H20020", baseSYmd), "");
		...
		// 직위
		var searchJikweeCd = convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","H20030", baseSYmd), "");
		...
		// 직급
		var searchJikgubCd = convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","H20010", baseSYmd), "");
	...
	function doAction(sAction) {
		...
		case "Search":
			...
			*/
			sheet1.DoSearch( "${ctx}/EndWorkDateEmpLst.do?cmd=getEndWorkDateEmpLstList", $("#sendForm").serialize() ); break;
		case "Down2Excel":
			var downcol = makeHiddenSkipCol(sheet1);
			var param = {DownCols:downcol, SheetDesign:1, Merge:1};
			var d = new Date();
			var fName = "정년대상자관리_" + d.getTime();
	...
	function chkInVal(sAction) {
		...
		if ($("#searchEndWorkYear").val() == "") {
			alert("정년기준을 입력하십시오.");
			$("#searchEndWorkYear").focus();
			return false;
		}
				...
				<div>
					<table>
						<tr>
							...
							<th>정년기준</th>
							<td> 
								<input id="searchEndWorkYear" name ="searchEndWorkYear" type="text" class="text w30" style="padding:0 4px;" value="60" />세
								</th>
							</td>
							<th>소속</th>
							<td>
