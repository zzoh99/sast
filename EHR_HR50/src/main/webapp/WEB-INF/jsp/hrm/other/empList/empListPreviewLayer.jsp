...
<script type="text/javascript">
	...
	$(function() {
		const modal = window.top.document.LayerModalUtility.getModal('empListPreviewLayer');
		searchGrpCd = modal.parameters.searchGrpCd;

		createIBSheet3(document.getElementById('empListPreviewLayerSht1-wrap'), "empListPreviewLayerSht1", "100%", "100%", "${ssnLocaleCd}");
		...
		]; IBS_InitSheet(empListPreviewLayerSht1, initdata);empListPreviewLayerSht1.SetEditable("${editable}");empListPreviewLayerSht1.SetVisible(true);empListPreviewLayerSht1.SetCountPosition(4);
		empListPreviewLayerSht1.SetImageList(0,"${ctx}/common/images/icon/icon_popup.png");

		sheetInit();

		// sheet 높이 계산
		var sheetHeight = $(".modal_body").height() - $("#srchFrm").height() - $(".sheet_title").height() - 2;
		empListPreviewLayerSht1.SetSheetHeight(sheetHeight);
	...
	function doAction1(sAction) {
		switch(sAction){
			...
			case "Down2Excel":  //엑셀내려받기
				var downcol = makeHiddenSkipCol(empListPreviewLayerSht1);
				var param  = {DownCols:downcol,SheetDesign:1,Merge:1,ExcelFontSize:"9",ExcelRowHeight:"20"};
				empListPreviewLayerSht1.Down2Excel(param);
			break;
		}
	}
	...
	function searchTitleList() {
		...
		if (titleList["colSaveName"].length > 1) {
			...
			IBS_InitSheet(empListPreviewLayerSht1, initdata1); empListPreviewLayerSht1.SetCountPosition(4);
	
			$("#multiStatusCd").val(getMultiSelect($("#statusCd").val()));
			
			empListPreviewLayerSht1.DoSearch( "${ctx}/EmpList.do?cmd=getEmpListList", $("#srchFrm").serialize() );
		} else {
	...
	// 조회 후 에러 메시지
	function empListPreviewLayerSht1_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			if (Msg != "") {
				alert(Msg);
			}
			if( empListPreviewLayerSht1.LastCol() < 10 ) {
				sheetResize();
			}
		...
		<table border="0" cellspacing="0" cellpadding="0" class="sheet_main">
			<tr>
				<td>
					...
					<div id="empListPreviewLayerSht1-wrap"></div>
				</td>
			</tr>
		</table>
	</div>
	<div class="modal_footer">
		<btn:a href="javascript:closeCommonLayer('empListPreviewLayer');" css="btn outline_gray" mid='110881' mdef="닫기"/>
	</div>
</div>
</body>
</html>
