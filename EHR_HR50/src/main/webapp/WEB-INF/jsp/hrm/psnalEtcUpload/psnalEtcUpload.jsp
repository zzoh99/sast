...
<!DOCTYPE html> <html class="hidden"><head> <title>기타사항업로드(기간)</title>
...
<script type="text/javascript">
	$(function() {
		...
		var userCd1 = convCode( ajaxCall("/CommonCode.do?cmd=getCommonNSCodeList&","queryId=getPsnalEtcUploadCodeList",false).codeList, "<tit:txt mid='103895' mdef='전체'/>");
...
// 		$("#toSdate").val("${curSysYyyyMMHyphen}-31") ;
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search":
			sheet1.DoSearch( "${ctx}/PsnalEtcUpload.do?cmd=getPsnalEtcUploadList",$("#sheet1Form").serialize() );
			break;
		case "Insert":
...
// 			if(!dupChk(sheet1,"sabun|gntCd|applYmd", true, true)){break;}
			...
			sheet1.DoSave( "${ctx}/PsnalEtcUpload.do?cmd=savePsnalEtcUpload" ,$("#sheet1Form").serialize());
			break;
		case "Down2Excel":
		...
		case "DownTemplate":
			...
			sheet1.Down2Excel({
					TitleText:"*기타사항자료업로드"
						+ "\n*필수입력항목 : '사번', '구분', '시작일자'"
						+ "\n*구분 : 노조가입, 상부회, 퇴직금누진대상자, 연차정산대상자 "
						+ "\n*파일로드시에는 이 행을 삭제해야 합니다."
					, SheetDesign:1,Merge:1,DownRows:0,DownCols:"sabun|divCd|sdate|edate|memo",ExcelRowHeight:"40"});
			break;
		case "LoadExcel":
			/* if($("#searchDivCd").val() == ""){
