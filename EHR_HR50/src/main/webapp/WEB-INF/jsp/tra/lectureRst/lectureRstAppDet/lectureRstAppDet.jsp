...
<!DOCTYPE html> <html class="hidden"><head> <title>사내교육결과보고신청</title>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#empCnt").mask('000,000,000,000,000', { reverse : true });
		$("#note").maxbyte(2000);
	...
	function doAction1(sAction) {
		switch (sAction) {
			case "Search": //신청내용 조회
				...
				//교육과정정보 조회
				var data = ajaxCall( "${ctx}/LectureRstApp.do?cmd=getLectureRstAppDetMap", $("#searchForm").serialize(),false);
				...
				if ( data != null && data.DATA != null ){ 
					...
					if (data.DATA.appedApplSeq) {
						$("#empCnt").val(data.DATA.empCnt).focusout();
	...
	function checkList(status) {
		...
		var data = ajaxCall( "${ctx}/LectureRstApp.do?cmd=getLectureRstAppDetDupChk", $("#searchForm").serialize(),false);
		...
		try {
			...
			if( adminRecevYn == "Y" ){
		        ...
		        if ( !checkList() ) {
		      	...
		      	//저장
				var data = ajaxCall("${ctx}/LectureRstApp.do?cmd=saveLectureRstAppDet",$("#searchForm").serialize(),false);
...
<div class="wrapper">
	...
	<table class="table">
		...
		<tr>
			...
			<td colspan="3">
				<input type="text" id="empCnt" name="empCnt" class="${textCss} w50 center" ${readonly} /> <span>&nbsp;명</span>
			</td>
		</tr>
		<tr>
