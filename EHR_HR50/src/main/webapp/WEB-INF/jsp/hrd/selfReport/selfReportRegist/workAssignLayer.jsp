...
<script type="text/javascript">
	...
	$(function() {
		createIBSheet3(document.getElementById('workAssignLayerSheet-wrap'), "workAssignLayerSheet", "100%", "100%", "${ssnLocaleCd}");

		const modal = window.top.document.LayerModalUtility.getModal('workAssignLayer');

		let chooseCds = modal.parameters.chooseCds || '';
		$("#chooseCds").val(chooseCds);
		...
		IBS_InitSheet(workAssignLayerSheet, initdata);
		workAssignLayerSheet.SetVisible(true);
		workAssignLayerSheet.SetCountPosition(4);

		var sheetHeight = $(".modal_body").height() - $(".sheet_title").height() - 2;
		workAssignLayerSheet.SetSheetHeight(sheetHeight);
	...
	function doAction1(sAction){
		switch (sAction) {
			case "Search":
				...
				workAssignLayerSheet.DoSearch( "${ctx}/SelfReportRegist.do?cmd=getWorkAssignPopupList", params);
				break;
		}
	}
	...
	function workAssignLayerSheet_OnSearchEnd(code, msg, stCode, stMsg){
		try{
			if (msg !="") alert(msg);
	...
	function workAssignLayerSheet_OnDblClick(Row, Col){
		try{
	...
	function returnWorkAssign(Row,Col){
		...
		// 중복체크
		/*if( $("#chooseCds").val().indexOf("|" + workAssignLayerSheet.GetCellValue(Row, "sWorkAssignCd") + "|") > -1 ) {
			isAlready = true;
		...
		if( !isAlready ) {
			...
			let p = {
				gWorkAssignCd : workAssignLayerSheet.GetCellValue(Row, "gWorkAssignCd"),
				gWorkAssignNm : workAssignLayerSheet.GetCellValue(Row, "gWorkAssignNm"),
				mWorkAssignCd : workAssignLayerSheet.GetCellValue(Row, "mWorkAssignCd"),
				mWorkAssignNm : workAssignLayerSheet.GetCellValue(Row, "mWorkAssignNm"),
				workAssignCd : workAssignLayerSheet.GetCellValue(Row, "sWorkAssignCd"),
				workAssignNm : workAssignLayerSheet.GetCellValue(Row, "sWorkAssignNm")
			};

			const modal = window.top.document.LayerModalUtility.getModal('workAssignLayer');
			modal.fire('workAssignLayerTrigger', p).hide();
		}
	}
</script>
</head>
...
<div class="wrapper modal_layer">
	<div class="modal_body">
		...
		<form id="workAssignLayerSheetForm" name="workAssignLayerSheetForm">
...
<%--		<script type="text/javascript">createIBSheet("workAssignLayerSheet", "100%", "100%", "${ssnLocaleCd}"); </script>--%>
		<div id="workAssignLayerSheet-wrap"></div>
	</div>
	<div class="modal_footer" style="padding-top:0px;">
		<btn:a href="javascript:closeCommonLayer('workAssignLayer');" css="gray large" mid='110881' mdef="닫기"/>
	</div>
</div>
</body>
</html>
