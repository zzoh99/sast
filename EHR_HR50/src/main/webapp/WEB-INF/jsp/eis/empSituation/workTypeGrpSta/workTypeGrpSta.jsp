...
var myChart2Data = null;
	...
	function CallChart1(Type)
	{
		...
		//연령별 데이터
		var result1 ;
		result1 = ajaxCall("${ctx}/WorkTypeGrpSta.do?cmd=getWorkTypeGrpStaList",$("#srchFrm").serialize(),false);
        var interval = -1;
		 var $timer = $("#timer");
...
// 		파이와 라벨의 거리, 두께, 색, padding, 부드러운 처리
	...
	function drawChartForAnime2(result1) {
		var pieChartTitle = "직군별 인원비율";//'직위별 인원비율' ;
		var data = "{";
		data += 		"IBCHART: {";
		data += 		"BACKCOLOR: '#FFFFFF',";
		data += 		"BORDERWIDTH: '1',";
	...
	function downChart(){
	 	if(chartType == "A"){
	 		myChart1.Down2Image({FileName:"WorkTypeChartImageA", Type:IBExportType.JPEG, Width:800, Url:"/common/plugin/IBLeaders/Sheet/jsp/Down2Image.jsp"});
	 	}else if(chartType == "B"){
	 		myChart2.Down2Image({FileName:"WorkTypeChartImageB", Type:IBExportType.JPEG, Width:800, Url:"/common/plugin/IBLeaders/Sheet/jsp/Down2Image.jsp"});
	 	}
	}
