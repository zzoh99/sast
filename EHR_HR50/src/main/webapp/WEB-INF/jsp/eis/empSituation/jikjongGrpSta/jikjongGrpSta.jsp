...
var myChart2Data = null;
	...
	function CallChart1(Type)
	{
		...
		//연령별 데이터
		var result1 ;
		result1 = ajaxCall("${ctx}/JikjongGrpSta.do?cmd=getJikjongGrpStaList",$("#srchFrm").serialize(),false);
        var interval = -1;
		 var $timer = $("#timer");
...
// 		파이와 라벨의 거리, 두께, 색, padding, 부드러운 처리
	...
	function drawChartForAnime2(result1) {
		var pieChartTitle = "직종별 인원비율";//'직위별 인원비율' ;
		var data = "{";
		data += 		"IBCHART: {";
		data += 		"BACKCOLOR: '#FFFFFF',";
		data += 		"BORDERWIDTH: '1',";
    ...
    function orgSearchPopup(){
        try{
        	if(!isPopup()) {return;}
			var args    = new Array();
			openPopup("/Popup.do?cmd=orgTreePopup", args, "740","520",function (rv){
...
/*             if(rv!=null){
	...
	function downChart(){
	 	if(chartType == "A"){
	 		myChart1.Down2Image({FileName:"JikjongChartImageA", Type:IBExportType.JPEG, Width:800, Url:"/common/plugin/IBLeaders/Sheet/jsp/Down2Image.jsp"});
	 	}else if(chartType == "B"){
	 		myChart2.Down2Image({FileName:"JikjongChartImageB", Type:IBExportType.JPEG, Width:800, Url:"/common/plugin/IBLeaders/Sheet/jsp/Down2Image.jsp"});
	 	}
	}
	...
	function openEmployeePopup(data){
		...
		args["jikweeCdL"]  = $("input[name=jikweeCd]").val();
		args["jikgubCdL"]  = $("input[name=jikgubCd]").val();
		args["workTypeL"]  = $("input[name=workType]").val();
		args["manageCdL"]  = $("input[name=manageCd]").val();
		args["jikchakCdL"] = $("input[name=jikchakCd]").val();
		args["orgCd"]      = $("input[name=searchOrgCd]").val();

		args["jikjongCd"] = data["code"];
			...
			<div>
				<table>
					<tr>
				        ...
				        <th class="hide">회사</th>
						<td class="hide"><select id="groupEnterCd" name="groupEnterCd" class="w150"></select></td>
...
<!-- 						<td class="hide"> <span><tit:txt mid='104295' mdef='소속 '/></span>  <input id="searchOrgNm" name="searchOrgNm" type="text" class="text w150" /> </td>
						 -->
					</tr>
					<tr>
	...
	<table border="0" cellspacing="0" cellpadding="0" class="sheet_main"  style="height: 350px;" >
		<colgroup>
			<col width="*" />
<!-- 			<col width="*" /> -->
		</colgroup>
<!-- 		<tr><td colspan="2">&nbsp;</td></tr> -->
		...
		<tr>
			<td>
				<span id="chart1" style="display: block;height: 350px;">
					<script type="text/javascript"> createIBChart("myChart1", "100%", "350px"); </script>
				</span>
				<span id="chart2" style="display: none;height: 350px;">
					<script type="text/javascript"> createIBChart("myChart2", "100%", "350px"); </script>
				</span>
			</td>
		</tr>
	</table>
