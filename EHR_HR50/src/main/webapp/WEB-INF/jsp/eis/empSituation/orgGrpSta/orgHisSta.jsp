...
<script type="text/javascript">
	$(function() {
		...
		//직위
		var searchJikgubCd = convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","H20030"), "");
	...
	function CallChart1(Type)
	{
		...
		result1 = ajaxCall("${ctx}/OrgGrpSta.do?cmd=getOrgGrpStaList2",$("#srchFrm").serialize(),false);
		result2 = ajaxCall("${ctx}/OrgGrpSta.do?cmd=getOrgGrpStaList2",param1.replace($("#searchYmd").val(),minusYmd ), false);
		result3 = ajaxCall("${ctx}/OrgGrpSta.do?cmd=getOrgGrpStaList2",param1.replace($("#searchYmd").val(),minusYmd2 ), false);
        var interval = -1;
		 var $timer = $("#timer");
	...
	function drawChartForAnime1(result1, result2, result3, minusYmd, minusYmd2) {
		...
		for(var i = 0; i < result1["DATA"].length; i++) {
			...
			data += 		"				 			 ] ";
			if(i == result1["DATA"].length-1 || ( i+1 <= result1["DATA"].length && result1["DATA"][i+1]["cnt"] == "0" && result2["DATA"][i+1]["cnt"] == "0" && result3["DATA"][i+1]["cnt"] == "0" ) ) {
				data += 		"	   		   }";
			} else {
				data += 		"	   		   },";
			}
	...
	function downChart(){
 		myChart1.Down2Image({FileName:"OrgHistImage", Type:IBExportType.JPEG, Width:800, Url:"/common/plugin/IBLeaders/Sheet/jsp/Down2Image.jsp"});
	}
				...
				<table>
					<table>
						<tr>
							...
							<td><a href="javascript:downChart()" id="btnDown" class="button"><tit:txt mid='113874' mdef='이미지다운'/></a></td>
						</tr>
					</table>
					<table>
						<tr>
