...
var myChart2Data = null;
	...
	function CallChart1(Type)
	{
		...
		result1 = ajaxCall("${ctx}/EmpEmpHisSta.do?cmd=getEmpEmpHisStaList2",$("#srchFrm").serialize(),false);
        var interval = -1;
		 var $timer = $("#timer");
	...
	function drawChartForAnime1(result1, result2, result3, minusYmd, minusYmd2) {
		...
		for(var i = 0; i < result1["DATA"].length; i++) {
			...
			data += 		"					  SERIES:[{ LEGENDLABEL:'인원', POINTLABEL:'', VALUE:"+result1["DATA"][i]["cnt"]+"}, { LEGENDLABEL:'입사율(%)', POINTLABEL:'', VALUE:"+result1["DATA"][i]["perCnt"]+"},{ LEGENDLABEL:'입사인원', POINTLABEL:'', VALUE:"+result1["DATA"][i]["enterCnt"]+"}, { SERIESTYPE: 'line',LEGENDLABEL:'입사율(%)', POINTLABEL:'123123123', VALUE:"+result1["DATA"][i]["perCnt"]+"}";
			data += 		"				 			 ] ";
...
// 		파이와 라벨의 거리, 두께, 색, padding, 부드러운 처리
	...
	function CallChart2(Type)
	{
		...
		//연령별 데이터
		var result1 ;
		result1 = ajaxCall("${ctx}/EmpEmpHisSta.do?cmd=getEmpEmpHisStaList1",$("#srchFrm").serialize(),false);
        var interval = -1;
		 var $timer = $("#timer");
         interval = setInterval(function() {
	...
	function drawChartForAnime2(result1) {
		var pieChartTitle = "입사구분";//'직위별 인원비율' ;
		var totCnt = 0;
		for(var i = 0; i < result1["DATA"].length; i++) {
			totCnt += result1["DATA"][i]["cnt"];
		}
		var pieSubTitle ="(입사자 " + totCnt + ")";
	...
	function downChart(){
	 	if(chartType == "hist"){
	 		myChart1.Down2Image({FileName:"EnterHistChartImage", Type:IBExportType.JPEG, Width:800, Url:"/common/plugin/IBLeaders/Sheet/jsp/Down2Image.jsp"});
	 	}else if(chartType == "gubun"){
	 		myChart2.Down2Image({FileName:"EnterGubunChartImage", Type:IBExportType.JPEG, Width:800, Url:"/common/plugin/IBLeaders/Sheet/jsp/Down2Image.jsp"});
	 	}
	}

	function myChart1_OnPointClick(Index,X,Y){
		if(Index == 0 || Index == 2){
			var data = { searchYmd: myChart1Data[X]["codeNm"]+"1231" };
			if(Index == 0) data["program"] = "empEmpHisSta";
			if(Index == 2) data["empYyyy"] = myChart1Data[X]["codeNm"];
			openEmployeePopup(data);
		}
	}
	
	function myChart2_OnPointClick(Index, X , Y){
		var data = {searchYmd: $("input[name=searchYyyy]").val()+"1231",
				    empYyyy: $("input[name=searchYyyy]").val(),
				    stfType: myChart2Data[X]["code"]};
		openEmployeePopup(data);
	}
			...
			<div>
				<table>
					<tr>
						...
						<td>
							<input type="radio" name="chartType" id="chartType" value="hist" checked/><span>  입사자 변동추이</span>
							<input type="radio" name="chartType" id="chartType" value="gubun"/><span>  입사구분</span>
						</td>
					</tr>
					<tr>
