...
<script type="text/javascript">
	...
	var g_spaceSize = 25, g_columnCnt = 6, g_defaultSize = 200;
	...
	function initPreset() {
		...
		if( presetCdList != undefined && presetCdList != null && presetCdList.codeList != undefined && presetCdList.codeList != null ) {
			...
			const searchPresetId = "${map.searchPresetId}";
			if (searchPresetId) {
	...
	function doAction1(sAction) {
		...
		case "Search":
			if( $("#searchPresetId").val() != "" ) {
				...
				// 데이터 조회
				statsData = ajaxCall("${ctx}/StatsSrch.do?cmd=getStatsPresetMngUseItemDtlData", $("#sheetForm").serialize(), false);
		...
		case "EmpPresetEdit":
			// 데이터 필수 정의 컬럼 팝업
			if(!isPopup()) {return;}
			var url = "/StatsSrch.do?cmd=viewStatsPresetMngEmpPresetEditLayer&authPg=${authPg}";
...
<script type="text/javascript">
	...
	function drawChartBeforeGetData(statsItem) {
		$.ajax({
			url: "${ctx}/StatsSrch.do?cmd=getStatsMngChartDataMap",
			type: "post",
			dataType: "json",
			async: true,
				...
				<table>
					<tr>
						...
						<td>
							...
							<a href="javascript:doAction1('EmpPresetEdit');" class="btn outline_gray">개인 통계구성 관리</a>
						</td>
					</tr>
				</table>
			</div>
	...
	<div class="inner">
		<div class="sheet_title">
			<ul>
				...
				<li class="btn">
					<a href="javascript:toggleSearchBox();" id="btn_searchBox" class="basic">검색박스 <span class="icon fa fa-toggle-on f_s20"></span></a>
					<a href="javascript:toggleSideBar();" id="btn_sideBar" class="basic">사이드바 <span class="icon fa fa-toggle-on f_s20"></span></a>
				</li>
			</ul>
		</div>
	</div>
