...
<script type="text/javascript">
	var chartItemMngRequiredColLayer = {id:'chartItemMngRequiredColLayer'};
	$(function() {
		createIBSheet3(document.getElementById('chsheet_wrap'), "chsheet", "100%", "100%", "${ssnLocaleCd}");
		const modal = window.top.document.LayerModalUtility.getModal(chartItemMngRequiredColLayer.id);
		var { pluginObjNm } = modal.parameters;
		...
		]; IBS_InitSheet(chsheet, initdata);chsheet.SetEditable("${editable}");chsheet.SetVisible(true);chsheet.SetCountPosition(4);
		...
		if( colIdArrs && colIdArrs != null && colIdArrs.length > 0 ) {
			...
			colIdArrs.forEach(function(item) {
				// add row
				row = chsheet.DataInsert(row + 1);
				
				// set value
				chsheet.SetCellValue(row, "colId", item);
				chsheet.SetCellValue(row, "colNm", HR_CHART_DATA_SQL_COL[item]);
				
				// 필수 정의 컬럼인지 체트
				if( requiredCols != null ) {
					requiredCols.forEach(function(col) {
						if( col ==  item) {
							chsheet.SetCellValue(row, "check", "1");
							return false;
						}
					});
				}
		...
		<table border="0" cellspacing="0" cellpadding="0" class="sheet_main">
			<tr>
				<td>
					...
					<div id="chsheet_wrap"></div>
				</td>
			</tr>
		</table>
	</div>
	<div class="modal_footer">
		<btn:a href="javascript:closeCommonLayer('chartItemMngRequiredColLayer');" css="btn outline_gray" mid='110881' mdef="닫기"/>
	</div>
</div>
</body>
</html>