...
<script type="text/javascript">
	...
	$(function() {
		...
		var searchPresetTypeCd = "";
		var searchPresetOwner = "";
		var searchPresetId = "";
		var searchPresetNm = "";
		
		if( arg != undefined ) {
			searchPresetTypeCd = arg["searchPresetTypeCd"];
			searchPresetOwner = arg["searchPresetOwner"];
			searchPresetId = arg["searchPresetId"];
			searchPresetNm = arg["searchPresetNm"];
		}else{
			if ( p.popDialogArgument("searchPresetTypeCd") !=null ) { 	searchPresetTypeCd = p.popDialogArgument("searchPresetTypeCd"); }
			if ( p.popDialogArgument("searchPresetOwner") !=null ) { 	searchPresetOwner = p.popDialogArgument("searchPresetOwner"); }
			if ( p.popDialogArgument("searchPresetId")     !=null ) { 	searchPresetId     = p.popDialogArgument("searchPresetId"); }
			if ( p.popDialogArgument("searchPresetNm")     !=null ) { 	searchPresetNm     = p.popDialogArgument("searchPresetNm"); }
		}
		...
		grid.on("added removed change resize", function(e, items) {
			...
			if( e.type == "added" ) {
				...
				//console.log('[GridStack] event', e.type, 'items', items, 'getGridItems', grid.getGridItems());
			}
			...
			//console.log('[GridStack] event', e.type, 'gridSerializedData', gridSerializedData);
		});
			...
			if( isContinues ) {
				if( widgetNode != undefined && widgetNode != null ) {
					...
					for(var i = 0; i < sheet1.RowCount(); i++) {
						Row = i + 1;
						statsCd = sheet1.GetCellValue(Row,"statsCd");
						if( gid == statsCd ) {
							sheet1.SetCellValue(Row, "btnAdd", '<a class="basic">추가</a>');
						}
					}
				}
			}
	...
	function sheet1_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			for(var i = 0; i < sheet1.RowCount(); i++) {
				...
				sheet1.SetCellValue(Row, "previewDetail", "1");
			}
	...
	function sheet1_OnClick(Row, Col, Value) {
		...
		try{
			if(Row >= sheet1.HeaderRows()){
				if(sheet1.GetCellValue(Row,"btnAdd") != ""){
					statsCd = sheet1.GetCellValue(Row,"statsCd");
					statsNm = sheet1.GetCellValue(Row,"statsNm");
					chartSizeW = parseInt(sheet1.GetCellValue(Row,"chartSizeW"));
					chartSizeH = parseInt(sheet1.GetCellValue(Row,"chartSizeH"));
					...
					if( $(".grid-stack-item[gs-id='" + statsCd + "']", ".grid-stack").length == 0 ) {
						...
						sheet1.SetCellValue(Row, "btnAdd", '');
					} else {
						alert("이미 추가된 통계입니다.");
					}
				}
...
<script type="text/javascript">
	...
	function doAction2(sAction) {
		switch (sAction) {
		case "Search":
			sheet2.DoSearch( "${ctx}/StatsPresetMng.do?cmd=getStatsPresetMngUseItemList", $("#sheetForm").serialize() );
			break;
		case "Save":
			// remove all rows
			sheet2.RemoveAll();
			...
			if( gridSerializedData != null && gridSerializedData.length > 0 ) {
				...
				gridSerializedData.forEach(function(item, idx, arr){
					var dummyYn = "N", statsCd, row = sheet2.DataInsert(0);
					...
					sheet2.SetCellValue(row, "presetTypeCd", $("#searchPresetTypeCd").val());
					sheet2.SetCellValue(row, "presetOwner", $("#searchPresetOwner").val());
					sheet2.SetCellValue(row, "presetId", $("#searchPresetId").val());
					sheet2.SetCellValue(row, "widgetId", item.id);
					sheet2.SetCellValue(row, "statsCd", statsCd);
					sheet2.SetCellValue(row, "seq", (idx + 1));
					sheet2.SetCellValue(row, "chartPositionX", item.x);
					sheet2.SetCellValue(row, "chartPositionY", item.y);
					sheet2.SetCellValue(row, "chartSizeW", item.w);
					sheet2.SetCellValue(row, "chartSizeH", item.h);
					sheet2.SetCellValue(row, "dummyYn", dummyYn);
				});
			}
			// call save
			IBS_SaveName(document.sheetForm,sheet2);
			sheet2.DoSave( "${ctx}/StatsPresetMng.do?cmd=saveStatsPresetMngUseItem", $("#sheetForm").serialize());
			break;
		}
	}
	...
	function sheet2_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			for(var i = 0; i < sheet2.RowCount(); i++) {
				Row = i + 1;
				widgetId = sheet2.GetCellValue(Row,"widgetId");
				dummyYn = sheet2.GetCellValue(Row,"dummyYn");
				...
				chartPositionX = parseInt(sheet2.GetCellValue(Row,"chartPositionX"));
				chartPositionY = parseInt(sheet2.GetCellValue(Row,"chartPositionY"));
		...
		<table border="0" cellspacing="0" cellpadding="0" class="sheet_main">
			...
			<tr>
				...
				<td class="alignC valignT">
					<div class="dummyWrap mat40" onclick="javascript:addDummyWidget();">
				...
				<td class="alignC valignT">
					...
					<br/>
					<a href="javascript:p.self.close();" class="button bg_gray_a large mat20 w70 h30 f_s16 padt20"><tit:txt mid='104157' mdef='닫기'/></a>
				</td>
			</tr>
		</table>
	</div>
...
<div class="hide">
	<script type="text/javascript"> createIBSheet("sheet2", "100%", "0%", "${ssnLocaleCd}"); </script>
</div>
