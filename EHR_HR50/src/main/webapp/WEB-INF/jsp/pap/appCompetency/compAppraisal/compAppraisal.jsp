...
var papAdminYn = "${sessionScope.ssnPapAdminYn}";
	$(function() {
		...
		initdata.Cols = [
			...
			{Header:"대상자(피평가자)",												Type:"Text",	 Hidden:0,	Width:80,	Align:"Center",	ColMerge:0,	SaveName:"name",			KeyField:0,	CalcLogic:"",	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:100 },
			...
			{Header:"<sht:txt mid='completionV1' mdef='완료여부'/>",		Type:"CheckBox",	 Hidden:0,	Width:50,	Align:"Center",	ColMerge:0,	SaveName:"ldsAppStatusCd",	KeyField:0,	CalcLogic:"",	Format:"",		PointCount:0,	UpdateEdit:1,	InsertEdit:0,	EditLen:100, TrueValue:"Y", FalseValue:"N"  },
			...
			{Header:"<sht:txt mid='jindanCnt' mdef='CHECKVAL'/>",		Type:"Text",	 Hidden:1,	Width:50,	Align:"Left",	ColMerge:0,	SaveName:"checkval",		KeyField:0,	CalcLogic:"",	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:1000 },
	...
	function doAction1(sAction){
		switch(sAction){
			case "Search":
				...
				sheet1.DoSearch( "${ctx}/CompAppraisal.do?cmd=getCompAppraisalList", $("#empForm").serialize() );
				break;

			case "Save":		//저장
				if ( sheet1.RowCount() > 0 ){
					for(var i = sheet1.HeaderRows(); i < sheet1.RowCount()+sheet1.HeaderRows(); i++){
						var statusVal = sheet1.GetCellValue(i, "sStatus");
						var ldsAppStatusCdVal = sheet1.GetCellValue(i, "ldsAppStatusCd");
						var name = sheet1.GetCellValue(i, "name");
						var checkval = sheet1.GetCellValue(i, "checkval");
						if(statusVal == "U" && ldsAppStatusCdVal == "Y" && checkval > 0){ // 확정여부가 Y인건 수정 못함
							alert(name+' 님의 평가되지않은 문항이 존재합니다.\n모든 문항을 평가한 후 평가완료 해주세요');
							return;
						}
						if(statusVal == "U" && ldsAppStatusCdVal == "Y" && sheet1.GetCellValue(i,"jindanCnt") == 0){
	        ...
	        case "SendMail":
                ...
                if (confirm(alertMsg)) {
                	IBS_SaveName(document.empForm,sheet1);
        			sheet1.DoSave( "${ctx}/CompAppraisal.do?cmd=sendMailCompAppraisal", $("#empForm").serialize() );
                }
				break;
		}
	}
	...
	function sheet1_OnSearchEnd(Code, ErrMsg, StCode, StMsg){
		try{
			...
			if ( sheet1.RowCount() > 0 ){
				for(var i = sheet1.HeaderRows(); i < sheet1.RowCount()+sheet1.HeaderRows(); i++){
					var ldsAppStatusCdVal = sheet1.GetCellValue(i, "ldsAppStatusCd");
					//alert(ldsAppStatusCdVal)
					if(ldsAppStatusCdVal == "Y"){ // 확정여부가 Y인건 수정 못함
						sheet1.SetCellEditable(i, "ldsAppStatusCd",0);
					}else{
						cnt = cnt + 1
					}
		...
		try{
		    if (Row > 0 && sheet1.ColSaveName(Col) == "result") {
				...
				var layer = new window.top.document.LayerModal({
					id: 'compAppraisalLayer',
					url: "${ctx}/CompAppraisal.do?cmd=viewCompAppraisalLayer&authPg="+ authPg,
					parameters: {
						CompAppraisalCd : sheet1.GetCellValue(Row,"compAppraisalCd"),
						wEnterNm : sheet1.GetCellValue(Row,"wEnterNm"),
						wEnterCd : sheet1.GetCellValue(Row,"wEnterCd"),
						empId : sheet1.GetCellValue(Row,"sabun"),
						name : sheet1.GetCellValue(Row,"name"),
						sabunOrgNm : sheet1.GetCellValue(Row,"sabunOrgNm"),
						sabunJikweeNm : sheet1.GetCellValue(Row,"sabunJikweeNm"),
						sabunJikchakNm : sheet1.GetCellValue(Row,"sabunJikchakNm"),
						appEmpId : sheet1.GetCellValue(Row,"appSabun"),
						appEnterCd : sheet1.GetCellValue(Row,"appEnterCd"),
						ldsAppStatusCd : sheet1.GetCellValue(Row,"ldsAppStatusCd"),
						appSeqCd : sheet1.GetCellValue(Row,"appSeqCd")
					},
					width: 960,
					height: 800,
					title: "다면평가",
					trigger: [
						{
							name: 'compAppraisalLayerTrigger',
					...
					<div class="sheet_title">
						<ul>
							...
							<li class="btn">
								...
								<a href="javascript:doAction1('Save')" id="btnSave1" class="btn filled authA"><tit:txt mid='114380' mdef='평가완료'/></a>
							</li>
						</ul>
					</div>
				</div>
