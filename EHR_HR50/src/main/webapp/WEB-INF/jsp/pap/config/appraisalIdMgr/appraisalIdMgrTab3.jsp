...
<script type="text/javascript">
	...
	$(function() {
		...
		/* {Header:"평가차수"			,Type:"Combo",	  Hidden:0,  Width:50,Align:"Center",  ColMerge:0,   SaveName:"appraisalSeq",	KeyField:1,   CalcLogic:"",   Format:"",			PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:10 }, */
		{Header:"평가시행\n시작일"		,Type:"Date",	  	Hidden:0,  Width:80,Align:"Center",  ColMerge:0,   SaveName:"appAsYmd",	 KeyField:0,   CalcLogic:"",   Format:"Ymd",		 PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:10 , EndDateCol:"appAeYmd"},
		{Header:"평가시행\n종료일"		,Type:"Date",	  	Hidden:0,  Width:80,Align:"Center",  ColMerge:0,   SaveName:"appAeYmd",		KeyField:0,   CalcLogic:"",   Format:"Ymd",		 PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:10 , StartDateCol:"appAsYmd"},
		...
		//평가차수
		/* var comboList4 = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList","queryId=getAppraisalIdMgrTab3CdList&useYn=Y&searchAppraisalCd=" + $("#searchAppraisalCd").val(),false).codeList, "");
	...
	function doAction2(sAction) {
		...
		switch (sAction) {
		case "Search": 	 	sheet2.DoSearch( "${ctx}/AppraisalIdMgr.do?cmd=getAppraisalIdMgrTab3", "appraisalCd="+appraisalCd ); break;
		case "Save":
			if(!chkInVal(sheet2, "appAsYmd", "appAeYmd")) {break;}
			if(chkDate(sheet2.GetSelectRow()) == 1){
	                return;
	        }
			if(sheet2.FindStatusRow("I") != ""){
				/* if(!dupChk(sheet2,"appraisalCd|appStepCd|appraisalSeq", true, true)){break;} */
			...
			sheet2.DoSave( "${ctx}/AppraisalIdMgr.do?cmd=saveAppraisalIdMgrTab3", $("#srchFrm").serialize()); break;
		case "Insert":
			if ( appraisalCd == "" ) {
				alert("평가ID를 선택해 주세요");
				return;
		...
		case "Copy":
			...
			sheet2.SetCellValue(row,"appStepCd");
			//sheet2.SetCellValue(row,"appraisalSeq");
			sheet2.SetCellValue(row, "appAsYmd");
            sheet2.SetCellValue(row, "appAeYmd"); 
		...
		<div class="sheet_title">
			<ul>
				...
				<li class="btn">
					...
					<a href="javascript:doAction2('Search')" class="btn dark authR">조회</a>
				</li>
			</ul>
		</div>
	</div>
