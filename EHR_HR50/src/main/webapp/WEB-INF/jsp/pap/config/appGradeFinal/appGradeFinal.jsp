...
<!DOCTYPE html> <html class="hidden"><head> <title>배분결과(최종)</title>
...
//=========================================================================================================================================
		...
		initdata1.Cols = [
			...
			{Header:"평가대상그룹명|평가대상그룹명",		Type:"Text",		Hidden:0,	Width:120,	Align:"Left",	ColMerge:0,	SaveName:"appGroupNm",	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 },
			{Header:"조직평가등급|조직평가등급",		Type:"Combo",		Hidden:0,	Width:80,	Align:"Center",	ColMerge:0,	SaveName:"orgGradeCd",	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:10  },
			...
			{Header:"최종|총원",					Type:"Int",			Hidden:0,	Width:80,	Align:"Center",	ColMerge:0,	SaveName:"totCntExec",	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 }
	...
	function initSheet1() {
		...
		initdata1.Cols = [
			...
			{Header:"평가대상그룹|평가대상그룹|평가대상그룹",		Type:"Combo",	Hidden:1,	Width:80,	Align:"Center",	ColMerge:0,	SaveName:"orgGradeCd",	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:10  },
		...
		if(data != null && data != undefined && data.DATA != null && data.DATA != undefined && data.DATA.length > 0) {
			...
			// 최종 총원 컬럼 추가
			initdata1.Cols.push({Header:"최종|총원|총원",					Type:"Int",			Hidden:0,	Width:80,	Align:"Center",	ColMerge:0,	SaveName:"totCntExec",	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100,	BackColor:bcc });

			// [최종] 등급배분 컬럼 셋팅
			for(var i = 0; i < data.DATA.length; i++) {
				item = data.DATA[i];
				var colHeaderNm = "최종|" + item.appClassNm + "|" + item.appClassNm;
			...
			initdata1.Cols.push({Header:"최종인원수meta|인원",		Type:"Text", 	Hidden:1,	 Width:60,	Align:"Center", ColMerge:0, SaveName:"cntArrExec",		KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0 });
		}
	...
	function doAction1(sAction) {
		switch (sAction) {
			case "Search":
				initSheet1();
				sheet1.DoSearch( "${ctx}/AppGradeFinal.do?cmd=getAppGradeFinalList", $("#sendForm").serialize() );
				break;
			case "Down2Excel":
