...
<script type="text/javascript">
	...
	$(function() {
		...
		initdata.Cols = [
			...
			{Header:"평가SHEET"		,Type:"Combo",	Hidden:0,	Width:100,	Align:"Center",	ColMerge:0,	SaveName:"appSheetCd",	KeyField:1,	Format:"",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:10},
		...
		//평가SHEET
		var comboList1 	= convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","P20005"));
		sheet1.SetColProperty("appSheetCd", 			{ComboText:comboList1[0], ComboCode:comboList1[1]} );
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search": 	 	sheet1.DoSearch( "${ctx}/GetDataList.do?cmd=getAppElementMgrTab2", $("#srchFrm").serialize() ); break;
		case "Search2": 	 	sheet1.DoSearch( "${ctx}/GetDataList.do?cmd=getAppElementMgrTab2ScopeCd", $("#srchFrm").serialize() ); break;
		case "Save":
			if(sheet1.FindStatusRow("I") != ""){
				if(!dupChk(sheet1,"appraisalCd|appSheetCd", true, true)){break;}
			}
			IBS_SaveName(document.srchFrm,sheet1);
			sheet1.DoSave( "${ctx}/AppElementMgr.do?cmd=saveAppElementMgrTab2", $("#srchFrm").serialize()); break;
	...
	function doAction2(sAction){
		switch (sAction) {
		case "Search2": 	sheet2.DoSearch( "${ctx}/GetDataList.do?cmd=getAppElementMgrTab2ScopeCd", $("#srchFrm").serialize() ); break;
	...
	function sheet1_OnClick(Row, Col, Value) {
		//try{
			...
			if( sheet1.ColSaveName(Col) == "scope" ) {
				...
				*/
				var args = new Array();
				args["searchUseGubun"] = "ST";
				args["searchItemValue1"] = sheet1.GetCellValue(Row,"appraisalCd");
				args["searchItemValue2"] = sheet1.GetCellValue(Row,"appSheetCd");
			...
			if(sheet1.GetCellValue(Row, "sStatus") != "I"){
				$("#appSheetCd").val(sheet1.GetCellValue(Row, "appSheetCd"));
				var seqInfo = ajaxCall("${ctx}/GetDataList.do?cmd=getAppElementMgrTab2TblNm", "queryId=getAppElementMgrTab2TblNm&"+ $("#srchFrm").serialize(), false);
				$("#txt1").html("&nbsp;");
				doAction2("Clear");
...
<div class="wrapper">
	<form id="srchFrm" name="srchFrm" >
		...
		<input type="hidden" id="appSheetCd" name="appSheetCd" />
