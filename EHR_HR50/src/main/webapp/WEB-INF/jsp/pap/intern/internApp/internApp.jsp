...
<script type="text/javascript">
	...
	$(function() {
		...
		initdata.Cols = [
			...
			{Header:"항목구분",		Type:"Combo",	Hidden:0,	Width:40,	Align:"Center",	ColMerge:1,	SaveName:"internItemType",	KeyField:1,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 },
			{Header:"역량코드",		Type:"Text",	Hidden:1,	Width:100,	Align:"Center",	ColMerge:0,	SaveName:"seq",				KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:100 },
			{Header:"역량항목",		Type:"Text",	Hidden:0,	Width:80,	Align:"Center",	ColMerge:0,	SaveName:"internItemNm",	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:100 },
			{Header:"역량정의",		Type:"Text",	Hidden:0,	Width:250,	Align:"Center",	ColMerge:0,	SaveName:"internItemMemo",	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:100 },
			...
			{Header:"평가코드",		Type:"Text",	Hidden:1,	Width:100,	Align:"Center",	ColMerge:0,	SaveName:"appraisalCd",		KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
			{Header:"사번",			Type:"Text",	Hidden:1,	Width:100,	Align:"Center",	ColMerge:0,	SaveName:"sabun",			KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 }
		...
		var comboCodeList2 	= convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","P10010"), "");		//역량종류
		var comboCodeList3  = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList","queryId=getAppraisalCdListMobOrgTargetReg&searchAppTypeCd=D,",false).codeList, "");	//평가명

		
		
		sheet1.SetColProperty("appClassCd", 			{ComboText:comboCodeList1[0], ComboCode:comboCodeList1[1]} );
		sheet1.SetColProperty("internItemType", 			{ComboText:comboCodeList2[0], ComboCode:comboCodeList2[1]} );
		...
		//평가진행상태
		 var v_status = ajaxCall("/InternApp.do?cmd=getInternAppStatusMap",$("#srchFrm").serialize(),false);
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search": 	 	sheet1.DoSearch( "${ctx}/InternApp.do?cmd=getInternAppList", $("#srchFrm").serialize() ); break;
		case "Save": 		
							IBS_SaveName(document.srchFrm,sheet1);
							sheet1.DoSave( "${ctx}/InternApp.do?cmd=saveInternApp", $("#srchFrm").serialize()); break;
...
//  Type이 POPUP인 셀에서 팝업버튼을 눌렀을때 발생하는 이벤트
    ...
    function calTotalPoint(){
    	     ...
    	     gradeInfo = ajaxCall("/CompAppSelfReg.do?cmd=getPapGradeInfoList","searchAppTypeCd=D,",false);
    	 var totalPoint = 0;
    	 var totalGrade = "";
    	 var totalGradeCd = "";
		...
		sheet1.SetCellValue(lr,"internItemMemo","점수/등급");
		sheet1.SetCellAlign(lr,"internItemMemo","Right");
    ...
    function goRequest(){
    	...
    	if (confirm("평가확정 후에는 평가내용을 수정 할 수 없습니다.\n평가확정을 취소 하시려면 평가담당자에게 문의 하세요.\n평가확정을 하시겠습니까?")){
    		var Request = ajaxCall("/InternApp.do?cmd=saevInternAppRequest",$("#srchFrm").serialize(),false);
