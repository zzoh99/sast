...
<script type="text/javascript">
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search": 	 	
			var comboData = ajaxCall("${ctx}/AppEval.do?cmd=getAppEvalComboList",$("#srchFrm").serialize(), false).DATA;
	...
	function makeSelfEval() {
		var selfEvalList = ajaxCall("${ctx}/AppEval.do?cmd=getAppSelfEvalList",$("#srchFrm").serialize(), false).DATA;
		...
		$(".self>.container>div").on("click", function(e) {
			...
			if (appStatus == "00") {
				title = "평가자조회";
				src = "AppEval.do?cmd=viewAppSelfEvalListPopup";
				height = "400px";
				width = "600px"
			} else {
	...
	function makeOtherEval() {
		var otherEvallist = ajaxCall("${ctx}/AppEval.do?cmd=getAppOtherEvalList",$("#srchFrm").serialize(), false).DATA;
		...
		for (var index=0; index<otherEvallist.length; index++) {
			...
			htmlProcess += makeContainer(obj.appTypeCd, obj.stepOnOff, index+1, obj.stepNm, obj.stepMsg)
		}
		
		$(".other").get(0).innerHTML = "";
		$(".other").append(htmlProcess);
		$(".other>.container>div").on("click", function(e) {
			var appTypeCd =  $(e.target.parentElement).children(".itemType").val();
			var popupTitle = $(e.target.parentElement).children(".itemNm").html();
			var title = $(e.target.parentElement).children(".itemNm").html();
			var src = "AppEval.do?cmd=viewAppOtherEvalListPopup";
			var width = "60%";
			var height = "60%";
			if (appTypeCd == "03") {
				title = "중간점검"; 
				...
				title = "1차평가"; 
			} else if (appTypeCd == "07") {
				title = "2차평가";
				src = "AppEval.do?cmd=viewAppOtherEvalListPopup2";
				width = "90%";
				height = "90%";
			}
			var param = {
...
<div class="wrapper">
	...
	<div class="main">
		<span id="spanAppraisalCd" class="txt">대상평가기준</span>&nbsp;&nbsp;&nbsp;<select id="cmbAppraisalCd" name="cmbAppraisalCd"></select>
