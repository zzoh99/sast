...
<title><tit:txt mid='' mdef='평가그룹매핑'/></title>
...
<script type="text/javascript">
	...
	$(function() {
		...
		initdata.Cols = [
			...
			{Header:"그룹명"		,Type:"Text",      	Hidden:0,  Width:60,  	Align:"Center",  ColMerge:0,   	SaveName:"appGroupNm", 	KeyField:0, CalcLogic:"",   Format:"",   	PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100 },
		...
		initdata3.Cols = [
			...
			{Header:"소속"			,Type:"Text",  	Hidden:0,  Width:60,  	Align:"Center",  ColMerge:0,   	SaveName:"appOrgNm", 	KeyField:0, CalcLogic:"",   Format:"",   	PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100 },
			{Header:"사번"			,Type:"Text",  	Hidden:0,  Width:0,  	Align:"Center",  ColMerge:0,   	SaveName:"sabun", 		KeyField:0, CalcLogic:"",   Format:"",   	PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100 },
			{Header:"성명"			,Type:"Text",  	Hidden:0,  Width:60,  	Align:"Center",  ColMerge:0,   	SaveName:"name", 		KeyField:0, CalcLogic:"",   Format:"",   	PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100 },
			{Header:"직위"			,Type:"Text",  	Hidden:0,  Width:60,  	Align:"Center",  ColMerge:0,   	SaveName:"jikweeNm", 	KeyField:0, CalcLogic:"",   Format:"",   	PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100 },
			{Header:"직책"			,Type:"Text",  	Hidden:0,  Width:60,  	Align:"Center",  ColMerge:0,   	SaveName:"jikchakNm", 	KeyField:0, CalcLogic:"",   Format:"",   	PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100 },
			{Header:"평가자"			,Type:"Text",  	Hidden:0,  Width:60,  	Align:"Center",  ColMerge:0,   	SaveName:"appName", 	KeyField:0, CalcLogic:"",   Format:"",   	PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100 },
		...
		//평가명
		var comboList1 = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList","queryId=getAppraisalCdList&searchAppTypeCd=A,B,C",false).codeList, "");
		$("#searchAppraisalCd").html(comboList1[2]);
		
		// 차수
		 var comboList2 = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList&","queryId=getComCodeNoteList&searchGrcodeCd=P00003&searchUseYn=Y&searchNote1=Y",false).codeList, "");
		$("#searchAppSeqCd").html(comboList2[2]);
		
		// 조직평가등급
		//var comboList3 = convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","P00002"), "전체");
		var comboList3 = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList&","queryId=getComCodeNoteList&searchGrcodeCd=P00010&searchUseYn=Y",false).codeList, "");
		$("#searchOrgGradeCd").html("<option value=''>전체</option>"+comboList3[2]);
		

		$("#searchAppraisalCd, #searchAppSeqCd, #searchOrgGradeCd").bind("change", function(event) {
			doAction1("Search");
		});
...
function doAction1(sAction){
	...
	switch(sAction){
		case "Search":		//조회
			sheet1.DoSearch("${ctx}/AppGroupMemMgr.do?cmd=getAppGroupMemMgrList1", $("#mySheetForm").serialize());
			break;
		case "Init"://초기화
			var appSeqNm = $("#searchAppSeqCd option:selected").text();
			var msg = "[" + appSeqNm + "] 평가그룹맵핑을 초기화 하시겠습니까?";
			if(confirm(msg)) {
				var data = ajaxCall("${ctx}/AppGroupMemMgr.do?cmd=updateInitializeAppGroupMem", $("#mySheetForm").serialize() + "&appSeqNm=" + appSeqNm, false);
...
function doAction2(sAction){
	...
	switch(sAction){
		case "Search":		//조회
			...
			sheet2.DoSearch("${ctx}/AppGroupMemMgr.do?cmd=getAppGroupMemMgrList2", 
					$("#mySheetForm").serialize() + "&searchAppGroupCd=" + sheet1.GetCellValue(Row, "appGroupCd"));
			break;
		case "Save":        //오른쪽 버튼
			var chk = "N";
			...
			sheet2.DoSave("${ctx}/AppGroupMemMgr.do?cmd=saveAppGroupMemMgr", 
					$("#mySheetForm").serialize() + "&searchAppGroupCd=");
			break;
	}
}
...
function doAction3(sAction){
	...
	switch(sAction){
		case "Search":		//조회
			sheet3.DoSearch("${ctx}/AppGroupMemMgr.do?cmd=getAppGroupMemMgrList3", $("#mySheetForm").serialize());
			break;
		case "Save":        //왼쪽버튼
			if (sheet1.GetSelectRow() < 1) {
				alert("선택된 평가그룹이 없습니다. 평가그룹을 선택해 주십시요!");
				return;
			}
			var chk = "N";
			...
			chk = "N";
			for(var i=1; i <= sheet3.LastRow(); i++){
				if(sheet3.GetCellValue(i,"sStatus") == "U"){
					if(sheet3.GetCellValue(i,"appSabun") == ""){
						chk = "Y";
						break;
					}
				}
			...
			if(chk == "Y"){
				alert("선택된 대상자중 평가자가 지정되지 않은 대상자가 있습니다. 평가자 지정 후 실행해 주십시요!");
				for(var i=0; i <= sheet3.LastRow(); i++){
			...
			var Row = sheet1.GetSelectRow();
			sheet3.DoSave("${ctx}/AppGroupMemMgr.do?cmd=saveAppGroupMemMgr", 
					$("#mySheetForm").serialize() + "&searchAppGroupCd=" + sheet1.GetCellValue(Row, "appGroupCd"));
			break;
	}
}
			...
			<div>
				<table>
					<tr>
						...
						<td class="hide"><!-- 무신사 조직평가등급 사용안함 -->
							<span>조직평가등급</span>
							<select name="searchOrgGradeCd" id="searchOrgGradeCd"></select>
						</td>
						<td>
							<span>평가그룹명</span>
		...
		<tr>
			<td>
				<div class="inner">
					<div class="sheet_title">
						<ul>
							<li id="txt" class="txt">평가그룹 적용 인원</li>
							<li class="btn">
								<a href="javascript:doAction1('Init')"	class="btn filled authA">평가그룹매핑 초기화</a>
							</li>
						</ul>
					</div>
				</div>
			...
			<td>
				<div class="inner">
					<div class="sheet_title">
						<ul>
							<li id="txt" class="txt"> [ <span id="inwonCnt" ></span> ]대상자 
							</li>
						</ul>
					</div>
				</div>
			...
			<td>
				<div class="inner">
					<div class="sheet_title">
						<ul>
							<li id="txt" class="txt">평가그룹 미적용자</li>
							<li class="btn">
								<span>소속/성명/평가자</span>
