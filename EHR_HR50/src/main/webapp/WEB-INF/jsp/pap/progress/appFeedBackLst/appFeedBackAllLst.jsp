...
<script type="text/javascript">
	$(function() {
		...
		initdata.Cols = [
			...
			{Header:"평가종류|평가종류",			Type:"Combo",		Hidden:0,  					Width:80,   Align:"Center", ColMerge:0, SaveName:"appTypeCd",         	KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:10 },
			...
			{Header:"이의제기|상태",			Type:"Text",		Hidden:1,					Width:40,	Align:"Center",	ColMerge:0,	SaveName:"protestYn"},
			...
			{Header:"rk|rk",            	Type:"Text",      	Hidden:1,  					Width:0,   	Align:"Center", ColMerge:0, SaveName:"rk",           		KeyField:0, Format:"",   	PointCount:0,   UpdateEdit:0,   InsertEdit:0}
		...
		// 숫자만 입력가능
		$("#searchSYear,#searchEYear").keyup(function() {
		     makeNumber(this,'A');
		 });
		
		$("#searchSYear,#searchEYear,#searchOrgNm,#searchNameSabun").bind("keyup",function(event){
			if( event.keyCode == 13){
				doAction1("Search"); 
				$(this).focus(); 
			}
		...
		$("#searchSYear, #searchEYear").change(function (){
			getCommonCodeList();
		});
	...
	function getCommonCodeList() {
		if($("#searchSYear").val() == "" || $("#searchSYear").val().length < 4) {
			alert("평가년도의 시작년도를 입력하시기 바랍니다.");
			$("#searchSYear").focus();
			return;
		}

		let baseSYmd = $("#searchSYear").val() + "-01-01";
		let baseEYmd = $("#searchEYear").val() + "-12-31";
		let appTypeCd      = convCode(codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList&useYn=Y","P10003", baseSYmd, baseEYmd), "");//재직상태
		$("#appTypeCd").html(appTypeCd[2]);
		$("#appTypeCd").select2({placeholder:""});
		$("#appTypeCd").val("").trigger("change");
		sheet1.SetColProperty("appTypeCd", 			{ComboText:appTypeCd[0], ComboCode:appTypeCd[1]} );
	}
	...
	function doAction1(sAction) {
		...
		case "Search": 	 	
							...
							$("#searchAppTypeCd").val(($("#appTypeCd").val()==null ? "" : getMultiSelect($("#appTypeCd").val())));
							sheet1.DoSearch( "${ctx}/AppFeedBackLst.do?cmd=getAppFeedBackAllLstList", $("#srchFrm").serialize() ); 
							break;
		case "Down2Excel":
	...
	function showRd() {
		...
		searchAppraisalCdSAbunAppOrgCd_s = searchAppraisalCdSAbunAppOrgCd_s.substr(0, searchAppraisalCdSAbunAppOrgCd_s.length-1);
		 ...
		 let rkList = [];
			let checkedRows = sheet1.FindCheckedRow('chk');
		...
		const data = {
				  rk : rkList
	            , searchAppraisalCdSAbunAppOrgCd_s : searchAppraisalCdSAbunAppOrgCd_s
		};
		
		
		window.top.showRdLayer('/AppFeedBackLst.do?cmd=getEncryptRd', data, null, '평가결과');
	}
</script>
</head>
<body class="hidden">
<div class="wrapper">
	...
	<input type="hidden" id="searchAppTypeCd" name="searchAppTypeCd" value=""/>
	<!-- 호출 팝업 넘겨줄 키 값 -->
	...
	<div class="sheet_search outer">
			<div>
				<table>
					<tr>
						<td>
							<span>평가년도</span>
							<input id="searchSYear" name="searchSYear" type="text" class="text" maxlength="4" value="<%=Integer.parseInt(DateUtil.getThisYear())-1%>" />&nbsp;~&nbsp;
							<input id="searchEYear" name="searchEYear" type="text" class="text" maxlength="4" value="<%= DateUtil.getThisYear() %>" />
							</select>
						</td>
						<td>
						...
						<td> 
							<span>평가종류</span>
							<select id="appTypeCd" name="appTypeCd" multiple></select>
						</td>
						<td>
		...
		<ul>
			...
			<li class="btn">
				...
				<a href="javascript:showRd()"  				 class="btn outline_gray authR">출력</a>
			</li>
		</ul>
		</div>
	</div>
