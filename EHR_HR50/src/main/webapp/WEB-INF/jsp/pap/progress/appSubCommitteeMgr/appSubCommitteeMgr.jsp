...
<script type="text/javascript">
	$(function() {
		...
		initdata.Cols = [
			...
			{Header:"인사소위원회|최종등급",	Type:"Combo",	Hidden:0,	Width:100,	Align:"Center",	ColMerge:0,	SaveName:"finalClassCd",	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
			{Header:"인사소위원회|조정사유",	Type:"Text",	Hidden:0,	Width:100,	Align:"Left",	ColMerge:0,	SaveName:"memo",			KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
...
*/
		...
		if("${ssnDataRwType}" == "A" || "${ssnGrpCd}" == "10") {
			orgCd = stfConvCode( codeList("${ctx}/AppSubCommitteeMgr.do?cmd=getAppSubCommitteeOrgListAdmin",""), "");
		} else {
			orgCd = stfConvCode( codeList("${ctx}/AppSubCommitteeMgr.do?cmd=getAppSubCommitteeOrgList",""), "");
		}
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search": 	 	sheet1.DoSearch( "${ctx}/AppSubCommitteeMgr.do?cmd=getAppSubCommitteeMgrList", $("#srchFrm").serialize() );

		  break;
		case "Save": 		
							IBS_SaveName(document.srchFrm,sheet1);
							sheet1.DoSave( "${ctx}/AppSubCommitteeMgr.do?cmd=saveAppSubCommitteeMgr", $("#srchFrm").serialize()); break;
	...
	function doAction2(sAction) {
		switch (sAction) {
		case "Search": 	 	sheet2.DoSearch( "${ctx}/AppSubCommitteeMgr.do?cmd=getAppSubCommitteeMgrList2", $("#srchFrm").serialize() ); break;
		}
	}
	// 조회 후 에러 메시지
...
//  사원 팝입
	...
	var getAppStatus = function(){
		var rtn = ajaxCall("${ctx}/AppSubCommitteeMgr.do?cmd=getAppSubCommitteeMgrMap&searchAppraisalCd=" + $("#searchAppraisalCd").val() ,$("#srchFrm").serialize(),false);
