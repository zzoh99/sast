...
<script type="text/javascript">
	...
	$(function() {
		var initdata = {};
		initdata.Cfg = {SearchMode:smLazyLoad,Page:22, ChildPage:5, SizeMode:2};
		...
		var orgType = convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","W20010"), "<tit:txt mid='103895' mdef='전체'/>");
 ...
 // 비교대상 화면으로 이동
    function goMenu() {
        ...
        $("#prgCd").val(prgCd);
    	var prgData = ajaxCall("${ctx}/OrgPersonSta.do?cmd=getCompareEmpOpenPrgMap",$("#srchFrm").serialize(),false);
		...
		var murl      	= prgData.map.murl;
		var menuSeq     = prgData.map.menuSeq;*/


		// parent.openContent(menuNm,prgCd,location,surl, '',menuId)
...
function sheet2_OnDblClick(Row, Col, Value, CellX, CellY, CellW, CellH) {
		...
		try{
			  //임직원을 더블클릭 했을 경우 조건절에 있는 비교대상 중  빈곳에 입력
			 if(Row >0){
				 if($('#name1').val() == ""){
                     $('#name1').val(sheet2.GetCellValue(Row,"name"));
                     $('#sabun1').val(sheet2.GetCellValue(Row,"sabun"));
                 }else if($('#name2').val() == ""){
                     $('#name2').val(sheet2.GetCellValue(Row,"name"));
                     $('#sabun2').val(sheet2.GetCellValue(Row,"sabun"));
                 }else if($('#name3').val() == ""){
                     $('#name3').val(sheet2.GetCellValue(Row,"name"));
                     $('#sabun3').val(sheet2.GetCellValue(Row,"sabun"));
                 }else{
				...
				<table>
					<tr>
                        ...
                        <td>
                         ...
                         <td>
                           <a href="javascript: goMenu()"     class="btn filled authR"><tit:txt mid='112850' mdef='비교'/></a>
                        </td>
					</tr>
				</table>
			</div>
