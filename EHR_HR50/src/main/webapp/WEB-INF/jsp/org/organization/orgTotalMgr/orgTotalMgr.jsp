...
// 		$("#edate").datepicker2({enddate:"edate"});		//수정불가하도록 처리[21.05.24]
		...
		var result = ajaxCall("${ctx}/OrgTotalMgr.do?cmd=getorgTotalMgrMemoTORG103",queryId="getorgTotalMgrMemoTORG103",false);
	...
	function getCommonCodeList() {
		//공통코드 한번에 조회
		let grpCds = "W20010,W20050,W20030,H20020,H20030,H20010,R10010";
		let params = "grpCd="+grpCds + "&baseSYmd=" + $("#baseDate").val();
		codeLists = convCodes(ajaxCall("${ctx}/CommonCode.do?cmd=getCommonCodeLists", params, false).codeList, "");
	...
	function init_sheet5() {
		...
		var applStatusCd 	= convCode( codeList("${ctx}/CommonCode.do?cmd=getCommonCodeList","R10010"), "");	//신청서상태코드
	...
	function init_sheet6() {
        ...
        initdata.Cols = [
            ...
            {Header:"<sht:txt mid='sYmd' mdef='시작일자'/>", Type:"Date",        Hidden:0,  Width:60,    Align:"Center",  ColMerge:0,   SaveName:"sdate",    KeyField:1,   CalcLogic:"",   Format:"Ymd",         PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:10, EndDateCol: "edate" },
            {Header:"<sht:txt mid='eYmd' mdef='종료일자'/>", Type:"Date",        Hidden:0,  Width:60,    Align:"Center",  ColMerge:0,   SaveName:"edate",    KeyField:0,   CalcLogic:"",   Format:"Ymd",         PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:10, StartDateCol: "sdate" }
	...
	function doAction1(sAction) {
		switch (sAction) {
			case "Search": 	 	
				if(!checkList()) return ;
				sheet1.DoSearch( "${ctx}/OrgTotalMgr.do?cmd=getOrgTotalMgrList", $("#srchFrm").serialize() ); 
				break;
			case "Save":
				IBS_SaveName(document.srchFrm,sheet1);
				sheet1.DoSave( "${ctx}/OrgTotalMgr.do?cmd=saveOrgTotalMgr", $("#srchFrm").serialize() ); 
				break ;
			case "Down2Excel":	
				sheet1.Down2Excel(); 
				break;
		...
		switch (sAction) {
			case "Search":
				...
				if(document.getElementById("searchType").checked == false){
					sheet2.DoSearch( "${ctx}/OrgTotalMgr.do?cmd=getOrgMeberList1", $("#srchFrm").serialize() );
				} else {
					sheet2.DoSearch( "${ctx}/OrgTotalMgr.do?cmd=getOrgMeberList2", $("#srchFrm").serialize() );
				}
				break;
			case "Down2Excel":
	...
	function doAction3(sAction) {
		switch (sAction) {
			case "Search": 	 	
				sheet3.DoSearch( "${ctx}/OrgTotalMgr.do?cmd=getOrgHistoryList", $("#srchFrm").serialize(),1 ); 
				break;
			case "Down2Excel":
	...
	function doAction4(sAction) {
		switch (sAction) {
			case "Insert":		
				...
				break;
			case "Search": 	 	
				sheet4.DoSearch( "${ctx}/OrgTotalMgr.do?cmd=getOrgChiefList", $("#srchFrm").serialize() ); 
				break;
			case "Save":
				// 필수값/유효성 체크
				if (!chkInVal(sheet4)) {break;}
				IBS_SaveName(document.srchFrm,sheet4);
				sheet4.DoSave( "${ctx}/OrgTotalMgr.do?cmd=saveOrgChiefList", $("#srchFrm").serialize() ); 
				break ;
			case "Down2Excel":
	...
	function doAction5(sAction) {
		switch (sAction) {
			case "Search": 	 	
				sheet5.DoSearch( "${ctx}/OrgTotalMgr.do?cmd=getOrgRNRList", $("#srchFrm").serialize() ); 
				break;
			case "Down2Excel":
    ...
    function doAction6(sAction) {
        switch (sAction) {
	        ...
	        break;
	        case "Search":      
	        	sheet6.DoSearch( "${ctx}/OrgTotalMgr.do?cmd=getOrgChiefList2", $("#srchFrm").serialize() ); 
	        	break;
	        case "Save":
	        	if (!dupChk(sheet6, "sdate", false, true)) {break;}
				if (!chkInVal(sheet6)) {break;}

	        	IBS_SaveName(document.srchFrm,sheet6);
	                            
	        	sheet6.DoSave( "${ctx}/OrgTotalMgr.do?cmd=saveOrgChiefList2", $("#srchFrm").serialize() ); 
	        	break ;
	        case "Copy":        
	        	sheet6.DataCopy(); 
	        	break;
  ...
  //-----------------------------------------------------------------------------------
	...
	function setSheetData() {
		...
		sheet1.SetCellValue(row,"edate",$('#edate').val().replace(/-/gi,"-"));
		sheet1.SetCellValue(row,"orgType",$("#orgType").val());
		sheet1.SetCellValue(row,"inoutType",$("#inoutType").val());
		sheet1.SetCellValue(row,"objectType",$("#objectType").val());
		sheet1.SetCellValue(row,"locationCd",$("#locationCd").val());
		sheet1.SetCellValue(row,"memo",$("#memo").val());
	...
	function searchSdatePopup(){
		try {
			...
			let orgTotalMgrLayer = new window.top.document.LayerModal({
				id: 'orgTotalMgrLayer',
				url: '/OrgTotalMgr.do?cmd=viewOrgTotalMgrLayer',
				parameters: {},
				width: 350,
				height: 520,
				title: '조직도명 검색',
	...
	function userSheetResize(){
		...
		$(".ibsheet").each(function() {
	        if($(this).attr("fixed") == "false" && this.id != "DIV_sheet1") {
	        	var h = $(this).height();
	        	var top_content = $("#orgInfoFrm").height();
	        	//console.log("top_content : " + top_content);
	        	var tab_h = $(".tab_bottom").outerHeight();
	        	//console.log("tab_h : " + tab_h);
	            $(this).height(h-top_content-tab_h); //조직기본정보 높이 & 탭 높이 차감
	        }
		});
	}
				...
				<table>
					<tr>
						...
						<td>
							...
							<a onclick="javascript:searchSdatePopup();" class="button6"><img src="/common/${theme}/images/btn_search2.gif"/></a>
	...
	<tr>
		<td class="sheet_left">
			<div class="sheet_title inner">
				<ul>
					<li id="txt1" class="txt"><tit:txt mid='112713' mdef='조직도 '/>&nbsp;
						<div class="util">
						<ul>
							<li	id="btnPlus"></li>
							<li	id="btnStep1"></li>
		...
		<td class="sheet_right">
			<form id="orgInfoFrm" name="orgInfoFrm" >
					...
					<ul>
						...
						<li class="btn">
							<btn:a href="javascript:setSheetData();" 	css="btn filled authA" mid='110708' mdef="저장"/>
						</li>
					</ul>
					</div> 
					<table class="table w100p" id="htmlTable">
						...
						<tr>
							...
							<td>
								<input id="memo" name="memo" type="text" class="text w200"/>
							</td>
						</tr>
					</table>
			</form>
			<div>
				<div id="tabs" style="height:100%;border:none;">
					...
					<div id="tabs-1">
						<div class="sheet_title inner">
							<ul>
								<li id="txt2" class="txt"><tit:txt mid='orgEmpNm' mdef='조직원'/></li>
								<li class="btn">
					...
					<div id="tabs-2">
						<div class="inner">
							<div class="sheet_title">
							<ul>
								<li id="txt3" class="txt"><tit:txt mid='orgTotalMgrT2' mdef='조직이력'/></li>
								<li class="btn">
					...
					<div id="tabs-3">
						<div class="inner">
							<div class="sheet_title">
							<ul>
								<li id="txt4" class="txt"><tit:txt mid='orgTotalMgrT3' mdef='조직장'/></li>
								<li class="btn">
									<a href="javascript:doAction4('Down2Excel')" 	class="btn outline-gray authR"><tit:txt mid='download' mdef='다운로드'/></a>
									<btn:a href="javascript:doAction4('Insert')" css="btn outline-gray authA" mid='110700' mdef="입력"/>
					...
					<div id="tabs-4" class="hide">
						<div class="inner">
							<div class="sheet_title">
							<ul>
								<li id="txt5" class="txt"><tit:txt mid='113226' mdef='조직 R&R'/></li>
								<li class="btn">
                        ...
                        <div class="inner">
                            ...
                            <ul>
                                ...
                                <li class="btn">
                                    ...
                                    <btn:a href="javascript:doAction6('Copy')"  css="btn outline-gray authA" mid='110696' mdef="복사"/>
                                    <btn:a href="javascript:doAction6('Insert')" css="btn outline-gray authA" mid='110700' mdef="입력"/>
                                    <btn:a href="javascript:doAction6('Save')"  css="btn filled authA" mid='110708' mdef="저장"/>
                                    <btn:a href="javascript:doAction6('Search')"    css="btn dark authR" mid='110697' mdef="조회"/>
                                </li>
                            </ul>
                            </div>
                        </div>
