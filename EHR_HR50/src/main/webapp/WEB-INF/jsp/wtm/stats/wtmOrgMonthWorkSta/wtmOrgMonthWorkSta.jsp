...
function getLastDayOfMonth(yearMonth) {
	...
	function setOrgCombo(){
		var orgCd = convCode( ajaxCall("${ctx}/WtmOrgMonthWorkSta.do?cmd=getWtmOrgMonthWorkStaOrgList","searchYmd="+$("#searchYm").val(),false).DATA, "");
	...
	function init_sheet1(){
		...
		var titleList = ajaxCall("${ctx}/WtmOrgMonthWorkSta.do?cmd=getWtmOrgMonthWorkStaTitleList", $("#sheet1Form").serialize(), false);
		if (titleList != null && titleList.DATA != null) {
			for(var i = 0 ; i<titleList.DATA.length; i++) {
			    var map = titleList.DATA[i];
				initdata1.Cols[v++] = {Header:str+map.vDay,  Type:"Text",    Hidden:0, Width:30, 	Align:"Center", ColMerge:0,  SaveName:map.saveName,  KeyField:0, Edit:0 };
			}
		}
	...
	function doAction1(sAction) {
		switch (sAction) {
			case "Search":
				...
				sheet1.DoSearch( "${ctx}/WtmOrgMonthWorkSta.do?cmd=getWtmOrgMonthWorkStaList", $("#sheet1Form").serialize());
				break;
			case "Down2Excel":
    ...
    function sheet1_OnClick(Row, Col, Value, CellX, CellY, CellW, CellH) {
        try {
            ...
            if( sheet1.ColSaveName(Col) == "detail" ) {
                ...
                // openPopup("/WtmOrgMonthWorkSta.do?cmd=viewOrgMonthWorkStaPop&authPg=R", args, "850", "650");
				...
				var layer = new window.top.document.LayerModal({
					id : 'wtmOrgMonthWorkStaLayer'
					, url : '/WtmOrgMonthWorkSta.do?cmd=viewWtmOrgMonthWorkStaLayer&authPg=${authPg}'
					, parameters: p
					, width : 850
					, height : 650
					, title : "개인별 월근태현황"
					, trigger :[
						{
							name : 'wtmOrgMonthWorkStaLayerTrigger'
