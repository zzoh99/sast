...
<head>
    ...
    <script>
        $(document).ready(async function () {
            const modal = window.top.document.LayerModalUtility.getModal('wtmWorkClassTargetLayer');
            const workClassCd = modal.parameters.workClassCd || '';

            initEvent(workClassCd);

            setEmpData(workClassCd, "");
        });
        ...
        function initEvent(workClassCd) {
            $("#moveBtn").on('click', async function () {
                ...
                closeCommonLayer('wtmWorkClassTargetLayer');
            });
            ...
            $("#saveTargetBtn").on('click', function (){
                ...
                const result = ajaxCall("/WtmWorkTypeMgr.do?cmd=saveWtmWorkClassTarget", param, false);

                if (result.Code > 0) {
                    const modal = window.top.document.LayerModalUtility.getModal('wtmWorkClassTargetLayer');
                    modal.fire('wtmWorkClassTargetLayerTrigger', '').hide();
                }
            });
        ...
        async function setEmpData(workClassCd, searchKey) {
            try {
                progressBar(true);

                const response = await fetch("/WtmWorkTypeMgr.do?cmd=getWtmWorkTargetList", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                    },
                    body: 'workClassCd=' + workClassCd + '&searchKey=' + searchKey
                });
                ...
                //         addBtn = '<button type="button" class="btn filled addTargetBtn">추가</button>';
                //     }
                //
...
<%--            <span class="page-title">기본설정</span>--%>
    ...
    <div class="modal_footer">
        <a href="javascript:closeCommonLayer('wtmWorkClassTargetLayer');" class="gray large">닫기</a>
    </div>
</div>
</body>
</html>
