...
<script type="text/javascript">
	...
	async function initTab4Sheet() {
		...
		initdata1.Cols = [
			...
			{Header:"<sht:txt mid='businessPlaceNm' mdef='사업장'/>",	Type:"Combo",		Hidden:0,					Width:100,	Align:"Center",	ColMerge:0,	SaveName:"businessPlaceCd",	KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen: 10 },
			...
			{Header:"<sht:txt mid='workDdCd' mdef='근무일집계코드'/>",		Type:"Combo",		Hidden:0,					Width:100,	Align:"Center",	ColMerge:0,	SaveName:"reportItemCd",	KeyField:1,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:10 },
			{Header:"<sht:txt mid='workDdCnt' mdef='근무일수'/>",		Type:"Int",			Hidden:0,					Width:100,	Align:"Center",	ColMerge:0,	SaveName:"totValue",		KeyField:0,	Format:"Integer",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:10 },
		]; IBS_InitSheet(tab4Sheet, initdata1); tab4Sheet.SetCountPosition(0);

		tab4Sheet.SetColProperty("businessPlaceCd", {ComboText:commonCodes["businessPlaceCd"][0], ComboCode:commonCodes["businessPlaceCd"][1]});
		tab4Sheet.SetColProperty("payType", {ComboText:"|"+commonCodes["payType"][0], ComboCode:"|"+commonCodes["payType"][1]});

		const data = await getReportItemCdList();
		const reportItemCdList = stfConvCode(data, "");
		tab4Sheet.SetColProperty("reportItemCd", {ComboText:"|"+reportItemCdList[0], ComboCode:"|"+reportItemCdList[1]});
	...
	const doActionSheet4 = {
		isInit: false,
		"init": function() {
			if (!doAction.tab4Sheet.isInit) {
				initTab4Sheet();
				doAction.tab4Sheet.isInit = true;
				doAction.tab4Sheet.Search();
			}
		},
		"Search": function() {
			tab4Sheet.DoSearch("${ctx}/WtmMonthlyCountMgr.do?cmd=getWtmMonthlyCountMgrWorkDays", $("#mainForm").serialize());
		},
		"Save": function() {
			// 중복체크
			if(!dupChk(tab4Sheet, "ym|businessPlaceCd|sabun|reportItemCd", false, true)) {return;}
			IBS_SaveName(document.mainForm, tab4Sheet);
			tab4Sheet.DoSave("${ctx}/WtmMonthlyCountMgr.do?cmd=saveWtmMonthlyCountMgrWorkDays", $("#mainForm").serialize());
		},
		"Insert": function() {
		...
		"DownTemplate": function() {
			// 양식다운로드
			tab4Sheet.Down2Excel({ SheetDesign:1, Merge:1, DownRows:"0", DownCols:"ym|sabun|reportItemCd"});
		}
	};
	...
	function getTab4ReturnValue(returnValue) {
		...
		tab4Sheet.SetCellValue(gPRow, "orgNm",				rv["orgNm"] );
		...
		tab4Sheet.SetCellValue(gPRow, "businessPlaceCd",	rv["businessPlaceCd"] );
	}
</script>
<div class="wrapper">
