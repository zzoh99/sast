...
function loadWidget20() {
	...
	if (widget20.size != 'bwide') {
		...
		widget20.selectDay = today.getDate();
		...
		base += '		<time id="widget20CalSelTime"></time>\n';
		base += '		<span>팀원 근태 현황</span>\n';
		base += '		<span class="attendance_label_desc">\n';
	...
	if (widget20.size != 'bwide') {
		...
		if (!data || data.length == 0) {
			...
			if ( widget20.size == 'wide' ) {
				...
				var rtable = '<table>\n';
				data.forEach((d, i) => {
					const color = c.type == 1 ? 'green'
								: c.type == 2 ? 'pink':'purple';
					const name = d.title;
		...
		var selectDate = new Date(widget20.selectYear + '-' + widget20.selectMonth + '-' + widget20.selectDay);
		const schedules = data.map(d => ({ title: d.title + ' ' + d.period, start: d.sYmd, end: d.eYmd, className: 'type_' + d.type }));
		widget20SetDetailContents(data, selectDate);
		widget20DrawCalendar(schedules, selectDate);
	}
}

function widget20SetDetailContents(data, date) {
	var datestring = dateFormatToString(date, '-');
	var darray = datestring.split('-');
	var dtext = darray[0] + '년 ' + darray[1] + '월 ' + darray[2] + '일'; 
	$('time#widget20CalSelTime').text(dtext);
	
	var todaydata = data.filter(d => d.sYmd <= datestring && datestring <= d.eYmd);
	const desc = todaydata.reduce((a, c) => {
					const color = c.type == 1 ? 'green' : c.type == 2 ? 'pink':'purple';
					const content = c.gntNm + ' ' + c.title + ' ' + c.period
					a += '<li><span class="state_line ' + color + '"></span><span class="text_content">' + content + '</span></li>\n'; 
					return a;
				}, '');

	const schedcnt = todaydata.filter(d => d.type == 1).length;
	const birthcnt = todaydata.filter(d => d.type == 2).length;
	const applycnt = todaydata.filter(d => d.type == 3).length;
	$('#widget20SelSched').html('<span class="state_icon type_1"></span>일정 ' + schedcnt);
	$('#widget20SelBirth').html('<span class="state_icon type_2"></span>생일 ' + birthcnt);
	$('#widget20SelApply').html('<span class="state_icon type_3"></span>근태 ' + applycnt);
	$('#widget20CalContent').html(desc);
}

function widget20DrawCalendar(schedules, initdate) {
	if (!initdate) initdate = new Date();
	var calendar = new FullCalendar.Calendar(document.getElementById("widget20Calendar"), {
		  //plugins: [interaction],
	      headerToolbar: {
	      ...
	      select: function (arg) {},
	      dateClick: (info) => {
		      var sdate = new Date(info.dateStr);
		      var syear = sdate.getFullYear();
		      var smonth = (sdate.getMonth() + 1);
		      var sday = sdate.getDate();
		      if (widget20.selectYear != syear 
			   || widget20.selectMonth != smonth
			   || widget20.selectDay != sday) {
		    	  widget20.selectYear = syear;
		    	  widget20.selectMonth = smonth;
		    	  widget20.selectDay = sday;
		    	  getContents();   
			  }
	...
	calendar.render();
	calendar.select(dateFormatToString(initdate, '-'));
}
