...
<script type="text/javascript">
	...
	// 위젯 사이즈별 로드
	function init_listBox705(size) {
		if (size == "normal"){
			widget705.makeMini();
			widget705.setDataMini();
		} else if (size == ("wide")){
			widget705.makeWide();
			widget705.setDataWide();
			widget705.drawChart();
		}
	}
	...
	var widget705 = {
		...
		elemId: 'widget705',  // 위젯 엘리면트 id
		$widget: null,
 
		severancePayData: null,  // 총급여 관련 데이터

		// 작은 위젯 마크업 생성
		makeMini: function(){
			var html =
				  '<div class="widget_header toggle-wrap">' +
				  '  <div class="widget_title">퇴직금 지급</div>' +
				  '</div>' +
				  '<div class="widget_body attendance_contents annual-status salary-average">' +
				  '  <div class="container_box">' +
				  '    <div class="container_left">' +
				  '      <div class="container_info">' +
				  '        <span class="info_title_num" id="totalSalary705"></span>' +
				  '        <span class="info_title desc" id="gapRate705"></span>' +
				  '      </div>' +
				  '      <div class="info_box" id="maxRate705">' +
				  '      </div>' +
				  '      <div class="info_box" id="minRate705">' +
				  '      </div>' +
				  '    </div>' +
				  '  </div>' +
				  '</div>';
		...
		setDataMini: function() {
            ...
            const draw = (severancePayData) => {
                const gapRateHtml = widget705.compareGapRate(severancePayData.gapRate);
                const maxGapRateHtml = widget705.compareMaxMinGapRate(severancePayData.maxGapRate);
                const minGapRateHtml = widget705.compareMaxMinGapRate(severancePayData.minGapRate);

                document.getElementById('totalSalary705').innerHTML =
                    `\${severancePayData.year4}<span class="info_title unit">백만원</span>`;
                document.getElementById('gapRate705').innerHTML = gapRateHtml;
                document.getElementById('maxRate705').innerHTML =
                    `<span class="title_kor">최대 인상률</span>
                     <span class="box_bnum">\${severancePayData.maxRate}</span>
                     <span class="title_kor">%</span>\${maxGapRateHtml}`;
                document.getElementById('minRate705').innerHTML =
                    `<span class="title_kor">최대 인상률</span>
                     <span class="box_bnum">\${severancePayData.minRate}</span>
                     <span class="title_kor">%</span>\${minGapRateHtml}`;
            }

			if(this.severancePayData == null) {
                ajaxCall2("${ctx}/getListBox705List.do"
                    , ""
                    , true
                    , null
                    , function(data) {
                        if (data && data.data && data.data.severancePayData) {

                            widget705.severancePayData = data.data.severancePayData;

                            const severancePayData = widget705.severancePayData;
                            draw(severancePayData);
                        }
                    })
			} else {
                const severancePayData = this.severancePayData;
                draw(severancePayData);
            }
		},
		...
		makeWide: function(){
			var html =
				  ...
				  '            <span id="selectedValue705">연도별</span><i class="mdi-ico">arrow_drop_down</i>' +
				  '          </button>' +
				  ...
				  '          </div>' +
				  '        </div>' +
				  '        <div class="sub-title" id="today705"></div>' +
				  '      </div>' +
				  '      <div class="chart-wrap">' +
				  '        <div id="annualChart705"></div>' +
				  '      </div>' +
				  '    </div>' +
				  '  </div>' +
				  '</div>';
		...
		setDataWide: function() {
            ...
            const draw = (severancePayData) => {
                ...
                document.getElementById('today705').textContent = `\${year}년 \${month}월 \${day}일`;
                document.getElementById('totalSalary705').innerHTML =
                    `\${severancePayData.year4}<span class="info_title unit">백만원</span>`;
                document.getElementById('gapRate705').innerHTML = gapRateHtml;
                document.getElementById('maxRate705').innerHTML =
		...
		setUIEvent: function(){
			...
			$widget.on('click', '.select_options > .option', function(){
				...
				document.getElementById('selectedValue705').textContent = optionText;
				$('#annualChart705').empty();
				widget705.drawChart(optionValue);
            ...
            ajaxCall2("${ctx}/getListBox705List.do"
                ...
                , function(data) {
                    if (data && data.data && data.data.severancePayData) {
                        ...
                        if (option === "year"){
                            for (var i = 0; i < 4; i++) {
                                yearData.push(severancePayData['year' + (i+1)]);
                            }
                        } else if (option === "month") {
                            for (var i = 0; i < 12; i++){
                                monthData.push(severancePayData['mon'+(i+1)]);
                            }
                        } else if (option === "quarter") {
                            for (var i = 0; i < 4; i++){
                                quarterData.push(severancePayData['quarter'+(i+1)]);
                            }
                        }
                        ...
                        const chart = new ApexCharts(document.querySelector("#annualChart705"), options);
                        chart.render();
                    }
                })
		}
...
<div class="widget" id="widget705"></div>

<script>
	// DOM 로드 후에 UI 설정
	$(document).ready(function () {
		widget705.$widget = $('#'+ widget705.elemId);

		widget705.setUI(); // UI생성
		widget705.setUIEvent(); // 이벤트 할당
	});
</script>