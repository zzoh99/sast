...
<script type="text/javascript">
	 ...
	 */
	function init_listBox814(size) {
		widget814.size = size;
		
		if (size == "normal"){
			createWidgetMini814();
			setDataWidgetMini814();
		} else if (size == ("wide")){
			createWidgetWide814();
			setDataWidgetWide814();
		}
	}

	// 위젯 mini html 코드 생성 
	function createWidgetMini814(){
		var html =
				'<div class="widget_header">' +
				'  <div class="widget_title">육아기단축 근로자 현황</div>' +
				'</div>' +
		...
		document.querySelector('#widget814Element').innerHTML = html;
	}

	// 위젯 mini 데이터 넣기  
	function setDataWidgetMini814() {

        ajaxCall2("getListBox814List.do"
            , ""
            , true
            , null
            , function(data) {
                if (data && data.data) {
		            const deemedWorker = data.data;
                    ...
                    setDataWidget814(deemedWorker.deemedWorkerData, deemedWorker.enterCd);
                }
            })
	}
	
	// 위젯 wide html 코드 생성
	function createWidgetWide814(){
		var html =
				'<div class="widget_header">' +
				'  <div class="widget_title">육아기단축 근로자 현황</div>' +
            ...
            , function(data) {
                if (data && data.data) {
                    ...
                    deemedWorker = {
                            enterCd: 'HR',
                            deemedWorkerData: [
                                {orgNm: '경남관리본부', jikweeNm: '차장', name: '강지원', sabun: '101123'},
                                {orgNm: '자금팀', jikweeNm: '대리', name: '김지민', sabun: '101130'},
                                {orgNm: '경남관리본부', jikweeNm: '사원', name: '김보연', sabun: '101133'},
                                {orgNm: '경남관리본부', jikweeNm: '대리', name: '김인희', sabun: '101136'},
                                {orgNm: '경남관리본부', jikweeNm: '사원', name: '김재영', sabun: '101140'},
	...
	function setDataWidget814(deemedData, enterCd){
...
<div class="widget" id="widget814Element"></div>