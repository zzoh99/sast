...
var loginMenuSecondLayer = { id: 'loginMenuSecondLayer' };
$(function() {
	const modal = window.top.document.LayerModalUtility.getModal(loginMenuSecondLayer.id);
...
function chkVal(){
	if ( $("input[name=imgClass]").is(":checked") == true ) {
		var p = { imgClass: $(":input:radio[name=imgClass]:checked").val() };
		const modal = window.top.document.LayerModalUtility.getModal(loginMenuSecondLayer.id);
		modal.fire(loginMenuSecondLayer.id + 'Trigger', p).hide();
	} else {
		alert("선택된 클래스가 없습니다.");
		return;
	}
...
<body>
	...
	<div class="btn_center">
		<a href="javascript:chkVal();" class="btn ">선택</a>
	</div>
</body>
</html>
