...
<script type="text/javascript">
	$(function() {	
		...
		initdata.Cols = [
			...
			{Header:"<sht:txt mid='manageCd' mdef='사원구분'/>",			Type:"Combo",     Hidden:1,  Width:70,  Align:"Center",  ColMerge:0,   SaveName:"manageCd",  		KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:10,	Cursor:"" },
		...
		var enterCdList	= convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList","queryId=getEnterCdAllList&enterCd=",false).codeList, "<tit:txt mid='103895' mdef='전체'/>");	//회사코드
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search": 	 	sheet1.DoSearch( "${ctx}/PtrComMgr.do?cmd=getPtrComMgrList", $("#srchFrm").serialize() ); break;
		case "Save": 		
							IBS_SaveName(document.srchFrm,sheet1);
							sheet1.DoSave( "${ctx}/PtrComMgr.do?cmd=savePtrComMgr", $("#srchFrm").serialize() ); break;
		case "Insert":	
			ptrComMgrPopup(0, "I") ;
	 ...
	 */
	function ptrComMgrPopup(Row, state){
  		var w 		= 750;
		var h 		= 620;
		var url 	= "${ctx}/PtrComMgr.do?cmd=ptrComMgrPopup&authPg=${authPg}";
		var args 	= new Array();
		if( state != "I" ) {
			args["enterCd"]	        = sheet1.GetCellValue(Row, "enterCd");
			...
			<div>
				<table>
					<tr>
						...
						<td>				
							<select id="searchEnterCd" name="searchEnterCd" onchange="doAction1('Search')">
							</select> 	
						</td>
			...
			<td>
				<div class="inner">
					<div class="sheet_title">
						<ul>
							<li id="txt" class="txt"><tit:txt mid='112569' mdef='협력업체관리'/></li>
							<li class="btn">
