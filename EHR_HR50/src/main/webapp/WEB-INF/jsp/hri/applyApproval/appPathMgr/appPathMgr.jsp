...
var onLoad      = false;
    $(function() {
        ...
        initdata.Cols = [
            ...
            {Header:"<sht:txt mid='pathNm' mdef='경로명'/>" ,  Type:"Text",        Hidden:0,                     Width:200,          Align:"Center",  ColMerge:0,   SaveName:"pathNm",       KeyField:1,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:100 },
            {Header:"<sht:txt mid='editable' mdef='수정가능여부'/>", Type:"CheckBox",    Hidden:1,                     Width:100,          Align:"Center",  ColMerge:0,   SaveName:"modifyYn",     KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100 ,TrueValue:"Y", FalseValue:"N"},
            ...
            {Header:"<sht:txt mid='keyLevel' mdef='레벨'/>",     Type:"Combo",       Hidden:0,                     Width:40,            Align:"Center",  ColMerge:0,   SaveName:"orgLevelCd",   KeyField:1,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:10 },
        ...
        initdata.Cols = [
            ...
            {Header:"<sht:txt mid='hochingNm' mdef='호칭'/>", Type:"Text",        Hidden:Number("${aliasHdn}"),  Width:40,           Align:"Center",  ColMerge:0,   SaveName:"alias",       KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100 },
        ...
        $(".name").prepend("<b>${ssnName}</b> ( "+"${ssnSabun}"+" )");
    });
    
    function setEmpPage(){
		$("#tSabun").val($("#searchUserId").val());
		$("#tName").val($("#searchKeyword").val());
        doAction1("Search");
        doAction2("Search");
        doAction3("Search");
	}
    ...
    function doAction1(sAction) {
        switch (sAction) {
        case "Search":  sheet1.DoSearch( "${ctx}/AppPathMgr.do?cmd=getAppPathMgrList", $("#empForm").serialize()); break;
        case "Save":
            ...
            sheet1.DoSave("${ctx}/AppPathMgr.do?cmd=saveAppPathMgr", $("#empForm").serialize() );
            break;
        case "Insert":
    ...
    function doAction2(sAction) {
        switch (sAction) { case "Search":  sheet2.DoSearch( "${ctx}/AppPathMgr.do?cmd=getAppPathOrgList", $("#empForm").serialize()); break; }
    }
    function doAction3(sAction) {
        switch (sAction) { case "Search":  sheet3.DoSearch( "${ctx}/AppPathMgr.do?cmd=getAppPathMgrOrgUserList", $("#empForm").serialize()); break; }
    }
    function doAction4(sAction) {
        switch (sAction) {
        case "Search":
            sheet4.DoSearch( "${ctx}/AppPathMgr.do?cmd=getAppPathMgrApplList", $("#empForm").serialize());
            break;
        case "Save":
            ...
            IBS_SaveName(document.empForm,sheet4);
            sheet4.DoSave("${ctx}/AppPathMgr.do?cmd=saveAppPathMgrAppl", $("#empForm").serialize(), -1,0 );
            break;
        }
    }
    function doAction5(sAction) {
        switch (sAction) {
        case "Search":  sheet5.DoSearch( "${ctx}/AppPathMgr.do?cmd=getAppPathMgrReferList", $("#empForm").serialize()); break;
        case "Save":
            var ps = sheet1.GetCellValue(sheet1.GetSelectRow(), "pathSeq");
            if(ps=="") { alert("<msg:txt mid='109572' mdef='결재 경로 저장후 시도 하십시오!'/>"); return;}
            ...
            IBS_SaveName(document.empForm,sheet5);
            sheet5.DoSave("${ctx}/AppPathMgr.do?cmd=saveAppPathMgrRefer", $("#empForm").serialize() );
            break;
        }
    }
...
//                  sheet1.SetCellEditable(row, "sDelete",0);
    ...
    function sheet1_OnClick(Row, Col, Value) {
        try{
            ...
            if(Col == sheet1.SaveNameCol("defaultYn") ){
                for(var r = 1 ; r < sheet1.LastRow()+1; r++){
                    if( sheet1.GetCellValue(r,"modifyYn") == "Y"){
                        ...
                        alert("<msg:txt mid='109732' mdef='시스템 기본결재선은 삭제 할 수 없습니다.'/>");
                    }
                }
            ...
            //delete
            if(Col == sheet1.SaveNameCol("sDelete") ){
                if( sheet1.GetCellValue(Row,"modifyYn") != "Y"){
                    alert("<msg:txt mid='109732' mdef='시스템 기본결재선은 삭제 할 수 없습니다.'/>");
                }
            }
    ...
    function mvAppl(){
        ...
        for(var i=0; i<chkArry.length; i++){
            ...
            if(chkdupTxt == -1){
                ...
                sheet4.SetCellValue(Row,"alias",sheet3.GetCellValue(chkArry[i],"alias") );
    ...
    function mvRefer(){
        ...
        for(i=0; i<chkArry.length; i++){
            ...
            if(chkdupTxt == -1){
                ...
                sheet5.SetCellValue(Row,"alias",sheet3.GetCellValue(chkArry[i],"alias") );
...
//      if(dupText != "") alert("중복된 사번 제외 복사\n"+dupText);
    ...
    function findUser(){
        ...
        args["name"] = "";
        args["sabun"] = "";
        //var result = window.showModalDialog(url, args, "dialogHeight="+h+"px; dialogWidth="+w+"px; scroll=no; status=no; help=no; center=yes");
        var result = openPopup(url,args,w,h);

//      if (result) {
//          $("#chgSabun").val( result["sabun"] );
//          $("#chgName").val( result["name"] );
//          $(".name").empty();
//          $(".name").prepend("<b>"+result["name"]+"</b> ( "+result["sabun"]+" )");
			...
			, trigger :[
				{
					...
					, callback : function(result){
			          $("#tSabun").val(result.sabun);
			          $("#tName").val(result.name);
			          $("#sabun").val(result.sabun);
			          $("#searchKeyword").val(result.name);
					}
				}
			]
		});
...
//         }
    ...
    function getReturnValue(returnValue) {
        ...
        if (pGubun == "fineUser") {
            $("#chgSabun").val( rv["sabun"] );
            $("#chgName").val( rv["name"] );
            $(".name").empty();
            $(".name").prepend("<b>"+rv["name"]+"</b> ( "+rv["sabun"]+" )");
            ...
            doAction3("Search");
            $("#sabun").val("");
        }else if( pGubun == "showEmployeePop" ){
    ...
    function appPathPsnlCreate(){

        if(!confirm("기본결재선 생성(개인별) 실행 시 기존 생성 되어 있는 데이타는 삭제됩니다.\n실행 하시겠습니까?")) {
            return;
        }
        var data = ajaxCall("/AppPathMgr.do?cmd=prcAppPathMgrPsnlCrt",$("#empForm").serialize(),false);
    ...
    function appPathAllCreate(){

        if(!confirm("기본결재선 생성(전인원) 실행 시 기존 생성 되어 있는 데이타는 삭제됩니다.\n실행 하시겠습니까? ")) {
            return;
        }
        var data = ajaxCall("/AppPathMgr.do?cmd=prcAppPathMgrAllCrt",$("#empForm").serialize(),false);
        if(data.Result.Code == null) {
            alert("<msg:txt mid='110120' mdef='처리되었습니다.'/>");
            doAction1('Search');
        } else {
...
<div class="wrapper">
    <form id="empForm" name="empForm">
            ...
            <input id="chgName"     name="chgName"      type="hidden" />
<!--    <div class="outer"> -->
...
<!--            </li> -->
<!--            <li> -->
<!--                <a id="toggleBtn" href="javascript:findUser();" class="basic"><tit:txt mid='114644' mdef='대상자 조회'/></a> -->
<!--            </li> -->
<!--        </ul> -->
<!--        </div> -->
<!--    </div> -->
        <div class="sheet_search outer">
            ...
            <tr>
                ...
                <td>
                    <input id="tSabun" name="tSabun" type="text" class="text required readonly" readonly />
                    <a onclick="javascript:showEmployeePop();return false;" class="button6"><img src="/common/${theme}/images/btn_search2.gif"/></a>
                </td>
				<th><tit:txt mid='103880' mdef='성명'/></th>
                <td>
					<!-- 
                    <input id="tName" name="tName" type="text" class="text readonly" readonly />
					 -->
					...
					<input type="hidden" id="searchGroupYn"  name="searchGroupYn" value="Y" />
                </td>
                <td class="sheet_main">
                	<div class="sheet_title">
                		<ul>
	                		<li class="btn">
	                			...
	                			<btn:a href="javascript:appPathPsnlCreate();"   css="btn outline authA" mid="111677" mdef="기본결재선 생성(개인별)"/>
	                    		<btn:a href="javascript:appPathAllCreate();"    css="btn outline authA" mid="111051" mdef="기본결재선 생성(전인원)"/>
	                		</li>
	                	</ul>
                	</div>
                </td>
        ...
        <div class="outer">
            <div class="sheet_title">
	            <ul>
	                <li class="txt">
	                    <btn:a id="toggleBtn" href="javascript:toggleSheet();" css="btn outline_gray" mid="111230" mdef="접기"/>
	                </li>
	                <li class="btn">
	                    <btn:a href="javascript:doAction1('Insert');" css="btn outline_gray" mid="110700" mdef="입력"/>
        ...
        <div class="sheet_search outer">
                ...
                <tr>
                    ...
                    <td id="btnOrg"  class="">
                        <btn:a href="javascript:orgList();" css="button" mid="110697" mdef="조회"/>
                    </td>
                </tr>
                </table>
                </div>
            ...
            <div class="sheet_button2">
                ...
                <div class="inner">
                    ...
                    <ul>
						...
						<li class="btn">
                            <btn:a href="javascript:doAction5('Save');" css="basic" mid="110708" mdef="저장"/>
                        </li>
                    </ul>
                    </div>
                </div>
