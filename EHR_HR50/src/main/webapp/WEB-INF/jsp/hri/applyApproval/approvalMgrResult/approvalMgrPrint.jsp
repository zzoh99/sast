...
$(function() {
	...
	$("#applTitle").html( $("#applTitle",opener.document).html() );
	$("#author_info").html( $("#author_info",opener.document).html() );
	$("#app_header").html( $(".app_header",opener.document).html() );
	$("#etcCommentDiv").html( $("#etcCommentDiv",opener.document).html() );
	$("#trComments").html( $("#trComments",opener.document).html() );
	$("#authorForm").html( $("#authorFormAttr",opener.document).html() );
	//$("#authorForm").html( $("#authorForm",opener.document).html() );
	$("#memoTable").html( $("#memoTable",opener.document).html() );
	$("#applBtnTop").hide();
	$("#commentWrite").hide();
	$(".button7").css("visibility", "hidden");

	//2019.09.24
	$(".author_left").css("padding", "0").css("margin-bottom", "5px");
	$(".author_right").css("padding", "0").css("margin-bottom", "5px");
...
<tr>
	...
	<td><btn:a href="javascript:p.self.close();" mid="close" mdef="닫기" css="gray large"/></td>
	<td><input type="checkbox" id="chkCmt" name="chkCmt" checked style="margin-bottom:-2px;"><label for="chkCmt"><span>&nbsp;<tit:txt mid="psnlWorkScheduleMgr2" mdef="유의사항" /></span></label></td>
<!-- 	<td><span style="color:red;">(화면 클릭 시 헤더 영역이 표시됩니다.)</span></td> -->
