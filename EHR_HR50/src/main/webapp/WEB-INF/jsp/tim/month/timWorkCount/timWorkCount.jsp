...
var pGubun = "";
	...
	$(function() {
		...
		//var url     = "queryId=getBusinessPlaceCdList"; // 2020.04.02 사용중인 사업장만 나오도록 처리
		//var businessPlaceCd = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList",url,false).codeList, "");	//사업장
		...
		$("#schYm").bind("keyup", function(event) {
			if( event.keyCode == 13 ) {
				doAction1("Search");
			}
		}) ;
		...
		$("#searchBusinessPlaceCd").bind("change", function(event) {
			doAction1("Search");
		}) ;
	...
	function doAction1(sAction) {
		...
		case "Search":
			...
			var data = ajaxCall("${ctx}/TimWorkCount.do?cmd=getTimWorkCount","schYm="+$("#schYm").val().replace(/-/gi,"")+"&searchBusinessPlaceCd="+$("#searchBusinessPlaceCd").val() ,false);
		...
		case "AppCancel":
			if($("#schYm").val() == "") {
				alert("<msg:txt mid='alertYmdCheck1' mdef='대상년월을 입력하여 주십시오.'/>");
				return;
			}
			//progressBar(true) ;
			if(!confirm(confirmMsg)) {
				...
				//추가하력 했으나 프로시져에서 이미 검증중... 2018.09.17
				/*
				if (sAction == "App") {
					//작업중인 급여 로그가 있는지 확인
					var ingYn = ajaxCall("${ctx}/GetDataMap.do?cmd=getTimWorkEndYn","searchBusinessPlaceCd="+$("#searchBusinessPlaceCd").val() ,false);
					if( ingYn.DATA.code != null && ingYn.DATA.code == "N" ) {
						if( confirm("<msg:txt mid='114804' mdef='이미 실행중입니다.\nClear 후 진행하시겠습니까?'/>") ) {
							//작업중이던 급여 로그를 강제 삭제 후 실행
							ajaxCall("/TimWorkCount.do?cmd=deleteTSYS906ForTimWorkCount&searchBusinessPlaceCd="+$("#searchBusinessPlaceCd").val(), "queryId=deleteTSYS906ForTimWorkCount", false) ;
						} else {
							return ;
						}
					}
			...
			var data;
			var param = "schYm="+$("#schYm").val().replace(/-/gi,"")+"&sabun="+$("#sabun").val()+"&placeCd="+$("#searchBusinessPlaceCd").val();
			var sync;
			...
			$.ajax({
				url : "${ctx}/TimWorkCount.do?cmd=prcTimWorkCount",
				type : "post",
				dataType : "json",
				async : sync,
				data : param,
		...
		case "CloseCancel":
			...
			var data;
			if(sAction == "Close") {
		    	data = ajaxCall("${ctx}/TimWorkCount.do?cmd=updateTimWorkCountEndYn",param+"&endYn=Y&searchBusinessPlaceCd="+$("#searchBusinessPlaceCd").val(),false);
			} else {
		    	data = ajaxCall("${ctx}/TimWorkCount.do?cmd=updateTimWorkCountEndYn",param+"&endYn=N&searchBusinessPlaceCd="+$("#searchBusinessPlaceCd").val(),false);
			}
		...
		<tr>
			<td class="top">
				...
				<tr>
					...
					<td>
						<input id="schYm" name="schYm" type="text" size="10" class="date2 required" value="<%= DateUtil.getCurrentTime("yyyy-MM")%>"/>
					</td>
				</tr>
				<tr>
				...
				<div class="center" style="margin-top: 10px">
					<btn:a href="javascript:doAction1('App');" css="btn soft" id="btnApp" style="display:none;" mid='appliedV3' mdef="작업"/>
					...
					<btn:a href="javascript:doAction1('CloseCancel');" css="btn filled large" id="btnCloseCancel" style="display:none;" mid='appCloseV1' mdef="마감취소"/>
				</div>
			</td>
		</tr>
		<tr>
