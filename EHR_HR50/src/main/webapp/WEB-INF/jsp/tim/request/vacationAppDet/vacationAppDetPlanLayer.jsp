...
<!DOCTYPE html><html><head><title>휴가계획가져오기</title>
...
$(function() {
	...
	var data = ajaxCall("/VacationApp.do?cmd=getVacationAppDetPlanPopupMap", 'searchApplSabun=' + vacationPlanOption.searchApplSabun, false);
	if ( data && data.DATA ){
...
function closeVacationPlanLayer() {
	...
	modal.fire('vacationAppDetPlanTrigger', null).hide();
}
...
function closeVacationPlanLayerRtnFunc(param) {
	...
	const modal = window.top.document.LayerModalUtility.getModal(vacationPlanOption.id);
	modal.fire('vacationAppDetPlanTrigger', p).hide();
}

function createVacationPlanSheet() {
	createIBSheet3($('#vacationAppDetLayerSheetArea').get(0), "vacationPlanSheet", "100%", "100%", '${ssnLocaleCd}');
	var initdata = {};
...
function vacationPlanAction(action) {
	...
	case 'Search':
		...
		vacationPlanSheet.DoSearch('/VacationApp.do?cmd=getVacationAppDetPlanPopupList', queryStringToJson(p));
		break;
	case 'Clear':
...
<body class="bodywrap">
	<div class="wrapper modal_layer">
			...
			<div class="sheet_search outer">
				<div>
					<table>
						<tr>
							<th>소속</th>
							<td><input id="vacationPlanOrgNm" type="text" class="text transparent readonly" readonly /></td>
							<th>사번/성명</th>
							<td><input id="vacationPlanName" type="text" class="text transparent readonly" readonly /></td>
							<th>년도</th>
							<td><input id="vacationPlanSearchYear" type="text" class="text" /></td>
							<td><a href="javascript:vacationPlanAction('Search')" class="btn dark">조회</a></td>
						</tr>
					</table>
				</div>
			</div>
			<table border="0" cellspacing="0" cellpadding="0" class="sheet_main">
				<tr>
					<td>
						...
						<div id="vacationAppDetLayerSheetArea"></div>
					</td>
				</tr>
			</table>
		</div>
		<div class="modal_footer">
			<ul>
				<li>
					<a href="javascript:closeVacationPlanLayerRtnFunc();" class="btn filled">선택</a>
					<a href="javascript:closeVacationPlanLayer();" class="btn outline_gray">닫기</a>
				</li>
			</ul>
		</div>
	</div>
