...
<!DOCTYPE html> <html class="hidden"><head> <title>해외출장신청/보고서 세부내역</title>
...
var searchBfApplSeq	 = "${etc01}"; //출장신청서 순번
var applStatusCd	 = "";
var pGubun           = "";
var gPRow = "";
var codeLists;
...
var hdn = 1, kfd = 0;
	...
	function init_sheet1(){
		...
		initdata.Cols = [
			...
			{Header:"<sht:txt mid='sDelete' mdef='삭제'/>",		Type:"${sDelTy}",	Hidden:hdn,					Width:"${sDelWdt}",	Align:"Center",	ColMerge:0,	SaveName:"sDelete",	Sort:0 },
  			...
  			{Header:"<sht:txt mid='name' mdef='성명'/>", 			Type:"Text",  		Hidden:0, Width:80, Align:"Center", SaveName:"empName",     KeyField:kfd, Format:"",   InsertEdit:1, UpdateEdit:0 },
	...
	function init_sheet2(){
		...
		initdata.Cols = [
			...
			{Header:"<sht:txt mid='sDelete' mdef='삭제'/>",		Type:"${sDelTy}",	Hidden:hdn,					Width:"${sDelWdt}",	Align:"Center",	ColMerge:0,	SaveName:"sDelete",	Sort:0 },
  			...
  			{Header:"<sht:txt mid='armyMemo' mdef='비고'/>", 		Type:"Text",  		Hidden:0, Width:150, Align:"Left",   SaveName:"workNote",  KeyField:0,   Format:"",    Edit:1 },
	...
	function init_sheet3(){
		...
		initdata.Cols = [
			...
			{Header:"<sht:txt mid='sDelete' mdef='삭제'/>",		Type:"${sDelTy}",	Hidden:hdn,					Width:"${sDelWdt}",	Align:"Center",	ColMerge:0,	SaveName:"sDelete",	Sort:0 },
  			...
  			{Header:"<sht:txt mid='cost_V1' mdef='사용금액'/>($)", 	Type:"Int",  		Hidden:0, Width:80,  Align:"Right",  SaveName:"payMonUs", KeyField:0, 	Format:"",   Edit:1 },
			...
			{Header:"<sht:txt mid='cost_V1' mdef='사용내역'/>", 		Type:"Text",  		Hidden:0, Width:200, Align:"Left",   SaveName:"useMemo",  KeyField:kfd, Format:"",   Edit:1 },
	...
	function doAction(sAction) {
		...
		case "Search":
			// 입력 폼 값 셋팅
			var data = ajaxCall( "${ctx}/BizTripApp.do?cmd=getBizTripAppDetMap", $("#searchForm").serialize(),false);
			if(data.Message != "" ){
			...
			sheet1.DoSearch( "${ctx}/BizTripApp.do?cmd=getBizTripAppDetList1", $("#searchForm").serialize() );
			sheet2.DoSearch( "${ctx}/BizTripApp.do?cmd=getBizTripAppDetList2", $("#searchForm").serialize() );
			sheet3.DoSearch( "${ctx}/BizTripApp.do?cmd=getBizTripAppDetList3", $("#searchForm").serialize() );
	...
	function sheet1_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			if( sheet1.RowCount() == 0 ){
				...
				//유저 데이터
				var data = ajaxCall("${ctx}/BizTripApp.do?cmd=getBizTripAppDetUserMap","searchSabun="+searchApplSabun+"&searchApplYmd="+searchApplYmd ,false);
				if(data.Message != "" ){
					alert(data.Message); return;
				} 
				if ( data != null && data.DATA != null ){ 
	...
	function sheet3_OnChange(Row, Col, Value){
		try{
			//달러금액 입력 시
		    if( sheet3.ColSaveName(Col) == "payMonUs" && $("#exchgRate").val() != "") {	
		    	var iexchgRate = parseInt($("#exchgRate").val().replace(/,/gi,""));
	    		sheet3.SetCellValue(Row, "payMon", Value * iexchgRate ); //원화금액 계산
			}
		}catch(ex){
	...
	function setSaveSheet(){
        ...
        //sheet3 출장경비내역
        var arr3 = ["paySabun","payGubunCd","payMonUs", "payMon", "useMemo", "payNote"];
	...
	function checkList(status) {
        ...
        if( ch ){
            ...
            for(var i = sheet1.HeaderRows(); i < sheet1.RowCount()+sheet1.HeaderRows() ; i++) {
    			...
    			//기 신청건 체크 
    			var data = ajaxCall("${ctx}/BizTripApp.do?cmd=getBizTripAppDetDupCnt", param,false);
    			if(data.Message != "" ){
    				alert("기 신청건 체크  시 오류가 발생 했습니다.");
    				return false;
    			}
    			if(data.DATA != null && data.DATA.dupCnt1 != "null" && data.DATA.dupCnt1 != "0"){
    				alert("[" + sheet1.GetCellValue(i, "empName") + "] 동일한 일자에 신청내역이 있습니다.");
    				return false;
    			}
    			if(data.DATA != null && data.DATA.dupCnt2 != "null" && data.DATA.dupCnt2 != "0"){
    				alert("[" + sheet1.GetCellValue(i, "empName") + "] 동일한 일자에 근태 신청내역이 있습니다.");
    				return false;
    			}
            }
	...
	function setValue(status) {
		...
		try {
            ...
            var params = $("#searchForm").serialize()+"&"+$("#dataForm").serialize()+"&"+saveSheet.GetSaveString(0)+"&exchgRate="+$("#exchgRate").val();

	      	//저장
			var data = ajaxCall("${ctx}/BizTripApp.do?cmd=saveBizTripAppDet",params,false);
 ...
 * 	아래코드 사용안함(참고소스로 남겨둠). common.js 의 resizeSheetHeight() 사용
	//시트행 갯수에 맞게 시트 높이 설정
	function setSheetHeight2(sheet){
		if(authPg == "A") return;
...
<div class="wrapper">
	...
	<input type="hidden" id="bizCd"				name="bizCd"	 		 value="20"/>
	...
	<div class="dh sheet_title">
		<ul>
			...
			<li class="btn"> 
				<b>환율 : </b> 
				<input type="text" id="exchgRate" name="exchgRate" class="text w70" readonly/>
