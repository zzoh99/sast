...
<!DOCTYPE html> <html class="hidden"><head> <title>연장근무사전신청 세부내역</title>
...
var pGubun = "";
	...
	$(function() {
		...
		if(authPg == "A") {
			...
			if(applStatusCd == "") { //최초 입력 시에만 수정 가능함. 임시저장상태에서는 못하게 함.
				...
				$("#searchYmdBase").val("${curSysYyyyMMddHyphen}");
				
				//근무일
				$("#searchYmd").datepicker2({
					//enddate:"searchYmdBase",
	...
	function initOrgCdCombo(){
		...
		sheet1.RemoveAll();
		var obj = ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList", "queryId=getOtWorkOrgAppDetOrgCdList&"+ $("#searchForm").serialize(), false).codeList;
	...
	function init_sheet(){
		...
		initdata1.Cols[v++] = {Header:"<sht:txt mid='workSchedule_lb_dbl' mdef='근무\n스케줄|근무\n스케줄'/>",	Type:"Text",		Hidden:0,  Width:70, 	Align:"Center", 	SaveName:"timeNm",		KeyField:kf, Format:"",     UpdateEdit:0,   InsertEdit:0 };
		...
		initdata1.Cols[v++] = {Header:"<sht:txt mid='weekOtTimeSum' mdef='주 연장시간|합계'/>",          	Type:"Text",		Hidden:0,  Width:40,	Align:"Center",  	SaveName:"weekOtTime",	KeyField:0,  Format:"",   Edit:0 };
		initdata1.Cols[v++] = {Header:"<sht:txt mid='weekOtTimeRemains' mdef='주 연장시간|잔여'/>",        Type:"Text",		Hidden:0,  Width:40,	Align:"Center",  	SaveName:"weekOtTimeRem",	KeyField:0,  Format:"",   Edit:0 };
		initdata1.Cols[v++] = {Header:"<sht:txt mid='termOtTimeSum' mdef='단위기간 연장시간|합계'/>",		Type:"Text",		Hidden:0,  Width:40,	Align:"Center",  	SaveName:"termOtTime",	KeyField:0,  Format:"",   Edit:0 };
		initdata1.Cols[v++] = {Header:"<sht:txt mid='termOtTimeRemains' mdef='단위기간 연장시간|잔여'/>",	Type:"Text",		Hidden:0,  Width:40,	Align:"Center",  	SaveName:"termOtTimeRem",	KeyField:0,  Format:"",   Edit:0 };
		...
		initdata1.Cols[v++] = {Header:"<sht:txt mid='inTime_V2' mdef='출근시간|출근시간'/>",								Type:"Date",		Hidden:0,  Width:50,	Align:"Center",  	SaveName:"reqSHm",		KeyField:kf,  Format:"Hm",   UpdateEdit:1,   InsertEdit:1,   EditLen:5};
		...
		initdata1.Cols[v++] = {Header:"<sht:txt mid='requestOtHour_lb_dbl' mdef='연장근무\n신청시간|연장근무\n신청시간'/>",	Type:"NullFloat",	Hidden:0,  Width:50,	Align:"Center",  	SaveName:"requestHour", KeyField:kf, Format:"",   	UpdateEdit:0,   InsertEdit:0};
		initdata1.Cols[v++] = {Header:"<sht:txt mid='reasonCdV1' mdef='신청사유|신청사유'/>",								Type:"Text",		Hidden:0,  Width:120,   Align:"Left",   	SaveName:"reason",		KeyField:0,  Format:"",     UpdateEdit:1,   InsertEdit:1,   EditLen:200 };
		...
		sheet1.SetRangeBackColor(0,sheet1.SaveNameCol("reqSHm"),1,sheet1.SaveNameCol("reqEHm"), "#fdf0f5");  //분홍이
	...
	function doAction(sAction) {
		...
		case "Search":
			// 입력 폼 값 셋팅
			var data = ajaxCall( "${ctx}/OtWorkOrgApp.do?cmd=getOtWorkOrgAppDetMap", $("#searchForm").serialize(),false);
			if(data.Message != "" ){
				alert(data.Message);
				return;
			}
		...
		case "SearchList": //대상자리스트
			...
			sheet1.DoSearch( "${ctx}/OtWorkOrgApp.do?cmd=getOtWorkOrgAppDetList", $("#searchForm").serialize()+"&searchType=list"  );
			break;
		case "Clear": //초기화
			if( $("#searchYmd").val() == "" ) return;
			...
			sheet1.DoSearch( "${ctx}/OtWorkOrgApp.do?cmd=getOtWorkOrgAppDetList", $("#searchForm").serialize()+"&searchType=clear" );
			break;
		}
	}
	...
	function sheet1_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			var data = ajaxCall( "${ctx}/OtWorkOrgApp.do?cmd=getOtWorkOrgAppDetTerDate", param,false);
			if(data.Message != "" ){
				alert(data.Message);
				return;
			}
	...
	function sheet1_OnChange(Row, Col, Value) {
		try{
			...
			}else if( sheet1.ColSaveName(Col) == "reqSHm" || sheet1.ColSaveName(Col) == "reqEHm") {
				if( Value.length == 4 ){
	...
	function sheet1_OnKeyUp(Row, Col) {
		try{
			var colName = sheet1.ColSaveName(Col);
			if( sheet1.ColSaveName(Col) == "reqSHm" || sheet1.ColSaveName(Col) == "reqEHm") {
	...
	function setSheetWorkTime(Row){
		...
		var workTimeList = ajaxCall("${ctx}/OtWorkOrgApp.do?cmd=getOtWorkOrgAppDetTimeMap", param, false);
	...
	function setValue(status) {
		...
		try {
			...
			//신청시간 및 연장근무 한도 체크 
			var chkTime = ajaxCall("${ctx}/OtWorkOrgApp.do?cmd=getOtWorkOrgAppDetCheckTime", saveStr+"&"+$("#searchForm").serialize(),false);
			if(chkTime.Message != "" ){
			...
			/* 대상자별  체크 */
			//기 신청건 체크 
			var chkDup = ajaxCall("${ctx}/OtWorkOrgApp.do?cmd=getOtWorkOrgAppDetDupCnt", saveStr+"&"+$("#searchForm").serialize(),false);
			if(chkDup.Message != "" ){
			...
			var data = eval("("+sheet1.GetSaveData("${ctx}/OtWorkOrgApp.do?cmd=saveOtWorkOrgAppDet", saveStr+"&"+$("#searchForm").serialize())+")");
	...
	function searchTimeSheet() {
		...
		var headerList = ajaxCall("${ctx}/OtWorkOrgApp.do?cmd=getPsnlWorkScheduleMgrDayWorkHeaderList", param, false);
		if (headerList != null && headerList.DATA != null) {
			...
			timeSheet.DoSearch( "${ctx}/OtWorkOrgApp.do?cmd=getPsnlWorkScheduleMgrDayWorkList", param );
...
<div class="wrapper">
	...
	<table class="table">
		...
		<tr>
			...
			<td>
				<select id="searchOrgCd" name="searchOrgCd" class="${selectCss} ${required}" ></select>
			</td>
	...
	<div class="sheet_title">
		<ul>
			...
			<li class="btn"><a href="javascript:doAction('Clear');" class="btn outline_gray" id="btnClear" style="display:none;"><tit:txt mid="112391" mdef="초기화" /></a></li>
		</ul>
	</div>
