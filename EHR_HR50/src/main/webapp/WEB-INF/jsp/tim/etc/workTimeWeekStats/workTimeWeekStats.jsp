...
<c:if test="${ssnSearchType == 'O'}">		
		...
		var user = ajaxCall("${ctx}/WorkTimeWeekStats.do?cmd=getWorkTimeWeekStatsMyWorkGrp","" ,false);
	...
	function doAction1(sAction) {
		...
		case "Title" :
			...
			var titleList = ajaxCall("${ctx}/WorkTimeWeekStats.do?cmd="+cmd, param, false);
		...
		case "Search":
			//sheet1 헤더 생성
			doAction1("Title");
			sheet1.DoSearch( "${ctx}/WorkTimeWeekStats.do?cmd=getWorkTimeWeekStatsList", $("#sheet1Form").serialize());
        	break;	
        case "Down2Excel":
	...
	function doAction2(sAction) {
		switch (sAction) {
			case "Search":
				...
				sheet2.DoSearch( "${ctx}/WorkTimeWeekStats.do?cmd=getWorkTimeWeekPerList", $("#sheet1Form").serialize());
	        	break;	
	        case "Down2Excel":
				var downcol = makeHiddenSkipCol(sheet1);
				var param  = {DownCols:downcol,SheetDesign:1,Merge:1,ExcelFontSize:"9",ExcelRowHeight:"20"};
