...
var isChangeDate = true;
	...
	$(function() {
		...
		$("#searchWorkGrpCd, #searchOrgCd, #searchJikweeCd, #searchManageCd, #searchBizPlaceCd").bind("change",function(event){
			//doAction1("Search");
		});
		...
		$("input[name='searchColFlag']").bind("change",function(event){
			...
			var hdn_u = ( $("#searchColFlag3").is(':checked') == true )?0:1;
	...
	function getCommonCodeList() {
		...
		$("#searchWorkGrpCd").html(codeLists["T11020"][2]);
	}
	...
	function doAction1(sAction) {
		...
		case "Search":
			if(!chkInVal()){break;}
			searchTitleList();
			sheet1.DoSearch( "${ctx}/WorkTimeDayStats.do?cmd=getWorkTimeDayStatsList", $("#sheetForm").serialize() );
			break;
        case "Down2Excel":
	...
	function searchTitleList() {
		...
		titleList = ajaxCall("${ctx}/WorkTimeDayStats.do?cmd=getWorkTimeDayStatsHeaderList", $("#sheetForm").serialize(), false);
		if (titleList != null && titleList.DATA != null) {
			...
			initdata1.Cols[v++] = {Header:"근무그룹|근무그룹|근무그룹",	Type:"Text",		Hidden:0,  Width:100,	Align:"Center",  ColMerge:0,   SaveName:"workGrpNm",		KeyField:0,   CalcLogic:"",   Format:"",		 PointCount:0,	 UpdateEdit:0,	 InsertEdit:0};
			initdata1.Cols[v++] = {Header:"근무조|근무조|근무조",		Type:"Text",		Hidden:0,  Width:70,	Align:"Center",  ColMerge:0,   SaveName:"workOrgNm",		KeyField:0,   CalcLogic:"",   Format:"",		 PointCount:0,	 UpdateEdit:0,	 InsertEdit:0};
			...
			for(var i=0; i<titleList.DATA.length; i++) {
				initdata1.Cols[v++] = {Header:titleList.DATA[i].year+"|"+titleList.DATA[i].shortDayNm+"|근무",	Type:"Text",	Hidden:hiddenFlag1,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:titleList.DATA[i].saveName,	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 };
				initdata1.Cols[v++] = {Header:titleList.DATA[i].year+"|"+titleList.DATA[i].shortDayNm+"|연장",	Type:"Text",	Hidden:hiddenFlag2,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:titleList.DATA[i].extSaveName,	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 };
				initdata1.Cols[v++] = {Header:titleList.DATA[i].year+"|"+titleList.DATA[i].shortDayNm+"|계",		Type:"Text",	Hidden:hiddenFlag3,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:titleList.DATA[i].totSaveName,	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 };
			}
			...
			<table>
				<tr>
					...
					<td>
						...
						<input id="searchColFlag3" name="searchColFlag" type="checkbox" class="checkbox" value="3" checked /><label for="searchColFlag3">&nbsp;계&nbsp;</label>&nbsp;
					</td>
				</tr>
				<tr>
					<th>소속</th>
