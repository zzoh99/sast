...
<script type="text/javascript">
	...
	$(function() {
		createIBSheet3(document.getElementById('orgMonthWorkStaLayerSheet-wrap'), "orgMonthWorkStaLayerSheet", "100%", "100%", "kr");
		...
		var modal = window.top.document.LayerModalUtility.getModal('orgMonthWorkStaLayer');
		var {ym, sabun, name, orgNm, jikgubNm} = modal.parameters;
		var titleTxt = "개인별 월근태현황"

		$('#modal-orgMonthWorkStaLayer').find('div.layer-modal-header span.layer-modal-title').text(titleTxt);
		...
		orgMonthWorkStaLayerSheet.SetSheetHeight(sheetHeight);
	...
	function initSheet(){
		...
		IBS_InitSheet(orgMonthWorkStaLayerSheet, initdata);
		orgMonthWorkStaLayerSheet.SetEditable("${editable}");
		orgMonthWorkStaLayerSheet.SetVisible(true);
		orgMonthWorkStaLayerSheet.SetCountPosition(4);
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search": //조회
            orgMonthWorkStaLayerSheet.DoSearch( "${ctx}/OrgMonthWorkSta.do?cmd=getOrgMonthWorkStaPopList", $("#mySheetForm").serialize());
            break;
		case "Down2Excel":
			var downcol = makeHiddenSkipCol(orgMonthWorkStaLayerSheet);
			var param  = {DownCols:downcol,SheetDesign:1,Merge:1,ExcelFontSize:"9",ExcelRowHeight:"20"};
			orgMonthWorkStaLayerSheet.Down2Excel(param);
			break;
		}
    } 
	...
	//-----------------------------------------------------------------------------------
	//		orgMonthWorkStaLayerSheet 이벤트
	//-----------------------------------------------------------------------------------
	// 조회 후 에러 메시지 
	function orgMonthWorkStaLayerSheet_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try{
			if(Msg != "") alert(Msg);    
			sheetResize();
	  	}catch(ex){
...
<%--		<script type="text/javascript">createIBSheet("orgMonthWorkStaLayerSheet", "100%", "100%"); </script>--%>
		<div id="orgMonthWorkStaLayerSheet-wrap"></div>
		<div class="modal_footer">
			<a href="javascript:closeCommonLayer('orgMonthWorkStaLayer');" class="gray large">닫기</a>
		</div>
	</div>
</div>
</body>
