...
<%
    ...
    /* Fasoo DRM 복호화 관련 변수 START */
    String drm_fsdinit_path = "/data/insa/FasooKey/";			// fsdinit 폴더 FullPath 설정 
	String fsn_domain_id = "0100000000001165";					// 교보증권 고유 코드
	String fsd_domain_id = "0100000000001173";					// 교보증권 고유 코드
	
	String tmpFilePath = "";
    String nfsUploadFilePath = StringUtil.getPropertiesValue("NFS.HRFILE.PATH");
    if(nfsUploadFilePath != null && nfsUploadFilePath.length() > 0) {
    	tmpFilePath = nfsUploadFilePath+StringUtil.getPropertiesValue("HRFILE.PATH")+File.separator+"tmp";
    } else {
    	tmpFilePath = StringUtil.getPropertiesValue("WAS.PATH")+StringUtil.getPropertiesValue("HRFILE.PATH")+File.separator+"tmp";
    }
...
try {
    ...
    for(int i = 0 ; i < v_fileCnt ; i++){
		...
		//////////////////////// START : Fasoo DRM 복호화 작업 //////////////////////
		// TEMP 디렉토리 생성
		File tmpDir = new File(tmpFilePath);
		if(!tmpDir.exists()) tmpDir.mkdirs();

		if ( fileItem != null ) {
			if ( 921600 < fileItem.getSize() )  { // BYTE 기준 : 900 * 1,024 = 921,600 BYTE (약 20페이지의 국세청 PDF 1본이 대략 350KB임. 그 3배를 한도로 제한.)
				throw new UserException("업로드 최대 용량은 900KB입니다.");
			}
			if ( ! "application/pdf".equalsIgnoreCase(fileItem.getContentType()) ) {
				throw new UserException("업로드는 PDF만 가능합니다.");
			}				
		}

		fileItem.write(new File(tmpFilePath, fileName));

		String extName = fileName.substring(fileName.lastIndexOf("."), fileName.length());
		String decFileName = fileName.substring(0, fileName.lastIndexOf(".")) + "_DEC" + extName;
		
		int iBret = 0;
		WorkPackager oWorkPackager = new WorkPackager();
		iBret = oWorkPackager.GetFileType(tmpFilePath+File.separator+fileName);		// 파일 타입 체크

		if (iBret == 103) {			// 암호화 문서(FSN Type)일 경우 복호화
			boolean bRet = false;
	 		bRet = oWorkPackager.DoExtract(drm_fsdinit_path, fsn_domain_id, tmpFilePath+File.separator+fileName, tmpFilePath+File.separator+decFileName);
		
			error_num = oWorkPackager.getLastErrorNum();
			error_str = oWorkPackager.getLastErrorStr();

			//복호화에 성공(DRM이 걸린 파일)하면 복호화된 파일을 업로드 
			if (error_num!=0){
				//out.println("<script>alert('error_num : "+error_num+"');</script>");
				//out.println("<script>alert('error_str : "+error_str+"');</script>");
	
				Error_Check = 1;
				Error_Message = "DRM_PKGING_ERROR";

				File f1 = new File(tmpFilePath, fileName);
				if (f1.exists()) f1.delete();
				File f2 = new File(tmpFilePath, decFileName);
				if (f2.exists()) f2.delete();
			}
		 	else {
				// FSN 문서 복호화 Success
				File f = new File(tmpFilePath, fileName);
				f.delete();
			}
		} else if (iBret == 26) {			// 암호화 문서(FSD Type)일 경우 복호화
			boolean bRet = false;
	 		bRet = oWorkPackager.DoExtract(drm_fsdinit_path, fsd_domain_id, fileName, decFileName);
		...
		//////////////////////// END : Fasoo DRM 복호화 작업 //////////////////////
		
		
		
        Log.Debug("fileName ===== "+decFileName);
        // PDF파일이 아닌 경우 skip
        if ( decFileName == null || !decFileName.toUpperCase().endsWith(".PDF") ) {
            throw new UserException("PDF 파일이 아닙니다.");
        }
        
        File decFile = new File(tmpFilePath, decFileName);
        FileInputStream input = new FileInputStream(decFile);
        int decFileLength = (int)decFile.length();
        int readBuffer = 0;
        
        // 파일내용을 읽음
        byte[] pdfBytes = new byte[decFileLength];
        while((readBuffer = input.read(pdfBytes)) != -1) {
        	System.out.write(pdfBytes, 0, readBuffer);
        }
        
		String pdfBytesEnc = new InputStreamReader(input).getEncoding().toLowerCase();
        ...
        if(dataList != null && dataList.size() > 0) {
            ...
            try{
                ...
                if(!fileItem.isFormField()) {
                    ...
                    if(fileSize > 0) { 
                    	String attFileNm = tmpFilePath+File.separator+decFileName;
                        String saveFileNm = paramSabun+attFileNm.substring(attFileNm.lastIndexOf("."));
                        String dbfilePath = ""; 
                        		//StringUtil.getPropertiesValue("HRFILE.PATH")+"/YEA_PDF/"+ssnEnterCd+"/"+paramYear+"/SUM";
                        String saveFilePath = "";
                        
                        String nfsUploadPath = StringUtil.getPropertiesValue("NFS.HRFILE.PATH");
                        if(nfsUploadPath != null && nfsUploadPath.length() > 0) {
                        	dbfilePath = nfsUploadPath+StringUtil.getPropertiesValue("HRFILE.PATH")+"/YEA_PDF/"+ssnEnterCd+"/"+paramYear+"/SUM";
                            saveFilePath = nfsUploadPath+StringUtil.getPropertiesValue("HRFILE.PATH")+"/YEA_PDF/"+ssnEnterCd+"/"+paramYear+"/SUM";
                        } else {
                        	dbfilePath = StringUtil.getPropertiesValue("HRFILE.PATH")+"/YEA_PDF/"+ssnEnterCd+"/"+paramYear+"/SUM";
                        ...
                        decFile = new File(tmpFilePath, decFileName);
                        input = new FileInputStream(decFile);
                        decFileLength = (int)decFile.length();
                        readBuffer = 0;
                        
                        byte[] buffer = new byte[decFileLength];
