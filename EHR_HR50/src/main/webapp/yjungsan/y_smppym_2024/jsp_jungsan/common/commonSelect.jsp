...
public List selectCommonSelectList(Map paramMap, Map queryMap) throws Exception {
...
public Map selectCommonSelectMap(Map paramMap, Map queryMap) throws Exception {
...
<%
	//쿼리 맵 셋팅
	Map queryMap = setQueryMap(xmlPath+"/common/commonSelect.xml");
	...
	if("commonSelectList".equals(cmd)) {
		...
		try {
			listData = selectCommonSelectList(mp, queryMap);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			mapData = selectCommonSelectMap(mp, queryMap);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
