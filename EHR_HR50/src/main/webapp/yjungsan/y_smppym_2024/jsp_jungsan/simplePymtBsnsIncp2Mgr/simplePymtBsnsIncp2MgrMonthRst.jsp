...
//사업소득관리 다건 조회
public List getSimplePymtBsnsIncp2Mgr(Map paramMap, Map queryMap) throws Exception {
...
//사업소득관리 저장
public int saveSimplePymtBsnsIncp2Mgr(Map paramMap, Map queryMap) throws Exception {
...
<%
	//쿼리 맵 셋팅
	Map queryMap = setQueryMap(xmlPath+"/simplePymtBsnsIncp2Mgr/simplePymtBsnsIncp2MgrMonth.xml");
	...
	if("getSimplePymtBsnsIncp2Mgr".equals(cmd)) {
		...
		try {
			listData = getSimplePymtBsnsIncp2Mgr(mp, queryMap);
		} catch(Exception e) {
			code = "-1";
	...
	//사업소득관리 저장
        ...
        try {
            int cnt = saveSimplePymtBsnsIncp2Mgr(mp, queryMap);
