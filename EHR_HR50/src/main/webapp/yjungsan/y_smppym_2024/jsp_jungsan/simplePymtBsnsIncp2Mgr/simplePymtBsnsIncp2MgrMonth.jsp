...
<!DOCTYPE html> <html class="hidden"><head> <title>연말_사업소득관리</title>
...
<script type="text/javascript">
	...
	businessIncomeList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getBsnsIncpCodeList&searchYM=<%=curSysYear%><%=curSysMon%>","C00508"), "");
	...
	$(function() {
		...
		initdata0.Cols = [
                ...
                {Header:"거주\n지국",		Type:"Combo",      	Hidden:0,  	Width:100,	Align:"Center",    	ColMerge:0,   	SaveName:"residency_nm",   		KeyField:0,   	CalcLogic:"",   Format:"",    		PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:500 , Wrap:1},
                {Header:"거주지국\n코드",	Type:"Text",      	Hidden:0,  	Width:100,	Align:"Center",    	ColMerge:0,   	SaveName:"residency_cd",   		KeyField:0,   	CalcLogic:"",   Format:"",    		PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:500},
                {Header:"발생기간",		Type:"Date",      	Hidden:0,  	Width:100,  Align:"Center",  	ColMerge:0,   	SaveName:"adj_s_ymd",			KeyField:0,   	CalcLogic:"",   Format:"Ymd",   	PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:8 },
                {Header:"사업별",			Type:"Combo",      	Hidden:0,  	Width:100,	Align:"Center",    	ColMerge:0,   	SaveName:"business_cd",   		KeyField:0,   	CalcLogic:"",   Format:"",    		PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:500 },

                {Header:"소득\n귀속연도",	Type:"Text",      	Hidden:0,  	Width:100,  Align:"Center",  	ColMerge:0,   	SaveName:"cre_work_yy",			KeyField:1,   	CalcLogic:"",   Format:"",   		PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:4 },
                {Header:"지급금액",		Type:"AutoSum",		Hidden:0,  	Width:100,	Align:"Right",    	ColMerge:0,   	SaveName:"tax_mon",   			KeyField:0,   	CalcLogic:"",   Format:"Integer",   PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:500 },
                ...
                {Header:"월구분",		    Type:"Text",		Hidden:1,  	Width:0,	Align:"Left",    	ColMerge:0,   	SaveName:"work_mm",   			KeyField:0,   	CalcLogic:"",   Format:"",          PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:500 },
        ...
        initdata2.Cols = [
			{Header:"",   Type:"Text",    Hidden:0,    Width:80,    Align:"Center",    ColMerge:0,    SaveName:"citizen_type" },		//내/외국인
			{Header:"",   Type:"Text",    Hidden:0,    Width:80,    Align:"Center",    ColMerge:0,    SaveName:"residency_type" },		//거주지구분
			{Header:"",   Type:"Text",    Hidden:0,    Width:80,    Align:"Center",    ColMerge:0,    SaveName:"residency_nm" },		//거주지국
			{Header:"",   Type:"Text",    Hidden:0,    Width:80,    Align:"Center",    ColMerge:0,    SaveName:"business_cd" },			//사업별
			{Header:"",   Type:"Text",    Hidden:0,    Width:80,    Align:"Center",    ColMerge:0,    SaveName:"send_type" },			//신고구분
			{Header:"",   Type:"Text",    Hidden:0,    Width:80,    Align:"Center",    ColMerge:0,    SaveName:"business_place_cd" }	//사업장
		...
		/* 거주지국 */
		sheet1.SetColProperty("residency_nm", 		{ComboText:"|"+residencyList[0], ComboCode:"|"+residencyList[1]} );
	...
	function sheet1_OnChange(Row, Col, Value) {
		try {
			if((sheet1.GetCellValue(Row,"sStatus") == "I" || sheet1.GetCellValue(Row,"sStatus") == "U" )
					&& (sheet1.ColSaveName(Col) == 'residency_nm')) {

				var residencyCd = sheet1.GetCellValue(Row, "residency_nm");

				sheet1.SetCellValue(Row, "residency_cd",residencyCd);
			}
    ...
    function doAction1(sAction) {
        switch (sAction) {
	        case "Search":
	        	...
	        	sheet1.DoSearch( "<%=jspPath%>/simplePymtBsnsIncp2Mgr/simplePymtBsnsIncp2MgrMonthRst.jsp?cmd=getSimplePymtBsnsIncp2Mgr", $("#mySheetForm").serialize() );
	        ...
	        case "Save":
		        ...
		        <%-- dupChk() 사용으로 변경 - 2020.01.14.
 				for(var j=1; j<=sheet1.LastRow()+1; j++) {
		        	...
		        	if(sheet1.GetCellValue(j, "sStatus") == "I") {
						...
						var data = ajaxCall("<%=jspPath%>/simplePymtBsnsIncp2Mgr/simplePymtBsnsIncp2MgrMonthRst.jsp?cmd=getSabunChk", param,false);
	        	...
	        	sheet1.DoSave( "<%=jspPath%>/simplePymtBsnsIncp2Mgr/simplePymtBsnsIncp2MgrMonthRst.jsp?cmd=saveSimplePymtBsnsIncp2Mgr", $("#mySheetForm").serialize());
	        ...
	        case "Insert":
        		...
        		sheet1.SetCellEditable(newRow, "residency_cd",false);
	        ...
	        case "Down2Template":
	        	...
	        	var titleText  = "작성방법 \n 1.내외국인/거주자구분/거주지국/사업별/신고구분/사업장 sheet2를 참조하여 작성\n"+
	        	                 "2.발생기간 : ex>2019-01-01\n"+
	        	                 "3.저장시 해당 Row 삭제 저장 후 Upload 해주시기 바랍니다.";

	            var titleText2 = "내/외\n국인|거주자\n구분|거주지국|사업별|신고구분|사업장" + "\r\n";
	            ...
	            var businessIncomeArr   = businessIncomeList[0].split("|");			//사업별
	            ...
	            var param  = {DownCols:"4|5|6|7|8|9|10|11|13|14|15|16",SheetDesign:1,Merge:1,DownRows:0,FileName:'Template',SheetName:'sheet1',UserMerge:"0,0,1,8", ExcelRowHeight:100, TitleText:titleText};
				...
				if (Msg != ""){
					...
					for(var i=1; i <= sheet1.LastRow(); i++){
						if(sheet1.GetCellValue(i, "final_close_yn") != "1"){
							...
							sheet1.SetCellEditable(i, "residency_nm",true);
	...
	function sheet1_OnLoadExcel(result) {
		...
		for(var i=1; i <= sheet1.LastRow(); i++){
        	...
        	sheet1.SetCellEditable(i, "final_close_yn",false);
        	/* 거주지국 코드 */
        	sheet1.SetCellEditable(i, "residency_cd",false);
        	var residencyCd = sheet1.GetCellValue(i, "residency_nm");
			sheet1.SetCellValue(i, "residency_cd",residencyCd);
        }
    }
	...
	function callProcPop() {
		...
		var url 	= "<%=jspPath%>/simplePymtBsnsIncp2Mgr/simplePymtBsnsIncp2MgrPopMonth.jsp";
        var args = new Array();
...
<div class="wrapper">
        ...
        <tr>
			...
			<td><span>월구분</span>
				<select id="searchWorkMm" name="searchWorkMm" onChange="javascript:searchWorkMm_onChange()" >
	                <option value="01" selected="selected" id="searchWorkMm1">1월</option>
				...
				<select id="searchHalfType" name="searchHalfType" hidden="false">
    ...
    <div class="outer">
        ...
        <ul>
            <li class="txt">연말_사업소득관리 <font color='red'>(주민번호 숫자만 입력해 주십시오)</font></li>
            <li class="btn">
