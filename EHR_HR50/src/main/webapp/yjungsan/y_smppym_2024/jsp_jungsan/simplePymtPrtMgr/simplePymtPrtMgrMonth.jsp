...
<script type="text/javascript">
	...
	$(function() {
		...
		/* 소득구분 */
		var includeGbList	= stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYM=<%=curSysYear%><%=curSysMon%>","YEA003"), "");
		...
		initdata1.Cols = [
			...
			{Header:"신고월",      	Type:"Text",      	Hidden:0,  Width:100,	Align:"Center",  ColMerge:0,   SaveName:"work_mm",  			KeyField:0,   CalcLogic:"",   Format:"",         PointCount:0,   UpdateEdit:0,   InsertEdit:0,	EditLen:100 },
	...
	function doAction1(sAction) {
		...
		case "Search":
			/* 거주자 기타소득만 선택가능하도록 (추후 수정) */
			$("#includeGb").prop("disabled", false);
			sheet1.DoSearch( "<%=jspPath%>/simplePymtPrtMgr/simplePymtPrtMgrMonthRst.jsp?cmd=selectSimplePymtPrtMgrList", $("#sheetForm").serialize() );
			break;
		}
	}
	...
	function print() {
		...
		if($("#includeGb").val() == "77"){
			var data = ajaxCall("<%=jspPath%>/simplePymtPrtMgr/simplePymtPrtMgrMonthRst.jsp?cmd=getHpChk", "",false);
	...
	function withHoldRcptStaPopup(sabuns, sortSabuns, sortNos, stamps, hpCnt){
		...
		if($("#includeGb").val() == '77'){
		   ...
		   /*  if($("#searchYear").val() < "2021" || ($("#searchYear").val() == "2021" && $("#searchHalfType").val() == "1")){
		    ...
		    rdFileNm        = "simplePymtReport01_2024.mrd";
            ppupTitle = "근로소득" ;//rd Popup제목
		}
		/* 사업소득 */
		...
		/* 거주자 기타소득 */
		else if($("#includeGb").val() == '55'){
			rdFileNm		= "simplePymtReport05_2024.mrd";
			popupTitle = "거주자의 기타소득" ;
		}
	        ...
	        <div>
		        <table>
			        <tr>
						...
						<td><span>신고월</span>
							<select id="searchWorkMm" name="searchWorkMm" onChange="javascript:doAction1('Search')" style="width: 119px;">
				            </select>
			        ...
			        <tr>
						<td><span>소득구분</span>
				            <select id="includeGb" name ="includeGb" onChange="javascript:workPartSet();doAction1('Search')" style="width: 118px;"></select>
						</td>
						<td><span>사업장</span>
