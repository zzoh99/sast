...
<script type="text/javascript">
	...
	//2021.07.16 업종코드
	var c00508 = ""; 	
	c00508 = codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYM=<%=curSysYear%><%=curSysMon%>", "C00508");
	...
	businessIncomeList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYM=<%=curSysYear%><%=curSysMon%>","C00508"), "");
...
// 	        	sheet1.ShowSubSum(info) ;
	        	sheet1.DoSearch( "<%=jspPath%>/simplePymtBsnsIncpMgr/simplePymtBsnsIncpMgrMonthRst.jsp?cmd=getSimplePymtBsnsIncpMgr", $("#mySheetForm").serialize() ); 
	        ...
	        case "Save":
		        ...
		        <%-- dupChk() 사용으로 변경 - 2020.01.14.
 				for(var j=1; j<=sheet1.LastRow()+1; j++) {
		        	...
		        	if(sheet1.GetCellValue(j, "sStatus") == "I") {
						...
						var data = ajaxCall("<%=jspPath%>/simplePymtBsnsIncpMgr/simplePymtBsnsIncpMgrMonthRst.jsp?cmd=getSabunChk", param,false);
	        	...
	        	sheet1.DoSave( "<%=jspPath%>/simplePymtBsnsIncpMgr/simplePymtBsnsIncpMgrMonthRst.jsp?cmd=saveSimplePymtBsnsIncpMgr", $("#mySheetForm").serialize());
	        ...
	        case "Insert":    
	        	...
	        	sheet1.SetCellEditable(newRow, "work_mm",false);
	        	break;
	        case "Copy":        
	...
	function callProcPop() {
		...
		var url 	= "<%=jspPath%>/simplePymtBsnsIncpMgr/simplePymtBsnsIncpMgrPopMonth.jsp";
        var args = new Array();
	...
	function finalCloseYnChk(val){
		...
		if(confirm(confirmMsg)){
			...
			ajaxCall("<%=jspPath%>/simplePymtBsnsIncpMgr/simplePymtBsnsIncpMgrMonthRst.jsp?cmd=updateSimplePymtEarnIncpMgr",params,false);
    ...
    */
	function workPartSet(){
            ...
            $('#searchWorkMm').empty();
            $('#searchWorkMm').append("<option value='01'>1월</option>");
            $('#searchWorkMm').append("<option value='02'>2월</option>");
            $('#searchWorkMm').append("<option value='03'>3월</option>");
            $('#searchWorkMm').append("<option value='04'>4월</option>");
            $('#searchWorkMm').append("<option value='05'>5월</option>");
            $('#searchWorkMm').append("<option value='06'>6월</option>");        
            $('#searchWorkMm').append("<option value='07'>7월</option>");
            $('#searchWorkMm').append("<option value='08'>8월</option>");
            $('#searchWorkMm').append("<option value='09'>9월</option>");
            $('#searchWorkMm').append("<option value='10'>10월</option>");
            $('#searchWorkMm').append("<option value='11'>11월</option>");
            $('#searchWorkMm').append("<option value='12'>12월</option>");
	}
    //값이 바뀔때 발생
