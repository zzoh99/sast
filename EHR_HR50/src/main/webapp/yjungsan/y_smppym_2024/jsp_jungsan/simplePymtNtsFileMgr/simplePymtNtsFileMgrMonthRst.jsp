...
<%!
//마감 체크
public List getBizLoc(Map paramMap, Map queryMap) throws Exception {
...
public String[] prcApply(Map paramMap) throws Exception {
	...
	String[] param = new String[]{"", "", ssnEnterCd, ssnSabun, tgtYear};

	String[] rstStr = null;
	try {
		rstStr = DBConn.executeProcedure("PKG_CPN_SYMPYM_MM_DISK.DISK_CODE_APPLY",type,param);
	} catch(Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("간이지급명세서 텍스트 파일 생성 프로시저(prcApply) 실행중 오류가 발생하였습니다.\n"+rstStr[1]);
	}
...
public String[] prcDiskAll(Map paramMap) throws Exception {
	...
	String tgtYear = (String)paramMap.get("tgtYear");
	/* 신고월 */
	String searchWorkMm = (String)paramMap.get("searchWorkMm");
	...
	String[] param = new String[]{"", ""
			, ssnEnterCd, tgtYear, searchWorkMm,  declClass, bizLoc
			, declYmdTemp, declPrsnClass, taxProxyNo, amendCd, termCd
			, declDept, declEmp, declEmpTel, hometaxId, ssnSabun};

	String[] rstStr = null;
	try {
		rstStr = DBConn.executeProcedure("PKG_CPN_SYMPYM_MM_DISK.DISK_ALL",type,param);
	} catch(Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("간이지급명세서 디스켓 생성 프로시저(prcDiskAll) 실행중 오류가 발생하였습니다.\n"+rstStr[1]);
	}
...
<%
	Map queryMap = XmlQueryParser.getQueryMap(xmlPath+"/simplePymtNtsFileMgr/simplePymtNtsFileMgrMonth.xml");
	...
	if("getBizLoc".equals(cmd)) {
		...
		try {
			listData = getBizLoc(mp, queryMap);
		...
		try {
			...
			//************************* 자료생성 프로시저 호출 시작 *************************//
			Log.Debug("== 간이지급명세서 텍스트 파일 생성 프로시저 호출 시작 ==");

			String[] rstStr = prcApply(paramMap);

			if(rstStr[1] != null && rstStr[1].length() != 0) {
				Log.Debug("== 간이지급명세서 텍스트 파일 생성 생성 프로시저(prcApply) 실행중 오류 발생:"+rstStr[1]+" ==");
				throw new UserException("간이지급명세서 텍스트 파일 생성 프로시저(prcApply) 실행중 오류가 발생하였습니다.\n"+rstStr[1]);
			}

			Log.Debug("== 간이지급명세서 텍스트 파일 생성 프로시저 호출 종료 ==");

			Log.Debug("== 간이지급명세서 디스켓생성 생성 프로시저 호출 시작 ==");


			String[] rstStr2 = prcDiskAll(paramMap);

			if(rstStr2[1] != null && rstStr2[1].length() != 0) {
				Log.Debug("== 간이지급명세서 텍스트 파일 생성 생성 프로시저(prcDiskAll) 실행중 오류 발생:"+rstStr2[1]+" ==");
				throw new UserException("간이지급명세서 텍스트 파일 생성 프로시저(prcDiskAll) 실행중 오류가 발생하였습니다.\n"+rstStr2[1]);
			}

			Log.Debug("== 간이지급명세서 디스켓생성 생성 프로시저 호출 종료 ==");
					...
					recodeCntC = 0;
						...
						if(listDataC != null && listDataC.size() > 0) {
							for(int i = 0; i < listDataC.size(); i++) {
								...
								mapDataC.put("searchWorkMm", (String)paramMap.get("searchWorkMm"));
