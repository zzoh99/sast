...
<%!
//컬럼명 조회
public List getSimplePymtNtstFileSta01(Map paramMap, Map queryMap) throws Exception {
...
//데이터 조회
public List getSimplePymtNtstFileSta02(Map paramMap, Map queryMap) throws Exception {
...
<%
	//쿼리 맵 셋팅
	Map queryMap = XmlQueryParser.getQueryMap(xmlPath+"/simplePymtNtstFileSta/simplePymtNtstFileSta.xml");
	...
	if("getSimplePymtNtstFileSta01".equals(cmd)) {
		...
		try {
			listData = getSimplePymtNtstFileSta01(mp, queryMap);
		} catch(Exception e) {
			code = "-1";
	...
	//데이터 조회
		...
		try {
			listData = getSimplePymtNtstFileSta02(mp, queryMap);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
