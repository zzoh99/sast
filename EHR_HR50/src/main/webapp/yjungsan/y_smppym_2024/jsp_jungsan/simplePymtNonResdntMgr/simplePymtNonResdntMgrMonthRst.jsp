...
<%!
//비거주자 사업기타소득관리 다건 조회
public List getSimplePymtNonResdntMgr(Map paramMap, Map queryMap) throws Exception {
...
//비거주자 사업기타소득관리 저장
public int saveSimplePymtNonResdntMgr(Map paramMap, Map queryMap) throws Exception {
...
<%  //쿼리 맵 셋팅 (xml 파서를 이용한 방법)
	Map queryMap = XmlQueryParser.getQueryMap(xmlPath+"/simplePymtNonResdntMgr/simplePymtNonResdntMgrMonth.xml");
	...
	if("getSimplePymtNonResdntMgr".equals(cmd)) {
		...
		try {
			listData = getSimplePymtNonResdntMgr(mp, queryMap);
		} catch(Exception e) {
			code = "-1";
	...
	//비거주자 사업기타소득관리 저장
        ...
        try {
            int cnt = saveSimplePymtNonResdntMgr(mp, queryMap);
