...
<!DOCTYPE html> <html class="hidden"><head> <title>비거주자 사업기타소득관리</title>
...
<script type="text/javascript">
	...
	etcList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYM=<%=curSysYear%><%=curSysMon%>","C00506"), "");
	...
	$(function() {
		...
		initdata0.Cols = [
                ...
                {Header:"주민(사업자)\n등록번호", 	Type:"Text",		Hidden:0,	Width:110,	Align:"Center",		ColMerge:0,		SaveName:"res_no",				KeyField:0,		CalcLogic:"",	Format:"IdNo",		PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:13 },
                {Header:"소득자의\n주소",			Type:"Text",      	Hidden:1,  	Width:150,	Align:"Left",    	ColMerge:0,   	SaveName:"addr",   				KeyField:0,   	CalcLogic:"",   Format:"",    		PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:500 , Wrap:1},
                ...
                {Header:"거주\n지국",				Type:"Combo",      	Hidden:0,  	Width:100,	Align:"Center",    	ColMerge:0,   	SaveName:"residency_cd",   		KeyField:0,   	CalcLogic:"",   Format:"",    		PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:500 , Wrap:1},
    ...
    function doAction1(sAction) {
        switch (sAction) {
	        case "Search":   
	        	...
	        	//소계 추가
	        	var info = [{StdCol:3 , SumCols:"15", CaptionCol:0}];
	        	sheet1.ShowSubSum(info) ;
	        	
	        	sheet1.DoSearch( "<%=jspPath%>/simplePymtNonResdntMgr/simplePymtNonResdntMgrMonthRst.jsp?cmd=getSimplePymtNonResdntMgr", $("#mySheetForm").serialize() ); 
	        ...
	        case "Save":
		        ...
		        <%-- dupChk() 사용으로 변경 - 2020.01.14.
 				for(var j=1; j<=sheet1.LastRow()+1; j++) {
		        	...
		        	if(sheet1.GetCellValue(j, "sStatus") == "I") {
						...
						var data = ajaxCall("<%=jspPath%>/simplePymtNonResdntMgr/simplePymtNonResdntMgrMonthRst.jsp?cmd=getSabunChk", param,false);
	        	...
	        	sheet1.DoSave( "<%=jspPath%>/simplePymtNonResdntMgr/simplePymtNonResdntMgrMonthRst.jsp?cmd=saveSimplePymtNonResdntMgr", $("#mySheetForm").serialize());
	        ...
	        case "Down2Template":
	            ...
	            var param  = {DownCols:"4|5|6|7|8|10|11|12|15",SheetDesign:1,Merge:1,DownRows:0,FileName:'Template',SheetName:'sheet1',UserMerge:"0,0,1,8", ExcelRowHeight:70,TitleText:titleText};
	...
	function callProcPop() {
		...
		var url 	= "<%=jspPath%>/simplePymtNonResdntMgr/simplePymtNonResdntMgrPopMonth.jsp";
        var args = new Array();
	...
	function finalCloseYnChk(val){
		...
		if(val == "C"){				
			confirmMsg = "[ "+$("#searchYear").val()+"년도 / "+$('#searchWorkMm option:selected').text()+"월 / 사업장("+$('#searchBusinessPlace option:selected').text()+") ]"+" 전체마감하시겠습니까?";
			closeYn    = "Y";
		}else{
			confirmMsg = "[ "+$("#searchYear").val()+"년도 / "+$('#searchWorkMm option:selected').text()+"월 / 사업장("+$('#searchBusinessPlace option:selected').text()+") ]"+" 전체마감 취소하시겠습니까?";
			closeYn    = "N";
		}		
		if(confirm(confirmMsg)){
			
			var params = $("#mySheetForm").serialize()+"&final_close_yn="+closeYn;	
			ajaxCall("<%=jspPath%>/simplePymtNonResdntMgr/simplePymtNonResdntMgrMonthRst.jsp?cmd=updateSimplePymtEarnIncpMgr",params,false);
...
<div class="wrapper">
    ...
    <div class="outer">
        ...
        <ul>
            <li class="txt">비거주자사업기타소득관리   <font color='red'>(주민번호 숫자만 입력해 주십시오)</font></li>
            <li class="btn">
