...
//프로그램관리 검색
public List selectPrgMgr(Map paramMap, Map queryMap) throws Exception {
...
//프로그램관리 저장.
public int savePrgMgr(Map paramMap, Map queryMap) throws Exception {
...
<%
	//쿼리 맵 셋팅
	Map queryMap = setQueryMap(xmlPath+"/auth/prgMgr.xml");
	...
	if("selectPrgMgr".equals(cmd)) {
		...
		try {
			listData = selectPrgMgr(mp, queryMap);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			int cnt = savePrgMgr(mp, queryMap);
