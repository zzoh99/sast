...
<script type="text/javascript">
	...
	$(function() {
		...
		initdata0.Cols = [
                ...
                {Header:"월구분",		Type:"Text",		Hidden:1,  	Width:0,		Align:"Left",    	ColMerge:0,   	SaveName:"work_mm",   			KeyField:0,   	CalcLogic:"",   Format:"",          PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:500 },
    ...
    function doAction1(sAction) {
        switch (sAction) {
	        case "Search":   
	        	sheet1.DoSearch( "<%=jspPath%>/simplePymtExBpCdMgr/simplePymtExBpCdMgrMonthRst.jsp?cmd=getSimplePymtExBpCdMgr", $("#mySheetForm").serialize() );
	        	break;
	        ...
	        case "Save":
	        	...
	        	sheet1.DoSave( "<%=jspPath%>/simplePymtExBpCdMgr/simplePymtExBpCdMgrMonthRst.jsp?cmd=savesimplePymtExBpCdMgr", $("#mySheetForm").serialize());
	        	break;
	...
	function callProcPop(sts) {
		...
		var url 	= "<%=jspPath%>/simplePymtExBpCdMgr/simplePymtExBpCdMgrPopMonth.jsp";
        var args = new Array();
	...
	function callProc(sts) {
		...
		//비거주자사업기타(49)	사업소득(50)	근로소득(77) 거주자의기타소득(55)
		if($("#searchIncomeType").val() == "49"){ 
			...
			text += "근로소득 ";
		}else if($("#searchIncomeType").val() == "55"){
			text += "거주자의기타소득 ";
		}
		...
		if(confirm(text)){			
			var data = ajaxCall("<%=jspPath%>/simplePymtExBpCdMgr/simplePymtExBpCdMgrMonthRst.jsp?cmd=P_CPN_SMPPYM_EX_BP_CHG_2024",params,false);
