...
<%!
//사업장예외관리 다건 조회
public List getSimplePymtExBpCdMgr(Map paramMap, Map queryMap) throws Exception {
...
//사업장예외관리 저장
public int saveSimplePymtExBpCdMgr(Map paramMap, Map queryMap) throws Exception {
...
<%
	Map queryMap = XmlQueryParser.getQueryMap(xmlPath+"/simplePymtExBpCdMgr/simplePymtExBpCdMgrMonth.xml");
	...
	if("getSimplePymtExBpCdMgr".equals(cmd)) {
		...
		try {
			listData = getSimplePymtExBpCdMgr(mp, queryMap);
		} catch(Exception e) {
			code = "-1";
	...
	//사업장예외관리 저장
        ...
        try {
            int cnt = saveSimplePymtExBpCdMgr(mp, queryMap);
	...
	//사업장예외관리 복사 프로시저 호출
	else if("P_CPN_SMPPYM_EX_BP_CHG_2024".equals(cmd)) {
		//연말정산 대상자 작업
		...
		String workMm = (String)mp.get("searchWorkMm");
		...
		String[] param = new String[]{"","",""
				,ssnEnterCd,workYy,workMm,sendType,incomeType,pgubun,businessCd,ssnSabun};
		...
		try {
			...
			String[] rstStr = DBConn.executeProcedure("P_CPN_SMPPYM_EX_BP_CHG_2024",type,param);
