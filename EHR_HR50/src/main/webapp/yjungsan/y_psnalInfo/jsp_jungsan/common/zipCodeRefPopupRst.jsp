...
//우편번호 갯수 조회
public String getZipCodePopupDoroListCnt(String path, Map paramMap) throws Exception {
	...
	try{
		...
		totalCnt = (mapData==null||mapData.get("cnt")==null) ? "0" : (String)mapData.get("cnt");
...
//우편번호 조회
public List getZipCodePopupDoroList(String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
	//파라메터 복사.
	Map pm =  StringUtil.getParamMapData(paramMap);
	List listData = null;
...
<%
	//쿼리 맵 셋팅
	//setQueryMap(xmlPath+"/common/zipCodeRefPopup.xml");
	...
	if("getZipCodePopupDoroList".equals(cmd)) {
		...
		try {
			totalCnt = getZipCodePopupDoroListCnt(xmlPath+"/common/zipCodeRefPopup.xml", mp);
			listData = getZipCodePopupDoroList(xmlPath+"/common/zipCodeRefPopup.xml", mp);
		} catch(Exception e) {
			code = "-1";
