...
//우편번호 갯수 조회
public String selectZipCodeListCnt(String path, Map paramMap) throws Exception {
...
//우편번호 조회
public List selectZipCodeList(String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
	//파라메터 복사.
	Map pm =  StringUtil.getParamMapData(paramMap);
	List listData = null;
...
<%
	//쿼리 맵 셋팅
	//setQueryMap(xmlPath+"/common/zipCodePopup.xml");
	...
	if("selectZipCodeList".equals(cmd)) {
		...
		try {
			totalCnt = selectZipCodeListCnt(xmlPath+"/common/zipCodePopup.xml", mp);
			listData = selectZipCodeList(xmlPath+"/common/zipCodePopup.xml", mp);
		} catch(Exception e) {
			code = "-1";
