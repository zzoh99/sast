...
try {
    ...
    Log.Debug("ssnSabun ===== "+ssnSabun);
  	...
  	//쿼리 맵 가져오기
    Map queryMap = XmlQueryParser.getQueryMap(xmlPath+"/evidenceDocMgr/evidenceFileUploadPop.xml");
  ...
  //폼필드 부분만 먼저 가져온다.
    ...
    Log.Debug("searchFileType ===== "+searchFileType);
    ...
    for(int i = 0 ; i < v_fileCnt ; i++){
        ...
        // 파일 확장자 체크
        String[] arrExt = {"pdf","hwp","txt","doc","docx","ppt","xls","xlsx","zip","rar","alz","egg","7z","jpg","jpeg","gif","png"};
        boolean isExt = true;
...
// 			saveFileNm = searchSabun+now+StringUtil.padLeft((int)(Math.random()*10000)+"", "0" ,4)+fileName.substring(fileName.lastIndexOf("."));
	...
	Log.Error("[evidenceFileUploadRst]: 파일다운로드중 오류가 발생하였습니다." + ex.getMessage());
    procYn = "N";
