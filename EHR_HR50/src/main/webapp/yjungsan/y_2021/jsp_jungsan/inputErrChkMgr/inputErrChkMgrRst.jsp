...
//공제자료오류검증 조회
public List selectInputErrChkMgrList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
//오류항목 팝업 조회
public List selectInputErrChkMgrPopupList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectInputErrChkMgrPopupList",pm);
		saveLog(null, pm, ssnYeaLogYn);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
<%
	String locPath = xmlPath+"/inputErrChkMgr/inputErrChkMgr.xml";
	...
	if("selectInputErrChkMgrList".equals(cmd)) {
		...
		try {
			listData = selectInputErrChkMgrList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
	...
	} else if("selectInputErrChkMgrPopupList".equals(cmd)) {
		//오류항목 팝업 조회
		...
		try {
			listData = selectInputErrChkMgrPopupList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
