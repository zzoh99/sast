...
<%!
//관리자 신용카드 자료 조회
public List selectYeaDataCardsList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	String inputCardType = String.valueOf(pm.get("inputCardType"));
...
// 	}
	if(inputCardType.trim().length() != 0){
		query.append(" AND A.CARD_TYPE = #inputCardType#");
	}
...
// 	String searchPeriodType1 = String.valueOf(pm.get("searchPeriodType1"));
	
// 	StringBuffer query   = new StringBuffer();
// 	query.setLength(0);

// 	if(searchPeriodType1.trim().length() != 0){
// 		query.append(" WHERE PERIOD_TYPE = #searchPeriodType1#");
// 	}
...
//추가공제율사용분 저장
public int saveYeaDataAddCards(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
<%
	...
	if("selectYeaDataCardsList".equals(cmd)) {
		...
		try {
			listData = selectYeaDataCardsList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			...
			if(saveList != null && saveList.size() > 0) {
				for(int i = 0; i < saveList.size(); i++) {
					...
					//2020. 소득구분 추가. 코로나로인한 임시 로직
					//String periodType = (String) mp.get("period_type");
					String useYyyy = (String) mp.get("use_yyyy");
		...
		try {
			int cnt = saveYeaDataAddCards(mp, locPath, ssnYeaLogYn);
        ...
        */
        ...
        Map rstMap = new HashMap();
        rstMap.put("Result", mapCode);
        rstMap.put("sumList", listData == null ? null : (List)listData);
        out.print((new org.json.JSONObject(rstMap)).toString());
	}
%>
