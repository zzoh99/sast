...
//작업 프로시저 호출
public String[] prcDayOTaxDisk(Map paramMap) throws Exception {
	...
	String termCd = (String)paramMap.get("termCd");
	...
	String[] param = new String[]{"", "", ssnEnterCd, belongYy, businessPlaceCd
			, termCd, sendYmd, hometaxId, taxNo, email
			, sendType, taxNumber, orgNm, name, telNumber
			, ssnSabun};
	
	String[] rstStr = null;
	try {
		rstStr = DBConn.executeProcedure("PKG_CPN_DAYTAX_DISK_2010.DISK_ALL",type,param);
	} catch(Exception e) {
...
//쿼리 리스트 조회
public List selectDayOTaxList(String path, String queryId, Map paramMap) throws Exception {
...
//쿼리 맵 조회
public Map selectDayOTaxMap(String path, String queryId, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
	//파라메터 복사.
	Map mapData = null;
...
<%
	...
	//쿼리 맵 셋팅
	//setQueryMap(xmlPath+"/dayIncomeTaxCre/dayIncomeTaxCre.xml");
	...
	try {
		...
		String[] rstStr = prcDayOTaxDisk(paramMap);
...
// 		}
		...
		Map fileData = selectDayOTaxMap(xmlPath+"/dayIncomeTaxCre/dayIncomeTaxCre.xml", "selectDayOTaxFileName", paramMap);
		...
		List listDataA = selectDayOTaxList(xmlPath+"/dayIncomeTaxCre/dayIncomeTaxCre.xml", "selectDayOTaxRecodeAContent", paramMap);
		...
		List listDataB = selectDayOTaxList(xmlPath+"/dayIncomeTaxCre/dayIncomeTaxCre.xml", "selectDayOTaxRecodeBContent", paramMap);
		...
		List listDataC = selectDayOTaxList(xmlPath+"/dayIncomeTaxCre/dayIncomeTaxCre.xml", "selectDayOTaxRecodeCContent", paramMap);
		String oldSabun = "";
		String sabun = "";
	...
	Log.Debug("[OTax(일용소득원천세) 결과]: "+rtnData);
