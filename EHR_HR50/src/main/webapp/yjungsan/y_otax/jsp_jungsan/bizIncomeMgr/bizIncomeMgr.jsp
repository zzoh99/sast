...
<!DOCTYPE html>	<html class="hidden"><head> <title>사업소득</title>
...
<script	type="text/javascript">
	...
	$(function() {
		...
		c00508 = codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYM="+$("#belongYm").val(), "C00508");
	...
	initdata.Cols =	[
				...
				{Header:"소득구분",		Type:"Combo",		Hidden:1,			Width:90,			Align:"Center",	ColMerge:0,	SaveName:"earner_cd",		KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 },
				...
				{Header:"업종코드",		Type:"Combo",		Hidden:0,			Width:90,			Align:"Right",	ColMerge:0,	SaveName:"business_cd",		KeyField:1,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:100 },
				{Header:"지급총액",		Type:"AutoSum",		Hidden:0,			Width:90,			Align:"Right",	ColMerge:0,	SaveName:"tot_mon",			KeyField:0,	Format:"Integer",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
				{Header:"세율(%)",		Type:"Combo",		Hidden:0,			Width:90,			Align:"Right",	ColMerge:0,	SaveName:"tax_rate",		KeyField:0,	Format:"Integer",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
				...
				{Header:"계",			Type:"AutoSum",		Hidden:0,			Width:90,			Align:"Right",	ColMerge:0,	SaveName:"total",			KeyField:0,	CalcLogic:"|tot_mon|-|itax_mon|-|rtax_mon|", Format:"Integer",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 }
	...
	initdata2.Cols = [
		...
		{Header:"성명",			Type:"Text",		Hidden:0,			Width:80,			Align:"Center",	ColMerge:0,	SaveName:"name",			KeyField:1,	Format:"",		PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
 		...
 		{Header:"Location",		Type:"Combo",		Hidden:0,			Width:90,			Align:"Center",	ColMerge:0,	SaveName:"location_cd",		KeyField:0,	Format:"",		PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:10 },
 		...
 		{Header:"업종코드",		Type:"Combo",		Hidden:0,			Width:90,			Align:"Right",	ColMerge:0,	SaveName:"business_cd",		KeyField:1,	Format:"",			PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
 		{Header:"지급총액",		Type:"Int",			Hidden:0,			Width:90,			Align:"Right",	ColMerge:0,	SaveName:"tot_mon",			KeyField:0,	Format:"Integer",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
 		{Header:"세율(%)",		Type:"Combo",		Hidden:0,			Width:90,			Align:"Right",	ColMerge:0,	SaveName:"tax_rate",		KeyField:0,	Format:"Integer",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
		...
		// 업종코드(C00508)
		var businessCd = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYM="+$("#belongYm").val(),"C00508"), "");
		sheet1.SetColProperty("business_cd", {ComboText:"|"+businessCd[0], ComboCode:"|"+businessCd[1]});
		...
		sheet1.SetColProperty("tax_rate",	{ComboText:"|3|5|20", ComboCode:"|3|5|20"}	);
	...
	function doAction1(sAction) {
		...
		case "Search":
			...
			sheet1.DoSearch( "<%=jspPath%>/bizIncomeMgr/bizIncomeMgrRst.jsp?cmd=selectBizIncomeMgrList", $("#empForm").serialize() );
			break;
	case "Insert":
		var Row	= sheet1.DataInsert(0) ;
		sheet1.SetCellValue(Row, "earner_cd",		"3" );	//사업소득
		break;
	case "Copy":
		sheet1.DataCopy();
		break;
		case "Save":
			...
			sheet1.DoSave( "<%=jspPath%>/bizIncomeMgr/bizIncomeMgrRst.jsp?cmd=saveBizIncomeMgr", $("#empForm").serialize() );
			break;
	case "Down2Excel":
		...
		case "Down2Template":
			...
			*/
			var templeteTitle = "업로드시 이 행은 삭제 합니다\n*지급일자 : yyyy-mm-dd, *귀속연월:yyyy-mm 형식으로 입력하여 주세요.";
			var param  = {DownCols:"3|4|5|6|8|9|10|11|12|13|14",SheetDesign:1,Merge:1,DownRows:'0',ExcelFontSize:"9"
		...
		try {
			...
			if(sheet1.RowCount() > 0) {
				for(var	i = 1; i < sheet1.RowCount()+1;	i++) {
					sheet1.SetCellValue(i, "earner_cd", "3");
					
					chgVal1 = sheet1.GetCellValue(i,"business_cd");
					chgVal2 = sheet1.GetCellValue(i,"tot_mon");
					chgVal3 = sheet1.GetCellValue(i,"tax_rate");
					chgFlag = false;
					sheet1.SetCellValue(i, "business_cd", "");
					sheet1.SetCellValue(i, "tot_mon", "");
					sheet1.SetCellValue(i, "tax_rate", "");
					chgFlag = true;
					sheet1.SetCellValue(i, "business_cd", chgVal1);
					sheet1.SetCellValue(i, "tot_mon", chgVal2);
					sheet1.SetCellValue(i, "tax_rate", chgVal3);		
				}
			}
		} catch(ex) {
	...
	function uploadPopup() {
		...
		pGubun = "bizIncomeMgrPopup";
		openPopup("<%=jspPath%>/bizIncomeMgr/bizIncomeMgrPopup.jsp?authPg=<%=authPg%>",args,800,500);
	}
	...
	function getReturnValue(returnValue) {
		...
		} else if ( pGubun == "bizIncomeMgrPopup" ){
			doAction1("Search");
		}
	}
	...
	function sheet1_OnChange(Row, Col, Value, OldValue) {
		try{
				...
				if(c00508 != null && c00508.length > 0)	{
					for(var	i = 0; i < c00508.length; i++) {
						if(c00508[i].code == businessCd) {			
							sheet1.SetCellValue(Row, "tax_rate", c00508[i].note1);
							break;
						}
					}
				}

			} else if( sheet1.ColSaveName(Col) == "tot_mon"	|| sheet1.ColSaveName(Col) == "tax_rate") {

				//총지급액, 세율 변경시	소득세 계산.
				if( sheet1.GetCellValue(Row, "tot_mon")	!= "" && sheet1.GetCellValue(Row, "tax_rate") != ""  ) {
					var itax = sheet1.GetCellValue(Row, "tot_mon") * sheet1.GetCellValue(Row, "tax_rate") /	100 ;

					// 소득세법 제86조 [소액 부징수] 가 2024.07.01 부로 시행되어 1천원 미만의 소득세에 대해서도 원천징수 하는 것으로 개정됨.
					if(itax < 1000 && $("#belongYm").val().replace(/-/gi, '') < "202407") {
			...
			if(sheet1.ColSaveName(Col) == "tot_mon"	|| sheet1.ColSaveName(Col) == "itax_mon" || sheet1.ColSaveName(Col) == "rtax_mon") {
				isNumberChk();
			}
