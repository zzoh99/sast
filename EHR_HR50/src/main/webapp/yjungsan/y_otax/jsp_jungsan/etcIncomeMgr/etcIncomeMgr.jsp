...
<!DOCTYPE html>	<html class="hidden"><head> <title>기타소득</title>
...
<script	type="text/javascript">
	...
	$(function() {
		...
		c00504 = codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYM="+$("#belongYm").val(), "C00504");
	...
	initdata.Cols =	[
				...
				{Header:"소득코드",			Type:"Combo",		Hidden:0,					Width:90,			Align:"Left",	ColMerge:0,	SaveName:"income_cd",			KeyField:1,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:100 },
				...
				{Header:"농특세",			Type:"AutoSum",		Hidden:0,					Width:90,			Align:"Right",	ColMerge:0,	SaveName:"atax_mon",			KeyField:0,	Format:"Integer",	PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
				{Header:"계",				Type:"AutoSum",		Hidden:0,					Width:90,			Align:"Right",	ColMerge:0,	SaveName:"total",				KeyField:0,	CalcLogic:"|earn_mon|-|itax_mon|-|rtax_mon|-|atax_mon|", Format:"Integer",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 }
	...
	initdata2.Cols = [
		...
		{Header:"성명",		Type:"Text",		Hidden:0,		Width:80,			Align:"Center",	ColMerge:0,	SaveName:"name",				KeyField:1,	Format:"",			PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:100 },
   		...
   		{Header:"소득구분",	Type:"Combo",		Hidden:1,		Width:90,			Align:"Center",	ColMerge:0,	SaveName:"earner_cd",			KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:100 },
   		// 기타소득관리 정보
		...
		// 소득코드(C00504)
		var incomeCd = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYM="+$("#belongYm").val(),"C00504"), "");
	...
	function doAction1(sAction) {
		...
		case "Search":
			...
			sheet1.DoSearch( "<%=jspPath%>/etcIncomeMgr/etcIncomeMgrRst.jsp?cmd=selectEtcIncomeMgrList", $("#empForm").serialize() );
			break;
	case "Insert":
		var Row	= sheet1.DataInsert(0) ;
		sheet1.SetCellValue(Row, "earner_cd",		"5" );	//기타소득
		break;
	case "Copy":
		sheet1.DataCopy();
		break;
		case "Save":
			...
			sheet1.DoSave( "<%=jspPath%>/etcIncomeMgr/etcIncomeMgrRst.jsp?cmd=saveEtcIncomeMgr", $("#empForm").serialize() );
			break;
	case "Down2Excel":
		...
		try {
			...
			if(sheet1.RowCount() > 0) { 
				for(var	i = 1; i < sheet1.RowCount()+1;	i++) {
					sheet1.SetCellValue(i, "earner_cd", "5");
					chgVal1 = sheet1.GetCellValue(i,"income_cd");
					chgVal2 = sheet1.GetCellValue(i,"tot_mon");
					
					chgFlag = false;
					sheet1.SetCellValue(i, "income_cd", "");
					sheet1.SetCellValue(i, "tot_mon", "");
					
					chgFlag = true;
					sheet1.SetCellValue(i, "income_cd", chgVal1);
	...
	function sheet1_OnChange(Row, Col, Value, OldValue) {
		try{ 
			if( sheet1.ColSaveName(Col) == "income_cd") {
				//소속코드 선택시 세율 C00504 코드의 NOTE1의로 세팅.
				var incomeCd = sheet1.GetCellValue(Row,	"income_cd");
				var earnMon = sheet1.GetCellValue(Row, "earn_mon");
				var note1 = "";				 
				if(c00504 != null && c00504.length > 0)	{
					for(var	i = 0; i < c00504.length; i++) {						
						if(c00504[i].code == incomeCd) {
							note1 =	c00504[i].note1;							
							break;
						}
					}

					if(note1 == "")	{
						//if(sheet1.GetCellValue(Row, "tax_rate") != "0") {
							sheet1.SetCellValue(Row, "tax_rate", 0);
							alert("해당 소득의 세율을 확인하여 직접 입력하여 주시기 바랍니다.");
						//}
					} else {
						sheet1.SetCellValue(Row, "tax_rate", note1);
					}
				}
			}

			if(sheet1.ColSaveName(Col) == "income_cd" || sheet1.ColSaveName(Col) ==	"tot_mon") {
				...
				if(incomeCd != "" && totMon != "") {
					...
					if(note3 == "0") {
						...
						sheet1.SetCellValue(Row, "expense_acct", parseInt(totMon*(nvl(note3,"0")*0.01),10));
					}
				}
			...
			if(sheet1.ColSaveName(Col) == "itax_mon" || sheet1.ColSaveName(Col) == "rtax_mon" || sheet1.ColSaveName(Col) ==	"atax_mon") {
			...
			if(sheet1.ColSaveName(Col) == "income_cd" || sheet1.ColSaveName(Col) ==	"tax_rate" || sheet1.ColSaveName(Col) == "earn_mon") {
	...
	function uploadPopup() {
		...
		pGubun = "etcIncomeMgrPopup";
		openPopup("<%=jspPath%>/etcIncomeMgr/etcIncomeMgrPopup.jsp?authPg=<%=authPg%>",args,800,500);
	}
	...
	function getReturnValue(returnValue) {
		...
		} else if ( pGubun == "etcIncomeMgrPopup" ){
			doAction1("Search");
		}
	}
...
<div class="wrapper">
	...
	<ul>
	    <li	class="txt">기타소득 - 소득코드에 따른 필요경비와 세율을 반드시	확인하시기 바랍니다.</li>
	    <li	class="btn">
