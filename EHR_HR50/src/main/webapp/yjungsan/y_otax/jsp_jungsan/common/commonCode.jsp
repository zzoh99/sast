...
//공통코드 검색
public List getCommonCodeList(String orderBy, String path, Map paramMap) throws Exception {
...
//공통코드 기타 검색
public List getCommonNSCodeList(String queryId, String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
	//파라메터 복사.
	Map pm =  StringUtil.getParamMapData(paramMap);
	List list = null;
...
public List getFamCodeList(String queryId, String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
	//파라메터 복사.
	Map pm =  StringUtil.getParamMapData(paramMap);
	List list = null;
...
//대상자(교육) 조회
public List getFamCodeEduList(String queryId, String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
	//파라메터 복사.
	Map pm =  StringUtil.getParamMapData(paramMap);
	List list = null;
...
//대상자(카드) 조회
public List getFamCodeCardList(String queryId, String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
	//파라메터 복사.
	Map pm =  StringUtil.getParamMapData(paramMap);
	List list = null;
...
<%
	...
	if("getCommonCodeList".equals(cmd)) {
		...
		try {
			listData = getCommonCodeList(orderBy, xmlPath+"/common/commonCode.xml", mp);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			listData = getCommonNSCodeList(queryId, xmlPath+"/common/commonCode.xml", mp);
		} catch(Exception e) {
			code = "-1";
			message = "공통 코드 조회중 오류가 발생하였습니다.";
		}
		...
		*/
		try {
			listData = getFamCodeList(queryId, xmlPath+"/common/commonCode.xml", mp);
		} catch(Exception e) {
			code = "-1";
		...
		*/
		try {
			listData = getFamCodeEduList(queryId, xmlPath+"/common/commonCode.xml", mp);
		} catch(Exception e) {
			code = "-1";
		...
		*/
		try {
			listData = getFamCodeCardList(queryId, xmlPath+"/common/commonCode.xml", mp);
		} catch(Exception e) {
			code = "-1";
