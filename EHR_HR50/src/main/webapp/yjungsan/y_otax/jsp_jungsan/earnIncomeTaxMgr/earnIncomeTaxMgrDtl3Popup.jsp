...
<!DOCTYPE html> <html class="hidden"><head> <head> <head>  <title>사업소득세</title>
	...
	switch (sAction) {
		case "Search":
			...
			if(dtl3Info.Data != null) {
				...
				//2025년 이후 신고면서 다음의 상황일 때 earnIncomeTaxMgrDtl3Popup_2025.jsp 양식으로 이동
				//     1. 기존에 생성된 자료가 없어서 신규로 자료생성을 하는 경우
				//     2. 생성일(20241231 개정에따른 구/신 양식 분기 기준) 값이 있다면
				if ( "202412" < $("#belongYm").val().replace(/\-/g,'') && (dtl3Info.Data.crtdate == undefined || dtl3Info.Data.crtdate != "") ) {
					window.location.replace("<%=jspPath%>/earnIncomeTaxMgr/earnIncomeTaxMgrDtl3Popup_2025.jsp?authPg=<%=authPg%>");
				}
				...
				addComma($("#tdTotMon")[0]);
			} else if ( "202412" < $("#belongYm").val().replace(/\-/g,'') ) {
				//기존 자료가 없는데 2025년 이후라면 earnIncomeTaxMgrDtl3Popup_2025.jsp 양식으로 이동
				window.location.replace("<%=jspPath%>/earnIncomeTaxMgr/earnIncomeTaxMgrDtl3Popup_2025.jsp?authPg=<%=authPg%>");
			}
			break;
		...
		case "Save":
			...
			if (confirm("수정사항을 반영하시겠습니까?")) {
				...
				var result = ajaxCall("<%=jspPath%>/earnIncomeTaxMgr/earnIncomeTaxMgrRst.jsp?cmd=saveEarnIncomeTaxMgrDtl3",$("#sheet1Form").serialize(),false);
