...
<%
	...
	if("selectYeaResultPayCreList".equals(cmd)) {
		...
		String[] param = new String[]{"","",ssnEnterCd,workYy,adjustType,bizPlaceCd,payApplyCd,ssnSabun};
		...
		try {
			...
			String[] rstStr = DBConn.executeProcedure("P_CPN_YEAREND_RESULT_2006",type,param);
			
			if(rstStr[1] == null || rstStr[1].length() == 0) {
				message = "연말정산 급여반영이 완료되었습니다.";
			} else {
				code = "-1";
				message = "연말정산 급여반영 처리도중 : "+rstStr[1];
			}
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
