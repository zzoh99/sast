...
<script type="text/javascript">
    ...
    $(function() {
        ...
        initdata1.Cols = [
            ...
            {Header:"자녀\n양육",           Type:"CheckBox",    Hidden:0,   Width:40,   Align:"Center", ColMerge:0, SaveName:"child_yn",            KeyField:0, Format:"",      PointCount:0,   UpdateEdit:1,   InsertEdit:0,   EditLen:1, TrueValue:"Y", FalseValue:"N" },
            {Header:"출산\n입양",           Type:"CheckBox",    Hidden:0,   Width:40,   Align:"Center", ColMerge:0, SaveName:"adopt_born_yn",       KeyField:0, Format:"",      PointCount:0,   UpdateEdit:1,   InsertEdit:0,   EditLen:1, TrueValue:"Y", FalseValue:"N" },
            ...
            {Header:"전년도\n대상여부",     Type:"CheckBox",    Hidden:0,   Width:50,   Align:"Center", ColMerge:0, SaveName:"pre_equals_yn",       KeyField:0, Format:"",      PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:1, TrueValue:"Y", FalseValue:"N" }
                ...
                if( sheet1.GetCellValue(row, "famres") != "" ) {
                    ...
                    if( sheet1.GetCellValue(row, "fam_cd") == "1" || sheet1.GetCellValue(row, "fam_cd") == "2" ) {
                        if( sheet1.GetCellValue(row, "dpndnt_yn") == "Y" && sheet1.GetCellValue(row, "hndcp_yn") == "N" ) {
                            if(age < 60) {
                                alert("직계존속 연령은 60세 이상이어야 합니다. <"+sheet1.GetCellValue(row, "fam_nm")+">") ;
                                return ;
                            }
                        }
                    }
                    ...
                    if( sheet1.GetCellValue(row, "fam_cd") == "4" || sheet1.GetCellValue(row, "fam_cd") == "5" ) {
                        if( sheet1.GetCellValue(row, "dpndnt_yn") == "Y" && sheet1.GetCellValue(row, "hndcp_yn") == "N" ) {
                            if(age > 20) {
                                alert( "직계비속 연령은 20세 이하이어야 합니다. <"+sheet1.GetCellValue(row, "fam_nm")+">" ) ;
                                return ;
                            }
                        }
                    }
    ...
    function sheet1_OnPopupClick(Row, Col){
        try{
            ...
            if(Row > 0 && sheet1.ColSaveName(Col) == "zip" ){
                ...
                gPRow = Row;
                pGubun = "zipCodePopup";
                var rst = openPopup("<%=jspPath%>/common/zipCodePopup.jsp", "", "740","620");
                /*
                if(rst != null){
    ...
    function getReturnValue(returnValue) {
        ...
        if ( pGubun == "zipCodePopup" ){
            //우편번호조회
            sheet1.SetCellValue(gPRow,"zip",rst[0]);
            sheet1.SetCellValue(gPRow,"addr1",rst[1]);
            sheet1.SetCellValue(gPRow,"addr2",rst[2]);
            ...
            if(sheet1.ColSaveName(Col) == "dpndnt_yn" ){
                if( sheet1.GetCellValue(Row,"dpndnt_yn") == "Y"){
                    ...
                    if(sheet1.GetCellValue(Row,"fam_cd") == "3"){
                        var flag = false;
                        if( confirm("배우자의 연간소득금액을 확인한 후 배우자 공제 여부를 결정하여 주십시오.\n * 참고 : 거주자의 배우자로서 연간소득금액이 없거나 연간소득금액의 합계액이 100만원 이하인 자는 배우자 공제가 가능합니다.  \n 배우자 공제를 선택 하시겠습니까?")){
                    ...
                    //수급자의 경우 경로우대 체크
                    } else if(sheet1.GetCellValue(Row,"fam_cd") == "7"){
                        //배우자 나이체크 후 경로우대 여부 판단
                        var sugub_age = 0;
                        if(sheet1.GetCellValue(Row,"famres").substring(6,7) == "3"
                                || sheet1.GetCellValue(Row,"famres").substring(6,7) == "4"
    ...
    function sheetSet(){
        ...
        if(comSheet.RowCount() > 0){
            ...
            $("#A020_09").val( comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A020_09"),"data_cnt") ) ;
            $("#A020_11").val( comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A020_11"),"data_cnt") ) ;
            $("#A020_12").val( comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A020_12"),"data_cnt") ) ;
        }else{
            $("#A010_05").val( "0" ) ;
            $("#A010_07").val( "0" ) ;
            $("#A010_09").val( "0" ) ;
        ...
        if(sheet1.GetCellValue(row, "dpndnt_yn") == "Y"){
            ...
            if(sheet1.GetCellValue(row,"famres") != ""){
                if(sheet1.GetCellValue(row,"fam_cd") == "1"
                    ...
                    if( sheet1.GetCellValue(row,"famres").substring(6,7) == "1"
                            || sheet1.GetCellValue(row,"famres").substring(6,7) == "3"
                            || sheet1.GetCellValue(row,"famres").substring(6,7) == "5"
                            || sheet1.GetCellValue(row,"famres").substring(6,7) == "7" ){
                        //경로우대 체크
                        if( age >= 70){
                            sheet1.SetCellEditable(row, "senior_yn", 0) ;
                    ...
                    } else if( sheet1.GetCellValue(row,"famres").substring(6,7) == "2"
                            || sheet1.GetCellValue(row,"famres").substring(6,7) == "4"
                            || sheet1.GetCellValue(row,"famres").substring(6,7) == "6"
                            || sheet1.GetCellValue(row,"famres").substring(6,7) == "8" ){
                        if( age >= 70){
                    ...
                    if( age <= 20){
                        ...
                        if( sheet1.GetCellValue(row,"famres").substring(6,7) == "1"
                                ...
                                || sheet1.GetCellValue(row,"famres").substring(6,7) == "5"
                                || sheet1.GetCellValue(row,"famres").substring(6,7) == "7" ) {
                            sheet1.SetCellEditable(row, "senior_yn", 1) ;
                            sheet1.SetCellValue(row,"senior_yn", "Y") ;
                        } else{
                            sheet1.SetCellEditable(row, "senior_yn", 0) ;
                            sheet1.SetCellValue(row,"senior_yn", "N") ;
                        ...
                        if( sheet1.GetCellValue(row,"famres").substring(6,7) == "2"
                                || sheet1.GetCellValue(row,"famres").substring(6,7) == "4"
                                || sheet1.GetCellValue(row,"famres").substring(6,7) == "6"
                                || sheet1.GetCellValue(row,"famres").substring(6,7) == "8" ) {
                            sheet1.SetCellEditable(row, "senior_yn", 1) ;
                    ...
                    //sheet1.CellValue(row,"spouse_yn") = "1";
                    sheet1.SetCellEditable(row, "child_yn", 0) ;
                    sheet1.SetCellValue(row,"child_yn", "N") ;
                    if(age < 70){
                        sheet1.SetCellEditable(row, "senior_yn", 0) ;
...
<div class="wrapper">
    ...
    <div class="outer">
        ...
        <ul>
            <li class="txt">기본공제
                <a href="javascript:yeaDataExpPopup('기본공제', helpText1, 520);"       class="cute_gray authR">기본공제 안내</a>
            </li>
        </ul>
        </div>
    </div>
    ...
    <div class="outer">
        ...
        <ul>
            <li class="txt">추가공제
                <a href="javascript:yeaDataExpPopup('추가공제', helpText2, 320);"       class="cute_gray authR">추가공제 안내</a>
            </li>
        </ul>
        </div>
    </div>
    ...
    <tr>
        ...
        <th class="center">자녀양육비공제</th>
        <th class="center">출산/입양자공제</th>
        <th class="center">다자녀추가공제</th>
    ...
    <tr>
        ...
        <td class="right">
            <input id="A020_09" name="A020_09" type="text" class="text w25p right transparent" readOnly />명
        </td>
        <td class="right">
            <input id="A020_12" name="A020_12" type="text" class="text w25p right transparent" readOnly />명
        </td>
        <td class="right">
            <input id="A020_11" name="A020_11" type="text" class="text w25p right transparent" readOnly />명
        </td>
        <td class="center">
