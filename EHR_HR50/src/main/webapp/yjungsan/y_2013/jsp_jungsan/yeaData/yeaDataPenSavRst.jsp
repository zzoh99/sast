...
<%!
//연금저축 자료 조회
public List selectYeaDataPenSavList(Map paramMap ,String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectYeaDataPenSavList",pm);
	} catch (Exception e) {
...
<%
	String locPath = xmlPath+"/yeaData/yeaDataPenSav.xml";

	String ssnEnterCd = (String)session.getAttribute("ssnEnterCd");
	String ssnSabun = (String)session.getAttribute("ssnSabun");
	String cmd = (String)request.getParameter("cmd");

	if("selectYeaDataPenSavList".equals(cmd)) {
		//연금저축 자료 조회
		...
		try {
			listData = selectYeaDataPenSavList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
	...
	} else if("saveYeaDataPenSav".equals(cmd)) {
		//연금저축 저장.
		...
		try {
			...
			if(saveList != null && saveList.size() > 0) {
				for(int i = 0; i < saveList.size(); i++) {
					...
					String[] rstStr = DBConn.executeProcedure("PKG_CPN_YEA_"+yeaYear+"_SYNC.SAVING_INS",type,param);
