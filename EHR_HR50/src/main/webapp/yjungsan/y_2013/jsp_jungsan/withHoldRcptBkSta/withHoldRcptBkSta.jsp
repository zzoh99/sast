...
<script type="text/javascript">
	...
	$(function() {
		...
		initdata1.Cols = [
			...
			{Header:"성명",      	Type:"Popup",      	Hidden:0,  Width:80,  	Align:"Center",  ColMerge:0,   SaveName:"name",       KeyField:0,   CalcLogic:"",   Format:"",         PointCount:0,   UpdateEdit:0,   InsertEdit:0,	EditLen:100 },
	...
	function print() {
	    ...
	    if(sheet1.RowCount() != 0) {
			...
			//call RD!
	        withHoldRcptBkStaPopup(sabuns, sortSabuns, sortNos) ;
	    } else {
	 ...
	 * by JSG
	 */
	function withHoldRcptBkStaPopup(sabuns, sortSabuns, sortNos){
  		var w 		= 1040;
		var h 		= 600;
		...
		args["rdParam"] = "[<%=removeXSS(session.getAttribute("ssnEnterCd"), "1")%>] ["+$("#searchWorkYy").val()+"] ["+$("#searchAdjustType").val()+"]"+
						  ...
						  "[4] ["+sortSabuns+"] ["+sortNos+"] ["+printYmd+"] ["+imgPath+"] ["+imgFile+"] ";//rd파라매터
        ...
        if( sheet1.RowCount() != 0 ) {               
            ...
            for(i=1; i<=sheet1.LastRow(); i++) {
            	rowSabun = sheet1.GetCellValue( i, "sabun" );
	   	    	var req = ajaxCall("<%=jspPath%>/withHoldRcptBkSta/withHoldRcptBkStaRst.jsp?cmd=selectEmpInfoUsingSabun","searchSabun="+rowSabun,false);
