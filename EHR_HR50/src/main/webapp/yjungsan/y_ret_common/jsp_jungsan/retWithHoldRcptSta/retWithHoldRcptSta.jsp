...
<script type="text/javascript">
	...
	function print() {
		...
		if(sheet1.RowCount() != 0) {
			...
			//call RD!
	        retWithHoldRcptStaPopup(sabuns, sortSabuns, payActionCds, stamps, sortNos) ;
	    } else {
	 ...
	 * by JSG
	 */
	function retWithHoldRcptStaPopup(sabuns, sortSabuns, payActionCds, stamps, sortNos){
  		var w 		= 800;
		var h 		= 600;
		...
		/* 2016-07-05 YHCHOI ADD START */
		/* 2014년 부터는 양식2, 양식3이 폐지되어 출력할 수 없으므로 알림창 띄우고 종료 */
		if(( $("#searchWorkYy").val()*1 ) >= 2014 && $("#searchReportType").val() != '1'){
			alert("폐지된 양식으로 출력하실 수 없습니다.");
			return;
		}
		/* 2016-07-05 YHCHOI ADD END */
		...
		//SORTING 파라메터 추가 - 2019.11.27
		if($("#searchWorkYy").val() !=null && ( $("#searchWorkYy").val()*1 ) >= 2019 ){
			args["rdParam"] += "["+sortNos+"]"; //13번째 파라메터
		}
