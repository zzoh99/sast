...
<%
	if(true) {
		Map testMap = StringUtil.getRequestMap(request);
		
		Map pm =  StringUtil.getParamMapData(testMap);
		
		String name = (String)pm.get("adj_process_nm");
		
		Map mapCode = new HashMap();
		mapCode.put("Code", "1");
		mapCode.put("Message", name);
...
public List selectYearEndItemMgrProcess(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		list  = DBConn.executeQueryList(queryMap,"selectYearEndItemMgrProcess",pm);
	} catch (Exception e) {
...
public List selectYearEndItemMgr(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		list  = DBConn.executeQueryList(queryMap,"selectYearEndItemMgr",pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
	}
...
public Map selectYearEndItemMgrPopup(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		mp  = DBConn.executeQueryMap(queryMap,"selectYearEndItemMgrPopup",pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
	}
