...
<script type="text/javascript">
	...
	$(function(){
		...
		/*2019.2.8 MODIFY 1주택 > 연말정산 옵션에 따라 활성 */
	...
	function subDoActionCheck() {
        ...
        if( result1.Result.Code == "1") {
        	sheet1.DoSave( "<%=jspPath%>/yeaData/yeaDataAddrRst.jsp?cmd=saveYeaDataAddr","","","0");
        	location.reload();
            doAction1("Search");
        }else{
        	location.reload();
	...
	function sheetToZipCode(){
		if(sheet1.RowCount() == 0){
			...
			//주택소유수
			$("input[id='house_cnt0']:input[value='"+sheet1.GetCellValue(sheet1.LastRow(),"house_cnt")+"']").attr("checked",true);
			$("input[id='house_cnt1']:input[value='"+sheet1.GetCellValue(sheet1.LastRow(),"house_cnt")+"']").attr("checked",true);
			$("input[id='house_cnt2']:input[value='"+sheet1.GetCellValue(sheet1.LastRow(),"house_cnt")+"']").attr("checked",true);
	...
	function houseInfo(){
		...
		if($("input[id='house_cnt0']:checked").val() == "0" 
				|| $("input[id='house_cnt1']:checked").val() == "1"
				|| $("input[id='house_cnt2']:checked").val() == "2") {
					$("#official_price").val("") ;
					$("#houseinfo3").hide() ;
		} else{
			$("#houseinfo3").show() ;
	...
	function initZipCode(){
		...
		$("input[id='house_cnt0']").attr("checked",false);
		$("input[id='house_cnt1']").attr("checked",false);
		$("input[id='house_cnt2']").attr("checked",false);
	...
	function subInputConfirmYn(){
		if($("#zipCodeResult").val() == ""){
			alert("우편번호를 입력해 주십시요.");
			return;
		}
		...
		if( ($("input[id='house_cnt0']:checked").val() != null) 
		   || ($("input[id='house_cnt1']:checked").val() != null)
		   || ($("input[id='house_cnt2']:checked").val() != null)) {
				sheet1.SetCellValue(1, "house_cnt", $("input[id='house_cnt0']:checked").val() ) ;
				sheet1.SetCellValue(1, "house_cnt", $("input[id='house_cnt1']:checked").val() ) ;
				sheet1.SetCellValue(1, "house_cnt", $("input[id='house_cnt2']:checked").val() ) ;
		}
	...
	<table border="0" cellpadding="0" cellspacing="0" class="default outer">
				...
				<ul>
					...
					<li class="btn">
						<span id="btnDisplayYn01">
						  <a href="javascript:saveConfirmYn('Save');" class="basic authA" id="btnSave">저장</a>
						  <a href="javascript:initZipCode();" class="basic authA" id="btnInit">초기화</a>
						  &nbsp;&nbsp;&nbsp;
						  <a href="javascript:saveConfirmYn('Confirm');" class="button authA" id="authTextA"><b id="authText">확정</b></a>
						</span>
					</li>
				</ul>
			</div>
		...
		<tr>
			...
			<td>
		        ...
		        <a href="#" class="basic authA" id="buttonMinus" style="display:none;"><b>우편번호-</b></a>
		...
		<tr>
			<th>소유주택수</th>
			<td class="left">
				<%-- <input type="radio" class="radio <%=disabled%>" name="house_cnt" id="house_cnt0" value="0" onclick="houseInfo();" <%=disabled%>>&nbsp;무주택 --%>
				<input type="radio" class="radio <%=disabled%>" name="house_cnt" id="house_cnt0" value="0" onclick="houseInfo();" <%=disabled%>><a href="#" id="house_cnt00">&nbsp;무주택</a>
				<input type="radio" class="radio <%=disabled%>" name="house_cnt" id="house_cnt1" value="1" onclick="houseInfo();" <%=disabled%>><a href="#" id="house_cnt11">&nbsp;1주택</a>
				<input type="radio" class="radio <%=disabled%>" name="house_cnt" id="house_cnt2" value="2" onclick="houseInfo();" <%=disabled%>><a href="#" id="house_cnt22">&nbsp;2주택이상</a>
				<font color='red'>☜ 주택자금공제를 위한 기초자료</font>
			</td>
		</tr>
		<tr class="hide" id='houseinfo3' style="display:none;">
			<th>주택공시지가</th>
			<td class="left">
			...
			<div>
				<table>
					<tr>
						...
						<td><a href="javascript:setValue();" class="basic authA" id="btnSave">확인</a></td>
					</tr>
				</table>
			</div>
		</div>
