...
<script type="text/javascript">
	...
	function getProcessStatus(){
		...
		if(manCnt == "" || manCnt == "0" ){
			...
			monpayRate = Round(nvl(processStatusData.Data.monpay1_cnt,"0")/nvl(manCnt,1)*100,1);
		}
		...
		if(manCnt == "" || manCnt == "0" ){
			...
			addrRate = Round(nvl(processStatusData.Data.addr_y_cnt,"0")/nvl(manCnt,1)*100,1);
		}
		...
		if(manCnt == "" || manCnt == "0" ){
			...
			perRate = Round(nvl(processStatusData.Data.per_y_cnt,"0")/nvl(manCnt,1)*100,1);
		}	    
		...
		if(manCnt == "" || manCnt == "0" ){
			...
			pdfRate = Round(nvl(processStatusData.Data.pdf_y_cnt,"0")/nvl(manCnt,1)*100,1);
		}	  	
		...
		if(manCnt == "" || manCnt == "0" ){
			...
			cardRate = Round(nvl(processStatusData.Data.card_y_cnt,"0")/nvl(manCnt,1)*100,1);
		}	  	
		...
		if(manCnt == "" || manCnt == "0" ){
			...
			inputCloseRate = Round(nvl(processStatusData.Data.input_close_y_cnt,"0")/nvl(manCnt,1)*100,1);
		}	  	
		...
		if(manCnt == "" || manCnt == "0" ){
			...
			apprvRate = Round(nvl(processStatusData.Data.apprv_y_cnt,"0")/nvl(manCnt,1)*100,1);
		}	  	
		...
		if(manCnt == "" || manCnt == "0" ){
			...
			confirmRate = Round(nvl(processStatusData.Data.confirm_y_cnt,"0")/nvl(manCnt,1)*100,1);
		}	  	
		...
		if(manCnt == "" || manCnt == "0" ){
			...
			finalCloseRate = Round(nvl(processStatusData.Data.final_close_y_cnt,"0")/nvl(manCnt,1)*100,1);
		}	    
		...
		if(manCnt == "" || manCnt == "0" ){
			...
			raxRate = Round(nvl(processStatusData.Data.tax_y_cnt,"0")/nvl(manCnt,1)*100,1);
		}	    
	    ...
	    if(resultApplyYn != "" && resultApplyYn == "반영") {
	...
	function clickBtnView(id) {
		...
		if(id == "clickBtn_payActionNm") {
			...
			title = "연말정산 마감";
			menu = "연말정산 > 정산계산/결과 > 연말정산계산<br>연말정산 > 소득공제자료관리 > 자료입력현황";
			message = " 연말정산 마감 정보입니다.<br>작업이 완료되면 연말정산 마감버튼을 누르거나,<br> 자료입력현황에서 개인별/전체 마감합니다.<br> 아래는 미마감 인원입니다.";			
			...
			//12.급여반영
			title = "급여반영";
			menu = "연말정산 > 정산계산/결과 > 연말정산결과 급여반영<br>연말정산 > 소득공제자료관리 > 원천징수세액조회(분납관련 등)";
...
<style type="text/css">
	...
	.viewCompleteImg_o {background-image: url('/yjungsan/common_jungsan/images/icon/icon_o.png') !important;}
	.viewCompleteImg_x {background-image: url('/yjungsan/common_jungsan/images/icon/icon_x.png') !important;}
