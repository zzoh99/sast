		...
		try{
			for(int i = 0; i < list.size(); i++ ) {
				...
				if("D".equals(sStatus)) {
					...
					//입력 중복체크
					Map dupMap = (queryMap == null) ? null : DBConn.executeQueryMap(conn, queryMap,"selectYearIncomeEachMgrCnt",mp);
	...
	if(list != null && list.size() > 0 && conn != null) {
		...
		try{
			for(int i = 0; i < list.size(); i++ ) {
				...
				20181114 by JSG
				*/
				Map dataMap = (queryMap == null) ? null : DBConn.executeQueryMap(conn, queryMap,"selectYearIncomeEachMgrEtcSum",mp);
				if(dataMap != null) {
