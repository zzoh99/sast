...
<%!
//연말정산결과 급여반영 조회
public List selectYeaResultPayCreList(Map paramMap, String locPath) throws Exception {
...
public List selectYeaResultRtrCreList(Map paramMap, String locPath) throws Exception {
...
//작업일자 조회
public List selectYeaResultPayCrePayDayPopupList(Map paramMap, String locPath) throws Exception {
...
//연말정산결과 급여반영 저장
public int saveYeaResultPayCre(Map paramMap, String locPath) throws Exception {
...
<%
	...
	if("selectYeaResultPayCreList".equals(cmd)) {
		...
		try {
			listData = selectYeaResultPayCreList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			listData = selectYeaResultRtrCreList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
		...
		try {
			listData = selectYeaResultPayCrePayDayPopupList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
		...
		try {
			int cnt = saveYeaResultPayCre(mp, locPath);
		...
		String[] type =  new String[]{"OUT","OUT","STR","STR","STR","STR","STR","STR","STR","STR","STR"};
		String[] param = new String[]{"","",ssnEnterCd,workYy,adjustType,bizPlaceCd,payApplyCd,jobGubun,monthSeq,rtrApplyCd,ssnSabun};
		...
		try {
			...
			String[] rstStr = DBConn.executeProcedure("P_CPN_YEAREND_RESULT_"+workYy,type,param);
