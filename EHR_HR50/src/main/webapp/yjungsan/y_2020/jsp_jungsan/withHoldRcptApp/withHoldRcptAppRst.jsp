...
//원천징수영수증/징수부 신청 조회
public List selectWithHoldRcptAppList(Map paramMap, String locPath) throws Exception {
...
//원천징수영수증/징수부 신청 저장
public int saveWithHoldRcptApp(Map paramMap, String locPath) throws Exception {
			...
			for(int i = 0; i < list.size(); i++ ) {
				...
				if("D".equals(sStatus)) {
					...
					if( "2".equals(print_type) ){
						...
						chkMap.put("workYy", workYy);
						chkMap.put("sabun", sabun);
						chkMap.put("adjustType", "1");
					...
					if(rstMap1 != null && rstMap1.size() > 0){
						...
						if("2".equals(print_type)){
							...
							if(rstMap3 != null && !rstMap3.get("cnt").equals("0")){
								...
								rstStr = DBConn.executeProcedure("P_CPN_YEAREND_MONPAY_"+workYy,type,param);
...
<%
	String locPath = xmlPath+"/withHoldRcptApp/withHoldRcptApp.xml";
	Map queryMap = XmlQueryParser.getQueryMap(locPath);
	...
	if("selectWithHoldRcptAppList".equals(cmd)) {
		...
		try {
			listData = selectWithHoldRcptAppList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			int cnt = saveWithHoldRcptApp(mp, locPath);
