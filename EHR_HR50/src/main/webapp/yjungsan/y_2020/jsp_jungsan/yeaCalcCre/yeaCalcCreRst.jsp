...
//대상인원전체 상태 저장.
public int savePayPeopleStatus(Map paramMap, String locPath) throws Exception {
...
// FAQ 조회
public List selectFaqActionInList(Map paramMap, String locPath) throws Exception{
...
//연말정산 급여일자 생성
public int insertYeaPayDayTCPN201(Map paramMap, String locPath) throws Exception {
...
//임직원 메뉴 오픈  
public int saveOrgAuthStatus(Map paramMap, String locPath) throws Exception {
...
// 임직원메뉴오픈 
public Map selectOrgAuthStatus(Map paramMap, String locPath) throws Exception {
...
<%
	...
	if("selectYeaPayActionInfo".equals(cmd)) {
		...
		try {
			int cnt = savePayPeopleStatus(mp, locPath);
		...
		try {
			listData = selectFaqActionInList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
		...
		try {
			int cnt = insertYeaPayDayTCPN201(mp, locPath);
		...
		try {
			int cnt = saveOrgAuthStatus(mp, locPath);
		...
		try {
			mapData = selectOrgAuthStatus(mp, locPath);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
