...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#searchStdWorkYy").val(searchWorkYy);
		...
		if(sheet1.RowCount() > 0){
			for (i = 1; i < sheet1.RowCount()+1; i++){
				...
				if(sheet1.GetCellValue(i,"std_cd")=="CPN_MONPAY_RETRY_YN"){   //연간급여재생성 여부
					...
					$("input:radio[name='cpn_work_income_print_button']:input[value='"+stdValue+"']").attr("checked",true);						
					...
					$("input:radio[name='cpn_yea_hou_info_yn']:input[value='"+stdValue+"']").attr("checked",true);
				} else if(sheet1.GetCellValue(i,"std_cd")=="CPN_YEA_ADD_FILE_YN"){   //파일첨부탭 사용 여부
					$("input:radio[name='cpn_yea_add_file_yn']:input[value='"+stdValue+"']").attr("checked",true);
				} else if(sheet1.GetCellValue(i,"std_cd")=="YEACALCLST_STAMP_YN"){   //파일첨부탭 사용 여부
					$("input:radio[name='yeacalclst_stamp_yn']:input[value='"+stdValue+"']").attr("checked",true);
				} 
			}			
		}		 
	}
						...
						<div class="sheet_title outer">
							<ul>
								<li id="txt" class="txt">기준년도
									...
									<font color='red'>연말정산 옵션은 사업장별 관리가 아닌 공통 사항입니다.</font>
								</li>
							</ul>
						</div>
					</td>
			...
			<table border="0" cellpadding="0" cellspacing="0" class="default outer">
				...
				<tr>
					<td class="left">
			            ...
			            &nbsp;자료등록 메뉴에서 세액계산 탭 및 결과보기 탭의 활성화를 선택할 수 있습니다.  <br>
						&nbsp;세액계산 / 결과보기는 임직원이 공제금액을 입력한 후 미리 세액을 계산해 볼 수 있는 기능으로, 최종 계산 결과와<br>
						&nbsp;그 값이 상이할 수 있습니다.<br>
						&nbsp;YES : 세액계산 및 결과보기 탭이 활성화되어 미리 세액계산을 할 수 있습니다.<br>
						&nbsp;NO : 세액계산 및 결과보기 탭이 비활성화되어 미리 세액계산을 할 수 없습니다.<br>
						&nbsp;관리자만 : 세액계산 및 결과보기 탭이 (관리자) 화면에서만 활성화되어 미리 세액계산을 할 수 있습니다.	
					</td>
					<td align="left">
				...
				<tr>
					...
					<td align="left">
						<input type="radio" class="radio" name="cpn_work_income_print_button" value = "1" >YES&nbsp;&nbsp;&nbsp;<br>    
						<input type="radio" class="radio" name="cpn_work_income_print_button" value = "0" >NO
					</td>		
				</tr>
				<tr>
				...
				<tr>
					...
					<td align="left">
						<input type="radio" class="radio" name="cpn_yea_hou_info_yn" value = "Y" >YES&nbsp;&nbsp;&nbsp;<br>   
						<input type="radio" class="radio" name="cpn_yea_hou_info_yn" value = "N" >NO
					</td>		
				</tr>
				<tr>
				...
				<tr>
					<td class="left">
						<b>[증빙자료탭 기능 사용 여부]</b><br>
						&nbsp;자료등록 증빙자료 탭에서 별도 파일등록 기능을 사용할 수 있습니다.(자료등록(관리자)에는 영향을 미치지 않습니다.)<br/>
						&nbsp;YES : 증빙자료 탭을 오픈하여 사용합니다.<br>
						&nbsp;NO : 증빙자료 탭을 숨깁니다.<br>
					</td>
					<td align="left">
						<input type="radio" class="radio" name="cpn_yea_add_file_yn" value = "Y" >YES&nbsp;&nbsp;&nbsp;<br>   
						<input type="radio" class="radio" name="cpn_yea_add_file_yn" value = "N" >NO
					</td>		
				</tr>
				<tr>
				...
				<tr>
					<td class="left">
						<b>[계산내역 도장 출력 여부]</b><br>
						&nbsp;계산내역 출력 화면에서 도장 출력 여부를 선택할 수 있습니다.(계산내역(관리자)에는 영향을 미치지 않습니다.)<br/>
						&nbsp;YES : 도장을 출력합니다.<br>
						&nbsp;NO : 도장을 출력하지 않습니다.<br>
					</td>
					<td align="left">
						<input type="radio" class="radio" name="yeacalclst_stamp_yn" value = "Y" >YES&nbsp;&nbsp;&nbsp;<br>   
						<input type="radio" class="radio" name="yeacalclst_stamp_yn" value = "N" >NO
					</td>		
				</tr>			
			</table>
		</div>
