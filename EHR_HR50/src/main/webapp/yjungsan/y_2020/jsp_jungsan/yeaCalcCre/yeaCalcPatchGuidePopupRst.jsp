...
<%!
//연말정산 패치현황 조회
public List selectYeaCalcPatchGuidePopupInfo(Map paramMap, String locPath) throws Exception {
...
//연말정산 패치 차수
public Map selectYeaCalcMaxPatchSeq(Map paramMap, String locPath) throws Exception {
...
<%
	...
	if("selectYeaCalcPatchGuidePopupInfo".equals(cmd)) {
		...
		try {
			listData = selectYeaCalcPatchGuidePopupInfo(mp, locPath);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			mapData = selectYeaCalcMaxPatchSeq(mp, locPath);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
