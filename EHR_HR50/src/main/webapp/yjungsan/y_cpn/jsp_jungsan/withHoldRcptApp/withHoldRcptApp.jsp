...
<script type="text/javascript">
    ...
    $(function() {
        ...
        initdata1.Cols = [
            ...
            {Header:"일련번호",			Type:"Text",		Hidden:0,  Width:80,    Align:"Center",	ColMerge:0, SaveName:"seq",			KeyField:0,   CalcLogic:"",   Format:"",        PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:25 },
            ...
            {Header:"PDF등록여부",	Type:"Combo",      	Hidden:0,  Width:50, 	Align:"Center",  ColMerge:0,   SaveName:"pdf_yn",     KeyField:0,   CalcLogic:"",   Format:"",         PointCount:0,   UpdateEdit:0,   InsertEdit:0,	EditLen:100 },
			...
			{Header:"파일다운",		Type:"Html",		Hidden:1,	Width:50,	Align:"Center",	ColMerge:0,	SaveName:"down_btn",	KeyField:0,	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100}
        ...
        var purposeCdList   = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYear="+<%=curSysYear%>,"C00315"), "");
        $("#searchPurposeCd").html(purposeCdList[2]);
        
        var adjustTypeList 	= stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYear="+<%=curSysYear%>,"C00303"), "");
    ...
    function doAction1(sAction) {
        ...
        case "Save":
        	...
        	for(var i = 1; i < sheet1.RowCount()+1; i++) {
            	...
            	if(sheet1.GetCellValue(i,"adjust_type") =="1") {
       				if(sheet1.GetCellValue(i,"work_yy") == year){
       					alert("정산구분이 연말정산인경우 전년도 까지만 신청 가능합니다.");
       					return;
       				}        				
            	}                   	
        	}
    ...
    function withHoldRcptStaPopup(sabuns, sortSabuns, /* sortNos,  */stamps){
        ...
        if($("#searchWorkYy").val() != null && $("#searchWorkYy").val() >= "2022"){
        	...
        	if(sheet1.RowCount() > 0 ){
            	...
            	var params = "&searchSabuns="+searchSabuns;
        			...
        			params += "&queryId=selectMigExistAppCnt";

	            var result = ajaxCall("<%=jspPath%>/withHoldRcptApp/withHoldRcptAppRst.jsp?cmd=selectMigExistAppCnt",params,false);
        ...
        args["rdParam"] = "[<%=removeXSS(session.getAttribute("ssnEnterCd"), "1")%>]"
                         ...
                         +" /rv sKey[] gKey[] sType[] qId[] themKey[] " //rd파라매터
                         ;//rd파라매터                         
	...
	function downloadFile(type, row) {
		...
		if(type == 'A') {
			...
			$(sCheckRow.split("|")).each(function(index,value){
				...
				// 이미 출력된 양식은 재출력 불가
	        	if(sheet1.GetCellValue(value, "print_yn") == "Y"){
	        		alert("이미 출력된 양식입니다.\n관리자에게 문의해 주시기 바랍니다.");
	        		isProc = false;
...
<div class="wrapper">
    ...
    <div class="outer">
        ...
        <ul>
            ...
            <li class="btn">
            	<font style="font-weight:bold;">귀속년도:&nbsp;&nbsp;</font><input class="text center required" type="text" id="searchWorkYy" name="searchWorkYy" size="4" maxlength= "4"  value="<%=yeaYear%>" />
