...
<script type="text/javascript">
	...
	$(function() {
		...
		initdata1.Cols = [
			...
			{Header:"일련번호",			Type:"Text",		Hidden:0,  Width:100,   Align:"Center",	 ColMerge:0,   SaveName:"seq",		  KeyField:0,   CalcLogic:"",   Format:"",         PointCount:0,   UpdateEdit:0,   InsertEdit:0,    EditLen:25 },
			...
			{Header:"신청일자",		Type:"Date",		Hidden:0,  Width:90,   Align:"Center",	 ColMerge:0,   SaveName:"appl_ymd",	  KeyField:0,   CalcLogic:"",   Format:"Ymd",      PointCount:0,   UpdateEdit:0,   InsertEdit:0,    EditLen:8 },
			{Header:"귀속년도",		Type:"Int",			Hidden:0,  Width:70,   Align:"Center",	 ColMerge:0,   SaveName:"work_yy",	  KeyField:1,   CalcLogic:"",   Format:"####",     PointCount:0,   UpdateEdit:0,   InsertEdit:1,    EditLen:4 },
            {Header:"출력구분",		Type:"Combo",		Hidden:0,  Width:120,   Align:"Center",	 ColMerge:0,   SaveName:"print_type", KeyField:1,   CalcLogic:"",   Format:"",         PointCount:0,   UpdateEdit:0,   InsertEdit:1,    EditLen:8 },
			...
			{Header:"처리상태",		Type:"Combo",		Hidden:0,  Width:80,   Align:"Center",	 ColMerge:0,   SaveName:"appl_status",KeyField:1,   CalcLogic:"",   Format:"",         PointCount:0,   UpdateEdit:1,   InsertEdit:1,    EditLen:10 },
			{Header:"처리일자",		Type:"Date",		Hidden:0,  Width:90,   Align:"Center",	 ColMerge:0,   SaveName:"appr_ymd",	  KeyField:0,   CalcLogic:"",   Format:"Ymd",      PointCount:0,   UpdateEdit:0,   InsertEdit:0,    EditLen:8 },
			...
			{Header:"이관대상자여부",     Type:"Combo",  	Hidden:0,  Width:100,	Align:"Center",  ColMerge:0,   SaveName:"mig_yn",   KeyField:0,   CalcLogic:"",   Format:"",         PointCount:0,   UpdateEdit:0,   InsertEdit:0,	EditLen:1, TrueValue:"Y", FalseValue:"N"},
			...
			{Header:"PDF등록여부",	Type:"Combo",      	Hidden:0,  Width:70, 	Align:"Center",  ColMerge:0,   SaveName:"pdf_yn",     KeyField:0,   CalcLogic:"",   Format:"",         PointCount:0,   UpdateEdit:0,   InsertEdit:0,	EditLen:100 },
			{Header:"파일구분",	Type:"Text",		Hidden:1,  Width:70,    Align:"Center",  ColMerge:0,   SaveName:"file_type",  KeyField:0,   CalcLogic:"",   Format:"",        PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100},
			{Header:"파일명",	Type:"Text",		Hidden:1,  Width:70,    Align:"Center",  ColMerge:0,   SaveName:"file_name",  KeyField:0,   CalcLogic:"",   Format:"",        PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100},
			{Header:"파일다운",		Type:"Html",		Hidden:0,	Width:50,	Align:"Center",	ColMerge:0,	SaveName:"down_btn",	KeyField:0,	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100}
        ...
        sheet1.SetColProperty("pdf_yn",    {ComboText:"존재|미존재", ComboCode:"Y|N"} );
        sheet1.SetColProperty("mig_yn",    {ComboText:"N|Y", ComboCode:"N|Y"} );
        
        var adjustTypeList 	= stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYear="+year,"C00303"), "");
	...
	function doAction1(sAction) {
        ...
        case "Insert":
            ...
            sheet1.SetCellValue(newRow, "pdf_yn",      "N");
            sheet1.SetCellValue(newRow, "mig_yn",      "");
            break;
        case "PDF":
			downloadFile('A', '0');
			break;
	...
	function sheet1_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try { 
			...
			for(var i = 1; i < sheet1.RowCount()+1; i++) {
				...
				var pdfYn = sheet1.GetCellValue(i,"pdf_yn");
				if(pdfYn == 'Y') {
        ...
        if(sheet1.RowCount() != 0) {
	        for(i=1; i<=sheet1.LastRow(); i++) {
	            ...
	            if (chk == "Y") {
	            	...
	            	//PDF 파일이 등록 되었는지 확인 - 2019.11.29.
	            	if(sheet1.GetCellValue(i,"pdf_yn") == 'Y') {
	            		if(isProc && !isMessage) {
	            			if(confirm("PDF가 등록된 항목이 존재합니다. 제외하고 출력 하시겠습니까?")) {
								isProc = true;
								isMessage = true;
							} else {
								isProc = false;
			    	...
			    	for(var j = 1; j < sheet1.RowCount()+1; j++) {
			    		...
			    		if(sheet1.GetCellValue(j,"chk") == "Y" && sheet1.GetCellValue(j,"pdf_yn") != 'Y') {
    ...
    function withHoldRcptStaPopup(sabuns, sortSabuns, /* sortNos,  */stamps){
        	...
        	if(sheet1.RowCount() > 0 ){
            	for(var i = 1; i < sheet1.RowCount()+1; i++) {
            		if(sheet1.GetCellValue(i,"chk") == "Y"){
            			...
            			if(i == 1) {
            				searchWorkYy 	 = sheet1.GetCellValue( i,"work_yy");
            				searchAdjustType = sheet1.GetCellValue( i,"adjust_type");
            			}
            		}
            	}
            	...
            	var params = "&searchSabuns="+searchSabuns;
        			params += "&searchWorkYy="+searchWorkYy;
        			params += "&searchAdjustType="+searchAdjustType;
        			params += "&queryId=selectMigExistAprCnt";
	            var result = ajaxCall("<%=jspPath%>/withHoldRcptApr/withHoldRcptAprRst.jsp?cmd=selectMigExistAprCnt",params,false);
        ...
        args["rdParam"] = "[<%=removeXSS(session.getAttribute("ssnEnterCd"), "1")%>]"
                         ...
                         +"["+ searchAdjustType +"]" //일괄다운로드 adjust_type리스트
                         ;//rd파라매터
	...
	function downloadFile(type, row) {
		...
		if(type == 'A') {
			...
			if ( sCheckRow == "" ){
				alert("선택된 내역이 없습니다. (출력)");
				return;
			}
			
			
			var isPdfCheck = false;
			$(sCheckRow.split("|")).each(function(index,value){
				
				if(sheet1.GetCellValue(value,"pdf_yn") == 'Y') {
					obj = new Object();
					...
					if(!isPdfCheck) {
						if(confirm("PDF가 등록되지 않은 항목이 존재합니다. 제외하고 다운로드 하시겠습니까?")) {
							isProc = true;
						} else {
							isProc = false;
							return;
					...
					if(isProc) {
						sheet1.SetCellValue(value,"chk", "N");
					}
				}
			});
			
			if(arr.length < 1) {
				alert("다운로드 가능한 대상자가 없습니다.");
				return;
			}
		...
		if(isProc && arr.length > 0) {
			$("#pWorkYy").val($("#searchWorkYy").val());
			$("#pValue").val(JSON.stringify(arr));
			$("#pfrm").attr("action", "<%=jspPath%>/withHoldRcptApr/withHoldRcptFileDownload.jsp");
			$("#pfrm").submit();
		}
	...
	<form id="sheetForm" name="sheetForm" >
			...
			<div>
				<table>
					<tr>
						...
						<td>
							<span>PDF등록여부</span>
							<select id="searchPdfYn" name ="searchPdfYn" onChange="" class="box">
								<option value="">전체</option>
								<option value="Y">존재</option>
								<option value="N">미존재</option>
							</select>
						</td>
					</tr>
					<tr>
						...
						<td>
	                        <span>이관자료여부</span>
	                        <select id="searchMigYn" name ="searchMigYn" onChange="" class="box">
		...
		<div class="outer">
	        <div class="explain">
		        ...
		        <div class="txt">
					<ul>
						<li> ▶ 이관한 대상자와 이관하지 않은 대상자 동시 선택하여 동시 출력에 제한이 있으므로, 각각 선택하여 출력하기 바랍니다.</li>
					</ul>
				</div>
        	</div>
	    </div>
				...
				<div class="inner">
					<div class="sheet_title">
						<ul>
							...
							<li class="btn">
								<span><font class="txt" style="color: red; text-overflow: ellipsis; white-space: nowrap;">&nbsp;[ 연말정산>연간소득관리>연간소득개별 ] 작업구분 원천징수부로 변경 후 금액 확인 및 수정 가능&nbsp;</font></span>
								...
								<a href="javascript:doAction1('PDF');" 			class="button authA">PDF다운로드</a>
