	...
	function shtToCtl(){		 
		if(sheet1.RowCount() > 0){
			for (i = 1; i < sheet1.RowCount()+1; i++){
				...
				} else if(sheet1.GetCellValue(i,"std_cd")=="CPN_YEA_PDF_YN"){   //연말정산 피드백 버튼 보여주기 유무
					$("input:radio[name='cpn_yea_pdf_yn']:input[value='"+stdValue+"']").attr("checked",true);
				} else if(sheet1.GetCellValue(i,"std_cd")=="CPN_YEA_DED_PRINT_YN"){   //연말정산 피드백 버튼 보여주기 유무
					$("input:radio[name='cpn_yea_ded_print_yn']:input[value='"+stdValue+"']").attr("checked",true);
				} else if(sheet1.GetCellValue(i,"std_cd")=="CPN_YEA_DOJANG_TYPE"){   //연말정산 피드백 버튼 보여주기 유무
					$("input:radio[name='cpn_yea_dojang_type']:input[value='"+stdValue+"']").attr("checked",true);
				} else if(sheet1.GetCellValue(i,"std_cd")=="CPN_YEA_JINGSUJA_TYPE"){   //연말정산 피드백 버튼 보여주기 유무
                    $("input:radio[name='cpn_yea_jingsuja_type']:input[value='"+stdValue+"']").attr("checked",true);
                } else if(sheet1.GetCellValue(i,"std_cd")=="CPN_YEA_TAB_DISABLED_YN"){   //연말정산 피드백 버튼 보여주기 유무
                    $("input:radio[name='cpn_yea_tab_disabled_yn']:input[value='"+stdValue+"']").attr("checked",true);
                }
			}			
		}		 
	}
		...
		if(sheet1.RowCount() > 0){
			if(confirm("연말정산 옵션 내역을 저장하시겠습니까?")){				
				for (i = 1; i < sheet1.RowCount()+1; i++){
					...
					} else if(sheet1.GetCellValue(i,"std_cd")=="CPN_YEA_DED_PRINT_YN"){   //pdf 업로드 탭 보여주기 유무
						sheet1.SetCellValue(i, "std_cd_value", $("input:radio[name='cpn_yea_ded_print_yn']:checked").val());
					} else if(sheet1.GetCellValue(i,"std_cd")=="CPN_YEA_DOJANG_TYPE"){   //pdf 업로드 탭 보여주기 유무
						sheet1.SetCellValue(i, "std_cd_value", $("input:radio[name='cpn_yea_dojang_type']:checked").val());
					} else if(sheet1.GetCellValue(i,"std_cd")=="CPN_YEA_JINGSUJA_TYPE"){   //pdf 업로드 탭 보여주기 유무
						sheet1.SetCellValue(i, "std_cd_value", $("input:radio[name='cpn_yea_jingsuja_type']:checked").val());
					} else if(sheet1.GetCellValue(i,"std_cd")=="CPN_YEA_TAB_DISABLED_YN"){   //pdf 업로드 탭 보여주기 유무
						sheet1.SetCellValue(i, "std_cd_value", $("input:radio[name='cpn_yea_tab_disabled_yn']:checked").val());
					}
				}
	...
	<div class="wrapper">
		...
		<div>
			<table border="0" cellpadding="0" cellspacing="0" class="default outer">
                ...
                <tr>
                    <td class="left">
                        <b>[자료등록 인적공제 탭만 보여주기]</b><br>
                        &nbsp;자료등록 화면의 탭들 중에서 인적공제 탭만 보여줍니다. (2014년 연말정산의 경우에만 사용되는 옵션) <br>
                        &nbsp;YES : 인적공제 탭만 보여주기<br>
                        &nbsp;NO : 모든 탭 보여주기<br>
                    </td>
                    <td align="left">
                        <input type="radio" class="radio" name="cpn_yea_tab_disabled_yn" value = "Y" >YES&nbsp;&nbsp;&nbsp;<br>   
                        <input type="radio" class="radio" name="cpn_yea_tab_disabled_yn" value = "N" >NO
                    </td>       
