...
<script type="text/javascript">
	...
	//전년도 합계 정보 조회
	function getPreYeaDataInfo() {
		
		var preyeaInfo = ajaxCall("<%=jspPath%>/yeaData/yeaDataCardsRst.jsp?cmd=selectPreYeaDataInfo", $("#sheetForm").serialize(),false);

		$("#preTotSum").val(nvl(preyeaInfo.Data.pre_tot_sum,"0"));
		$("#billSum").val(nvl(preyeaInfo.Data.bill_sum,"0"));
		$("#checkSum").val(nvl(preyeaInfo.Data.check_sum,"0")) ;
		$("#marketSum").val(nvl(preyeaInfo.Data.market_sum,"0")) ;
		$("#busSum").val(nvl(preyeaInfo.Data.bus_sum,"0")) ;
		$("#checkBillSum").val(nvl(preyeaInfo.Data.check_bill_sum,"0")) ;
	}
		...
		try{
			...
			if( sheet1.ColSaveName(Col) == "famres" ) {
				...
				if( (typeof cardType === "string" && cardType.indexOf("2013") > -1)
					...
					alert("근로제공기간 이외 (본인) 신용카드등 사용액의 대상은"
							...
							+ "\n 1. 명의인 : 본인"
							+ "\n 2. 반기구분 : 당해 전체"
							+ "\n * 종전근무지를 포함하여 2014.1.1 ~ 2014.12.31 기간중 "
			...
			if( sheet1.ColSaveName(Col) == "card_type" ) {
				...
				if(typeof cardType === "string" && cardType.indexOf("2013") > -1) {
					...
					sheet1.SetCellValue(Row, "famres", gOwnFamCd) ;
					sheet1.SetCellValue(Row, "half_gubun", "20") ;
					return;
					
				} else {
					if ( (nvl(sheet1.GetCellText(Row, "card_type"), "") != "")
					  && (typeof cardType === "string" && cardType.indexOf("2013") > -1 ) ) {
...
<div class="wrapper">
	...
	<div class="outer">
		...
		<ul>
			<li class="txt">2014 신용카드<span class="txt"> ※ 총급여 25% 미만의 연간 총액은 공제금액이 발생하지 않습니다.</span>
	...
	<tr>
		<td class="right">
			<input id="preTotSum" name="preTotSum" type="text" class="text w50p right transparent" readonly /> 원
		</td>
		<td class="right">
			<input id="billSum" name="billSum" type="text" class="text w50p right transparent" readOnly /> 원
		</td>
		<td class="right">
			<input id="checkSum" name="checkSum" type="text" class="text w50p right transparent" readOnly /> 원
		</td>
		<td class="right">
			<input id="marketSum" name="marketSum" type="text" class="text w50p right transparent" readOnly /> 원
		</td>
		<td class="right">
			<input id="busSum" name="busSum" type="text" class="text w50p right transparent" readOnly /> 원
		</td>
		<!-- 
		<td class="right">
