...
<div class="wrapper">
	...
	<div class="inner" id="div_button">
		...
		<ul>
			<li class="right">
				<span id="spanMagam" style="display:none;"><a href="javascript:doAction('InputClose');" class="basic" style="border:2px solid #FF8080;">입력마감</a></span>
				<span id="spanMagamCancel" style="display:none;"><a href="javascript:doAction('CancelClose');" class="basic">마감취소</a></span>
	...
	<div class="insa_tab" style="top:175px;" id="div_insa_tab">
...
<script type="text/javascript">
	...
	function createTabFrame() {
		...
		//인적공제 탭만 활성화 여부 2014년 재계산 대응
        var perOnlyVisibleYn = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_TAB_DISABLED_YN", "queryId=getSystemStdData",false).codeList;
        var perOnlyTabHide = "";
		
        if(perOnlyVisibleYn[0].code_nm == "Y") {
        	perOnlyTabHide = "hide";
        }
		...
		.append("<li ><a href='#tabs-1'  id='tabs1'>주소사항</a></li>")
		.append("<li><a href='#tabs-2'  id='tabs2'>인적공제</a></li>")
		.append("<li class='"+pdfTabHide+"'>    <a href='#tabs-3'  id='tabs3'>PDF등록</a></li>")
		.append("<li class='"+perOnlyTabHide+"'><a href='#tabs-4'  id='tabs4'>연금</a></li>")
		.append("<li class='"+perOnlyTabHide+"'><a href='#tabs-5'  id='tabs5'>보험</a></li>")
		.append("<li class='"+perOnlyTabHide+"'><a href='#tabs-6'  id='tabs6'>의료</a></li>")
		.append("<li class='"+perOnlyTabHide+"'><a href='#tabs-7'  id='tabs7'>교육</a></li>")
		.append("<li class='"+perOnlyTabHide+"'><a href='#tabs-8'  id='tabs8'>주택자금</a></li>")
		.append("<li class='"+perOnlyTabHide+"'><a href='#tabs-9'  id='tabs9'>기부</a></li>")
		.append("<li class='"+perOnlyTabHide+"'><a href='#tabs-10' id='tabs10'>저축</a></li>")
		.append("<li class='"+perOnlyTabHide+"'><a href='#tabs-11' id='tabs11'>카드등</a></li>")
		.append("<li class='"+perOnlyTabHide+"'><a href='#tabs-12' id='tabs12'>기타공제</a></li>")
		.append("<li class='"+perOnlyTabHide+"'><a href='#tabs-13' id='tabs13'>세액감면/기타세액공제</a></li>");		

		tabObj.append("<div id='tabs-1'><div class='layout_tabs'><iframe src='<%=jspPath%>/common/hidden.jsp' frameborder='0' class='tab_iframes'></iframe></div></div>")
	...
	function showIframe() {
		...
		if(iframeIdx == 0) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataAddr.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 1) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataPer.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 2) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataPdf.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 3) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataPen.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 4) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataIns.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 5) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataMed.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 6) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataEdu.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 7) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataHou.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 8) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataDon.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 9) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataHouSav.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 10) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataCards.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 11) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataEtc.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		} else if(iframeIdx == 12) {
			newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataTax.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
		}
	}
	...
	function getYearDefaultInfoObj() {
		...
		if(result.Result.Code == 1) {
			...
			$("#tabs4").html("연금("+result.Data.cnt4+")");
			$("#tabs5").html("보험("+result.Data.cnt5+")");
			$("#tabs6").html("의료("+result.Data.cnt6+")");
			$("#tabs7").html("교육("+result.Data.cnt7+")");
			$("#tabs8").html("주택자금("+result.Data.cnt8+")");
			$("#tabs9").html("기부("+result.Data.cnt9+")");
			$("#tabs10").html("저축("+result.Data.cnt10+")");
			$("#tabs11").html("카드등("+result.Data.cnt11+")");
			$("#tabs12").html("기타공제("+result.Data.cnt12+")");
			$("#tabs13").html("세액감면/기타세액공제("+result.Data.cnt13+")");
		}			
	...
	function openFeedbackPopup() {
		...
		var authPg = $('<input name="authPg" type="hidden" value="R">');
	...
	function doReport(report_type){
		...
		args["rdTitle"] = popupTitle ;//rd Popup제목
		args["rdMrd"] = "<%=cpnYearEndPath%>/"+rdFileNm;//( 공통경로 /html/report/는 공통에서 처리)업무경로+rd파일명
		args["rdParam"] = "[<%=removeXSS(session.getAttribute("ssnEnterCd"), "1")%>] ["+$("#searchWorkYy").val()+"] ["+$("#searchAdjustType").val()+"] ['"+$("#searchSabun").val()+"'] [00000] [ALL]";//rd파라매터
		args["rdParamGubun"] = "rp" ;//파라매터구분(rp/rv)
		args["rdToolBarYn"] = "Y" ;//툴바여부
		args["rdZoomRatio"] = "100" ;//확대축소비율
