...
<%!
//장기주식형저축 자료 조회
public List selectYeaDataStockSavList(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectYeaDataStockSavList",pm);
	} catch (Exception e) {
...
<%
	String locPath = xmlPath+"/yeaData/yeaDataStockSav.xml";

	String ssnEnterCd = (String)session.getAttribute("ssnEnterCd");
	String ssnSabun = (String)session.getAttribute("ssnSabun");
	String cmd = (String)request.getParameter("cmd");

	if("selectYeaDataStockSavList".equals(cmd)) {
		//장기주식형저축 자료 조회
		...
		try {
			listData = selectYeaDataStockSavList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
	...
	} else if("saveYeaDataStockSav".equals(cmd)) {
		//장기주식형저축 저장.
		...
		try {
			...
			if(saveList != null && saveList.size() > 0) {
				for(int i = 0; i < saveList.size(); i++) {
					...
					String payingNumCd = (String)mp.get("paying_num_cd");
					...
					String[] param = new String[]{"","",sStatus,ssnEnterCd,workYy
							,adjustType,sabun,financeOrgCd,payingNumCd,accountNo
							,applMon,adjInputType,ntsYn,ssnSabun};

					String[] rstStr = DBConn.executeProcedure("PKG_CPN_YEA_"+yeaYear+"_SYNC.STOCK_SAVING_INS",type,param);
