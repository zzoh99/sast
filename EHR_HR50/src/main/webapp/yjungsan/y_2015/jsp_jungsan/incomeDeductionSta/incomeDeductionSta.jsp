...
<script type="text/javascript">
	...
	$(function() {
		...
		initdata1.Cols = [
			...
			{Header:"PDF파일보기", Type:"Html",		Hidden:0,	Width:70,	Align:"Center",		ColMerge:0,	  SaveName:"file_link",	 KeyField:0,	               Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100}
	...
	function doAction1(sAction) {
		...
		case "Down2Excel":
			...
			break;
		case "Down2Template":
			var param  = {DownCols:"1|2|5",SheetDesign:1,Merge:1,DownRows:"0",ExcelFontSize:"9"
		...
		try {
			...
			if(Code == 1) {
				for(var i = 1; i < sheet1.RowCount()+1; i++) {
					sheet1.SetCellValue(i,"file_link","<a href=\"javascript:openPdfViewPopup('"+sheet1.GetCellValue(i,"sabun")+"')\" class='basic'>보기</a>");
				}	
			}
...
// 	}
	...
	//pdf 업로드 팝업
	function openPdfViewPopup(sabun){
		
		if(!isPopup()) {return;}
		
		var file_path = "";
		var file_name = "";
		var param = "searchSabun="+sabun+"&searchWorkYy="+$("#searchWorkYy").val()+"&searchAdjustType="+$("#searchAdjustType").val() ;

		var result = ajaxCall("<%=jspPath%>/incomeDeductionSta/incomeDeductionStaRst.jsp?cmd=getFileInfo", param,false);
		...
		if ( file_path==null || file_path=="" || file_name==null || file_name=="" ) {
			alert("등록된 PDF파일이 없습니다.");
			return;
		} else {
			var filePath = "<%=serverBaseUrl%>"+file_path+"/"+file_name;
			openPopup(filePath,"","700","600");
		}
	}
