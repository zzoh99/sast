...
try {
    ...
    /* MultipartRequest 로 파일 업로드 temp폴더로(LOADEXCEL.PATH)로 업로드 */
	MultipartRequest req = new MultipartRequest(request, StringUtil.getPropertiesValue("LOADEXCEL.PATH"), 5*1024000, sysEnc);
    ...
    while(params.hasMoreElements()) {
        ...
        if ( "paramPwd".equals(key)) {
            paramPwd = value;
        } else if("paramSabun".equals(key) ) {
            paramSabun = value;
        } else if("paramYear".equals(key) ) {
        	paramYear = yeaYear;
        } else if("paramAdjustType".equals(key) ) {
            paramAdjustType = value;
        }
    }
    ...
    while(files.hasMoreElements()) {
    	String fileItem = (String)files.nextElement();

        File fileData = req.getFile(fileItem);

        // path 제외한 파일명만 취득
        String[] filePath = fileData.getName().split("\\\\");
        ...
        // 파일내용을 읽음
        byte[] pdfBytes = FileUtils.readFileToByteArray(fileData);
        FileInputStream fileInputS = new FileInputStream(fileData);
        InputStreamReader fileInputRd = new InputStreamReader(fileInputS);

        String pdfBytesEnc = fileInputRd.getEncoding().toLowerCase();
...
//                                 /* 2014-11-24 JEKIM MODIFY */
//                                 // 2013 신용카드 로직 추가(종류 3 [일반, 전통시장, 대중교통])
//                                 for(int halfIndex = 0; halfIndex < 3; halfIndex++ ) {
//                                     dataMap = getDefaultDataMap(ssnEnterCd,ssnSabun,paramYear,paramAdjustType,paramSabun,docType,docSeq,formCd,manResId,manName);

//                                     dataMap.put("A3",""); //자료코드
//                                     dataMap.put("A4",""); //사업자번호
//                                     dataMap.put("A5",""); //상호
//                                     dataMap.put("A8", "2013"); //반기구분 2013전체

//                                     if ( halfIndex == 0 ) {
//                                         dataMap.put("A6","19"); //종류 (일반 신용카드)
//                                         dataMap.put("A7",((Element)childList.get(manIndex)).getAttributeValue("pre_tot_amt")); //2013  일반 신용카드
//                                     } else if ( halfIndex == 1 ) {
//                                         dataMap.put("A6","25"); //종류 (전통시장사용분(신용카드))
//                                         dataMap.put("A7",((Element)childList.get(manIndex)).getAttributeValue("pre_market_tot_amt")); //2013 전통시장 신용카드
//                                     } else {
//                                         dataMap.put("A6","29"); //종류 (대중교통이용분(신용카드))
//                                         dataMap.put("A7",((Element)childList.get(manIndex)).getAttributeValue("pre_tmoney_tot_amt")); //2013 대중교통 신용카드
//                                     }
...
//                                     /* 2014-11-24 JEKIM MODIFY */
//                                     // 2014 상반기 하반기 반기구분코드 로직 추가(반기구분 2 [상반기, 하반기])
...
//                                         /* 2014-11-24 JEKIM MODIFY */
//                                         // 상/하반기 신용카드 사용액, 반기구분코드
//                                         if ( halfIndex == 0 ) {
//                                             dataMap.put("A7",((Element)childList2.get(dataIndex)).getAttributeValue("first_tot_amt")); //상반기 신용카드
//                                             dataMap.put("A8", "10"); //반기구분 2014상반기
//                                         } else {
//                                             dataMap.put("A7",((Element)childList2.get(dataIndex)).getAttributeValue("second_tot_amt")); //하반기 신용카드
//                                             dataMap.put("A8", "20"); //반기구분 2014하반기
//                                         }
...
//                                 }
                            } else if("G103Y".equals(formCd)) {
                                //신용카드
                                /* 2015-12-15 JEKIM MODIFY */

                                // 2014, 2013 신용카드 로직 추가(종류 3 [일반, 전통시장, 대중교통])
                                for(int halfIndex = 0; halfIndex < 6; halfIndex++ ) {
                                    dataMap = getDefaultDataMap(ssnEnterCd,ssnSabun,paramYear,paramAdjustType,paramSabun,docType,docSeq,formCd,manResId,manName);
                                    ...
                                    if ( halfIndex == 0 ) {
                                    	dataMap.put("A8", "2014"); //반기구분 2014전체
                                        dataMap.put("A6","41"); //종류 (일반 신용카드)
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("pre_tot_amt"),"")); //2014 (본인)일반 신용카드
                                    } else if ( halfIndex == 1 ) {
                                    	dataMap.put("A8", "2014"); //반기구분 2014전체
                                        dataMap.put("A6","44"); //종류 (전통시장사용분(신용카드))
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("pre_market_tot_amt"),"")); //2014 (본인)전통시장 신용카드
                                    } else if ( halfIndex == 2 ) {
                                    	dataMap.put("A8", "2014"); //반기구분 2014전체
                                        dataMap.put("A6","46"); //종류 (대중교통이용분(신용카드))
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("pre_tmoney_tot_amt"),"")); //2014 (본인)대중교통 신용카드

                                    } else if ( halfIndex == 3 ) {
                                    	dataMap.put("A8", "2013"); //반기구분 2013전체
                                        dataMap.put("A6","19"); //종류 (일반 신용카드)
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("ftyr_tot_amt"),"")); //2013 (본인)일반 신용카드
                                    } else if ( halfIndex == 4 ) {
                                    	dataMap.put("A8", "2013"); //반기구분 2013전체
                                        dataMap.put("A6","25"); //종류 (전통시장사용분(신용카드))
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("ftyr_market_tot_amt"),"")); //2013 (본인)전통시장 신용카드
                                    } else {
                                    	dataMap.put("A8", "2013"); //반기구분 2013전체
                                        dataMap.put("A6","29"); //종류 (대중교통이용분(신용카드))
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("ftyr_tmoney_tot_amt"),"")); //2013 (본인)대중교통 신용카드
                                    }
                                ...
                                for(int dataIndex = 0; dataIndex < childList2.size(); dataIndex++ ) {
                                    ...
                                    // 당해 상반기 하반기 반기구분코드 로직 추가(반기구분 2 [상반기, 하반기])
                                    for(int halfIndex = 0; halfIndex < 2; halfIndex++ ) {
                                        ...
                                        // 상/하반기 신용카드 사용액, 반기구분코드
                                        if ( halfIndex == 0 ) {
                                            dataMap.put("A7",((Element)childList2.get(dataIndex)).getAttributeValue("first_year_tot_amt")); //상반기 신용카드
                                            dataMap.put("A8", "10"); //반기구분 당해상반기
                                        } else {
                                            dataMap.put("A7",((Element)childList2.get(dataIndex)).getAttributeValue("second_year_tot_amt")); //하반기 신용카드
                                            dataMap.put("A8", "20"); //반기구분 당해 하반기
                                        }
...
//                                 /* 2014-11-24 JEKIM MODIFY */
//                                 // 2013 현금영수증 로직 추가(종류 3 [일반, 전통시장, 대중교통])
//                                 for(int halfIndex = 0; halfIndex < 3; halfIndex++ ) {
//                                     dataMap = getDefaultDataMap(ssnEnterCd,ssnSabun,paramYear,paramAdjustType,paramSabun,docType,docSeq,formCd,manResId,manName);

//                                     dataMap.put("A3",""); //자료코드
//                                     dataMap.put("A6", "2013"); //반기구분 2013전체

//                                     if ( halfIndex == 0 ) {
//                                         dataMap.put("A4","23"); //종류 (현금영수증)
//                                         dataMap.put("A5",((Element)childList.get(manIndex)).getAttributeValue("pre_tot_amt")); //2013  일반 현금영수증
//                                     } else if ( halfIndex == 1 ) {
//                                         dataMap.put("A4","27"); //종류 (전통시장사용분(현금영수증))
//                                         dataMap.put("A5",((Element)childList.get(manIndex)).getAttributeValue("pre_market_tot_amt")); //2013 전통시장 현금영수증
//                                     } else {
//                                         dataMap.put("A4","31"); //종류 (대중교통이용분(현금영수증))
//                                         dataMap.put("A5",((Element)childList.get(manIndex)).getAttributeValue("pre_tmoney_tot_amt")); //2013 대중교통 현금영수증
//                                     }
...
//                                         /* 2014-11-24 JEKIM MODIFY */
//                                         // 상/하반기 현금영수증 사용액, 반기구분코드
//                                         if ( halfIndex == 0 ) {
//                                             dataMap.put("A5",((Element)childList2.get(dataIndex)).getAttributeValue("first_tot_amt")); //상반기 현금영수증
//                                             dataMap.put("A6", "10"); //반기구분 2014상반기
//                                         } else {
//                                             dataMap.put("A5",((Element)childList2.get(dataIndex)).getAttributeValue("second_tot_amt")); //하반기 현금영수증
//                                             dataMap.put("A6", "20"); //반기구분 2014하반기
//                                         }

//                                         dataList.add(dataMap);
//                                     }
//                                 }
                            } else if("G204M".equals(formCd)) {
                                //현금영수증
                                /* 2015-12-15 JEKIM MODIFY */

                                // 2014, 2013 현금영수증 로직 추가(종류 3 [일반, 전통시장, 대중교통])
                                for(int halfIndex = 0; halfIndex < 6; halfIndex++ ) {
                                    dataMap = getDefaultDataMap(ssnEnterCd,ssnSabun,paramYear,paramAdjustType,paramSabun,docType,docSeq,formCd,manResId,manName);
                                    ...
                                    if ( halfIndex == 0 ) {
                                    	dataMap.put("A6", "2014"); //반기구분 2014전체
                                        dataMap.put("A4","43");    //종류 (현금영수증)
                                        dataMap.put("A5",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("pre_tot_amt"),"")); //2014  일반 현금영수증
                                    } else if ( halfIndex == 1 ) {
                                    	dataMap.put("A6", "2014"); //반기구분 2014전체
                                        dataMap.put("A4","45"); //종류 (전통시장사용분(현금영수증))
                                        dataMap.put("A5",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("pre_market_tot_amt"),"")); //2014 전통시장 현금영수증
                                    } else if ( halfIndex == 2 ) {
                                    	dataMap.put("A6", "2014"); //반기구분 2014전체
                                        dataMap.put("A4","47"); //종류 (대중교통이용분(현금영수증))
                                        dataMap.put("A5",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("pre_tmoney_tot_amt"),"")); //2014 대중교통 현금영수증

                                    } else if ( halfIndex == 3 ) {
                                    	dataMap.put("A6", "2013"); //반기구분 2013전체
                                        dataMap.put("A4","23"); //종류 (현금영수증)
                                        dataMap.put("A5",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("ftyr_tot_amt"),"")); //2013  일반 현금영수증
                                    } else if ( halfIndex == 4 ) {
                                    	dataMap.put("A6", "2013"); //반기구분 2013전체
                                        dataMap.put("A4","27"); //종류 (전통시장사용분(현금영수증))
                                        dataMap.put("A5",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("ftyr_market_tot_amt"),"")); //2013 전통시장 현금영수증
                                    } else {
                                    	dataMap.put("A6", "2013"); //반기구분 2013전체
                                        dataMap.put("A4","31"); //종류 (대중교통이용분(현금영수증))
                                        dataMap.put("A5",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("ftyr_tmoney_tot_amt"),"")); //2013 대중교통 현금영수증
                                    }
                                ...
                                for(int dataIndex = 0; dataIndex < childList2.size(); dataIndex++ ) {
                                    ...
                                    for(int halfIndex = 0; halfIndex < 2; halfIndex++ ) {
                                        ...
                                        // 상/하반기 현금영수증 사용액, 반기구분코드
                                        if ( halfIndex == 0 ) {
                                            dataMap.put("A5",((Element)childList2.get(dataIndex)).getAttributeValue("first_year_tot_amt")); //상반기 현금영수증
                                            //dataMap.put("A5",((Element)childList2.get(dataIndex)).getAttributeValue("tfhy_tot_amt"));  //상반기 현금영수증
                                            dataMap.put("A6", "10"); //반기구분 당해상반기
                                        } else {
                                            dataMap.put("A5",((Element)childList2.get(dataIndex)).getAttributeValue("second_year_tot_amt")); //하반기 현금영수증
                                            //dataMap.put("A5",((Element)childList2.get(dataIndex)).getAttributeValue("shfy_tot_amt")); //하반기 현금영수증
                                            dataMap.put("A6", "20"); //반기구분 당해하반기
                                        }
...
//                                 /* 2014-11-24 JEKIM MODIFY */
//                                 // 2013 직불카드 로직 추가(종류 3 [일반, 전통시장, 대중교통])
//                                 for(int halfIndex = 0; halfIndex < 3; halfIndex++ ) {
//                                     dataMap = getDefaultDataMap(ssnEnterCd,ssnSabun,paramYear,paramAdjustType,paramSabun,docType,docSeq,formCd,manResId,manName);
...
//                                     if ( halfIndex == 0 ) {
//                                         dataMap.put("A6","21"); //종류 (직불 기명식선불카드)
//                                         dataMap.put("A7",((Element)childList.get(manIndex)).getAttributeValue("pre_tot_amt")); //2013  직불 기명식선불카드
//                                     } else if ( halfIndex == 1 ) {
//                                         dataMap.put("A6","27"); //종류 (전통시장사용분(직불카드))
//                                         dataMap.put("A7",((Element)childList.get(manIndex)).getAttributeValue("pre_market_tot_amt")); //2013 전통시장 직불카드
//                                     } else {
//                                         dataMap.put("A6","31"); //종류 (대중교통이용분(직불카드))
//                                         dataMap.put("A7",((Element)childList.get(manIndex)).getAttributeValue("pre_tmoney_tot_amt")); //2013 대중교통 직불카드
//                                     }
...
//                                         /* 2014-11-24 JEKIM MODIFY */
//                                         // 상/하반기 직불카드 사용액, 반기구분코드
//                                         if ( halfIndex == 0 ) {
//                                             dataMap.put("A7",((Element)childList2.get(dataIndex)).getAttributeValue("first_tot_amt")); //상반기 직불카드
//                                             dataMap.put("A8", "10"); //반기구분 2014상반기
//                                         } else {
//                                             dataMap.put("A7",((Element)childList2.get(dataIndex)).getAttributeValue("second_tot_amt")); //하반기 직불카드
//                                             dataMap.put("A8", "20"); //반기구분 2014하반기
//                                         }
...
//                                     }
//                                 }
                            } else if("G303Y".equals(formCd)) {
                                //직불카드
                                /* 2015-12-15 JEKIM MODIFY */

                                // 2014, 2013 직불카드 로직 추가(종류 3 [일반, 전통시장, 대중교통])
                                for(int halfIndex = 0; halfIndex < 6; halfIndex++ ) {
                                    dataMap = getDefaultDataMap(ssnEnterCd,ssnSabun,paramYear,paramAdjustType,paramSabun,docType,docSeq,formCd,manResId,manName);
                                    ...
                                    if ( halfIndex == 0 ) {
                                    	dataMap.put("A8", "2014"); //반기구분 2014전체
                                        dataMap.put("A6","42"); //종류 (직불 기명식선불카드)
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("pre_tot_amt"),"")); //2014  직불 기명식선불카드
                                    } else if ( halfIndex == 1 ) {
                                    	dataMap.put("A8", "2014"); //반기구분 2014전체
                                        dataMap.put("A6","45"); //종류 (전통시장사용분(직불카드))
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("pre_market_tot_amt"),"")); //2014 전통시장 직불카드
                                    } else if ( halfIndex == 2 ) {
                                    	dataMap.put("A8", "2014"); //반기구분 2014전체
                                        dataMap.put("A6","47"); //종류 (대중교통이용분(직불카드))
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("pre_tmoney_tot_amt"),"")); //2014 대중교통 직불카드

                                    } else if ( halfIndex == 3 ) {
                                    	dataMap.put("A8", "2013"); //반기구분 2013전체
                                        dataMap.put("A6","21"); //종류 (직불 기명식선불카드)
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("ftyr_tot_amt"),"")); //2013  직불 기명식선불카드
                                    } else if ( halfIndex == 4 ) {
                                    	dataMap.put("A8", "2013"); //반기구분 2013전체
                                        dataMap.put("A6","27"); //종류 (전통시장사용분(직불카드))
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("ftyr_market_tot_amt"),"")); //2013 전통시장 직불카드
                                    } else {
                                    	dataMap.put("A8", "2013"); //반기구분 2013전체
                                        dataMap.put("A6","31"); //종류 (대중교통이용분(직불카드))
                                        dataMap.put("A7",StringUtil.nvl(((Element)childList.get(manIndex)).getAttributeValue("ftyr_tmoney_tot_amt"),"")); //2013 대중교통 직불카드
                                    }
                                ...
                                for(int dataIndex = 0; dataIndex < childList2.size(); dataIndex++ ) {
                                    ...
                                    for(int halfIndex = 0; halfIndex < 2; halfIndex++ ) {
                                        ...
                                        // 상/하반기 직불카드 사용액, 반기구분코드
                                        if ( halfIndex == 0 ) {
                                            dataMap.put("A7",((Element)childList2.get(dataIndex)).getAttributeValue("first_year_tot_amt")); //상반기 직불카드
                                            //dataMap.put("A7",((Element)childList2.get(dataIndex)).getAttributeValue("tfhy_tot_amt")); //상반기 직불카드
                                            dataMap.put("A8", "10"); //반기구분 당해상반기
                                        } else {
                                            dataMap.put("A7",((Element)childList2.get(dataIndex)).getAttributeValue("second_year_tot_amt")); //하반기 직불카드
                                            //dataMap.put("A7",((Element)childList2.get(dataIndex)).getAttributeValue("shfy_tot_amt")); //하반기 직불카드
                                            dataMap.put("A8", "20"); //반기구분 당해하반기
                                        }
        ...
        if(dataList != null && dataList.size() > 0) {
            ...
            try{
                ...
                if(fileData.isFile() ) {
                    ...
                    if(fileSize > 0) {
                    	String attFileNm = fileData.getName().trim();
                        ...
                        //파일명 암호화
                        //Map mapData  = DBConn.executeQueryMap(queryMap,"selectPdfFileEncrypt",fileMp);
                        String file_name = "";
                        //if ( mapData != null && mapData.get("file_name") != null && !mapData.get("file_name").equals("") ) {
                        	SecureRandom secureRandom = new SecureRandom();
                        	file_name = paramSabun + StringUtil.padLeft(secureRandom.nextInt(10000)+"", "0" ,4);
...
//     ex.printStackTrace();
    Log.Debug("[pdfUploadRstCp Exception]" + ex.getMessage());
%>
    <script>
