...
public List selectWithHoldRcptStaList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	try{
		if(pm.get("searchPage") != null && !"".equals(pm.get("searchPage").toString()) ) {
			...
			listData = DBConn.executeQueryList(queryMap,"selectWithHoldPaging",pm);

		} else {
			listData  = DBConn.executeQueryList(queryMap,"selectWithHoldRcptStaList",pm);
		}
...
public Map selectEmpInfoUsingSabun(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		pm  = DBConn.executeQueryMap(queryMap,"selectEmpInfoUsingSabun",pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
	} finally {
...
public Map selecWithHoldTotCnt(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		map  = DBConn.executeQueryMap(queryMap,"selecWithHoldTotCnt",pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
	} finally {
...
public Map selectMigExistCnt(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		mp  = DBConn.executeQueryMap(queryMap, queryId, pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
	}
...
<%
	String locPath = xmlPath+"/withHoldRcptSta/withHoldRcptSta.xml";
