...
public List getCommonCodeList(String orderBy, Map paramMap, String locPath) throws Exception {
	...
	try{
		...
		//쿼리 실행및 결과 받기.
		list  = DBConn.executeQueryList(queryMap, "getCommonCodeList", pm);
	} catch (Exception e) {
		Log.Error("[Exception] \n" + e);
		throw new Exception();
	} finally {
...
// 		}
		...
		//쿼리 실행및 결과 받기.
		list  = DBConn.executeQueryList(queryMap, "getCardTypeCodeList", pm);
	} catch (Exception e) {
		Log.Error("[Exception] \n" + e);
		throw new Exception();
	} finally {
...
//사업장 권한범위 적용 조회
public List getBizPlaceCdAuthList(String queryId, Map paramMap, String locPath) throws Exception {
...
<%
	...
	if("getCommonCodeList".equals(cmd)) {
		...
		*/
		try {
			listData = getBizPlaceCdAuthList(queryId, mp, locPath);
		} catch(Exception e) {
			code = "-1";
