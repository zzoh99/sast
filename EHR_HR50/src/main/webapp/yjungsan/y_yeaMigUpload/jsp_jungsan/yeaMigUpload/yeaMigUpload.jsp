...
<script type="text/javascript">
	...
	$(function() {
		...
		var adjustTypeList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYear="+$("#searchWorkYy").val(),"C00303"), "");
		$("#searchAdjustType").html(adjustTypeList[2]);

		newIframe = $('#tabs-1 iframe');
		iframeIdx = 0;
		newIframe.attr("src","<%=jspPath%>/yeaMigUpload/yeaMigInfoTab.jsp?authPg=<%=authPg%>");
		$( "#tabs" ).tabs({
		...
		function showIframe() {
			...
			if (iframeIdx == 0) {
				...
				newIframe.attr("src","<%=jspPath%>/yeaMigUpload/yeaMigCurWpTab.jsp?authPg=<%=authPg%>");
				$("#searchWorkCd").val("A");
				$("#useDiv").show();
			} else if (iframeIdx == 2) {
				newIframe.attr("src","<%=jspPath%>/yeaMigUpload/yeaMigIncomeDdctTab.jsp?authPg=<%=authPg%>");
			} else if (iframeIdx == 3) {
				newIframe.attr("src","<%=jspPath%>/yeaMigUpload/yeaMigTaxReductDdctTab.jsp?authPg=<%=authPg%>");
			}
		}
		setIframeHeight();
	});
	...
	function allTabSearch(){
		newIframe[0].contentWindow.setWorkYy();
