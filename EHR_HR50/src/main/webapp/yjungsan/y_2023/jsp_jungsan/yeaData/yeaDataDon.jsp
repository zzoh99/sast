...
<script type="text/javascript">
	...
	$(function() {
		...
		initdata1.Cols = [
			...
			{Header:"기부금종류|기부금종류",				Type:"Combo",		Hidden:0,	Width:140,	Align:"Left",	ColMerge:1,	SaveName:"contribution_cd",	KeyField:1,	Format:"",			PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:10 },
			{Header:"노조비구분|노조비구분",				Type:"Combo",		Hidden:0,	Width:180,	Align:"Left",	ColMerge:1,	SaveName:"nojo_gubun",	KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:10 },
...
<%-- 			{Header:"금액자료|담당자용",					Type:"AutoSum",			Hidden:0,	Width:80,	Align:"Right",	ColMerge:1,	SaveName:"appl_mon",		KeyField:1,	Format:"",			PointCount:0,	UpdateEdit:<%=applEdit%>,	InsertEdit:<%=applEdit%>,	EditLen:35 }, --%>
		...
		var nojoGubunList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList","YEA995"), "");
		...
		sheet1.SetColProperty("nojo_gubun",	    {ComboText:"|"+nojoGubunList[0], ComboCode:"|"+nojoGubunList[1]} );
...
<%--         sheet1.SetRangeBackColor(0,0,1, sheet1.LastCol(),"<%=headerColorA%>") ; --%>
	...
	function doAction1(sAction) {
		...
		case "Save":
			...
			/* 노조비 구분 중복 등록 가능. 자동생성된 건만 SEQ 1~4까지 선점. 직접입력은 5이후부터 순서대로 채번 20231215 
			//2023.12.14 노조비 구분 중복체크 로직 추가
			var duprows = sheet1.ColValueDup("nojo_gubun",{
				"IncludeDelRow" : 0,
				"IncludeEmpty" : 0
				});
			if(duprows != -1){
				alert("'노조비 구분'이 중복되었습니다. 확인해주세요.") ;
				return ;
			}*/
			...
			for( var i = 2; i < sheet1.LastRow()+2; i++) {
				...
				if( sheet1.GetCellValue(i, "sStatus") == "U" ||
					...
					if( sheet1.GetCellValue(i, "contribution_cd") == "20" || sheet1.GetCellValue(i, "contribution_cd") == "42" || sheet1.GetCellValue(i, "contribution_cd") == "43" ) {
						if( sheet1.GetCellValue(i, "fam_nm") != $("#searchRegNo", parent.document).val() ) {
							alert("정치자금이나 고향사랑기부금, 우리사주조합 기부금은 본인만 해당됩니다.") ;
							return ;
						}
					}

					//정치자금, 고향사랑기부금을 제외한 나머지는 사업자 등록번호화 상호가 필수사항이다.
					if( ! (sheet1.GetCellValue(i, "contribution_cd") == "20" || sheet1.GetCellValue(i, "contribution_cd") == "43") ) {
		...
		case "Copy":
			...
			if ( orgAuthPg=="A" ) { //담당자용
				...
				sheet1.SetCellValue( newRow, "input_mon", "" ); // 20231115 금액자료(직원용)은 초기화
			} else {
		...
		try {
			...
			if (Code == 1) {
				for(var i = sheet1.HeaderRows(); i < sheet1.LastRow(); i++){
					...
					//우리사주조합, 정치자금기부금, 고향사랑기부금은 기부장려금 신청금액 불가능 by JSG 20161202//
					if( sheet1.GetCellValue(i, "contribution_cd") == "20" || sheet1.GetCellValue(i, "contribution_cd") == "42" || sheet1.GetCellValue(i, "contribution_cd") == "43" ) {
						sheet1.SetCellEditable(i, "contribution_sup_mon", 0) ;
					} else if(sheet1.GetCellValue(i, "contribution_cd") == "40") {
						sheet1.SetCellEditable(i, "nojo_gubun", 1) ;
					} else {
						sheet1.SetCellEditable(i, "contribution_sup_mon", 1) ;
						sheet1.SetCellEditable(i, "nojo_gubun", 0) ;
					}
					/*****************************************************************/
		...
		try{
			if(sheet1.ColSaveName(Col) == "sDelete" ) {
				...
				if( sheet1.GetCellValue(Row, "adj_input_type") == "07" ) {
					...
					sheet1.SetCellEditable(Row, "nojo_gubun", 0);
	...
	function sheet1_OnChange(Row, Col, Value, OldValue) {
		try{
			...
			//우리사주조합, 정치자금기부금, 고향사랑기부금은 기부장려금 신청금액 불가능 by JSG 20161202
			if( sheet1.ColSaveName(Col) == "contribution_cd" && (sheet1.GetCellValue(Row, "contribution_cd") == "20" || sheet1.GetCellValue(Row, "contribution_cd") == "42" || sheet1.GetCellValue(Row, "contribution_cd") == "43") ) {
				sheet1.SetCellValue(Row, "contribution_sup_mon", "") ;
				sheet1.SetCellEditable(Row, "contribution_sup_mon", 0) ;
			} else if( sheet1.ColSaveName(Col) == "contribution_cd" && !(sheet1.GetCellValue(Row, "contribution_cd") == "20" || sheet1.GetCellValue(Row, "contribution_cd") == "42" || sheet1.GetCellValue(Row, "contribution_cd") == "43") ){
			...
			*/
			...
			//기부금종류이 일반기부금(종교단체외)일때만 노조비구분 활성화 
			if ( sheet1.ColSaveName(Col) == "contribution_cd") {
				if(sheet1.GetCellValue(Row,"contribution_cd")=="40") {
					sheet1.SetCellEditable(Row, "nojo_gubun", 1) ;
				} else {
					sheet1.SetCellValue(Row, "nojo_gubun", "");
					sheet1.SetCellEditable(Row, "nojo_gubun", 0) ;
				}
			}
	...
	function sheetSet(){
		...
		if(comSheet.RowCount() > 0){
			...
			$("#A080_17").val( comSheet.GetCellText(comSheet.FindText("adj_element_cd", "A080_17"),"input_mon"));
		} else {
			$("#A080_03").val("0");
			$("#A080_05").val("0");
			//$("#A080_07").val("0");
...
<div class="wrapper">
		...
		<div class="sheet_title">
			<ul>
				<li class="txt">기부금
					...
					<a href="#layerPopup" class="basic btn-white ico-question" id="InfoPlus"><b>기부금 안내+</b></a>
	            	<a href="#layerPopup" class="basic btn-white ico-question" id="InfoMinus" style="display:none"><b>기부금 안내-</b></a>
					<!-- <a href="javascript:doAction1('DonationFrdPop');" class="basic authR">이월자료</a> -->
					<a href="javascript:void(0);" class="basic btn-white out-line authR" id="buttonPlus"><b>이월자료+</b></a>
					<a href="javascript:void(0);" class="basic btn-white out-line authR" id="buttonMinus" style="display:none"><b>이월자료-</b></a>
				</li>
			</ul>
		</div>
	</div>
	...
	<table border="0" cellpadding="0" cellspacing="0" class="default line outer">
		...
		<tr>
			...
			<th class="center">고향사랑기부금&nbsp;[43]</th>
			<th class="center">특례기부금<sup><small>(舊법정기부금)</small></sup>&nbsp;[10]</th>
			<!--th class="center">50%한도 적용기부금</th-->
			<th class="center">우리사주조합기부금&nbsp;[42]</th>
			<th class="center">일반기부금(종교단체외)<sup><small>(舊지정기부금)</small></sup>&nbsp;[40]</th>
			<th class="center">일반기부금(종교단체)<sup><small>(舊지정기부금)</small></sup>&nbsp;[41]</th>
		</tr>
		<tr>
			<td class="right">
			...
			<td class="right">
				<input id="A080_17" name="A080_17" type="text" class="text w50p right transparent" readonly /> 원
			</td>
			<td class="right">
