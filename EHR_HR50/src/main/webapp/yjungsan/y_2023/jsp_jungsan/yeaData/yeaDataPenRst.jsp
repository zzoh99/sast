...
//연금보험료 자료 조회
public List selectYeaDataPenList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectYeaDataPenList",pm);
...
//50세이상확인 유무 저장
public int saveAgeChk(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		rstCnt  = DBConn.executeUpdate(queryMap, "saveAgeChk", pm);
		saveLog(null, pm, ssnYeaLogYn);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("저장에 실패하였습니다.");
...
// 50세이상 확인 유무 조회
public Map getAgeChk(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
//외국인단일세율적용 조회
public Map getFrgTaxChk(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
//주민번호 조회
public Map getResNo(Map paramMap, String locPath) throws Exception {
...
<%
	String locPath = xmlPath+"/yeaData/yeaDataPen.xml";
	...
	if("selectYeaDataPenList".equals(cmd)) {
		//연금보험료 자료 조회
		...
		try {
			listData = selectYeaDataPenList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
	...
	} else if("saveYeaDataPen".equals(cmd)) {
		//연금보험료 저장.
		...
		try {
			...
			if(saveList != null && saveList.size() > 0) {
				for(int i = 0; i < saveList.size(); i++) {
					...
					String retPenType = (String)mp.get("ret_pen_type");
                    ...
                    String applMonIsa  = (String)mp.get("appl_mon_isa");
					...
					if( "07".equals(adjInputType) && "D".equals(sStatus) ) {
						...
						param = new String[]{"","",sStatus,ssnEnterCd,workYy
								,adjustType,sabun,savingDeductType,retPenType,financeOrgCd,accountNo,applMon
 								,applMonIsa,adjInputType,feedbackType,ssnSabun,regDt,docSeq,docSeqDetail,mthPer};

						rstStr = DBConn.executeProcedure("PKG_CPN_YEA_"+yeaYear+"_SYNC.PENSION_INS",type,param);

						if(rstStr[1] == null || rstStr[1].length() == 0) {
							cnt++;
						} else {
							//2022.11.10 - 시스템 에러 메시지 주석 처리 
							//message = message + "\n\n" + rstStr[1];
						}
					}
				}
		...
		try {
			int cnt = saveAgeChk(mp, locPath, ssnYeaLogYn);
		...
		try {
			mapData = getAgeChk(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
	...
	}else if("getFrgTaxChk".equals(cmd)) {
        // 외국인단일세율적용 조회
        ...
        try {
            mapData = getFrgTaxChk(mp, locPath, ssnYeaLogYn);
        } catch(Exception e) {
            code = "-1";
            message = e.getMessage();
        }
