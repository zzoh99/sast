...
<%!
	...
	private DiskFileItem createFileItem(String filePath, String fileName) throws IOException {
		...
		try (InputStream inputStream = Files.newInputStream(new File(filePath).toPath());
	            ...
	            Log.Debug("[yeaDataPdfUploadRstMSEAT IOException]" + e.getMessage());
// 	            e.printStackTrace();
	        }
...
try {
    ...
    for(int i = 0 ; i < v_fileCnt ; i++){
        ...
        sFile.SettingPathForProperty("C:\\project\\workspace\\MSEAT_HR\\resources\\softcamp.properties"); // 33서버
        ...
        //복호화
        int retDec;
        retDec = sFile.CreateDecryptFileDAC("C:\\softcamp\\04_KeyFile\\keyDAC_SVR0.sc", "SECURITYDOMAIN", fileName, decFileName);
        ...
        if(retDec == 0) {
        	...
        	if(retDec == -36){
        		...
        		System.out.println("[ NOT DRM FILE  : "+ fileName + " ] ");
        	}else{
        		//그 밖의 에러일때 신규 키로 재시도
        		int retDecNew;
        		retDecNew = sFile.CreateDecryptFileDAC("C:\\softcamp\\04_KeyFile\\mseat\\keyDAC_SVR1.sc", "SECURITYDOMAIN", fileName, decFileName);

                if(retDecNew == 0){
                    File f = new File(tmpFilePath, fileName);
                    f.delete();
                    System.out.println("[ DRM FILE  : "+ decFileName + " ] ");
                }else{
                    File f = new File(decFileName);
                    f.delete();
                    System.out.println("[ NOT DRM FILE  : "+ fileName + " ] ");
...
<%
	...
	Log.Error("[yeaDataPdfUploadRstMSEAT]: " + ex.getMessage());
%>
    <script>
