...
%>
    ...
    $(function() {
        ...
        rentTypeList 	 = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList","C00324"), "");
    ...
    function loadSheet2() {
        ...
        initdata2.Cols = [
            ...
            {Header:"계약서상임대차계약기간|개시일",                Type:"Date",    Hidden:0,   Width:90,   Align:"Center", ColMerge:1, SaveName:"con_s_ymd",       KeyField:1, Format:"Ymd",     UpdateEdit:1,   InsertEdit:1,   EditLen:8 },
            {Header:"계약서상임대차계약기간|종료일",                Type:"Date",    Hidden:0,   Width:90,   Align:"Center", ColMerge:1, SaveName:"con_e_ymd",       KeyField:1, Format:"Ymd",     UpdateEdit:1,   InsertEdit:1,   EditLen:8 },
            {Header:"계약기간\n전체\n월세액|계약기간\n전체\n월세액",Type:"Int",     Hidden:0,   Width:80,   Align:"Right",  ColMerge:1, SaveName:"rent_mon",        KeyField:1, Format:"Integer", UpdateEdit:1,   InsertEdit:1,   EditLen:35, MinimumValue: 0 },
            ...
            {Header:"월세대상금액|직원용",                          Type:"AutoSum", Hidden:0,   Width:80,   Align:"Right",  ColMerge:1, SaveName:"input_mon",       KeyField:0, Format:"Integer",        UpdateEdit:<%=inputEdit%>,   InsertEdit:<%=inputEdit%>,   EditLen:35, MinimumValue: 0 },
            {Header:"월세대상금액|담당자용",                        Type:"AutoSum", Hidden:0,   Width:80,   Align:"Right",  ColMerge:1, SaveName:"appl_mon",        KeyField:1, Format:"Integer",        UpdateEdit:<%=applEdit%>,    InsertEdit:<%=applEdit%>,    EditLen:35, MinimumValue: 0 },
...
<%--         sheet2.SetRangeBackColor(0,0,1, sheet2.LastCol(),"<%=headerColorA%>") ; --%>
    ...
    function loadSheet4() {
        ...
        var houseDecCdList = stfConvCode( codeList("<%=jspPath%>/yeaData/yeaDataHouRst.jsp?cmd=selectHouseDecCdList&codeType=1",""), "");
...
<%				}	%>
	...
	function sheetToZipCode(){
		...
		//세대주여부
		$("input[id='house_owner_yn']:input[value='"+sheet5.GetCellValue(sheet5.LastRow(),"house_owner_yn")+"']").attr("checked",true);
    ...
    function rentInputCheck(obj, gubun) {
        ...
        4. 총급여액 : 주택임차차입금(거주자) : 5천만원이하인 자, 월세액 : 7천만원이하인 자
        */
	        ...
	        if(houseOwnerYn != "Y") {	//세대원
	        	...
	        	if(citizenType == "9" || foreignTaxType == "5"){		//외국인
					if(foreignChk != "Y"){
						...
						if(confirm("출입국관리법 또는 재외동포법에 해당하는 외국인이며\n배우자 및 생계를 같이하는 부양가족이 주택자금공제를\n받지 않는 경우만 입력이 가능합니다.\n입력하시겠습니까?")){
							return true;
						}else{
							return false;
						}
...
<div class="wrapper">
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="default outer" style="margin-bottom: 30px;">
		<tr>
			...
			<th>소유주택수</th>
			<td class="left">
				<input type="radio" class="radio disabled" name="house_cnt" id="house_cnt" value = "0" disabled>&nbsp;무주택
				<input type="radio" class="radio disabled" name="house_cnt" id="house_cnt" value = "1" disabled>&nbsp;1주택
				<input type="radio" class="radio disabled" name="house_cnt" id="house_cnt" value = "2" disabled>&nbsp;2주택이상
			</td>
		</tr>
		<tr>
			<th>주택기준시가</th>
   ...
   <table border="0" cellpadding="0" cellspacing="0" class="default line outer">
    ...
    <div class="outer">
        ...
        <ul>
            <li class="txt">
                ※ 월세액 :
                <span>월세액 및 계약기간은 당해년도분이 아니라 전체 계약기간과 총 합계액을 입력하되, 월세액이 실 지급액과 상이한 경우 실 지급액을 입력 하여 주시기 바랍니다.</span>
                ...
                <span id="paytotMonViewYn2">
                    <a href="javascript:paytotMonView(2, 70000000);" class="basic authA btn-red-outline">총급여 7천만원 초과여부</a>
                </span>
