...
//보험료 자료 조회
public List selectYeaDataInsList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
//보험료 자료 조회 (현근무지)
public Map selectYeaDataInsCurMap(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		mapData  = (queryMap == null) ? null : DBConn.executeQueryMap(queryMap,"selectYeaDataInsCurMap",pm);
		saveLog(null, pm, ssnYeaLogYn);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
//보험료 자료 조회 (종전근무지)
public Map selectYeaDataInsBefMap(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		mapData  = (queryMap == null) ? null : DBConn.executeQueryMap(queryMap,"selectYeaDataInsBefMap",pm);
		saveLog(null, pm, ssnYeaLogYn);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
//pdf 파일 상세 저장.
public int saveYeaDataPdf(Map paramMap, String orgAuthPg, String locPath, String ssnYeaLogYn) throws Exception {
...
<%
	...
	if("selectYeaDataInsList".equals(cmd)) {
		...
		try {
			listData = selectYeaDataInsList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
	...
	} else if("selectYeaDataInsCurMap".equals(cmd)) {
		//보험료 자료 조회
		...
		try {
			mapData = selectYeaDataInsCurMap(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
		...
		Map rstMap = new HashMap();
		rstMap.put("Result", mapCode);
		rstMap.put("Data", mapData);
		out.print((new org.json.JSONObject(rstMap)).toString());

	} else if("selectYeaDataInsBefMap".equals(cmd)) {
		//보험료 자료 조회
		...
		try {
			mapData = selectYeaDataInsBefMap(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
			...
			if(saveList != null && saveList.size() > 0) {
				for(int i = 0; i < saveList.size(); i++) {
					...
					if( "07".equals(adjInputType) && "D".equals(sStatus) ) {
						...
						int tempCnt = saveYeaDataPdf(mp, orgAuthPg, locPath, ssnYeaLogYn);
						cnt += tempCnt ;
					...
					//일반 데이터 (기존 저장 로직)
						...
						param = new String[]{"","",sStatus,ssnEnterCd,workYy
								...
								,famresInsured,applMon,adjInputType,ntsYn,feedbackType,ssnSabun,docSeq,docSeqDetail};

						rstStr = DBConn.executeProcedure("PKG_CPN_YEA_"+yeaYear+"_SYNC.INSURANCE_INS",type,param);
