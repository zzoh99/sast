...
<%!
	...
	private DiskFileItem createFileItem(String filePath, String fileName) throws IOException {
		...
		try (InputStream inputStream = Files.newInputStream(new File(filePath).toPath());
	            ...
	            Log.Debug("[yeaDataPdfUploadRstKOH IOException]" + e.getMessage());
// 	            e.printStackTrace();
	        }
...
<%
	...
	String tmpFilePath = StringUtil.getPropertiesValue("WAS.PATH")+StringUtil.getPropertiesValue("HRFILE.PATH")+"/tmp";
...
try {
    ...
    */
    ...
    for(int i = 0 ; i < v_fileCnt ; i++){
        FileItem fileItem = (FileItem) fileItemList.get(i);
        FileItem fileItem2 = (FileItem) fileItemList.get(i);
		...
		fileItem2.write(new File(tmpFilePath, fileName));

		//String decFileName = fileName.substring(0, fileName.lastIndexOf(".")) +"_Dec"+ fileName.substring(fileName.lastIndexOf("."), fileName.length());
		String pdfBytesEnc = "";
		...
		bret = oWorkPackager.GetFileType(tmpFilePath+File.separator+fileName);		// 파일 타입 체크
		//bret = oWorkPackager.GetFileType(fileName);		// 파일 타입 체크
		...
		if(bret == 103){
			...
			chk = oWorkPackager.IsSupportFile("E:\\app\\OPTI_HR\\common\\plugin\\fsdinit", "0100000000001020", tmpFilePath+File.separator+fileName);

			if(chk){
				nRet = oWorkPackager.DoExtract("E:\\app\\OPTI_HR\\common\\plugin\\fsdinit","0100000000001020",tmpFilePath+File.separator+fileName, tmpFilePath+File.separator+decFileName);

				if(!nRet) {
					out.println("복호화 실패!! ["+ oWorkPackager.getLastErrorNum()+"] "+oWorkPackager.getLastErrorStr());
					...
					if(f1.exists()){
						if(f1.delete()){
							...
							Log.Error("[yeaDataPdfUploadRstKOH] 파일 삭제 중 에러");
						}
					}
					...
					out.println("복호화 성공!!" + oWorkPackager.getContainerFilePathName());
					//ibExcel.setFileName(decFileName);
					//File f = new File(decFileName);
					//f.delete();
        	...
        	if ( !decFileName.toUpperCase().endsWith(".PDF") ) {
            ...
            // 파일내용을 읽음
            pdfBytes = new byte[decFileLength];
            while((readBuffer = input.read(pdfBytes)) != -1) {
            	System.out.write(pdfBytes, 0, readBuffer);
            }

    		pdfBytesEnc = new InputStreamReader(input).getEncoding().toLowerCase();
            ...
            pdfBytesEnc = new InputStreamReader(fileItem.getInputStream()).getEncoding().toLowerCase();
            //Log.Debug("pdf 파일인코딩 ==== "+pdfBytesEnc);
		}
...
<%
		...
		try{
			//쿼리 실행및 결과 받기.
			mpCnt  = DBConn.executeQueryMap(queryMap,"selectPdfFileCnt",pm);
		} catch (Exception e) {
            ...
            try{
                ...
                if(!fileItem.isFormField()) {
                    ...
                    if(fileSize > 0) {
                    	...
                    	if(bret == 103){
                    		attFileNm = tmpFilePath+File.separator+decFileName;
                    	}else{
                    		attFileNm = fileItem.getName().trim();
                    	}
        ...
        try{
    		//쿼리 실행및 결과 받기.
    		perList  = DBConn.executeQueryList(queryMap,"selectPdfPerList",pm);
...
<%
	...
	Log.Error("[yeaDataPdfUploadRstKOH]: " + ex.getMessage());
%>
    <script>
