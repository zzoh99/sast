...
try {
    ...
    if("saveNmTxt".equals(cmd)) {
	    ...
	    for(int i = 0 ; i < v_fileCnt ; i++){
			...
			try{
				//쿼리 실행및 결과 받기.
				finmCnt  = DBConn.executeQueryMap(queryMap,"selectFileNmChkCnt",pm);
			} catch (Exception e) {
				Log.Error("[Exception] " + e);
				throw new Exception("조회에 실패하였습니다.");
			}
	        ...
	        try{
	            ...
	            if(fileUploadType.equals("1")){
	            	...
	            	if("1".equals(searchFileType)) {
						...
						infoMap.put("filePath", saveFilePath); 			//파일경로정보
					} else {
						saveFilePath = "/YEA_ADDFILE/"+ssnEnterCd+"/"+searchWorkYy;
					}
	            	infoMap.put("filePath", saveFilePath); 			//파일경로정보
					saveFileToS3(saveFilePath, ssnSabun, fileName, fileItem.getInputStream(), tempFileNm);
	            } else {
	            	//디렉토리 생성
...
//교육비 자료 조회
public int saveNmTxt(Map paramMap, String locPath) throws Exception {
