...
%>
		...
		initdata3.Cols = [
			...
			{Header:"등록일시",			Type:"Text",		Hidden:0,	Width:70,	Align:"Center",	ColMerge:0,	SaveName:"chkdate",			KeyField:0,	Format:"YmdHms",PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100},
			{Header:"반영여부",			Type:"Text",		Hidden:0,	Width:40,	Align:"Center",	ColMerge:0,	SaveName:"fileapply",		KeyField:0,	Format:"",      PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100},
			...
			{Header:"파일삭제 (인원별 최종 PDF는 삭제불가)",			Type:"Html",		Hidden:0,	Width:130,	Align:"Center",	ColMerge:0,	SaveName:"delfile_link",	KeyField:0,	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100}
	...
	function doAction(sAction) {
		...
		case "Search":
			...
			if(data.Result.Code == 1) {
				...
				$("#btnLongSav > .pdfAmt").text( comma(parseInt(map.long_sav_amt)+parseInt(map.venture_amt)) );			$("#btnLongSav > .pdfCnt").text("반영("+ (parseInt(map.long_sav_cnt,10) + parseInt(map.venture_cnt,10)) +"), 미반영("+ (parseInt(map.long_sav_cnt_err,10) + parseInt(map.venture_cnt_err,10)) +")");
	...
	function goAction(){
		...
		var vsfileUploadType = $("#fileUploadType").val();

		$("#paramSabun").val(vsSearchSabun);
		$("#paramAdjustType").val(vsSearchAdjustType);
		$("#fileUploadType").val(vsfileUploadType);
		...
		/*}else if(vsSsnEnterCd == "MSEAT"){
            ...
            */
			...
			$("#uploadForm").attr({"method":"POST","target":"ifrmPdfUpload","action":"yeaDataPdfUploadRst_IE.jsp"}).submit();
...
<div class="wrapper" id="topNav">
	<form id="uploadForm" method="post"  enctype="multipart/form-data">
		...
		<input type="hidden" id="paramSabun" name="paramSabun" value="<%=session.getAttribute("ssnSabun")%>"/>
		<input type="hidden" id="paramAdjustType" name="paramAdjustType" value="1"/>
		<input type="hidden" id="fileUploadType" name="fileUploadType" value="0"/>
		<table border="0" cellpadding="0" cellspacing="0" class="default outer" id="pdfUpload" style="display:none">
		    ...
		    <tr>
		        ...
		        <td class="center"><a href="javascript:goAction();" class="basic btn-upload" title="전자문서 제출">업로드</a></td>
		    </tr>
	    </table>
		<!-- PDF 등록 파일업로드 End -->
	</form>
	...
	<div class="outer new" id="infoLayer" style="display:none">
		<table>
			<tr>
				<td>
					<p class="red">* 국세청 연말정산간소화서비스 사이트에서 <b>아래의 이미지에 해당하는 PDF를 다운로드 하셔서 등록</b>하시기 바랍니다.<br>
					* PDF 자료를 국세청에서 다운받을 때 <b>비밀번호를 설정</b>하였을 경우, <b>해당 비밀번호를 입력</b>하셔야 합니다.
					</p>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td>
					<p>* 찾아보기 버튼을 클릭하여 PDF파일을 선택하신 후, 업로드 버튼을 클릭하여 주시기 바랍니다.</p>
					<ul>
						<li class="red"><strong>PDF 파일 업로드 시 이전 자료 모두 삭제(별도 수기 입력 제외) 후 업로드 파일 내용으로 최종 등록되오니 참고 바랍니다.</strong></li>
						<li><span class="under-line">본인 + 배우자 PDF 자료 업로드 후 본인 PDF 자료만 재 업로드 시 본인 + 배우자 데이터 삭제되고 마지막으로 업로드 파일을 최종 자료로 등록되오니 이점 유의 바랍니다.</span></li>
						<li><span class="under-line">본인의 모든 PDF자료(보험료/신용카드 등) 업로드 후 본인 보험료 PDF 자료만 다시 받아 재 업로드 시 본인 모든 기존자료 삭제 후 보험료 데이터만 업로드 되오니 이점 유의 바랍니다.</span></li>
					</ul>
				</td>
			</tr>
		</table>
	...
	<div class="outer" id="pdfFileList">
		<div class="sheet_title">
			<ul>
	            ...
	            <li class="btn pd-left4"><a href="javascript:doAction3('Down2Excel')" class="basic btn-download">다운로드</a></li>
	        </ul>
			<ul>
	            <li class="btn">
					<!-- <a href="javascript:openPdfUploadPopup();" class="blue authA">PDF 등록 원본</a> -->
					<font class="txt red"><strong>※ XREF 정보검색 오류, ByteRange 오류 발생시 국세청 PDF 원본 파일로 다운받아 재업로드 해주시기 바랍니다.</strong>&nbsp;</font>
					<span id="btn_1">
						<a href="javascript:void(0);" class="basic btn-white out-line authA" id="buttonPlus"><b>PDF 등록+</b></a>
			            <a href="javascript:void(0);" class="basic btn-white authA" id="buttonMinus" style="display:none"><b>PDF 등록-</b></a>
		            </span>
		            <a href="#layerPopup" class="basic btn-white ico-question authR" id="buttonInfoPlus"><b>PDF 등록안내+</b></a>
		            <a href="#layerPopup" class="basic btn-white ico-question authR" id="buttonInfoMinus" style="display:none"><b>PDF 등록안내-</b></a>
	            </li>
	        </ul>
        </div>
	...
	<div class="outer">
		<table class="table-pdf">
			...
			<tr>
				...
				<td>
					<a href="javascript:doAction2('Search', 'B101Y,B201Y');" class="pdf" id="btnMed">
				...
				<td>
					<a href="javascript:doAction2('Search', 'C102Y,C202Y,C301Y,C401Y,C501Y');" class="pdf" id="btnEdu">
				...
				<td>
					<a href="javascript:doAction2('Search', 'G111Y');" class="pdf" id="btnCreditCard">
				...
				<td>
					<a href="javascript:doAction2('Search', 'G311Y,G411Y');" class="pdf" id="btnCheckCard">
						<span class="pdfItem">직불카드 등</span>
						<span class="pdfAmt"></span>
						<span class="pdfCnt"></span>
					</a>
				</td>
				<td>
					<a href="javascript:doAction2('Search', 'G211M');" class="pdf" id="btnCash">
			...
			<tr>
				<td>
					<a href="javascript:doAction2('Search', 'D101Y,E103Y,E101Y,F103Y');" class="pdf" id="btnPen">
						<span class="pdfItem">개인연금저축/<br/>연금계좌</span>
						<span class="pdfAmt"></span>
						<span class="pdfCnt"></span>
					</a>
				</td>
				<td>
					<a href="javascript:doAction2('Search', 'J101Y,J203Y,J401Y,J501Y');" class="pdf" id="btnHou">
						<span class="pdfItem">주택자금/월세액</span>
						<span class="pdfAmt"></span>
						<span class="pdfCnt"></span>
					</a>
				</td>
				<td>
					<a href="javascript:doAction2('Search', 'J301Y');" class="pdf" id="btnHouSav">
						<span class="pdfItem">주택마련저축</span>
						<span class="pdfAmt"></span>
						<span class="pdfCnt"></span>
					</a>
				</td>
				<td>
					<a href="javascript:doAction2('Search', 'N102Y,Q101Y,Q201Y,Q301Y');" class="pdf" id="btnLongSav">
		...
		<div class="sheet_title under-line" style="margin-top:0px; padding-top:0px;">
		<ul>
			<li class="txt" style="font-weight:normal;">
				...
				<select id="formCd">
					...
					<option value="G111Y">신용카드</option>
					<option value="G311Y,G411Y">직불카드 등</option>
					<option value="G211M">현금영수증</option>
			...
			<li class="btn">
				<font class="red"><b><span id="spanMsg"></span></b></font>
