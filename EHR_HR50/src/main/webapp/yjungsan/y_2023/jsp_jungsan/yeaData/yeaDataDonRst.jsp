...
//기부금 자료 조회
public List selectYeaDataDonList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
<%
	...
	if("selectYeaDataDonList".equals(cmd)) {
		...
		try {
			listData = selectYeaDataDonList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			...
			if(saveList != null && saveList.size() > 0) {
				for(int i = 0; i < saveList.size(); i++) {
					...
					String contribution_sup_mon = (String)mp.get("contribution_sup_mon");
					String sum_mon = (String)mp.get("sum_mon");
					String docSeq = (String)mp.get("doc_seq");
					String docSeqDetail = (String)mp.get("doc_seq_detail");
					String donationType = (String)mp.get("donation_type");
					String nojoGubun = (String)mp.get("nojo_gubun");
					...
					if( "07".equals(adjInputType) && "D".equals(sStatus) ) {
						...
						param = new String[]{"","",sStatus,ssnEnterCd,workYy
								...
								,ntsYn,feedbackType,ssnSabun,contribution_sup_mon,sum_mon,docSeq,docSeqDetail,donationType,nojoGubun};

						rstStr = DBConn.executeProcedure("PKG_CPN_YEA_"+yeaYear+"_SYNC.DONATION_INS",type,param);
