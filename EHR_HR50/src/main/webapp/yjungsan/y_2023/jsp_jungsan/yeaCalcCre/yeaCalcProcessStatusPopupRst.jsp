...
//작업현황조회
public Map selectYeaCalcProcessStatusPopupInfo(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
public List selectYeaCalcProcessStatusPopupDetail(String queryId, Map paramMap, String locPath) throws Exception {
	...
	Log.Debug(" [ 1차 length ] ==============       "+searchBusinessCd.trim().length());
	if(searchBusinessCd.trim().length() != 0){

		Log.Debug(" [ 2차 length ] ==============       "+searchBusinessCd.trim().length());
...
<%
	...
	if("selectYeaCalcProcessStatusPopupInfo".equals(cmd)) {
		...
		try {
			mapData = selectYeaCalcProcessStatusPopupInfo(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
