...
//담당자 FeedBack 검색
public List selectYeaFeedbackLstList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
//담당자 FeedBack  저장
public int saveYeaFeedback(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	if(list != null && list.size() > 0 && conn != null) {
		...
		try{
			for(int i = 0; i < list.size(); i++ ) {
				...
				if("I".equals(sStatus)) {
					//입력
					rstCnt += DBConn.executeUpdate(conn, queryMap, "mergeYeaFeedback", mp);
...
<%
	...
	if("selectYeaFeedbackLstList".equals(cmd)) {
		...
		try {
			listData = selectYeaFeedbackLstList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
	...
	} else if("saveYeaFeedback".equals(cmd)) {
		//특이사항 팝업 저장
		...
		try {
			...
			int cnt = saveYeaFeedback(mp, locPath, ssnYeaLogYn);
