...
<script type="text/javascript">
	...
	$(function() {
		...
		initdata.Cols = [
	        ...
	        {Header:"노조비구분",			Type:"Combo",     Hidden:0,  Width:180,	Align:"Left",    ColMerge:1,   SaveName:"nojo_gubun",	KeyField:0,   CalcLogic:"",   Format:"",     	PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:10 },
	        {Header:"선택",		Type:"CheckBox",  Hidden:0,  Width:50,	Align:"Center",  ColMerge:1,   SaveName:"jeokyong_yn",			KeyField:0,   CalcLogic:"",   Format:"",     	PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:1, TrueValue:"Y", FalseValue:"N" },
	        ...
	        {Header:"기부장려금신청금액",	    Type:"AutoSum",       Hidden:0,  Width:90,	Align:"Right",   ColMerge:1,   SaveName:"contribution_sup_mon",			KeyField:0,   CalcLogic:"",   Format:"",     	PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:35 },
		...
		//기부금종류
        var contributionCdList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&useYn=Y","C00307"), "전체");
        //노조비구분
		var nojoGubunList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList","YEA995"), "전체");
        ...
        $("#searchNojoGubun").html(nojoGubunList[2]);
        
        //일괄변경노조비구분
		var creNojoGubunList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList","YEA995"), "");
        $("#creNojoGubun").html(creNojoGubunList[2]);
	...
	function doAction1(sAction) {
		...
		case "Down2Template":
			...
			var param  = {DownCols:"work_yy|adjust_type|sabun|famres|enter_no|firm_nm|contribution_cd|nojo_gubun|contribution_sup_mon|donation_type|sum_mon|appl_mon|adj_input_type"
					,SheetDesign:1,Merge:1,DownRows:'0',ExcelFontSize:"9"
				,TitleText:templeteTitle1,UserMerge :"0,0,1,13",menuNm:$(document).find("title").text()};
			sheet1.Down2Excel(param);
			break;
        case "LoadExcel":
	...
	function sheet1_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			for(var i = sheet1.HeaderRows(); i <= sheet1.LastRow(); i++){
				if(sheet1.GetCellValue(i,"contribution_cd")=="40") {
					sheet1.SetCellEditable(i, "nojo_gubun", 1) ;
					sheet1.SetCellEditable(i, "jeokyong_yn", 1) ;
				} else {
					sheet1.SetCellEditable(i, "nojo_gubun", 0) ;
					sheet1.SetCellValue(i, "nojo_gubun", "");
					sheet1.SetCellEditable(i, "jeokyong_yn", 0) ;
					sheet1.SetCellValue(i, "jeokyong_yn", "");
		...
		try{
			...
			if ( sheet1.ColSaveName(Col) == "contribution_cd") {
				if(sheet1.GetCellValue(Row,"contribution_cd")=="40") {
					...
					sheet1.SetCellEditable(Row, "jeokyong_yn", 1) ;
				} else {
					sheet1.SetCellValue(Row, "nojo_gubun", "");
					sheet1.SetCellEditable(Row, "nojo_gubun", 0) ;
					sheet1.SetCellValue(Row, "jeokyong_yn", "");
					sheet1.SetCellEditable(Row, "jeokyong_yn", 0) ;
				}
			}
		} catch(ex) {
	...
	function nojoGubunChg() {

		for(var i = sheet1.HeaderRows(); i <= sheet1.LastRow(); i++){
			if(sheet1.GetCellValue(i,"jeokyong_yn")=="Y") {
				sheet1.SetCellValue(i, "nojo_gubun", $("#creNojoGubun").val());
			}
		}
	}
</script>
...
<div class="wrapper">
    ...
    <div class="sheet_search outer">
        ...
        <table>
			...
			<tr>
                ...
                <td><span>노조비구분</span>
                    <select id="searchNojoGubun" name ="searchNojoGubun" onChange="javascript:doAction1('Search')" class="box"></select>
                </td>
				<td><span>사번/성명</span>
    ...
    <div class="outer">
        ...
        <ul>
            ...
            <li class="btn">
	            <select id="creNojoGubun" name ="creNojoGubun" class="box"></select>
				<a href="javascript:nojoGubunChg()" class="basic btn-green authA">일괄적용</a>
