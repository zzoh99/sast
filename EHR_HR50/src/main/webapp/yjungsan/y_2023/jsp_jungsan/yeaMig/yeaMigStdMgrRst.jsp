...
//이관내역 조회
public List selectMigMgrList(Map paramMap, String locPath) throws Exception {
	//파라메터 복사.
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = DBConn.executeQueryList(queryMap,"selectMigMgrList",pm);
		saveLog(null, pm);
	} catch (Exception e) {
...
//감면기간 저장
public int saveMigMgr(Map paramMap, String locPath) throws Exception {
...
<%
	...
	if("selectMigMgrList".equals(cmd)){
		...
		try {
			listData = selectMigMgrList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
