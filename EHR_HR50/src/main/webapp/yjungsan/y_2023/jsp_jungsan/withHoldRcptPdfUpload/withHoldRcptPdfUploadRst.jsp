...
//증빙자료 목록 조회
public List selectWithHoldRcptPdfUploadList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	if(searchAdjustType.trim().length() != 0){
		query.append("AND A.ADJUST_TYPE = #searchAdjustType#");
	}
...
public int deleteWithHoldRcptData(Map paramMap, String locPath) throws Exception {
    ...
    if (conn != null) {
	    try {
	    	...
	    	if(paramMap.get("fileUploadType").equals("1")){
	    		rstCnt = DBConn.executeUpdate(conn, queryMap, "deleteTsys201", paramMap);
	    	}
...
<%
	...
	if("selectWithHoldRcptPdfUploadList".equals(cmd)) {
		...
		try {
			listData = selectWithHoldRcptPdfUploadList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
	    ...
	    String fileUploadType       = request.getParameter("fileUploadType") != null ? request.getParameter("fileUploadType").toString() : "";
			    ...
			    if(fileUploadType.equals("1")){
			    	...
			    	try {
						...
						if (responseEntity.getStatusCodeValue() == 200) {
							...
							// 파일 데이터 삭제
							if(deleteWithHoldRcptData(param, locPath) != 1) {
								flag = false;
							}
						} else {
							// 에러 응답 처리
