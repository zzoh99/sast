...
public void download(HttpServletRequest request, HttpServletResponse response, String filePath, String fileName, boolean isDel) throws Exception {
    ...
    try {
    	...
    	inputStream = new BufferedInputStream(new FileInputStream(downfile));
		...
		if(isDel) {
			if(downfile.exists()){
				if(downfile.delete()){
					...
					Log.Error("[withHoldRcptFileDownlad] 파일 삭제 중 에러");
				}
			}
		}
	}
	...
	try {
		...
		if(jsonArr == null || jsonArr.length() == 0) {
			...
			if(fileUploadType.equals("1")){
            	...
            	downloadToS3(filePath, fileName, sfileName, response);
            } else {
            	download(request, response, filePath+"/"+fileName, fileName, false);
            }
		} else {
			//여러개 다운시에는 압축해서 다운
			...
			if(fileUploadType.equals("1")){
				...
				for(int i=0; i<jsonArr.length(); i++) {
					...
					rfileName = jsonObj.getString("fileName");
					...
					zipFiledownloadToS3(filePath, rfileName, sfileName, response, zipOutputStream, zipName);
		...
		Log.Error("[withHoldRcptFileDownload]:" + e.getMessage());
	}
