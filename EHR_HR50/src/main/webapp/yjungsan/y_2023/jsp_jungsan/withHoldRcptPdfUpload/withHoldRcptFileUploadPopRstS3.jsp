...
try {
  ...
  //xml 파서를 이용한 방법;
    ...
    try{
		//쿼리 실행및 결과 받기.
		systemInfo  = DBConn.executeQueryMap(queryMap,"selectSystemInfo",param);
	} catch (Exception e) {
  ...
  //폼필드 부분만 먼저 가져온다.
    for(int i = 0 ; i < v_fileCnt ; i++){
        ...
        if ( fileItem.isFormField() ) {
            if("fileUploadType".equals(fileItem.getFieldName())) {
    ...
    for(int i = 0 ; i < v_fileCnt ; i++){
		...
		System.out.println("##################################111111111");
	    System.out.println(ssnSabun);

		//사번 체크
		Map sabunCnt = null;
		try{
			//쿼리 실행및 결과 받기.
			sabunCnt  = DBConn.executeQueryMap(queryMap,"selectSabunCnt",pm);
		} catch (Exception e) {
			Log.Error("[Exception] " + e);
		...
		try{
			//쿼리 실행및 결과 받기.
			mpCnt  = DBConn.executeQueryMap(queryMap,"selectFileCnt",pm);
		} catch (Exception e) {
			Log.Error("[Exception] " + e);
			throw new Exception("조회에 실패하였습니다.");
		}
		...
		if(Integer.valueOf(pdfCnt) > 0) {
			...
			try{
				//쿼리 실행및 결과 받기.
				mpInfo  = DBConn.executeQueryMap(queryMap,"selectFileInfo",pm);
			} catch (Exception e) {
				Log.Error("[Exception] " + e);
				throw new Exception("조회에 실패하였습니다.");
			}
        ...
        try{
            ...
            infoMap.put("attr1", tempFileNm); 				//ATTR1
            infoMap.put("fileSize", String.valueOf(fileSize)); 	//filseSize
            infoMap.put("attr2", ""); 						//ATTR2
            infoMap.put("attr3", ""); 						//ATTR3
            infoMap.put("attr4", ""); 						//ATTR4

          //2023.12.19 : s3 or 기본 분기처리 로직 추가 
            if(fileUploadType.equals("1")){
            	String fileSeq = DBConn.executeQueryMap(conn, queryMap, "getFileSeq", infoMap).get("seq").toString();
				infoMap.put("fileSeq", fileSeq); 						//ATTR4
				if("1".equals(fileType)) {
            		dbfilePath = "/YEA_INCOME/"+ssnEnterCd+"/"+searchWorkYy;
					saveFilePath = "/YEA_INCOME/"+ssnEnterCd+"/"+searchWorkYy;
				} else {
					dbfilePath = "/YEA_ADDFILE/"+ssnEnterCd+"/"+searchWorkYy;
					saveFilePath = "/YEA_ADDFILE/"+ssnEnterCd+"/"+searchWorkYy;
				}
				infoMap.put("filePath", dbfilePath); 			//파일경로정보
	            infoMap.put("fileName", saveFileNm); 			//파일명
				...
				if(Integer.valueOf(pdfCnt) > 0) {
					...
					rstCnt = DBConn.executeUpdate(conn, queryMap, "insertFileInfo2", infoMap);
					rstCnt = DBConn.executeUpdate(conn, queryMap, "withHoldTsys200save", infoMap);
					rstCnt = DBConn.executeUpdate(conn, queryMap, "withHoldTsys201save", infoMap);
				}
				
				saveFileToS3(saveFilePath, ssnSabun, fileName, fileItem.getInputStream());
            } else {
            	if(Integer.valueOf(pdfCnt) > 0) {
                	//파일이 존재하면 일자 업데이트 후 파일 처리
                	rstCnt = DBConn.executeUpdate(conn, queryMap, "updateFileInfo", infoMap);
