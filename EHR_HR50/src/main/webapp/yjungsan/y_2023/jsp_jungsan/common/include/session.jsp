...
<%
	...
	if("1".equals(chkSysVersion)) {
		//if(session.getAttribute("ssnEnterCd") == null || session.getAttribute("ssnEnterCd").toString().length() == 0) {
			...
			session.setAttribute("ssnMailId", session.getAttribute("SESSION_MAIL_ID"));
			session.setAttribute("ssnOfficeTel", session.getAttribute("SESSION_OFFICE_TEL"));
			...
			//GS만 권한이 조금 다름.
			if("GSS".equals(session.getAttribute("SESSION_ENTER_CD"))) {
				if(!"10".equals(session.getAttribute("SESSION_AUTHORITY_GROUP"))&&!"34".equals(session.getAttribute("SESSION_AUTHORITY_GROUP"))&&!"37".equals(session.getAttribute("SESSION_AUTHORITY_GROUP"))){
					session.setAttribute("ssnSearchType", "P");
				}
			}
		//}
	}
...
<%@ include file="../../../../common_jungsan/jsp/security/securityMgr2.jsp" %>
<%
	String authPg = "";

	//권한관련 셋팅
	if( sUrlAuthPg != null && !"".equals(sUrlAuthPg) ) {
		//메뉴에서 surl이 적용되거나 보안체크를 할 경우
		authPg = sUrlAuthPg;

	} else if(request.getParameter("authPg") != null && !"".equals((String)request.getParameter("authPg")) ) {
		authPg = request.getParameter("authPg");

	} else if(cloudHr != null && "Y".equals(cloudHr) && request.getAttribute("authPg") != null && !"".equals((String)request.getAttribute("authPg")) )  {
		// cloud hr 경우 authPg 가 메뉴에 없고 Filter에서 attribute 로 추가됨.
		authPg = (String)request.getAttribute("authPg");
...
<%@ include file="../../../../common_jungsan/jsp/sessionProc/sessionProc.jsp" %>