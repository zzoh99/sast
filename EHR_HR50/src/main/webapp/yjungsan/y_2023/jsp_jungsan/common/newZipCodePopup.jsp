...
<%@ include file="include/meta.jsp"%><!-- Meta -->
<%@ include file="include/jqueryScript.jsp"%>


<%
	request.setCharacterEncoding("utf-8");  // 한글깨지면 주석제거
	/* 20210112 개발용 발급*/
	//String confmKey = ""; // 검색API 승인키
	...
	if(secure || requestUrl.indexOf("https://") > -1 || (cloudHr != null && "Y".equals(cloudHr))){

		domain = "https://www.juso.go.kr"; // 인터넷망

	}else{

		domain = "http://www.juso.go.kr"; // 인터넷망
...
<link rel="stylesheet" type="text/css" href="../../../common_jungsan/css/addrlink.css"/>
<script src="include/addrlink.js" type="text/javascript" charset="utf-8"></script>
...
var p = eval("<%=popUpStatus%>");
	...
	function searchUrlJuso(){
		...
		$.ajax({
			...
			,type:"post"
			,data:decodeURI( $("#AKCFrm").serialize() )
			,dataType:"jsonp"
			,crossDomain:true
	...
	function setParent(){
		...
		if(p.window.opener == null || p.window.opener == undefined) {
			alert("리턴경로를 확인해주세요.");
		}

	 	var doroFullAddr 		= rtAddrPart1+" "+rtAddrPart2+" "+ rtAddrDetail;
	 	var resDoroFullAddr 	= rtAddrPart1+" "+rtAddrPart2+" "+rtAddrDetail;
	 	var doroFullAddrEng 	= rtEngAddr;
	 	var resDoroFullAddrEng  = rtEngAddr;

		var textString = "\"zip\" : \""+rtZipNo+"\", ";
			textString += "\"doroFullAddr\" : \""+doroFullAddr+"\", ";
			textString += "\"resDoroFullAddr\" : \""+resDoroFullAddr+"\", ";
			textString += "\"doroFullAddrEng\" : \""+doroFullAddrEng+"\", ";
			textString += "\"resDoroFullAddrEng\" : \""+resDoroFullAddrEng+"\" ";
		p.window.opener.getReturnValue(textString);
		p.self.close();
	...
	function init(){
		var browerName = navigator.appName; //사용안함
		var browerAgent = navigator.userAgent; //사용안함

		var data = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=ZIPCODE_LICENSE", "queryId=getSystemStdData",false).codeList;
        if ( data != null && data.length>0) {

        	$("#confmKey").val($.trim(data[0].code_nm));
		}
		//var license = convCode( ajaxCall("${ctx}/CommonCode.do?cmd=getCommonNSCodeList&searchStdCd=ZIPCODE_LICENSE", "queryId=getSystemStdData",false).codeList, "");
		//var confmKey = license[0];
		/******* 운영배포시 주석 해제 필요 */
...
<body onload="init();" class ="visualSection">
	...
	<form name="AKCFrm" id="AKCFrm" method="post">
		...
		<!-- 배포시 주석 해제필요  -->
		<input type="hidden" name="confmKey" id="confmKey"  value=""/>
