...
//소득공제 한도차감관리내역(관리자) 조회
public List selectIncomeDedLimitUploadList(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = DBConn.executeQueryList(queryMap,"selectIncomeDedLimitUploadList",pm);
		saveLog(null, pm);
	} catch (Exception e) {
...
//소득공제 한도차감관리내역(관리자) 상세 저장
public int saveIncomeDedLimitUploadDetail(Map paramMap, String locPath) throws Exception {
...
public List selectIncomeDedEmployeeList(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = DBConn.executeQueryList(queryMap,"selectIncomeDedEmployeeList",pm);
		saveLog(null, pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
public List getIncomeDedEmployeeDetailList(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = DBConn.executeQueryList(queryMap,"getIncomeDedEmployeeDetailList",pm);
		saveLog(null, pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
public List getDedItemList(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = DBConn.executeQueryList(queryMap,"getDedItemList",pm);
		saveLog(null, pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
<%
	...
	if("selectIncomeDedLimitUploadList".equals(cmd)) {
		...
		try {
			listData = selectIncomeDedLimitUploadList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			int cnt = saveIncomeDedLimitUploadDetail(mp, locPath);
		...
		try {
			listData = selectIncomeDedEmployeeList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
		...
		try {
			listData = getIncomeDedEmployeeDetailList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
		...
		try {
			listData = getDedItemList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
