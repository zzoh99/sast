...
<script type="text/javascript">
	...
	function doAction1(sAction) {
		...
		case "Save":
			...
			sheet1.DoSave( "<%=jspPath%>/yearEtcMgr/yearEtcMgrRst.jsp","cmd=saveYearEtcMgr&menuNm="+$("#menuNm").val());
			break;
		case "Insert":
       ...
       case "Down2Template":
           var downcol = makeHiddenSkipCol(sheet1);
           var param  = {DownCols:downcol,SheetDesign:1,Merge:1,DownRows:'0',menuNm:$(document).find("title").text()};
           sheet1.Down2Excel(param);
           break;
           /*
	...
	function sheet1_OnChange(Row, Col, Value) {
		try{
			var params = "searchWorkYy="+$("#srchYear").val();
			    params += "&searchStdCd="+sheet1.GetCellValue(Row, "adj_element_cd");
			...
			if(result.Data.code == "Y"){
				// 총액 = 과세액 (mon == tax_mon)
				sheet1.SetCellValue(Row, "tax_mon"  ,sheet1.GetCellValue(Row, "mon"));
				sheet1.SetCellValue(Row, "notax_mon",		"0"                     );
			}else{
				// 총액 = 비과세액or감면세액 (mon == notax_mon)
				sheet1.SetCellValue(Row, "notax_mon",sheet1.GetCellValue(Row, "mon"));
    ...
    function sheet1_OnLoadExcel(result) {
        try {
            ...
            for(var i = 1; i < sheet1.RowCount()+1; i++) {
            	...
            	if(sheet1.GetCellValue(i,"adj_element_cd") == "C010_79" || sheet1.GetCellValue(i,"adj_element_cd") == "C010_80"){
            		sheet1.SetCellValue( i, "adj_element_cd","");
            		flag = "true";
            	}else{
            		sheet1.SetCellValue( i, "work_yy", $("#srchYear").val());
                    sheet1.SetCellValue( i, "adjust_type", $("#srchAdjustType").val());
            ...
            if(flag == "true"){
            	alert("외국인기술자감면세액 소득구분은 "+"\n자료등록(세액감면/기타세액공제)에서 등록해 주시기 바랍니다.");
            }
            */
        } catch(ex) {
