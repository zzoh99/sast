...
//개인정보파기기준설정 조회
public List getRetInfoDestMgrList1(String path, Map paramMap) throws Exception {
    ...
    try{
        //쿼리 실행및 결과 받기.
        list  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"getRetInfoDestMgrList1",pm);
    } catch (Exception e) {
...
//개인정보파기기준설정 조회
public List getRetInfoDestMgrList2(String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
  //파라메터 복사.
  Map pm =  StringUtil.getParamMapData(paramMap);
  List list = null;
  ...
  try{
      //쿼리 실행및 결과 받기.
      list  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"getRetInfoDestMgrList2",pm);
  } catch (Exception e) {
      Log.Error("[Exception] " + e);
      throw new Exception("조회에 실패하였습니다.");
  }
...
//개인정보파기대상 저장.
public int saveRetInfoDestMgr1(String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
		...
		try{
			for(int i = 0; i < list.size(); i++ ) {
				...
				if("I".equals(sStatus) || "U".equals(sStatus)) {
					//입력
					rstCnt += DBConn.executeUpdate(conn, queryMap, "saveRetInfoDestMgr1", mp);
				}
			}
...
<%
    //쿼리 맵 셋팅
    //setQueryMap(xmlPath+"/retInfoDestMgr/retInfoDestMgr.xml");

    String ssnEnterCd = (String)session.getAttribute("ssnEnterCd");
    String ssnSabun = (String)session.getAttribute("ssnSabun");
    String cmd = (String)request.getParameter("cmd");

    if("getRetInfoDestMgrList1".equals(cmd)) {
        //인적사항관리 조회 
        ...
        try {
            listData = getRetInfoDestMgrList1(xmlPath+"/retInfoDestMgr/retInfoDestMgr.xml", mp);
        } catch(Exception e) {
            code = "-1";
    ...
    }  else if("getRetInfoDestMgrList2".equals(cmd)) {
        //인적사항관리 저장
        ...
        try {
            listData = getRetInfoDestMgrList2(xmlPath+"/retInfoDestMgr/retInfoDestMgr.xml", mp);
        } catch(Exception e) {
            code = "-1";
            message = e.getMessage();
        }
    ...
    } else if("saveRetInfoDestMgr1".equals(cmd)){
		...
		try {
			int cnt = saveRetInfoDestMgr1(xmlPath+"/retInfoDestMgr/retInfoDestMgr.xml", mp);
