...
public String[] prcYeaNtsDisk(Map paramMap) throws Exception {
	...
	String[] rstStr = null;
	try {
		rstStr = DBConn.executeProcedure("PKG_CPN_YEA_"+tgtYear+"_DISK.DISK_DONATION_ALL",type,param);
	} catch(Exception e) {
...
<%
	...
	//쿼리 맵 셋팅
	Map queryMap = setQueryMap(xmlPath+"/yeaNtsTax/yeaNtsTaxH.xml");
	...
	*/
	...
	String totDonationMon = "0";//결과(기부금액총액)
	String totDedMon = "0";		//결과(공제금액총액)
...
// 		}
		...
		List listDataA = selectYeaNtsTaxList("selectYeaNtsTaxHRecodeAContent", paramMap, queryMap);
		...
		List listDataB = selectYeaNtsTaxList("selectYeaNtsTaxHRecodeBContent", paramMap, queryMap);
		...
		List listDataC = selectYeaNtsTaxList("selectYeaNtsTaxHRecodeCContent", paramMap, queryMap);
		String oldSabun = "";
		String sabun = "";
		...
		if(listDataC != null && listDataC.size() > 0) {			
			for(int i = 0; i < listDataC.size(); i++) {
				...
				sabun = (String)mapDataC.get("sabun");
				
				if(!"".equals(oldSabun) && !oldSabun.equals(sabun)){					
					paramMap.put("oldSabun", oldSabun);	
					
					List listDataSubC = selectYeaNtsTaxList("selectYeaNtsTaxHRecodeDContent", paramMap, queryMap);
					if(listDataSubC != null && listDataSubC.size() > 0) {
						for(int j = 0; j < listDataSubC.size(); j++) {
							strBuf.delete(0, strBuf.toString().length());
							Map mapDataD = (Map)listDataSubC.get(j);
			...
			paramMap.put("oldSabun", oldSabun);
			List listDataD = selectYeaNtsTaxList("selectYeaNtsTaxHRecodeDContent", paramMap, queryMap);
			if(listDataD != null && listDataD.size() > 0) {				
				for(int i = 0; i < listDataD.size(); i++) {
					strBuf.delete(0, strBuf.toString().length());
					Map mapDataD = (Map)listDataD.get(i);
		...
		if(recodeCntA == 0 && recodeCntB == 0 && recodeCntC == 0) {
			Log.Debug("== content 데이터 조회중 오류 발생:신고대상 기부금 레코드가 존재하지 않습니다. ==");
			throw new UserException("신고대상 기부금 레코드가 존재하지 않습니다.");
		...
		Map rstData = selectYeaNtsTaxMap("selectYeaNtsTaxHResult", paramMap, queryMap);
		if(rstData != null && rstData.get("tot_donation_mon") != null) {
			totDonationMon = (String)rstData.get("tot_donation_mon");
		}
		if(rstData != null && rstData.get("tot_ded_mon") != null) {
			totDedMon = (String)rstData.get("tot_ded_mon");
		}
	...
	if("1".equals(code)) {
		...
		rtnData.put("totDonationMon", StringUtil.formatMoney(totDonationMon)); //결과(기부금액총액)
		rtnData.put("totDedMon", StringUtil.formatMoney(totDedMon)); //결과(공제금액총액)
		rtnData.put("filePrefix", filePrefix); //구분
	}
	Log.Debug("[NTSTAX H(기부금) 결과]: "+rtnData);
