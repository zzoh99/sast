...
<script type="text/javascript">
	...
	$(function() {
		...
		var rtnval = ajaxCall("<%=jspPath%>/withHoldRcptSta/rcptInvoker.jsp?cmd=onload&rdMrd="+rdMrd, $("#srchFrm").serialize(), false);
	...
	function doAction(sAction) {
		...
		switch (sAction) {
	        case "chk":
	        	chkDisk(ajaxCall("<%=jspPath%>/withHoldRcptSta/rcptInvoker.jsp?cmd=CheckDisk", $("#srchFrm").serialize(), false));
			break;
	        ...
	        case "cre":
        		...
        		var rtnData = ajaxCall("<%=jspPath%>/withHoldRcptSta/rcptInvoker.jsp?cmd=creFile", $("#srchFrm").serialize(), true
	        			,function(){
		                    /* $("#progressCover").show(); */
		                    workingYn = "Y";
	        				timer_start();
		                ...
		                ,function(rtnData){
			        		...
			        		if(rtnData.downFileName.length > 0) {
			        			$("iframe").attr("src",'<%=jspPath%>/withHoldRcptSta/rcptInvoker.jsp?cmd=downFile&downfileName='+$("#fileName").val()+'&searchWorkYy='+searchWorkYy);	
			        		}
			        		workingYn = "N";
	...
	function Timer2(){
		...
		fileCountChk(ajaxCall("<%=jspPath%>/withHoldRcptSta/rcptInvoker.jsp?cmd=cntChk&searchWorkYy="+searchWorkYy, "", false));
	}
