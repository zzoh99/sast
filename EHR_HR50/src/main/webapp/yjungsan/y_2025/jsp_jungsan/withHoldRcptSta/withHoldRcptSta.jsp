...
<script type="text/javascript">
    ...
    $(function() {
        ...
        var data = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_PAGE_UNIT&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
        ...
        data = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_RD_COMMENT_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
        if ( data != null && data.length>0) {
        	$("#searchRdCmtYn").val($.trim(data[0].code_nm));
		}
        
      	//파일생성 사용 여부
		var fileCreYn = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_PDF_CRE_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
		...
		//영문출력 사용 여부
		var engPrintYn = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_ENG_PRINT_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;

		$("#btnDisplayYn2").hide() ;
		if ( engPrintYn != null && engPrintYn.length>0) {
			if(engPrintYn[0].code_nm == "Y") {
				$("#btnDisplayYn2").show() ;
			}
		}
    ...
    function doAction1(sAction) {
        ...
        case "SaveRdCmtYn":
			...
			//TSYS955 UPDATE!
			ajaxCall("<%=jspPath%>/withHoldRcptSta/withHoldRcptStaRst.jsp?cmd=SaveRdCmtYn",params,false);
            break;
    ...
    function withHoldRcptDownPopup(sabuns, sortSabuns, sortNos, stamps, printGbn, sabun, empName, sabunCnt, arrName, adjustTypes){
        var w       = 900;
        var h       = 300;
        var url     = "<%=jspPath%>/withHoldRcptSta/callRcptInvokerPop.jsp";
        var args    = new Array();
        // args의 Y/N 구분자는 없으면 N과 같음
        var rdFileNm ;
        if($("#searchWorkYy").val() !=null && ( $("#searchWorkYy").val()*1 ) >= 2007 ){
    ...
    function initPage() {
    	if($("#searchDivPage").val() != "" && !isNaN($("#searchDivPage").val())) {
    		...
    		var rstData = ajaxCall("<%=jspPath%>/withHoldRcptSta/withHoldRcptStaRst.jsp?cmd=selecWithHoldTotCnt",param,false);
...
<div class="wrapper">
    <form id="sheetForm" name="sheetForm" >
        ...
        <div class="sheet_search outer"><div>
             <table>
                 <tr>
                    <td>
                        <span>귀속년월</span>
                    ...
                    <div class="sheet_title">
                        <ul>
                            ...
                            <li class="btn">
                                ...
                                <span id="btnDisplayYn2">
                                <a href="javascript:print('printEng')" id="rdEngBtn" name="rdEngBtn" class="basic btn-white ico-print">출력(영문)</a>
                                </span>
