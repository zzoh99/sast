...
<script type="text/javascript">
	...
	$(function() {
		...
		var foreignTaxTypeList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYear=<%=yeaYear%>","C00170"), "");
	...
	function shtToCtl(){
		if(sheet1.RowCount() > 0){
			for (i = 1; i < sheet1.RowCount()+1; i++){
				...
				} else if(sheet1.GetCellValue(i,"std_cd")=="CPN_TAX_SPC_YN"){   // 납부특례세액 자동 계산 여부
					$("input:radio[name='cpn_tax_spc_yn']:input[value='"+stdValue+"']").prop("checked",true);
				}
			}
			...
			if(confirm("연말정산 옵션 내역을 저장하시겠습니까?")){
				for (i = 1; i < sheet1.RowCount()+1; i++){
					...
					if(sheet1.GetCellValue(i,"std_cd")=="CPN_MONPAY_RETRY_YN"){   //연간급여재생성 여부
						...
						sheet1.SetCellValue(i, "std_cd_value", $("input:radio[name='cpn_tax_spc_yn']:checked").val());
					}
				}
			...
			<table border="0" cellpadding="0" cellspacing="0" class="default outer line">
				...
				<tr>
					<td class="left">
						...
						자료등록 출력 화면에서 전자 서명 사용 여부를 선택할 수 있습니다. (Internet Explorer 브라우저를 사용하실 경우는 버튼으로 대체됩니다.)<br/>
						YES : 전자 서명을 사용합니다.<br>
				...
				<tr>
					<th class="left" colspan="2">연말정산 옵션 23</th>
				</tr>
				<tr>
					<td class="left">
						<b class="table-title">납부특례세액 자동 계산 여부</b><br>
						벤처기업 주식매수 선택권 행사이익 납부특례(조특법 제16조의3)에 따른 납부특례세액을 자동 계산할 수 있습니다.<br/>
						YES : 자동계산. [자료등록 > (탭) 세액감면/기타세액공제]에서 수기 등록한 납부특례세액이 변경될 수 있음을 주의. <br>
						No : 기본(기존). [자료등록 > (탭) 세액감면/기타세액공제]에서 납부특례세액을 수기 등록해야 함.<br>
					</td>
					<td align="left">
						<input type="radio" class="radio" name="cpn_tax_spc_yn" id="cpn_tax_spc_yn_yn1" value = "Y" > <label for="cpn_tax_spc_yn_yn1">YES</label><br>
						<input type="radio" class="radio" name="cpn_tax_spc_yn" id="cpn_tax_spc_yn_yn2" value = "N" > <label for="cpn_tax_spc_yn_yn2">NO</label>
					</td>
				</tr>
			</table>
		</div>
