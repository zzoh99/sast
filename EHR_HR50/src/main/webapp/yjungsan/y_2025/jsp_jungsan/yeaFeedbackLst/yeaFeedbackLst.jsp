...
<script type="text/javascript">
	...
	$(function() {
		...
		initdata1.Cols = [
			...
			//{Header:"전체내역",		Type:"Image",     	Hidden:1,  	Width:50,   Align:"Center",		ColMerge:1,	SaveName:"detail_popup",    KeyField:0,   CalcLogic:"",   Format:"",	PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100, Cursor:"Pointer" },
		...
		var appTypeList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYear=<%=yeaYear%>","C00329"), "");
	...
	function doAction1(sAction) {
		...
		case "Search":
			...
			break;
		case "Save":
			sheet1.DoSave( "<%=jspPath%>/yeaFeedbackLst/yeaFeedbackLstRst.jsp?cmd=saveYeaFeedback", $("#sheetForm").serialize() );
			break;
        case "Down2Excel":
			...
			break;
        case "Down2Template":
			var param  = {DownCols:"5|6|7|18",SheetDesign:1,Merge:1,DownRows:"0",ExcelFontSize:"9"
	...
	function sheet1_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			if(Code == 1 && sheet1.RowCount() == 0) {
				var param = "searchWorkYy=&searchAdjustType=&searchSabun=&searchGubunCd=";
	...
	function sheet1_OnClick(Row, Col, Value, CellX, CellY, CellW, CellH) {
		try{
			/* 2023.10.26 전체내역 필드 및 팝업 부분 주석처리
        ...
        try {
        	if(sheet1.RowCount() != 0) {
	        	...
	        	for(var i = 1; i < sheet1.RowCount()+1; i++) {
					sheet1.SetCellValue2(i,"work_yy",searchWorkYy);
					sheet1.SetCellValue2(i, "adjust_type", searchAdjustType);
					sheet1.SetCellValue2(i, "gubun_nm", sheet1.GetCellValue(i, "gubun_cd"));
				}
        	}
        } catch(ex) {
