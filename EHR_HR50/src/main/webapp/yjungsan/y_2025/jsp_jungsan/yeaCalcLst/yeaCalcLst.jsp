...
// 외국납부 상세팝업
$(function() {
	$("#foreignPayDetailBtn").hide();	// 2024.09.30 hide 처리
	$("#menuNm").val($(document).find("title").text()); //엑셀,CURD
...
<script type="text/javascript">
	...
	$(function(){
		...
		if ( orgAuthPg == "A" ) {
			...
			var data = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_PAYTAX_YN&searchWorkYy=<%=yeaYear%>", "queryId=getYeaSystemStdData",false).codeList;
			var data2 = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_PAYTAX_INS_YN&searchWorkYy=<%=yeaYear%>", "queryId=getYeaSystemStdData",false).codeList;
	...
	$(function(){
		...
		//세액계산,결과 권한 조회
	    printButtonYn = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_WORK_INCOME_PRINT_BUTTON&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
		...
		var yeacalclstStampYn = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=YEACALCLST_STAMP_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
	...
	function sheet3_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			if(Code == 1) {
				...
				else if(sheet3.GetCellValue(1, "apprv_yn") == "Y" && sheet3.GetCellValue(1, "result_open_yn") == "Y"){
					radioDisable("N");
				} else {
					radioDisable("Y");
				}
	...
	function print(){
		...
		args["rdParam"] = "[<%=session.getAttribute("ssnEnterCd")%>]"
		                ...
		                + stampYn
		                + "[N]" //사대보험출력여부
                        + "["+$("#searchAdjustType").val()+"]"; //일괄다운로드 adjust_type리스트
						//rd파라매터
