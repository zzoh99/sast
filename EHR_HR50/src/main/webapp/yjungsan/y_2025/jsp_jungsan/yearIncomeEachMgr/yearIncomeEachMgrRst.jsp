...
//연간소득_개별 검색
public List selectYearIncomeEachMgr(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
//연간소득_개별 저장.
public int saveYearIncomeEachMgr(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
//연간소득_개별 기타소득 저장.
public int saveYearIncomeEachMgrEtc(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	if(list != null && list.size() > 0 && conn != null) {
		...
		try{
			for(int i = 0; i < list.size(); i++ ) {
				...
				Map mp = (Map)list.get(i);
				String sStatus = (String)mp.get("sStatus");
				String reduceChk = (String)mp.get("reduceChk");
                Map mp2 = (Map)list.get(0);
                String menuNm = (String)mp2.get("menuNm");
                mp.put("menuNm", menuNm);
				if("D".equals(sStatus)) {
				...
				*/
				...
				if(dataMap != null) {
					...
					*/
					...
					/*
						22.11.23 추가
						B010_46: 조세특례제한법 (제29조의6)-50% 감면소득(청년-중견)
						B010_47: 조세특례제한법 (제29조의6)-90% 감면소득(청년-중소)
					*/
					String B010_46 = (String)dataMap.get("b010_46")  ;
					if(!B010_46.equals("")) {
						mp.put("input_mon", B010_46) ;
						mp.put("adj_element_cd", "B010_46") ;
						DBConn.executeUpdate(conn, queryMap, "updateYearIncomeEachMgrEtcSum", mp);
					}
					
					String B010_47 = (String)dataMap.get("b010_47")  ;
					if(!B010_47.equals("")) {
						mp.put("input_mon", B010_47) ;
						mp.put("adj_element_cd", "B010_47") ;
						DBConn.executeUpdate(conn, queryMap, "updateYearIncomeEachMgrEtcSum", mp);
					}
...
<%
	...
	if("selectYearIncomeEachMgr".equals(cmd)) {
		...
		try {
			listData = selectYearIncomeEachMgr(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			int cnt = saveYearIncomeEachMgr(mp, locPath, ssnYeaLogYn);
		...
		try {
			int cnt = saveYearIncomeEachMgrEtc(mp, locPath, ssnYeaLogYn);
