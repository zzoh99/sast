	...
	$(function() {
		...
		initdata.Cols = [
          ...
          <%// 20250502. 시트 7.0.0.0-20131223-17 버전에서는 SetColProperty 로 DefaultValue가 지원되지 않음. => initdata 세팅으로 조정
	        ...
	        {Header:"대상년도",		Type:"Text",      Hidden:0,  Width:60,    Align:"Center",  ColMerge:1,   SaveName:"work_yy",			KeyField:1,   	CalcLogic:"",   Format:"",	PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:4, DefaultValue:"<%=yeaYear%>" },
            ...
            {Header:"계약기간(개월수)",	Type:"Int",		Hidden:0,	Width:80,	Align:"Center",	ColMerge:1,	SaveName:"mth_per",				KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:4 },
            {Header:"납입횟수",       Type:"Text",      Hidden:1,  Width:60,   Align:"Left",   ColMerge:1, SaveName:"paying_num_cd",       KeyField:1, Format:"",  PointCount:0,   UpdateEdit:0,   InsertEdit:1,   EditLen:10 },
	...
	function doAction1(sAction) {
		...
		case "Save":
			...
			if(vsInsertCheck != "" && vsInsertCheck != null){
				...
				for(var j = 0; j < insertArray.length; j++){
					...
					if(sheet1.GetCellValue(insertArray[j],"saving_deduct_type")=='55' || sheet1.GetCellValue(insertArray[j],"saving_deduct_type")=='60' || sheet1.GetCellValue(insertArray[j],"saving_deduct_type")=='65'){
					...
					//청년형장기집합투자증권저축 선택시 
					if(sheet1.GetCellValue(insertArray[j],"saving_deduct_type")=='70' ){

						if( sheet1.GetCellValue(insertArray[j], "mth_per") == "") {
							alert("청년형장기집합투자증권저축일때 계약기간은 필수입력입니다.");
							sheet1.SelectCell(insertArray[j], "mth_per");
							return;
						}
					}
				}
		...
		case "Down2Template":
			var param  = {DownCols:"adjust_type"
                                     +"|sabun|saving_deduct_type|finance_org_cd|reg_dt|mth_per|account_no|appl_mon|adj_input_type",SheetDesign:1,Merge:1,DownRows:'0',ExcelFontSize:"9"
			...
			if (Code == 1) {
				for(var i = sheet1.HeaderRows(); i <= sheet1.LastRow(); i++){
					...
					//중소기업창업투자조합(벤처등) 또는 중소기업창업투자조합(조합등) 로드시
					 if(sheet1.GetCellValue(i,"saving_deduct_type")=='55' || sheet1.GetCellValue(i,"saving_deduct_type")=='60' || sheet1.GetCellValue(i,"saving_deduct_type")=='65'){
						//텍스트타입
					...
					} else if(sheet1.GetCellValue(i,"saving_deduct_type")=='70'){
						sheet1.SetCellEditable(i, "mth_per", 1); //청년형장기집합투자증권저축 로드시 계약기간(개월수) 수정 가능
					}
	...
	function sheet1_OnChange(Row, Col, Value, OldValue) {
		try{
			...
			if(sheet1.ColSaveName(Col) == "saving_deduct_type"){
				...
				}else if(sheet1.GetCellValue(Row,"saving_deduct_type")=='55' || sheet1.GetCellValue(Row,"saving_deduct_type")=='60' || sheet1.GetCellValue(Row,"saving_deduct_type")=='65'){
					//텍스트타입
				...
				//청년형장기집합투자증권저축 선택시 
				if(sheet1.GetCellValue(Row,"saving_deduct_type")=='70'){
					sheet1.SetCellEditable(Row, "mth_per", 1); //계약기간(개월수) 수정 가능
				} else {
					sheet1.SetCellValue(Row, "mth_per", "");
					sheet1.SetCellEditable(Row, "mth_per", 0);
				}
			}
		} catch(ex) {
