...
<script type="text/javascript">
	...
	$(function() {
		...
		var rtnval = ajaxCall("<%=jspPath%>/withHoldRcptBkSta/rcptBkInvoker.jsp?cmd=onload", $("#srchFrm").serialize(), false);
	...
	function doAction(sAction) {
		...
		switch (sAction) {
	        case "chk":
	        	...
	        	chkDisk(ajaxCall("<%=jspPath%>/withHoldRcptBkSta/rcptBkInvoker.jsp?cmd=CheckDisk", $("#srchFrm").serialize(), false));
			break;
	        ...
	        case "cre":
	        	...
	        	var rtnData = ajaxCall("<%=jspPath%>/withHoldRcptBkSta/rcptBkInvoker.jsp?cmd=creFile", $("#srchFrm").serialize(), true
	        			,function(){
			        		/* $("#progressCover").show(); */
		                    workingYn = "Y";
		    				timer_start();
		                ...
		                ,function(rtnData){
		                	...
		                	if (rtnData.Result.Code > 0) {
				        		...
				        		if(rtnData.downFileName.length > 0) {
				        			$("iframe").attr("src",'<%=jspPath%>/withHoldRcptBkSta/rcptBkInvoker.jsp?cmd=downFile&downfileName='+$("#fileName").val()+'&searchWorkYy='+searchWorkYy);	
				        		}
		                	} else {
	...
	function Timer2(){
		...
		fileCountChk(ajaxCall("<%=jspPath%>/withHoldRcptBkSta/rcptBkInvoker.jsp?cmd=cntChk&searchWorkYy="+searchWorkYy, "", false));
	}
