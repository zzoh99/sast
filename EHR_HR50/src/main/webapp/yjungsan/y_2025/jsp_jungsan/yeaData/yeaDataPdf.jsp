...
<script type="text/javascript">
    ...
    var bulkRegistYn = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_PDF_BULK_REGIST_TYPE&searchWorkYy="+$("#searchWorkYy", parent.document).val(), "queryId=getYeaSystemStdData",false).codeList[0];
	...
	$(function() {
		...
		var result = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_PDF_TYPE&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
		if(result != null && result.length > 0) {
			pdfTypeCd = result[0].code_nm;
		}
...
//		if(orgAuthPg == "R" && (empStatus == "close_2" || empStatus == "close_3" || empStatus == "close_4")) { //임직원 조회 화면에서 본인마감 or 최종 or 담당자마감일경우
		...
		if(document.getElementById('uploadBtn').innerText == '반영'){
			...
			try{
				...
				if(vsSsnEnterCd == "KYBS"){
				   ...
				   if($("#fileUploadType").val() == "1"){//S3
					   $("#newForm").attr({"method":"POST","target":"ifrmPdfUpload","action":"yeaDataPdfUploadRstS3.jsp"}).submit();
		           }
				   else if($("#fileUploadType").val() == "2"){//HMM S3 TYPE2
		        	   $("#newForm").attr({"method":"POST","target":"ifrmPdfUpload","action":"yeaDataPdfUploadRstS3_TYPE2.jsp"}).submit();
		           }
				   else{
			...
			if(vsSsnEnterCd == "KYBS"){
			   ...
			   if($("#fileUploadType").val() == "1"){//S3
	        	   ...
	        	   $("#uploadForm").attr({"method":"POST","target":"ifrmPdfUpload","action":"yeaDataPdfUploadRstS3_TYPE2.jsp"}).submit();
	           }
			   else{
	...
	function sheet3_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			for(var i = 1; i < sheet3.RowCount()+1; i++) {
				...
				else if($("#fileUploadType").val() == "2"){//S3 HMM TYPE2
					sheet3.SetCellValue(i,"file_link","<a href=\"javascript:openPrintS3_TYPE2('"+file_name+"')\" class='basic btn-download'>보기</a>");
					if(sheet3.GetCellValue(i, "fileapply") == "N") {
						sheet3.SetCellValue(i,"delfile_link","<a href=\"javascript:deletePdf('"+i+"', '"+filePath+"')\" class='basic btn-white'>삭제</a>");
					} else {
						sheet3.SetCellValue(i,"delfile_link","&nbsp;");
	...
	function fileDownFromS3(fileNm){//S3
		var params = "";
	...
	function openPrintS3_TYPE2(fileNm){//HMM S3 TYPE2 PDF 출력
		var params = "";
		params += "fileName="      + fileNm;		
		var pdfUrl = "<%=jspPath%>/yeaData/yeaDataPdfRstS3_TYPE2.jsp?cmd=downloadPdf&fileName=" +fileNm;
		var viewer = "<iframe src='" + pdfUrl + "' width='100%' height='800px'></iframe>";
		var popup = window.open("", "_blank", "width=1024,height=768");
        popup.document.write(viewer);
	}
	...
	function deletePdf(row, filePath){
		...
		if($("#fileUploadType").val() == "1"){ //S3
            ...
            var rtnResult = ajaxCall("<%=jspPath%>/yeaData/yeaDataPdfRstS3_TYPE2.jsp?cmd=deleteFile",params,false); 	
        }
		else{
