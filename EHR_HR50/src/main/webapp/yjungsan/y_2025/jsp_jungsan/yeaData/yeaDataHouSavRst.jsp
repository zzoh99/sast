...
//주택마련저축 자료 조회
public List selectYeaDataHouSavList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectYeaDataHouSavList",pm);
...
<%
	String locPath = xmlPath+"/yeaData/yeaDataHouSav.xml";
	...
	if("selectYeaDataHouSavList".equals(cmd)) {
		//주택마련저축 자료 조회
		...
		try {
			listData = selectYeaDataHouSavList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
	...
	} else if("saveYeaDataHouSav".equals(cmd)) {
		//주택마련저축 저장.
			...
			if(saveList != null && saveList.size() > 0) {
				for(int i = 0; i < saveList.size(); i++) {
					...
					if( "07".equals(adjInputType) && "D".equals(sStatus) ) {
						...
						rstStr = DBConn.executeProcedure("PKG_CPN_YEA_"+yeaYear+"_SYNC.HOUSE_SAVING_INS",type,param);
