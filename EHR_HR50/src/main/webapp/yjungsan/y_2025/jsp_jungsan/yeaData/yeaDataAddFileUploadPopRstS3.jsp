    ...
    if("saveNmTxt".equals(cmd)) {
	    ...
	    for(int i = 0 ; i < v_fileCnt ; i++){
	        ...
	        if(ext != null && !"".equals(ext)) {
				...
				// 분할 압축한 확장자 "z01" 과 같이 [z + 두자리 숫자] 형식 확장자도 업로드 가능하도록 정규식 처리
				String regex = "^[z]\\d{2}$";
	    ...
	    for(int i = 0 ; i < v_fileCnt ; i++){
			...
			if("".equals(saveFileNm)) {
				...
				saveFileNm = searchSabun+now+StringUtil.padLeft(secureRandom.nextInt(10000)+"", "0" ,4)+fileName.substring(fileName.lastIndexOf("."));				
			}
			...
			if (conn != null) {
				...
				memoObj.put("errorMessage", "conn 객체가 null입니다.");
				memoObj.put("errorCode", -1);
		    	memoArr.put(fileIndex, memoObj);
