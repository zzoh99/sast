...
//의료비 자료 조회
public List selectYeaDataMedList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectYeaDataMedList",pm);
...
<%
	String locPath = xmlPath+"/yeaData/yeaDataMed.xml";
	...
	if("selectYeaDataMedList".equals(cmd)) {
		//의료비 자료 조회
		...
		try {
			listData = selectYeaDataMedList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
	...
	} else if("saveYeaDataMed".equals(cmd)) {
		//의료비 저장.
		...
		try {
			...
			if(saveList != null && saveList.size() > 0) {
				for(int i = 0; i < saveList.size(); i++) {
					...
					String ymd = (String)mp.get("ymd");
					String famres = (String)mp.get("famres");
					String adjFamCd = (String)mp.get("adj_fam_cd");
					String specialYn = (String)mp.get("special_yn");
					String medicalImpCd = (String)mp.get("medical_imp_cd");
					String medicalType = (String)mp.get("medical_type");
					...
					String preBabyYn     = (String)mp.get("pre_baby_yn");
					String nanim_yn = (String)mp.get("nanim_yn");
					...
					if( "07".equals(adjInputType) && "D".equals(sStatus) ) {
						...
						param = new String[]{"","",sStatus,ssnEnterCd,workYy
								,adjustType,sabun,seq,ymd,famres
								,adjFamCd,specialYn,medicalImpCd,medicalType,enterNo
								,firmNm,sCnt,applMon,adjInputType,ntsYn,restrictCd
								,feedbackType,preBabyYn,nanim_yn,ssnSabun,docSeq,docSeqDetail};

						rstStr = DBConn.executeProcedure("PKG_CPN_YEA_"+yeaYear+"_SYNC.MEDICAL_INS",type,param);
