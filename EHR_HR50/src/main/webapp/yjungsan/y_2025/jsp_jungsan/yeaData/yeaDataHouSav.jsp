...
<script type="text/javascript">
    ...
    var toYear = "<%=toYear%>";
    //도움말    
    var helpText;  //안내메세지 주택마련저축
    var helpText2; //안내메세지 개인연금/장기집합투자증권저축/중소기업창업투자조합
    var helpText3; //안내메세지 청년형 장기집합투자증권저축
    //기준년도
    var systemYY;
    //총급여
    var paytotMonStr;
...
--%>
        ...
        if(houseCnt == "0" || houseCnt == "") {
            ...
            $("#houseInfo").html("취득일자 : " + houseGetYmd + " / 국민주택규모여부 " + (houseArea == 1 ? "O" : "X")) ;
        }

      	//중소기업창업투자조합(벤처등) 또는 중소기업창업투자조합(조합등) 선택시 날짜 기준을 귀속년도로 변경. 2019-11-22
        toYear = systemYY;
        var dt = new Date(toYear, 1, 1);
        beforeYear = dt.getFullYear() - 2;
    ...
    $(function() {
        ...
        var financeOrgCdList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&orderBy=code_nm&searchYear="+<%=yeaYear%>,"C00319"), "");
        var savingDeductTypeList = stfConvCode( ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&queryId=getSavingDeductList&searchYear="+<%=yeaYear%>,"searchGubun=4",false).codeList, "");
        ...
        sheet1.SetColProperty("finance_org_cd",     {ComboText:"|"+financeOrgCdList[0]+"|해당없음", ComboCode:"|"+financeOrgCdList[1]+"|ZZZ"} );
...
//         doAction2("Search");
            ...
            if(sheet1.ColSaveName(Col) == "saving_deduct_type"){
                ...
                //2015년도 추가 내역 start
                    ...
                    if(houseOwnerYn != 'Y') {
                    	...
                    	result = ajaxCall("<%=jspPath%>/common/commonSelect.jsp?cmd=commonSelectMap",param+"&searchNumber=1",false);
    ...
    function initDefaultData() {
        ...
        var result1 = ajaxCall("<%=jspPath%>/common/commonSelect.jsp?cmd=commonSelectMap",param1+"&adjProcessCd=A071",false);
        helpText = nvl(result1.Data.help_text1,"") + nvl(result1.Data.help_text2,"") + nvl(result1.Data.help_text3,"");
        $("#infoLayer").html(helpText).hide();

        //안내메시지 개인연금/장기집합투자증권저축/중소기업창업투자조합        
        param1 = "searchWorkYy=" + $("#searchWorkYy").val() 
                    + "&queryId=getYeaDataHelpTextSub" 
                    + "&adjProcessCd=A071" // A071(특별소득공제_저축) //TSYS005.NOTE2 = 프로세스 코드 
                    + "&helpGrpCd=A071_10" // A071_10(개인연금/장기집합투자증권저축/중소기업창업투자조합 안내) // TSYS005.NOTE3 = 도움말 그룹 (MEMO 4000 바이트가 모자를 경우 분리해서 저장)
                    ;
        result1 = ajaxCall("<%=jspPath%>/common/commonSelect.jsp?cmd=commonSelectMap",param1,false);
        helpText2 = nvl(result1.Data.help_text1,"") + nvl(result1.Data.help_text2,"") + nvl(result1.Data.help_text3,"") + nvl(result1.Data.help_text4,"");
		...
		param1 = "searchWorkYy=" + $("#searchWorkYy").val() 
                    + "&queryId=getYeaDataHelpTextSub" 
                    + "&adjProcessCd=A071" // A071(특별소득공제_저축) //TSYS005.NOTE2 = 프로세스 코드 
                    + "&helpGrpCd=A071_20" // A071_20(청년형장기집합투자증권저축 안내) // TSYS005.NOTE3 = 도움말 그룹 (MEMO 4000 바이트가 모자를 경우 분리해서 저장)
                    ;
        result1 = ajaxCall("<%=jspPath%>/common/commonSelect.jsp?cmd=commonSelectMap",param1,false);
        helpText2 = nvl(result1.Data.help_text1,"") + nvl(result1.Data.help_text2,"") + nvl(result1.Data.help_text3,""); */
        $("#infoLayer3").html(helpText2).hide();
        //------------------------------ 
        // 도움말 조회 END
        //------------------------------
        ...
        //총급여 확인 버튼 유무
        var result4 = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_MON_SHOW_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
    ...
    function sheetSet(){
        ...
        if(comSheet.RowCount() > 0){
            ...
            $("#A100_41").val(comSheet.GetCellText(comSheet.FindText("adj_element_cd", "A100_41"),"input_mon"));
            
			var intMon = 0 ;
			intMon = intMon + comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A100_84"),"input_mon") ; // 중소기업창업투자2022(조합1)
			intMon = intMon + comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A100_85"),"input_mon") ; // 중소기업창업투자2022(조합2)
			intMon = intMon + comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A100_86"),"input_mon") ; // 장기집합투자저축2022(벤처)
			intMon = intMon + comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A100_87"),"input_mon") ; // 중소기업창업투자2023(조합1)
			intMon = intMon + comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A100_88"),"input_mon") ; // 중소기업창업투자2023(조합2)
			intMon = intMon + comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A100_89"),"input_mon") ; // 장기집합투자저축2023(벤처)
			intMon = intMon + comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A100_90"),"input_mon") ; // 중소기업창업투자2024(조합1)
			intMon = intMon + comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A100_91"),"input_mon") ; // 중소기업창업투자2024(조합2)
			intMon = intMon + comSheet.GetCellValue(comSheet.FindText("adj_element_cd", "A100_92"),"input_mon") ; // 장기집합투자저축2024(벤처)
            $("#A100_80").val(intMon);
    ...
    function paytotMonView(gubun){
        ...
        if(paytotMonStr != ""){
            ...
            if( ( paytotMonStr.replace(/,/gi, "") *1 ) > 80000000 ) {
                if(gubun == '1'){
                    $("#span_paytotMonView1").html("<B>YES</B>&nbsp;<a class='under-line bold' href='javascript:paytotMonViewClose(1)'>닫기</a>");
                }else if(gubun == '3'){
                    $("#span_paytotMonView3").html("<B>YES</B>&nbsp;<a class='under-line bold' href='javascript:paytotMonViewClose(3)'>닫기</a>");
                }else{
                	$("#span_paytotMonView2").html("<B>YES</B>&nbsp;<a class='under-line bold' href='javascript:paytotMonViewClose(2)'>닫기</a>");
                }

            } else {
                if(gubun == '1'){
                    $("#span_paytotMonView1").html("<B>NO</B>&nbsp;<a class='under-line bold' href='javascript:paytotMonViewClose(1)'>닫기</a>");
                }else if(gubun == '3'){
                	$("#span_paytotMonView3").html("<B>NO</B>&nbsp;<a class='under-line bold' href='javascript:paytotMonViewClose(3)'>닫기</a>");
                }else{
                    if( ( paytotMonStr.replace(/,/gi, "") *1 ) > 70000000 ) {
                        $("#span_paytotMonView2").html("<B>YES</B>&nbsp;<a class='under-line bold' href='javascript:paytotMonViewClose(2)'>닫기</a>");
                    }else{
                        $("#span_paytotMonView2").html("<B>NO</B>&nbsp;<a class='under-line bold' href='javascript:paytotMonViewClose(2)'>닫기</a>");
                    }
                }
            }
        } else {
...
<script type="text/javascript">
	function loadSheet2() {
		...
		initdata.Cols = [
			...
			{Header:"정산구분|정산구분",					Type:"Text",		Hidden:1,	Width:60,	Align:"Left",	ColMerge:1,	SaveName:"adjust_type",			KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:10 },
			{Header:"사번|사번",							Type:"Text",		Hidden:1,	Width:60,	Align:"Left",	ColMerge:1,	SaveName:"sabun",				KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:13 },
			{Header:"저축구분|저축구분",							Type:"Combo",		Hidden:0,	Width:80,	Align:"Left",	ColMerge:1,	SaveName:"saving_deduct_type",	KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:10 },
			...
			{Header:"금융기관코드|금융기관코드",					Type:"Combo",		Hidden:0,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:"finance_org_cd",		KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:10 },
			{Header:"가입일자\n(계약시작일자)\n(투자(연도)시기)|가입일자\n(계약시작일자)\n(투자(연도)시기)",	Type:"Date",       Hidden:0,   Width:60,   Align:"Center",   ColMerge:1, SaveName:"reg_dt",      KeyField:1, Format:"Ymd",  PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:8 },
			{Header:"가입일자\n(계약시작일자)\n(투자(연도)시기)|가입일자\n(계약시작일자)\n(투자(연도)시기)",							Type:"Text",       Hidden:1,   Width:60,   Align:"Center",   ColMerge:1, SaveName:"reg_dt2",      KeyField:0, Format:"",  PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:8 },
			...
			{Header:"계좌번호\n(또는증권번호)|계좌번호\n(또는증권번호)",Type:"Text",		Hidden:0,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:"account_no",			KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:35 },
			{Header:"금액자료|직원용",						Type:"AutoSum",			Hidden:0,	Width:60,	Align:"Right",	ColMerge:1,	SaveName:"input_mon",			KeyField:0,	Format:"#,###",			PointCount:0,	UpdateEdit:<%=inputEdit%>,	InsertEdit:<%=inputEdit%>,	EditLen:35, MinimumValue: 0 },
			{Header:"금액자료|담당자용",						Type:"AutoSum",			Hidden:0,	Width:60,	Align:"Right",	ColMerge:1,	SaveName:"appl_mon",			KeyField:1,	Format:"#,###",			PointCount:0,	UpdateEdit:<%=applEdit%>,	InsertEdit:<%=applEdit%>,	EditLen:35, MinimumValue: 0 },
			...
			{Header:"금액자료|Dummy",						Type:"Int",			Hidden:1,	Width:60,	Align:"Right",	ColMerge:1,	SaveName:"appl_mon_isa",			KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:35 },
			{Header:"자료입력유형|자료입력유형",					Type:"Combo",		Hidden:0,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:"adj_input_type",		KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:10 },
			{Header:"국세청\n자료여부|국세청\n자료여부",			Type:"CheckBox",	Hidden:1,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:"nts_yn",				KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:1, TrueValue:"Y", FalseValue:"N" },
			{Header:"담당자확인|담당자확인",			Type:"Combo",		Hidden:0,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:"feedback_type",		KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:10 }
		...
		var savingDeductTypeList2 = stfConvCode( ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&queryId=getSavingDeductList&searchYear="+<%=yeaYear%>,"searchGubun=3",false).codeList, "");

		sheet2.SetColProperty("adj_input_type",		{ComboText:"|"+adjInputTypeList[0], ComboCode:"|"+adjInputTypeList[1]} );
		sheet2.SetColProperty("finance_org_cd",		{ComboText:"|"+financeOrgCdList[0]+"|", ComboCode:"|"+financeOrgCdList[1]+"|ZZZ"} );
	...
	function doAction2(sAction) {
		...
		case "Save":
			...
			var vsInsertCheck = sheet2.FindStatusRow("I|U");
			...
			if(vsInsertCheck != "" && vsInsertCheck != null){
				...
				for(var j = 0; j < insertArray.length; j++){
					...
					if(smallCompYn == "N"){
						//금융기관코드 필수체크
						if(sheet2.GetCellValue(insertArray[j],"finance_org_cd")=="" || sheet2.GetCellValue(insertArray[j],"finance_org_cd")==null){
							alert("금융기관 코드를 입력 해주세요");
							return;

						}

						if(sheet2.GetCellValue(insertArray[j],"account_no")=="" || sheet2.GetCellValue(insertArray[j],"account_no")==null){
							alert("계좌번호를 입력 해주세요");
							return;
					...
					/* 계좌번호 숫자 체크 제거 - 2020.01.30
					//계좌번호 문자 및 숫자여부 체크
					if(isNaN(sheet2.GetCellValue(insertArray[j], "account_no"))){
					...
					if(sheet2.GetCellValue(insertArray[j],"saving_deduct_type")=='55' || sheet2.GetCellValue(insertArray[j],"saving_deduct_type")=='60' || sheet2.GetCellValue(insertArray[j],"saving_deduct_type")=='65'){

						if((sheet2.GetCellValue(insertArray[j], "reg_dt") < beforeYear) || (sheet2.GetCellValue(insertArray[j], "reg_dt") > toYear) ){
							alert("귀속년도부터 3년전까지만 입력할수 있습니다. ex)2022 ~ 2024");
			                sheet2.SetCellValue(insertArray[j],"reg_dt", "");
			                return;
						}

						if(!yearPattern.test(sheet2.GetCellValue(insertArray[j], "reg_dt"))) {
							alert("년도 형식이 다릅니다. ex)2022 ~ 2024");
							sheet2.SetCellValue(insertArray[j],"reg_dt", "");
							return;
						}
					}
			...
			if(Code == 1) {
				for(var i = sheet2.HeaderRows(); i <= sheet2.LastRow(); i++){
					...
					//중소기업창업투자조합(벤처등) 또는 중소기업창업투자조합(조합등) 로드시
					 if(sheet2.GetCellValue(i,"saving_deduct_type")=='55' || sheet2.GetCellValue(i,"saving_deduct_type")=='60' || sheet2.GetCellValue(i,"saving_deduct_type")=='65'){
						//텍스트타입
						var info = {Type: "Text", Align: "Center", Edit: 0};
					    sheet2.InitCellProperty(i, "reg_dt", info);

					    var vsHvalue= sheet2.GetCellValue(i, "reg_dt2");
					    sheet2.SetCellValue(i,"reg_dt", vsHvalue);
	...
	function sheet2_OnChange(Row, Col, Value, OldValue) {
		try{
			...
			if(sheet2.ColSaveName(Col) == "saving_deduct_type"){
				if(sheet2.GetCellValue(Row,"saving_deduct_type")=='21' || sheet2.GetCellValue(Row,"saving_deduct_type")=='50'){
					//달력타입
					var info = {Type: "Date", Align: "Center", Edit: 1};
				    sheet2.InitCellProperty(Row, "reg_dt", info);
				}

				if(sheet2.GetCellValue(Row,"saving_deduct_type")=='55' || sheet2.GetCellValue(Row,"saving_deduct_type")=='60' || sheet2.GetCellValue(Row,"saving_deduct_type")=='65'){
					//텍스트타입
			...
			//중소기업창업투자조합(벤처등) 또는 중소기업창업투자조합(조합등) 선택시
			/* if(sheet2.GetCellValue(Row,"saving_deduct_type")=='55' || sheet2.GetCellValue(Row,"saving_deduct_type")=='60'){
				//가입일자 계약시작일자가 현재년도 부터 3년전 까지만 입력 가능 체크
				if(sheet2.ColSaveName(Col) == "reg_dt"){
					if((Value < beforeYear) || (Value > toYear) ){
						alert("귀속년도부터 3년전까지만 입력할수 있습니다. ex)2017 ~ 2019");
	                    /*sheet2.SetCellValue(sheet2.GetSelectRow(),"reg_dt", "");
	                    return;
					}

					if(!yearPattern.test(Value)) {
						alert("년도 형식이 다릅니다. ex)2017 ~ 2019");
						/*sheet2.SetCellValue(sheet2.GetSelectRow(),"reg_dt", "");
						return;
					}
				}
			...
			if(sheet2.ColSaveName(Col) == "saving_deduct_type"){
				//중소기업창업투자조합(벤처등), 중소기업창업투자조합(조합등) 선택여부     Y: 중소기업창업투자조합 선택, N: 중소기업창업투자조합 선택안함
				if(sheet2.GetCellValue(Row,"saving_deduct_type")=='55' || sheet2.GetCellValue(Row,"saving_deduct_type")=='60' || sheet2.GetCellValue(Row,"saving_deduct_type")=='65'){
					smallCompYn = "Y";
				}else{
					smallCompYn = "N";
	...
	$(document).ready(function(){
        ...
        /* 청년형장기집합투자증권저축 안내 버튼 기능End */
  
    });

	//주택마련저축 안내 팝업 실행후 클릭시 창 닫음
    ...
    $(document).mouseup(function(e){
  	...
  	//청년형 장기집합투자증권저축 안내 팝업 실행후 클릭시 창 닫음
    $(document).mouseup(function(e){
...
<script type="text/javascript">
	function loadSheet3() {
		...
		initdata.Cols = [
			...
			{Header:"금융기관코드|금융기관코드",					Type:"Combo",		Hidden:0,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:"finance_org_cd",		KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:10 },
			{Header:"가입일자\n(계약시작일자)\n(투자(연도)시기)|가입일자\n(계약시작일자)\n(투자(연도)시기)",	Type:"Date",       Hidden:0,   Width:60,   Align:"Center",   ColMerge:1, SaveName:"reg_dt",      KeyField:1, Format:"Ymd",  PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:8 },
			{Header:"계약기간(개월수)|계약기간(개월수)",							Type:"Int",		Hidden:0,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:"mth_per",				KeyField:1,	Format:"",			PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:4 },
			{Header:"가입일자|가입일자",							Type:"Text",       Hidden:1,   Width:60,   Align:"Center",   ColMerge:1, SaveName:"reg_dt2",      KeyField:0, Format:"",  PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:8 },
	        {Header:"납입횟수코드|납입횟수코드",					Type:"Text",		Hidden:1,	Width:60,	Align:"Left",	ColMerge:1,	SaveName:"paying_num_cd",		KeyField:0,	Format:"",			PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:35 },
		...
		var feedbackTypeList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYear=<%=yeaYear%>","C00329"), "");
		var retPenTypeList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYear=<%=yeaYear%>","C00318"), "");
		var savingDeductTypeList2 = stfConvCode( ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&queryId=getSavingDeductList&searchYear="+<%=yeaYear%>,"searchGubun=10",false).codeList, "");

		sheet3.SetColProperty("adj_input_type",		{ComboText:"|"+adjInputTypeList[0], ComboCode:"|"+adjInputTypeList[1]} );
		sheet3.SetColProperty("finance_org_cd",		{ComboText:"|"+financeOrgCdList[0]+"|해당없음", ComboCode:"|"+financeOrgCdList[1]+"|ZZZ"} );
		sheet3.SetColProperty("feedback_type",		{ComboText:"|"+feedbackTypeList[0], ComboCode:"|"+feedbackTypeList[1]} );
		sheet3.SetColProperty("ret_pen_type",		{ComboText:"|"+retPenTypeList[0], ComboCode:"|"+retPenTypeList[1]} );
		sheet3.SetColProperty("saving_deduct_type", {ComboText:"|"+savingDeductTypeList2[0], ComboCode:"|"+savingDeductTypeList2[1]} );
	}
	...
	function doAction3(sAction) {
		...
		switch (sAction) {
		case "Search":
			sheet3.DoSearch( "<%=jspPath%>/yeaData/yeaDataHouSavRst.jsp?cmd=selectYeaDataHouSavList", $("#sheetForm").serialize() +"&searchNote1=10" );
			break;
		case "Save":
			if(!parent.checkClose())return;

			tab_setAdjInputType(orgAuthPg, sheet3);

			var vsInsertCheck = sheet3.FindStatusRow("I|U");
			var insertArray    = vsInsertCheck.split(";");
			...
			if(vsInsertCheck != "" && vsInsertCheck != null){
				...
				for(var j = 0; j < insertArray.length; j++){
					...
					if(sheet3.GetCellValue(insertArray[j],"finance_org_cd")=="" || sheet3.GetCellValue(insertArray[j],"finance_org_cd")==null){
						alert("금융기관 코드를 입력 해주세요");
						return;

					}

					if(sheet3.GetCellValue(insertArray[j],"account_no")=="" || sheet3.GetCellValue(insertArray[j],"account_no")==null){
						alert("계좌번호를 입력 해주세요");
						return;
			...
			for(var i = sheet3.HeaderRows(); i <= sheet3.LastRow(); i++){
				tab_setAuthEdtitable(orgAuthPg, sheet3, i);
			}

			sheet3.DoSave("<%=jspPath%>/yeaData/yeaDataPenRst.jsp?cmd=saveYeaDataPen&orgAuthPg="+orgAuthPg);
			break;
		case "Insert":
			//개인별 총급여
	        var param = "searchWorkYy="+$("#searchWorkYy").val();
	...
	function sheet3_OnChange(Row, Col, Value, OldValue) {
		try{
			...
			if(sheet3.ColSaveName(Col) == "input_mon") {
				sheet3.SetCellValue(Row,"appl_mon", sheet3.GetCellValue(Row,Col)) ;
			}


			if(sheet3.ColSaveName(Col) == "saving_deduct_type"){

				if ( nvl(sheet3.GetCellValue(Row,"saving_deduct_type"), "") == "" ) {
                    return;
                }
	...
	function sheet3_OnClick(Row, Col, Value) {
		try{
			if(sheet3.ColSaveName(Col) == "sDelete" ) {
				tab_clickDelete(sheet3, Row);

				/* 금액 및 국세청 자료여부 Editable 풀림 방지*/
				if( sheet3.GetCellValue(Row, "adj_input_type") == "07" ) {
					sheet3.SetCellEditable(Row, "input_mon", 0);
					sheet3.SetCellEditable(Row, "appl_mon", 0);
					sheet3.SetCellEditable(Row, "nts_yn", 0);
					sheet3.SetCellEditable(Row, "reg_dt", 0);
					sheet3.SetCellEditable(Row, "mth_per", 0);
				}
			}
		}catch(ex){
			alert("OnClick Event Error : " + ex);
...
<div class="wrapper">
	...
	<table class="default line outer" style="margin-top:10px !important;">
	    ...
	    <tr>
	        ...
	        <th class="center" rowspan="2" >중소기업창업투자조합<br>(2025.12.31까지 출자금액)</th>
	        <th class="center" rowspan="2" >청년형장기집합투자증권저축<br></th>
	    </tr>
	    <tr>
	    ...
	    <tr>
	        ...
	        <td class="right">
	            <input id="A100_40" name="A100_40" type="text" class="text w50p right transparent" readOnly /> 원
	        </td>
	        <td class="right">
	            <input id="A100_03" name="A100_03" type="text" class="text w50p right transparent" readOnly /> 원
	        </td>	        
	        <td class="right">
	            <input id="A100_80" name="A100_80" type="text" class="text w50p right transparent" readOnly /> 원
	        </td>
	        <td class="right">
	            <input id="A100_41" name="A100_41" type="text" class="text w50p right transparent" readOnly /> 원
	        </td>
	    </tr>
	</table>
        ...
        <ul>
            <li class="txt">주택마련저축
                <span id="paytotMonViewYn2">
                	<a href="javascript:paytotMonView('2');" class="basic btn-red-outline authA">총급여 7천만원 초과여부</a>
                </span>
                <span id="span_paytotMonView2" ></span>
                <!-- <a href="javascript:yeaDataExpPopup('주택자금', helpText, 570)" class="cute_gray authA">주택마련저축 안내</a> -->
                <!-- <a href = "#" class="cute_gray" id="cute_gray_authR" >주택마련저축 안내</a> -->
                <a href="#layerPopup" class="basic btn-white ico-question" id="InfoPlus1"><b>주택마련저축 안내+</b></a>
	            <a href="#layerPopup" class="basic btn-white ico-question" id="InfoMinus1" style="display:none"><b>주택마련저축 안내-</b></a>
               	<font class="red" style="font-size: 12px;">주택정보 ▶ <span id="houseInfo"></span></font>
            </li>
            <li class="btn">
...
<%} %>
    ...
    <div class="sheet_title outer">
        <ul>
            <li class="txt">개인연금저축 / 장기집합투자증권저축 / 중소기업창업투자조합
            	<span id="paytotMonViewYn1">
                	<a href="javascript:paytotMonView('1');" class="basic btn-red-outline authA">총급여 8천만원 초과여부</a>
                </span>
                ...
                <!-- <a href = "#" class="cute_gray" id="cute_gray_authR" >개인연금/장기집합저축 안내</a> -->
                <a href="#layerPopup" class="basic btn-white ico-question" id="InfoPlus2"><b>개인연금/장기집합저축/중소기업창업투자조합 안내+</b></a>
	            <a href="#layerPopup" class="basic btn-white ico-question" id="InfoMinus2" style="display:none"><b>개인연금/장기집합저축 안내-</b></a>
            </li>
            <li class="btn">
            <strong class='blue'>중소기업창업투자조합 과세연도 변경시 "출력 탭>소득공제변경신청서" 참조</strong>
    ...
    <div class="sheet_title outer">
        <ul>
            <li class="txt">청년형 장기집합투자증권저축 소득공제
            	<span id="paytotMonViewYn3">
                	<a href="javascript:paytotMonView('3');" class="basic btn-red-outline authA">총급여 8천만원 초과여부</a>
                </span>
                <span id="span_paytotMonView3" ></span>
                <!-- <a href="javascript:yeaDataExpPopup('개인연금/장기집합저축', helpText2, 500)" class="cute_gray authA">개인연금/장기집합저축 안내</a> -->
                <!-- <a href = "#" class="cute_gray" id="cute_gray_authR" >개인연금/장기집합저축 안내</a> -->
                <a href="#layerPopup" class="basic btn-white ico-question" id="InfoPlus3"><b>청년형 장기집합투자증권저축 안내+</b></a>
	            <a href="#layerPopup" class="basic btn-white ico-question" id="InfoMinus3" style="display:none"><b>개인연금/장기집합저축 안내-</b></a>

            </li>
            <li class="btn">
            <strong class='blue'>"계약기간" 란은 계약기간을 개월 수로 적습니다.(월수 계산시 1월 미만은 1월로 합니다.)</strong>
	...
	<!-- Sample Ex&Image End -->
    <div style="height:200px">
    <script type="text/javascript">createIBSheet("sheet3", "100%", "200px"); </script>
    </div>
