...
<script type="text/javascript">
	...
	var helpText;  //도움말 - 소기업소상공인 공제부금 안내
    var helpText2; //도움말 - 창업투자조합 출자 등 소득공제 안내    
    var helpText3; //도움말 - 우리사주조합출연금 안내 
    var helpText4; //도움말 - 고용유지중소기업근로자 안내 
	...
	$(function() {
		...
		<%//2024년 추가 start %>
		$("#A100_90").mask('000,000,000,000,000', {reverse: true});
		$("#A100_91").mask('000,000,000,000,000', {reverse: true});
		$("#A100_92").mask('000,000,000,000,000', {reverse: true});
		<%//2024년 추가 end %>
	...
	function initDefaultData() {
		...
		param1 += "&queryId=getYeaDataHelpTextSub";
		
	    //도움말 - 창업투자조합 출자 등 소득공제 안내        
        var param2 = "&adjProcessCd=A100" // A100(그 밖의 소득공제) //TSYS005.NOTE2 = 프로세스 코드 
                    + "&helpGrpCd=A100_10" // 10(투자조합출자공제 안내) // TSYS005.NOTE3 = 도움말 그룹 (MEMO 4000 바이트가 모자를 경우 분리해서 저장)
                    ;
        var result1 = ajaxCall("<%=jspPath%>/common/commonSelect.jsp?cmd=commonSelectMap",param1+param2,false);
        helpText2 = nvl(result1.Data.help_text1,"") + nvl(result1.Data.help_text2,"") + nvl(result1.Data.help_text3,"");

        //도움말 - 우리사주조합출연금 안내        
        param2 = "&adjProcessCd=A100" // A100(그 밖의 소득공제) //TSYS005.NOTE2 = 프로세스 코드 
                    + "&helpGrpCd=A100_20" // 10(우리사주조합출연금공제 안내) // TSYS005.NOTE3 = 도움말 그룹 (MEMO 4000 바이트가 모자를 경우 분리해서 저장)
                    ;
        var result1 = ajaxCall("<%=jspPath%>/common/commonSelect.jsp?cmd=commonSelectMap",param1+param2,false);
        helpText3 = nvl(result1.Data.help_text1,"") + nvl(result1.Data.help_text2,"") + nvl(result1.Data.help_text3,"");

        //도움말 - 고용유지중소기업근로자 안내        
        param2 = "&adjProcessCd=A100" // A100(그 밖의 소득공제) //TSYS005.NOTE2 = 프로세스 코드 
                    + "&helpGrpCd=A100_30" // 10(고용유지중소기업근로자 소득공제) // TSYS005.NOTE3 = 도움말 그룹 (MEMO 4000 바이트가 모자를 경우 분리해서 저장)
                    ;
        var result1 = ajaxCall("<%=jspPath%>/common/commonSelect.jsp?cmd=commonSelectMap",param1+param2,false);
        helpText4 = nvl(result1.Data.help_text1,"") + nvl(result1.Data.help_text2,"") + nvl(result1.Data.help_text3,"");
        

        param2 = "searchWorkYy="+$("#searchWorkYy").val();
        ...
        yeaMonShowYn = nvl(result4[0].code_nm,"");
      	...
      	//안내메세지 1
        $("#infoLayer").html(helpText).hide(); $('#infoLayer').css('top', '10px');
      	//안내메세지 2
        $("#infoLayer2").html(helpText2).hide(); //$('#infoLayer2').css('top', '100px'); //디폴트
      	//안내메세지 3
        $("#infoLayer3").html(helpText3).hide(); $('#infoLayer3').css('top', '440px');
      	//안내메세지 4
        $("#infoLayer4").html(helpText4).hide(); $('#infoLayer4').css('top', '480px');
	}
	...
	function sheetSet(){
		...
		if(comSheet.RowCount() > 0){
			...
			<%//2024년 추가 start %>
			$("#A100_90").val(comSheet.GetCellText(comSheet.FindText("adj_element_cd", "A100_90") > 0 ? comSheet.FindText("adj_element_cd", "A100_90") : "","input_mon"));
			$("#A100_91").val(comSheet.GetCellText(comSheet.FindText("adj_element_cd", "A100_91") > 0 ? comSheet.FindText("adj_element_cd", "A100_91") : "","input_mon"));
			$("#A100_92").val(comSheet.GetCellText(comSheet.FindText("adj_element_cd", "A100_92") > 0 ? comSheet.FindText("adj_element_cd", "A100_92") : "","input_mon"));
			<%//2024년 추가 end %>
	...
	function saveCommonData(){
		...
		if(comSheet.RowCount() == 0) {
			...
			<%//2024년 추가 start %>
			parent.doInsertCommonSheet("A100_90",$("#A100_90").val());
			parent.doInsertCommonSheet("A100_91",$("#A100_91").val());
			parent.doInsertCommonSheet("A100_92",$("#A100_92").val());
			<%//2024년 추가 end %>
...
<div class="wrapper">
	...
	<tr>
		...
		<td class="left">
			...
			&nbsp;공제금액 : 공제한도 내의 부금 납부액 * (1 - 부동산임대업 소득금액/사업소득금액)
		</td>
	</tr>
	</table>
...
<!--             &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;5천만원 이하분은 50%, 5천만원 초과분은 30% <br> -->
	...
	<%--<tr>
        ...
        <td class="left" rowspan="9" style="vertical-align: top;">
            &nbsp;<font class='blue'>[자료등록>저축 탭] 에서 등록 (벤처등:직접, 조합1/조합2:간접)</font><br>
            &nbsp;거주자가 중소기업창업투자조합 등에 2024년 12월 31일까지 출자 또는 투자한 금액<br>
            &nbsp;그 출자일 또는 투자일이 속하는 과세연도부터 2년이 되는 날이 속하는<br>
            &nbsp;과세연도까지 거주자가 선택하는 1과세연도의 종합소득금액에서 공제<br>
            &nbsp;공제한도 : Min[ㄱ+ㄴ+ㄷ, ㄹ]<br>
            &nbsp;ㄱ. 조합1 출자 또는 투자한 금액 * 10% <br>
	...
	<tr>
		...
		<td class="left" rowspan="9" style="vertical-align: top;">
			...
			&nbsp;ㄴ. Min[조합2 출자 또는 투자한 금액, 3000만원] * 10% <br>
			&nbsp;ㄷ. 벤처등 출자 또는 투자한 금액 <br>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2022년 이후분 ： 개인이 직접 또는 개인투자조합을 통해 벤처기업에 투자하는 경우 <br>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;3천만원 이하분은 100%, 5천만원 이하분은 70%, 5천만원 초과분은 30% <br>
			&nbsp;ㄹ. 해당년도 종합소득금액 * 50% <br>
		</td>
    </tr>
	...
	<tr>
		...
		<th class="left">조합1</th>
		<td class="right">
			<input id="A100_90" name="A100_90" type="text" class="text w90 right" readonly disabled/> 원
		</td>
	</tr>
	<tr>
		<th class="left">조합2</th>
		<td class="right">
			<input id="A100_91" name="A100_91" type="text" class="text w90 right" readonly disabled/> 원
		</td>
	</tr>
	<tr>
		<th class="center">직접출자</th>
		<th class="left">벤처등</th>
		<td class="right">
			<input id="A100_92" name="A100_92" type="text" class="text w90 right" readonly disabled/> 원
		</td>
	</tr>
	<%// 2024년 추가 End %>
	</table>
	...
	<tr>
		...
		<td class="left" rowspan="3">
			...
			&nbsp;연간 임금총액 고용유지중소기업에 근로를 제공하는 상시근로자에 대하여 2024년 12월 31일이<br>
