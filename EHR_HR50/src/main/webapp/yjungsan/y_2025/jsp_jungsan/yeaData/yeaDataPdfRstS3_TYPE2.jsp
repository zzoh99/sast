...
public List selectYeaDataPdfInfo(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectYeaDataPdfInfo",pm);
	} catch (Exception e) {
...
//pdf 파일 조회
public List selectYeaDataPdfList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
public Map selectYeaDataPdfFormCdCount(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		mp  = (queryMap==null) ? null : DBConn.executeQueryMap(queryMap, "selectYeaDataPdfFormCdCount", pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
	} finally {
...
public List selectYeaDataPdfA1(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectYeaDataPdfA1",pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
	} finally {
...
public List selectPdfTypeCode(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectPdfTypeCode",pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
	} finally {
...
public List selectBulkPdf(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		bulkPdf  = (queryMap==null) ? null : DBConn.executeQueryMap(queryMap,"selectBulkPdfYn",pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
	} finally {
		if (bulkPdf != null) {
			...
			if ("Y".equals(bulkPdfYn)) {
	        	listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectBulkPdf",pm);
	        } else listData = null;
		}
		queryMap = null;
	}
...
public List selectConfirmedYn(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		list  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectConfirmedYn",pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
	} finally {
...
<%
	...
	if("selectYeaDataPdfList".equals(cmd)) {
		...
		try {
			listData = selectYeaDataPdfList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
	    ...
	    //PDF 파일 삭제
	    //HMM S3 TYPE2에서 파일 삭제 시, EHR 내 데이터만 삭제 합니다.
        Map mp = StringUtil.getRequestMap(request);
		...
		rstMap.put("Data", (List)listData);
		out.print((new org.json.JSONObject(rstMap)).toString());
	} else if("downloadPdf".equals(cmd)){
	 	Log.Debug("============== [ PDF등록 파일 다운로드 시작] ===============");
		String fileName = request.getParameter("fileName").toString();					
		int pos = fileName.lastIndexOf(".");
		String fileId = fileName.substring(0, pos);
		String filePath = "/yjungsan/pdf/s3/type2";
		String url = "";
		String message = "";
		String code = "1";
		
		try{				
        	downloadToPdfS3(filePath, fileName, fileId, response);	        	
        } catch(Exception e){
        	code = "-1";
        	message = "파일다운로드중 오류가 발생하였습니다.";
        	out.println("<script>alert('파일다운로드중 오류가 발생하였습니다.');</script>");
        }
		Log.Debug("============== [ PDF등록 파일 다운로드 종료] ===============");
	}
%>