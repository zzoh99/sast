...
<div id="progressCover" style="display:none;position:absolute;top:0;bottom:0;left:0;right:0;background:url(<%=imagePath%>/common/process.png) no-repeat 50% 50%; z-index:99;"></div>
	<div class="wrapper">
		...
		<div id="subSheet" style="overflow-y: auto; height: auto;">
			<!-- 담당자피드백 Start -->
...
<script type="text/javascript">
	...
	function sheet2_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			for(var i = 1; i < sheet2.RowCount()+1; i++){
				var chkType = sheet2.GetCellValue(i, "chk_type");
				if(chkType == "E"){
					sheet2.SetRowBackColor(i, "#FFFFCC");
					sheet2.SetCellFontColor(i, "chk_gubun", "#EE0000");	//구분
					sheet2.SetCellFontColor(i, "chk_type", "#EE0000");	//결과유형
				}

				var chkGubunC = sheet2.GetCellValue(i, "chk_gubun_c");
				if (chkGubunC != "") {
					sheet2.SetCellValue(i,"btn_link","<a href=\"javascript:moveTabIdx('"+chkGubunC+"')\" class='basic btn-white'>바로가기</a>");
				}
			}
	...
	function moveTabIdx(gubunCd) {
		    ...
		    case "TAX_": iframeIdx = 13; break;
		}

	    var yeaDefaultInfo = getYearDefaultInfoObj();
		if(yeaDefaultInfo.Data.apprv_yn == "Y") {
			; // 담당자 입력 마감이면, 확정하라는 메시지 생략 20231115
		} else {
			if(iframeIdx > 1){
			    if($("#inputStatus").val() == "00") {
					alert('기본 주소사항과 인적공제 현황을 확정 하시기 바랍니다.');
			        return doAction1("Search");
				}else if($("#inputStatus").val().substring(1,2) == "0"){
		...
		if(iframeIdx == 0) { newIframe.attr("src","<%=jspPath%>/yeaData/yeaDataAddr.jsp?authPg="+authPg+"&orgAuthPg="+orgAuthPg);
			...
			if( //IE 7...
			 ...
			 || //IE 11,10,9,8
			    ( trident != null && ( trident[1] == "7.0" || trident[1] == "6.0" || trident[1] == "5.0" || trident[1] == "4.0" ) ) ) 
		    {
...
<script type="text/javascript">
	...
	$(function() {
		...
		var feedBack = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_FEEDBACK_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
		...
		//관리자권한이 true 이거나 직원용 권한이 true 이면 FeedBack 버튼을 보여준다.
		var totalPay = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_MON_SHOW_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
	...
	function createTabFrame() {
		...
		//pdf등록 탭 출력유무
		var pdfTab = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_PDF_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
		var pdfTabHide = "";
		...
		//연간소득, 종전근무지 탭 출력유무
		var incomeEachTab = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_PAYTOT_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
		...
		var befComTab = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_PREWORK_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
		var befComTabHide = "";
		...
		//모의계산,결과 권한 조회
		var simYn = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_SIM_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
		var simYnHide ="";
		...
		//증빙자료탭 사용 여부
		var addfileTab = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEA_ADD_FILE_YN&searchWorkYy="+$("#searchWorkYy").val(), "queryId=getYeaSystemStdData",false).codeList;
		var addfileTabHide ="";
	...
	function doAction(sAction) {
		...
		case "InputClose":
            ...
            if($("#inputStatus").val().substring(0,1) == "0"){
                alert("인적공제 현황을 확정 하시기 바랍니다.");
				return;
            }
...
$(function() {
		...
		initdata2.Cols = [
			...
			{Header:"구분코드",	Type:"Text",		Hidden:1,	Width:60,	Align:"Center",	ColMerge:0,	SaveName:"chk_gubun_c",	KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:100 },
			...
			{Header:"바로가기",   	Type:"Html",     	Hidden:0,  	Width:60,  Align:"Center",    	ColMerge:1, SaveName:"btn_link",   KeyField:0,   CalcLogic:"",   Format:"",     PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:1000, MultiLineText:1, Cursor:"pointer" }

        ];IBS_InitSheet(sheet2, initdata2); sheet2.SetEditable(true); sheet2.SetCountPosition(4);
    	...
    	sheet2.SetColProperty("chk_type", {ComboText: "|" + chkTypeList[0], ComboCode: "|" + chkTypeList[1]} );
...
function doAction2(sAction) {
	switch (sAction) {
	case "Search": 		//조회
		sheet2.DoSearch( "<%=jspPath%>/yeaData/yeaDataRst.jsp?cmd=selectErrChkPerMgr", $("#mainForm").serialize());
		break;
    case "Down2Excel":
