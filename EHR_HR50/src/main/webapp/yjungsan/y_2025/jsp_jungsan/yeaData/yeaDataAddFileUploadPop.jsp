...
//<!--
	...
	$(function(){
		...
		//업로드 파일사이즈 설정 조회
		limitFileSize = ajaxCall("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&searchStdCd=CPN_YEAREND_LIMIT_SIZE&searchWorkYy="+$("#searchWorkYy").val(), 'queryId=getYeaSystemStdData',false).codeList[0].code_nm;
		//파일업로드 type 조회
		...
		if(exeYn == 'N') {
		    ...
		    if(fileCnt > 10){
				...
				if($("#fileUploadType").val() == '1'){//S3					
					...
					$("#uploadForm").attr({"method":"POST","target":"ifrmFileUpload","action":"yeaDataAddFileUploadPopRstS3_TYPE2.jsp"}).submit();
				}
				else{
