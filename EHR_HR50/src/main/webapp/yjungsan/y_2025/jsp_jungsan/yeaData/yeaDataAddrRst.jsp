...
// 과거 결혼세액공제 적용 여부
public Map checkWedTaxDedPrevious(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		mapData  = (queryMap==null) ? null : DBConn.executeQueryMap(queryMap,"checkWedTaxDedPrevious",pm);
		saveLog(null, pm, ssnYeaLogYn);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
<%
	...
	if("selectYeaDataAddrList".equals(cmd)) {
        ...
        rstMap.put("Data", (Map)mapData);
        out.print((new org.json.JSONObject(rstMap)).toString());
    } else if("checkWedTaxDedPrevious".equals(cmd)) {
		//자료등록 필수기입사항 msg
		Map mp = StringUtil.getRequestMap(request);
		mp.put("ssnEnterCd", ssnEnterCd);
		mp.put("ssnSabun", ssnSabun);
		...
		try {
			mapData = checkWedTaxDedPrevious(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
