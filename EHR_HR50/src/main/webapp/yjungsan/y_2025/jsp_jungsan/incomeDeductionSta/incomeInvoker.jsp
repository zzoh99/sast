...
private Map getRecentFile(String pWorkYy, String report_type, String tmpSabun, String ssnEnterCd) throws Exception {
	// 시큐어코딩으로 인하여 전달받은 파라미터 XSS체크를 매번 수행 20240712
   	...
   	String serverFilePath = StringUtil.getPropertiesValue("WAS.PATH") + StringUtil.getPropertiesValue("HRFILE.PATH") 
                               + "/YEA/income/" + ssnEnterCd + "/" +  workYy + "/" + sabun ;
...
private Map CheckDisk(String pWorkYy, String report_type, String sabunCnt, String printOption, String tmpSabun, String ssnEnterCd) throws Exception {
	// 시큐어코딩으로 인하여 전달받은 파라미터 XSS체크를 매번 수행 20240712
   	String workYy = pWorkYy ;
   	String sabun  = tmpSabun;
	...
	try {
	    ...
	    long totFile = Integer.parseInt(sabunCnt) * 250;
...
//파일 디렉토리 생성
private void fileDir(String pWorkYy, String report_type, String tmpSabun, String ssnEnterCd) throws Exception {
	// 시큐어코딩으로 인하여 전달받은 파라미터 XSS체크를 매번 수행 20240712
   	String workYy = pWorkYy ;
   	String sabun  = tmpSabun;
...
private Map cntChk(String pWorkYy, String report_type, String tmpSabun,String ssnEnterCd) throws Exception {
	// 시큐어코딩으로 인하여 전달받은 파라미터 XSS체크를 매번 수행 20240712
   	String workYy = pWorkYy ;
   	String sabun  = tmpSabun;
...
String report_type = String.valueOf(request.getParameter("report_type"));
...
String serverFilePath = StringUtil.getPropertiesValue("WAS.PATH") + StringUtil.getPropertiesValue("HRFILE.PATH") 
						+ "/YEA/income/" + ssnEnterCd + "/" +workYy + "/" + tmpSabun ;
...
if("onload".equals(cmd)) {
	...
	if(rstMap.get("fileExist") == "Y") {
		try {
			rstMap = getRecentFile(workYy, report_type, tmpSabun , ssnEnterCd);
		} catch(Exception e) {
			throw new Exception("파일 정보를 가져오는 중에 오류가 발생했습니다: " + e.getMessage());
		}
	}
	...
	fileDir(workYy, report_type, tmpSabun,ssnEnterCd);
	
	Map rstMap  = new HashMap();
	
	try {
		rstMap = CheckDisk(workYy, report_type, sabunCnt, printOption, tmpSabun,ssnEnterCd);
	} catch(Exception e) {
		throw new Exception("파일 정보를 가져오는 중에 오류가 발생했습니다: " + e.getMessage());
	}
	...
	try {
		rstMap = cntChk(workYy, report_type, tmpSabun,ssnEnterCd);
	} catch(Exception e) {
		throw new Exception("파일 정보를 가져오는 중에 오류가 발생했습니다: " + e.getMessage());
	}
	...
	for(int i=1; i<17; i++) {
		...
		if (!"".equals(rdParam1) && rdParam1!=null) {			
			Log.Debug("rdParam1" + Integer.toString(i) + "============== " + URLDecoder.decode(rdParam1) );
			// 개별파일 생성이면서 파라미터가 사번이면..
			if ( "N".equals(printOption) && i==4 ) {
				rdParam2 += "[&#'사번'&#]"; // ''로 엮인 놈 
			} else if ( "N".equals(printOption) && (i==7 &&   "INCOME".equals(report_type))	) {	 // 소득공제서
				rdParam2 += "[&#사번&#]";   // ''로 엮이지 않은 놈
			} else if ( "N".equals(printOption) && (i==8 && !("INCOME".equals(report_type)))) {	 // 신용카드등, 기부금명세서, 의료비명세서
				rdParam2 += "[&#사번&#]";   // ''로 엮이지 않은 놈
			} else if ( "N".equals(printOption) && (i==8 &&   "INCOME".equals(report_type))	) {	 // 소득공제서
				rdParam2 += "[1]";   // 정렬순서
			} else { 
	...
	if("Y".equals(printOption)) {
		...
		//invoker.addParameter("mrd_param", rdParam1);
		creFileName += ".pdf";
		Log.Debug(creFileName);
		Log.Debug(rdParam1);
		...
		for (int i = 0; i < splitSabun.length; i++) {
			...
			String rdParam01 = URLDecoder.decode(request.getParameter("rdParam1"));
			String rdParam03 = URLDecoder.decode(request.getParameter("rdParam3"));
			...
			if (rdParam1 != null && !"".equals(rdParam1) && splitSabun[i] != null && !"".equals(splitSabun[i]) && splitName[i] != null && !"".equals(splitName[i])) {
				...
				invoker.addParameter("mrd_param", "/rfn ["+rdUrl+"/DataServer/rdagent.jsp] /rsn ["+rdAliasName+"] /rreportopt [256] /rp "+rdParam01+"['"+splitSabun[i]+"']"+rdParam03+" /rv securityKey[]  sKey[null]  gKey[null]  sType[null]  qId[null]  themKey[''] ");
			}
