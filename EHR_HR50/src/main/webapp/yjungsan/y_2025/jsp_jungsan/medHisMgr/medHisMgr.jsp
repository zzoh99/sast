...
<script type="text/javascript">
	...
	$(function() {
		...
		var initdata = {};
		initdata.Cfg = {FrozenCol:0,SearchMode:smServerPaging,Page:<%=defaultRow%>,MergeSheet:msHeaderOnly};
		...
		initdata.Cols = [
			...
			{Header:"대상년도",			Type:"Text",		Hidden:1, Width:60,	Align:"Center",	ColMerge:1,		SaveName:"work_yy",			KeyField:1, Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:4, DefaultValue:"<%=yeaYear%>" },
			...
			{Header:"정산구분",			Type:"Text",		Hidden:1, Width:160,	Align:"Center",	ColMerge:1,		SaveName:"adjust_type_hid",		KeyField:0, Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:10 },
			...
			{Header:"구분",				Type:"Text",		Hidden:1,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:"special_yn_hid",		KeyField:0,	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:10 },
			...
			{Header:"의료비\n증빙코드",		Type:"Text",		Hidden:1,	Width:90,	Align:"Left",	ColMerge:1,	SaveName:"medical_imp_cd_hid",	KeyField:0,	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:10 },
			...
			{Header:"상세구분",			Type:"Text",		Hidden:1,	Width:90,	Align:"Center",	ColMerge:1,	SaveName:"restrict_cd_hid",		KeyField:0,	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:10 },
			...
			{Header:"재가/시설급여",		Type:"Text",		Hidden:1,	Width:90,	Align:"Center",	ColMerge:1,	SaveName:"medical_type_hid",	KeyField:0,	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:10 },
			...
			{Header:"금액\n(담당자용)",	Type:"Float",		Hidden:0,	Width:90,	Align:"Right",	ColMerge:1,	SaveName:"appl_mon",		KeyField:1,	Format:"",		PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:35 },
			...
			{Header:"자료\n입력유형",		Type:"Text",		Hidden:1,	Width:60,	Align:"Right",	ColMerge:1,	SaveName:"adj_input_type_hid",	KeyField:0,	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:10 },
            ...
            <%// 20250502. 시트 7.0.0.0-20131223-17 버전에서는 SetColProperty 로 DefaultValue가 지원되지 않음. => initdata 세팅으로 조정
            // 20250418. OnLoadExcel 이벤트에서 RowCount 반복 수행으로 인한 성능저하. case "LoadExcel"의 디폴트 값 세팅으로 개선.%>
			{Header:"지급일자",				Type:"Text",	Hidden:1,	Width:60,	Align:"Center", ColMerge:1, SaveName:"ymd",				KeyField:0, Format:"",		PointCount:0,	UpdateEdit:1,	InsertEdit:1,	EditLen:8, DefaultValue:"<%=yeaYear%>1231" },
			...
			{Header:"미숙아ㆍ선천성이상아\n해당여부",   Type:"CheckBox",    Hidden:0,   Width:85,   Align:"Center", ColMerge:1, SaveName:"pre_baby_yn",        KeyField:0, Format:"",      PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:1, TrueValue:"Y", FalseValue:"N" },
			{Header:"난임시술비\n해당여부",   Type:"CheckBox",    Hidden:0,   Width:85,   Align:"Center", ColMerge:1, SaveName:"nanim_yn",        KeyField:0, Format:"",      PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:1, TrueValue:"Y", FalseValue:"N" },
            ...
            {Header:"담당자확인",			Type:"Text",	Hidden:1,	Width:80,	Align:"Center", ColMerge:1, SaveName:"feedback_type_hid",	KeyField:0, Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:10 },
		...
		// 전체조회 엑셀 다운로드를 위한 시트 생성
		IBS_InitSheet(sheet2, initdata);sheet1.SetVisible(false);
	...
	function doAction1(sAction) {
		switch (sAction) {
		case "Search":
			var totCnt = ajaxCall("<%=jspPath%>/medHisMgr/medHisMgrRst.jsp?cmd=selectMedHisMgrListCnt", $("#sheetForm").serialize(),false);
			var param = {"Param":"cmd=selectMedHisMgrList&totalCnt="+totCnt.Data[0].total_cnt+"&defaultRow=<%=defaultRow%>&"+$("#sheetForm").serialize()};
			sheet1.DoSearchPaging( "<%=jspPath%>/medHisMgr/medHisMgrRst.jsp", param );
			break;
        ...
        case "LoadExcel":
            ...
            //              adj_input_type => 엑셀 양식에서 key-in으로 조정 
            //              ymd         => initdata 세팅으로 조정 
            ...
            //sheet1.SetColProperty(0, "adj_input_type", { DefaultValue:  '02' } );
            //sheet1.SetColProperty(0, "ymd", { DefaultValue: $("#searchYear").val()+"1231" } );
    ...
    *************************************************************/
    function callDownFile(returnValue){
    	...
    	for(i = 0; i < colLength; i++) {
			if(sheet2.ColSaveName(i).substring(sheet2.ColSaveName(i).length-4 , sheet2.ColSaveName(i).length) == "_hid") {
				sheet2.SetColHidden(sheet2.ColSaveName(i).substring(0, sheet2.ColSaveName(i).length-4), 1);
				sheet2.SetColHidden(sheet2.ColSaveName(i), 0);
			}
		}

    	var downcol = makeHiddenSkipCol(sheet2);
		var param = {
			URL:"<%=jspPath%>/medHisMgr/medHisMgrRst.jsp"
			,ExtendParam:"&cmd=selectMedHisMgrListDown&" + $("#sheetForm").serialize()
			,FileName:"MedHisMgrList${curSysYyyyMMdd}.xls"
			, Merge:1 /*속도 문제가 있지만 크게 문제 없으므로 추가함*/
			, DownCols:downcol
		};
		sheet2.DirectDown2Excel(param);
		<%-- 
		sheet2.DoSearch( "<%=jspPath%>/medHisMgr/medHisMgrRst.jsp?cmd=selectMedHisMgrList", $("#sheetForm").serialize() );
		$("#loading").css("display", "");
		$("#mainSheet").css("opacity", 0.5);
		$("#mainSheet").css("pointer-events", "none"); --%>
    }
	...
	function sheet2_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			if (Code == 1) {
				...
				$("#mainSheet").css("opacity", 1);
				$("#mainSheet").css("pointer-events", "auto");
			}
		} catch(ex) {
			alert("OnSearchEnd Event Error : " + ex);
		}
...
<div class="wrapper">
	...
	<div class="outer">
		...
		<ul>
			...
			<li class="btn">
				<font class='blue'><%=defaultRow%>건 이상의 자료는 scroll을 할 때마다 <%=defaultRow%>건씩 추가 조회하여 append합니다.</font>
<!-- 				<a href="javascript:doAction1('Down2Excel')"	class="basic btn-download authR">다운로드/업데이트 양식다운로드</a> -->
				...
				<a href="javascript:doAction1('Down2Excel')"	class="basic btn-download authR">다운로드(조회된건)</a>
				<a href="javascript:doAction1('DirectDown2Excel')"	class="basic btn-download authR">다운로드(전체건)</a>
			</li>
		</ul>
		</div>
	</div>
	<div style="position: relative;">
		<div style="position: absolute; top: 40%; left: 40%; display: none;" id="loading">
			<img src="../../../common_jungsan/plugin/IBLeaders/Sheet/css/image/loading.gif" />
		</div>
		<div id= "mainSheet">
