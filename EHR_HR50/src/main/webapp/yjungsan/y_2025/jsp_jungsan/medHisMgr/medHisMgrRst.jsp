...
<%!
public List selectMedHisMgrListCnt(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectMedHisMgrListCnt",pm);
	} catch (Exception e) {
...
public List selectMedHisMgrList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	String searchIbpage          = pm.get("ibpage") == null ? "" : String.valueOf(pm.get("ibpage"));
	String searchDefaultRow      = pm.get("defaultRow") == null ? "" : String.valueOf(pm.get("defaultRow"));
	
	StringBuffer query = new StringBuffer();
	
	query.setLength(0);
	if(!"".equals(searchIbpage)) { query.append(" AND B.RN BETWEEN ((TO_NUMBER("+searchIbpage+")-1)*TO_NUMBER("+searchDefaultRow+"))+1 AND ( (TO_NUMBER("+searchIbpage+")-1)*TO_NUMBER("+searchDefaultRow+") ) + TO_NUMBER("+searchDefaultRow+")" ); }
	
	pm.put("query1", query.toString());
...
<%
	...
	if("selectMedHisMgrListCnt".equals(cmd) || "selectMedHisMgrList".equals(cmd) || "selectMedHisMgrListDown".equals(cmd)) {
		//의료비 내역 조회
		...
		try {
			if("selectMedHisMgrListCnt".equals(cmd)) {
				listData = selectMedHisMgrListCnt(mp, locPath);	
			} else {
				listData = selectMedHisMgrList(mp, locPath, ssnYeaLogYn);
			}
			
			if("selectMedHisMgrListDown".equals(cmd)) {
				// 데이터를 반드시 SHEETDATA라는 이름으로 담는다.
				request.setAttribute("SHEETDATA", listData);
				RequestDispatcher rd = request.getRequestDispatcher(jspPath+"/common/include/yeaDirectDown2Excel.jsp");
				rd.forward(request,response);
	            return; // 여기서 명확히 종료시켜야 함
			}
		...
		if(!"selectMedHisMgrListDown".equals(cmd)) {
			...
			if("selectMedHisMgrList".equals(cmd)) {
				rstMap.put("TOTAL", (String)request.getParameter("totalCnt"));
			}
