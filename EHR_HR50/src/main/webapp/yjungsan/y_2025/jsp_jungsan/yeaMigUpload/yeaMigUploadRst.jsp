...
public List selectHeaderList(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		list = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectHeaderList",pm);
	} catch (Exception e) {
...
;	String searchTable  = "TYEA00";
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"getColDataList",pm);
...
public List selectMigInfoMgrList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	if ( list != null && !list.isEmpty() ){
		for ( int i=0; i<list.size(); i++){
			if (list.get(i) != null) {
				...
				if (header_col_nm != null) {
					column_nm_arr = header_col_nm.split("\\|");
					
					if ( column_nm_arr != null && column_nm_arr.length > 0 ){
	...
	pm.put("InfoColumn1"  , InfoColumn1.toString().trim().replaceAll(",$", "")); // 20250414. ORA-00936: 누락된 표현식 오류 수정. 문자열 끝에 있는 쉼표 제거. [데이터이관 > 이관기준관리]가 등록되지 않은 년도일 경우라도 오류는 뜨지 않아야...
	
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectMigInfoMgrList",pm);
		saveLog(null, pm, ssnYeaLogYn);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
public List selectMigCurWpMgrList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	pm.put("curWpColumn1"  , curWpColumn1.toString().trim().replaceAll(",$", "")); // 20250414. ORA-00936: 누락된 표현식 오류 수정. 문자열 끝에 있는 쉼표 제거. [데이터이관 > 이관기준관리]가 등록되지 않은 년도일 경우라도 오류는 뜨지 않아야...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectMigCurWpMgrList",pm);
		saveLog(null, pm, ssnYeaLogYn);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
public List selectIncomeDdctList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	pm.put("incomeDdctColumn"  , incomeDdctColumn.toString().trim().replaceAll(",$", "")); // 20250414. ORA-00936: 누락된 표현식 오류 수정. 문자열 끝에 있는 쉼표 제거. [데이터이관 > 이관기준관리]가 등록되지 않은 년도일 경우라도 오류는 뜨지 않아야...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectIncomeDdctList",pm);
		saveLog(null, pm, ssnYeaLogYn);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
public List selectTaxReductDdctList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	pm.put("taxRdtDdctColumn"  , taxRdtDdctColumn.toString().trim().replaceAll(",$", "")); // 20250414. ORA-00936: 누락된 표현식 오류 수정. 문자열 끝에 있는 쉼표 제거. [데이터이관 > 이관기준관리]가 등록되지 않은 년도일 경우라도 오류는 뜨지 않아야...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectTaxReductDdctList",pm);
		saveLog(null, pm, ssnYeaLogYn);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
//System.out.println("headerListData : " + headerListData);
	if ( headerListData != null && !headerListData.isEmpty() ){
