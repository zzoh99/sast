	...
	if("selectCardHisMgrList".equals(cmd)) {
		...
		try {
			...
			for(int i = 0; i < listMap.size(); i++) {
				...
				/* 주석 처리) 동일 사용처에서 여러번 소비를 하고 N건으로 수기 등록할 수 있기 때문에				
				if("I".equals(sStatus) || "U".equals(sStatus) ) { // 입력,수정일 경우는 중복값 사전 체크 20241205
					...
					if(dupMap != null && Integer.parseInt((String)dupMap.get("cnt")) > 0 ) {
						...
						grcdPmap.put("grCd", "C00305");
						grcdPmap.put("code", String.valueOf(data.get("card_type")));
						grcdPmap.put("searchYear", String.valueOf(data.get("work_yy")));
						Map cdNmMap = DBConn.executeQueryMap(queryMap,"getCommonCodeNm",grcdPmap);
						
						errMsg = "\n - 사번 : " + String.valueOf(data.get("sabun")) 
				               + "\n - 사용연도 : " + String.valueOf(data.get("use_yyyy"))
					           + "\n - 명의인주민번호 : " + String.valueOf(data.get("famres"))
					           + "\n - 카드구분 : " + String.valueOf(cdNmMap.get("code_nm"))
					           + "\n - 카드명 : " + String.valueOf(data.get("card_enter_nm"))
					           + "\n - 금액 : " + String.valueOf(data.get("appl_mon"))
						       ;

						throw new UserException("중복된 신용카드 자료가 " + dupMap.get("cnt") + "건 이상 존재합니다.\n조정 후 다시 진행하십시오." + errMsg);
					}
				} */
