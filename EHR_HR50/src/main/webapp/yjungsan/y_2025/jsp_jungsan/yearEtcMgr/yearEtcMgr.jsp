...
<script type="text/javascript">
	...
	$(function() {
		...
		initdata.Cols = [
	        ...
	        {Header:"년도",		Type:"Text",      	Hidden:0,  Width:50,    Align:"Center", ColMerge:1,   SaveName:"work_yy",			KeyField:0,   CalcLogic:"",   Format:"",        PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:4, DefaultValue:"<%=yeaYear%>" },
    ...
    //20250502 DOM 변경 직후 비동기 연산으로 인해 진행률 렌더링이 생략되는 이슈가 있어서, 비동기 루프(async/await)로 처리 (for await 기반으로 순차 처리하면, 진행률 업데이트 후 브라우저가 DOM 그릴 기회를 가질 수 있음)
    async function checkValidnSave() {
        sheet1.ShowProcessDlg("Save"); // 저장중 대기이미지를 표시 한다..
        await new Promise(resolve => setTimeout(resolve, 1)); // 렌더링 시간 확보 (0.001초만 줘도 충분)
        
        if(getBrowser() != "IE") {
            $(".GMProcess").append('<p id="txt_progress" style="font-size:13px; background-color:#fff; color:#333; font-weight:bold;">Checking validity..</p>');
            await new Promise(resolve => setTimeout(resolve, 1)); // 렌더링 시간 확보 (0.001초만 줘도 충분)
        }
        ...
        for(var i=1; i <= sheet1.LastRow(); i++) {
            ...
            {
                if(sheet1.GetCellValue(i, "adj_element_cd") == "C010_156" || sheet1.GetCellValue(i, "adj_element_cd") == "C010_157")
                {
                    alert("출산지원금(Q03, Q04) 관련정보는(" + i + "번째 행 등) [출산지원금내역관리]화면에서 입력해 주시길 바랍니다.");
                    $(".GMProcess").html("");
                    sheet1.HideProcessDlg();
                    return false;
                }
            ...
            if (i < sheet1.LastRow())
            {
                ...
                if (getBrowser() != "IE") {
                    if ($(".GMProcess #txt_progress").length > 0) {
                        $(".GMProcess #txt_progress").text(i + "/" + sheet1.LastRow() + " Passed validation...");
                    }
                }
            ...
            //console.log("Passed validation", i + "/" + sheet1.LastRow() + " Passed");
        }
        ...
        // 중복체크
        if (!dupChk(sheet1, "work_yy|adjust_type|sabun|adj_element_cd|ym", true, true)) { return false; }
	...
	function doAction1(sAction) {
       ...
       case "Down2Template":
           //var downcol = makeHiddenSkipCol(sheet1);
           var downcol = "adjust_type"
                            +"|sabun|adj_element_cd|ym|mon|memo"; //notax_mon, tax_mon 시스템이 처리함
	...
	function sheet1_OnChange(Row, Col, Value, OldValue) {
		try{
            ...
            if(sheet1.ColSaveName(Col) == "mon"||sheet1.ColSaveName(Col) == "sDelete"){
                if(sheet1.ColSaveName(Col) == "sDelete" || OldValue == 'D') {
                    return;
                }
