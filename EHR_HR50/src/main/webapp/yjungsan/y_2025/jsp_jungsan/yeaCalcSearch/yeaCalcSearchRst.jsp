...
public List selectYeaReSeqList(Map paramMap, String locPath) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		list  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectYeaReSeqList",pm);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
	} finally {
...
<%
	...
	} else if("selectYeaReSeqList".equals(cmd)) {

		List listData  = new ArrayList();
		try {
			listData = selectYeaReSeqList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
