<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>

<!DOCTYPE html>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>서명등록</title>
<link rel="stylesheet" type="text/css" href="../../../common_jungsan/css/theme3_style.css">
<script src="../../../common_jungsan/js/jquery/1.8.3/jquery.min.js" type="text/javascript" charset="utf-8"></script>
<script type="text/javascript" src="../../../common_jungsan/js/signature_pad.min.js"></script>
<style type="text/css">
body {
/*
  background: url("data:image/png;base64,") repeat scroll center center #fff;
*/
  -moz-user-select: none;
  -webkit-user-select: none;
  -ms-user-select: none;
}

.m-signature-pad {
  	position: absolute;
	top: 0;
	left: 0;
	right: 0;
	bottom: 0;
	width: auto;
	height: auto;
	min-width: 250px;
	min-height: 140px;
	margin: 10px;
  	border: 1px solid #b1b1b1;
	background-color: #f6f9fa;
  
}

.m-signature-pad:before, .m-signature-pad:after {
	position: absolute;
	z-index: -1;
	content: "";
	width: 40%;
	height: 10px;
	left: 20px;
	bottom: 10px;
	background: transparent;
	/*
	-webkit-transform: skew(-3deg) rotate(-3deg);
	-moz-transform: skew(-3deg) rotate(-3deg);
	-ms-transform: skew(-3deg) rotate(-3deg);
	-o-transform: skew(-3deg) rotate(-3deg);
	transform: skew(-3deg) rotate(-3deg);
	box-shadow: 0 8px 12px rgba(0, 0, 0, 0.4);*/
}

.m-signature-pad:after {
	left: auto;
	right: 20px;
	/*
	-webkit-transform: skew(3deg) rotate(3deg);
	-moz-transform: skew(3deg) rotate(3deg);
	-ms-transform: skew(3deg) rotate(3deg);
	-o-transform: skew(3deg) rotate(3deg);
	transform: skew(3deg) rotate(3deg);*/
}

.m-signature-pad--body {
	position: absolute;
	left: 0;
	right: 0;
	top: 0;
	bottom: 32px;
	border: 1px solid #f4f4f4;
  	background-color: #fff;

}

.m-signature-pad--body
  canvas {
    position: absolute;
    left: 0;
    top: 0;
    width: 100%;
    height: 100%;
    /*box-shadow: 0 0 5px rgba(0, 0, 0, 0.02) inset;*/
  }

.m-signature-pad--footer {
	position: absolute;
	left: 20px;
	right: 20px;
	bottom: 4px;
	height: 28px;
}

.m-signature-pad--footer .description {
    color: #4d575d;
    text-align: center;
    font-size: 13px;
  }

.m-signature-pad--footer table { 
	width: 100%; 
	margin-top:3px;
}

.button.save {
	padding: 4px 12px;
}

.clearBtn {
	padding: 3px 12px;
	line-height: normal;
	margin-right: .1em;
	cursor: pointer;
	vertical-align: middle;
	text-align: center;
	overflow: visible;
	background:url(../../../common_jungsan/images/common/bg_button01.gif) repeat-x;
	font-weight: bold;
	color: #777 !important;
	font-size:12px;
	border:1px solid #b8c6cc;
	letter-spacing:-1px;
	word-break:keep-all
}
</style>
<body>

	<div id="signature-pad" class="m-signature-pad">
		<div class="m-signature-pad--body">
			<canvas></canvas>
		</div>
		<div class="m-signature-pad--footer">
			<table>
				<colgroup>
					<col width="70"/>
					<col width="" />
					<col width="70" />
				</colgroup>
				<tr>
					<td><a id="btnClear" class="clearBtn">지우기</a></td>
					<td align="center"><div id="description" class="description">서명을 해주세요.</div></td>
					<td align="right"><a id="btnSave" class="button save">서명완료</a></td>
				</tr>
			</table>
		</div>
	</div>

    <script>
		var canvas = $("#signature-pad canvas")[0];
		var sign = new SignaturePad(canvas, {
			minWidth: 2,
			maxWidth: 4,
			penColor: "rgb(0, 0, 0)"
		});
		
		$("#btnClear").bind("click", function(){
			sign.clear();
		});

		$("#btnSave").bind("click", function(){
			if (sign.isEmpty()) {
				alert("서명을 해주세요.");
			} else {
				var forSubmit 	 ="";
				var incomeSubmit ="";
				var penSubmit	 ="";
				var rentSubmit 	 ="";
				var medSubmit 	 ="";
				var doSubmit 	 ="";
				var fileUploadType = parent.$("#fileUploadType").val();
				var nullChk = 0;

				if(parent.$("#forSubmitYn1").prop("checked")) {
					forSubmit = "Y"; 
				}else if(parent.$("#forSubmitYn2").prop("checked")){
					forSubmit = "N";
				}else{
					nullChk++;
				}
				if(parent.$("#incomeSubmitYn1").prop("checked")) {
					incomeSubmit = "Y"; 
				}else if(parent.$("#incomeSubmitYn2").prop("checked")){
					incomeSubmit = "N";
				}else{
					nullChk++;
				}
				if(parent.$("#penSubmitYn1").prop("checked")) {
					penSubmit = "Y"; 
				}else if(parent.$("#penSubmitYn2").prop("checked")){
					penSubmit = "N";
				}else{
					nullChk++;
				}
				if(parent.$("#rentSubmitYn1").prop("checked")) {
					rentSubmit = "Y"; 
				}else if(parent.$("#rentSubmitYn2").prop("checked")){
					rentSubmit = "N";
				}else{
					nullChk++;
				}
				if(parent.$("#medSubmitYn1").prop("checked")) {
					medSubmit = "Y"; 
				}else if(parent.$("#medSubmitYn2").prop("checked")){
					medSubmit = "N";
				}else{
					nullChk++;
				}
				if(parent.$("#doSubmitYn1").prop("checked")) {
					doSubmit = "Y"; 
				}else if(parent.$("#doSubmitYn2").prop("checked")){
					doSubmit = "N";
				}else{
					nullChk++;
				}

				if(nullChk > 0){
					alert("추가 제출 서류에 대한 제출 여부를 선택 후 \n서명완료 버튼을 눌러주세요.");
					return;
				}

				if(confirm("제출 이후 수정할 수 없습니다. 제출하시겠습니까?")){
					$.ajax({
						url : "./signPadPopupRst.jsp",
						type : "POST",
						dataType : "json",
						data : {
							sign : sign.toDataURL()
							,searchWorkYy : parent.$("#searchWorkYy").val()
							,searchAdjustType : parent.$("#searchAdjustType").val()
							,searchSabun : parent.$("#searchSabun").val()
							,forSubmitYn : forSubmit
							,incomeSubmitYn : incomeSubmit
							,penSubmitYn : penSubmit
							,rentSubmitYn : rentSubmit
							,medSubmitYn : medSubmit
							,doSubmitYn : doSubmit
							,fileUploadType : fileUploadType
						},
						success : function(r){
							alert("저장되었습니다.");
							parent.returnSignPad(r);
							sign.clear();
						},
						error : function(res){
							//console.log(res); //ie 구버전 사용 고객사 오류 발생. 20240105
							alert(res);
						}
					});
				}
			}
		});
		

		function resizeCanvas(){
			var canvas = $("#signature-pad canvas")[0];
	
			var ratio =  Math.max(window.devicePixelRatio || 1, 1);
			canvas.width = canvas.offsetWidth * ratio;
			canvas.height = canvas.offsetHeight * ratio;
			canvas.getContext("2d").scale(ratio, ratio);
		}
	    
	    $(window).on("resize", function(){
			resizeCanvas();
		});

		resizeCanvas();
    </script>

</body>
</html>
