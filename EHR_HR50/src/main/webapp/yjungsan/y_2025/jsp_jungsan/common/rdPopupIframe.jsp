...
<script>
    ...
    $(document).on('click', '#crownix-toolbar-print_pdf', function(e) {
        ...
        var eventHandler = function(event){
            ...
            $(document).off('mouseover').on('mouseover', '#crownix-toolbar-print_pdf, #crownix-toolbar-save .dropdown li[id^="crownix-toolbar-"]', function() {
                $(this).off('click tab').on('click tab', function() {
                    ...
                    // security key 취득 후 Rd 재 조회
                    $.ajax({
                        url 		: '../yeaData/yeaDataPrtViewGetSecKey.jsp',
                        type 		: 'post',
                        dataType 	: 'json',
                        async 		: true,
                        data 		: '',
                        success : function(data) {
                            if(data != null) {
                                mrd = copyMrd;
                                param = copyParam.replaceAll(securityKey, data.securityKey); // 만료된 securityKey 값을 신규 securityKey 값으로 교체

                                isDown = true;
                                rdEncryptAndOpenFile(mrd, param);
                            }
                        },
