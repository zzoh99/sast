...
<%
	...
	String fileUploadType	= (String)request.getParameter("fileUploadType");
	...
	// String realPath = fileConfig.getDiskPath(); // 5.x 패키지 전용 소스 (시큐어코딩)
	...
	String strEnterCd = String.valueOf(session.getAttribute("ssnEnterCd")) ;

	if(strEnterCd!=null && !"".equals(strEnterCd)){
		strEnterCd = strEnterCd.replaceAll("/","");
	    strEnterCd = strEnterCd.replaceAll("\\\\","");
	    strEnterCd = strEnterCd.replaceAll("\\.","");
	    strEnterCd = strEnterCd.replaceAll("&","");
	}
	  
	String targetPath = StringUtil.replaceAll(realPath + "/" + strEnterCd, "//", "/");
	...
	if("1".equals(fileUploadType)) {
		param.put("confirmImg" , sign.replace("data:image/png;base64,", ""));
	} else if("2".equals(fileUploadType)){ //TYPE2도 이미지 저장해야 합니다.
		param.put("confirmImg" , sign.replace("data:image/png;base64,", ""));
	} else {
		param.put("confirmImg" , "");
	}
