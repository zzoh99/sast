...
<script type="text/javascript">
	...
	residencyList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYM=<%=yjungsan.util.DateUtil.getDateTime("yyyy")%>12","H20290"), "");
	...
	businessIncomeList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getBsnsIncpCodeList&searchYM=<%=yjungsan.util.DateUtil.getDateTime("yyyy")%>12","C00508"), "");
    ...
    function doAction1(sAction) {
        switch (sAction) {
	        case "Search":   
	        	...
	        	sheet1.DoSearch( "<%=jspPath%>/simplePymtBsnsIncp2Mgr/simplePymtBsnsIncp2MgrRst.jsp?cmd=getSimplePymtBsnsIncp2Mgr", $("#mySheetForm").serialize() ); 
	        ...
	        case "Save":
		        ...
		        <%-- dupChk() 사용으로 변경 - 2020.01.14.
 				for(var j=1; j<=sheet1.LastRow()+1; j++) {
		        	...
		        	if(sheet1.GetCellValue(j, "sStatus") == "I") {
						...
						var data = ajaxCall("<%=jspPath%>/simplePymtBsnsIncp2Mgr/simplePymtBsnsIncp2MgrRst.jsp?cmd=getSabunChk", param,false);
	        	...
	        	sheet1.DoSave( "<%=jspPath%>/simplePymtBsnsIncp2Mgr/simplePymtBsnsIncp2MgrRst.jsp?cmd=saveSimplePymtBsnsIncp2Mgr", $("#mySheetForm").serialize());
	...
	function callProcPop() {
		...
		var url 	= "<%=jspPath%>/simplePymtBsnsIncp2Mgr/simplePymtBsnsIncp2MgrPop.jsp";
        var args = new Array();
