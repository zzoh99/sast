...
//public Map queryMap = null; 
	...
	//기타소득관리 다건 조회
	public List getsimplePymtEtcIncpMgr(String path, Map paramMap) throws Exception {
	...
	//근로소득관리 저장
	public int saveSimplePymtEtcIncpMgr(String path, Map paramMap) throws Exception {
		Map queryMap = XmlQueryParser.getQueryMap(path);
...
<%
	//쿼리 맵 셋팅
	//setQueryMap(xmlPath+"/simplePymtEtcIncpMgr/simplePymtEtcIncpMgr.xml");
	...
	if("getsimplePymtEtcIncpMgr".equals(cmd)) {
		...
		try {
			listData = getsimplePymtEtcIncpMgr(xmlPath+"/simplePymtEtcIncpMgr/simplePymtEtcIncpMgr.xml", mp);
		} catch(Exception e) {
			code = "-1";
        ...
        try {
            int cnt = saveSimplePymtEtcIncpMgr(xmlPath+"/simplePymtEtcIncpMgr/simplePymtEtcIncpMgr.xml", mp);
