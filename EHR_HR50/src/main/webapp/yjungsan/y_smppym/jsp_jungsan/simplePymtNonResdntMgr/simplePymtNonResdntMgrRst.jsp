...
//비거주자 사업기타소득관리 다건 조회
public List getSimplePymtNonResdntMgr(String path, Map paramMap) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"getSimplePymtNonResdntMgr",pm);
	} catch (Exception e) {
...
//비거주자 사업기타소득관리 저장
public int saveSimplePymtNonResdntMgr(String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
  ...
  if(conn != null && list != null && list.size() > 0) {
      ...
      try{
          ...
          for(int i = 0; i < list.size(); i++ ) {
              ...
              if("D".equals(sStatus)) {
                  //삭제
                  rstCnt += DBConn.executeUpdate(conn, queryMap, "deleteSimplePymtNonResdntMgr", mp);
              } else if("I".equals(sStatus) || "U".equals(sStatus) ) {
                //수정
                  rstCnt += DBConn.executeUpdate(conn, queryMap, "saveSimplePymtNonResdntMgr", mp);
              }                                                   
          }
...
<%
	//쿼리 맵 셋팅
	//setQueryMap(xmlPath+"/simplePymtNonResdntMgr/simplePymtNonResdntMgr.xml");
	...
	//비거주자 사업기타소득관리 다건 조회
	if("getSimplePymtNonResdntMgr".equals(cmd)) {
		...
		try {
			listData = getSimplePymtNonResdntMgr(xmlPath+"/simplePymtNonResdntMgr/simplePymtNonResdntMgr.xml", mp);
		} catch(Exception e) {
			code = "-1";
	...
	//중복 사번 체크
		...
		try {
			listData = getSabunChk(xmlPath+"/simplePymtNonResdntMgr/simplePymtNonResdntMgr.xml", mp);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
	...
	//비거주자 사업기타소득관리 저장
	else if("saveSimplePymtNonResdntMgr".equals(cmd)) {
        ...
        try {
            int cnt = saveSimplePymtNonResdntMgr(xmlPath+"/simplePymtNonResdntMgr/simplePymtNonResdntMgr.xml", mp);
	...
	// 전체마감/전체마감취소 
				...
				try {
					int cnt = updateSimplePymtEarnIncpMgr(xmlPath+"/simplePymtNonResdntMgr/simplePymtNonResdntMgr.xml", mp);
		        ...
		        String[] param = new String[]{"","",""
		                ,ssnEnterCd,"",halfType,incomeType,workYy,creWorkYy,sabun,businessPlace,delYn,calcType,ssnSabun};
