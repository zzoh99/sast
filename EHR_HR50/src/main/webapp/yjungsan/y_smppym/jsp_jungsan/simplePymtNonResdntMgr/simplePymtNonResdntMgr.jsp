...
<script type="text/javascript">
	...
	etcList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYM="+searchYM,"C00506"), "");
    ...
    function doAction1(sAction) {
        switch (sAction) {
	        case "Search":   
	        	...
	        	sheet1.DoSearch( "<%=jspPath%>/simplePymtNonResdntMgr/simplePymtNonResdntMgrRst.jsp?cmd=getSimplePymtNonResdntMgr", $("#mySheetForm").serialize() ); 
	        ...
	        case "Save":
		        ...
		        <%-- dupChk() 사용으로 변경 - 2020.01.14.
 				for(var j=1; j<=sheet1.LastRow()+1; j++) {
		        	...
		        	if(sheet1.GetCellValue(j, "sStatus") == "I") {
						...
						var data = ajaxCall("<%=jspPath%>/simplePymtNonResdntMgr/simplePymtNonResdntMgrRst.jsp?cmd=getSabunChk", param,false);
	        	...
	        	sheet1.DoSave( "<%=jspPath%>/simplePymtNonResdntMgr/simplePymtNonResdntMgrRst.jsp?cmd=saveSimplePymtNonResdntMgr", $("#mySheetForm").serialize());
	...
	function callProcPop() {
		...
		var url 	= "<%=jspPath%>/simplePymtNonResdntMgr/simplePymtNonResdntMgrPop.jsp";
        var args = new Array();
	...
	function finalCloseYnChk(val){
		...
		if(confirm(confirmMsg)){
			...
			ajaxCall("<%=jspPath%>/simplePymtNonResdntMgr/simplePymtNonResdntMgrRst.jsp?cmd=updateSimplePymtEarnIncpMgr",params,false);
