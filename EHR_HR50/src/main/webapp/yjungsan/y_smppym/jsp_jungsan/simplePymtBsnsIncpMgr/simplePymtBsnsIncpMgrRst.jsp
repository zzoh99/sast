...
//사업소득관리 다건 조회
public List getSimplePymtBsnsIncpMgr(String path, Map paramMap) throws Exception {
	...
	try{
		...
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"getSimplePymtBsnsIncpMgr",pm);
	} catch (Exception e) {
...
//사업소득관리 저장
public int saveSimplePymtBsnsIncpMgr(String path, Map paramMap) throws Exception {
  Map queryMap = XmlQueryParser.getQueryMap(path);
  ...
  if(conn != null && list != null && list.size() > 0) {
      ...
      try{
          ...
          for(int i = 0; i < list.size(); i++ ) {
              ...
              if("D".equals(sStatus)) {
                  //삭제
                  rstCnt += DBConn.executeUpdate(conn, queryMap, "deleteSimplePymtBsnsIncpMgr", mp);
              } else if("I".equals(sStatus) || "U".equals(sStatus) ) {
                //수정
                  rstCnt += DBConn.executeUpdate(conn, queryMap, "saveSimplePymtBsnsIncpMgr", mp);
              }
          }
...
<%
	//쿼리 맵 셋팅
	//setQueryMap(xmlPath+"/simplePymtBsnsIncpMgr/simplePymtBsnsIncpMgr.xml");
	...
	//사업소득관리 다건 조회
	if("getSimplePymtBsnsIncpMgr".equals(cmd)) {
		...
		try {
			listData = getSimplePymtBsnsIncpMgr(xmlPath+"/simplePymtBsnsIncpMgr/simplePymtBsnsIncpMgr.xml", mp);
		} catch(Exception e) {
			code = "-1";
	...
	//중복 사번 체크
		...
		try {
			listData = getSabunChk(xmlPath+"/simplePymtBsnsIncpMgr/simplePymtBsnsIncpMgr.xml", mp);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
	...
	//사업소득관리 저장
	else if("saveSimplePymtBsnsIncpMgr".equals(cmd)) {
        ...
        try {
            int cnt = saveSimplePymtBsnsIncpMgr(xmlPath+"/simplePymtBsnsIncpMgr/simplePymtBsnsIncpMgr.xml", mp);
	...
	// 전체마감/전체마감취소
			...
			try {
				int cnt = updateSimplePymtEarnIncpMgr(xmlPath+"/simplePymtBsnsIncpMgr/simplePymtBsnsIncpMgr.xml", mp);
