...
//프로그램관리 검색
public List selectPrgMgr(String path, Map paramMap) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		list = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectPrgMgr",pm);
	} catch (Exception e) {
...
//프로그램관리 저장.
public int savePrgMgr(String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
...
<%
	//쿼리 맵 셋팅
	//setQueryMap(xmlPath+"/auth/prgMgr.xml");
	...
	if("selectPrgMgr".equals(cmd)) {
		...
		try {
			listData = selectPrgMgr(xmlPath+"/auth/prgMgr.xml", mp);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			int cnt = savePrgMgr(xmlPath+"/auth/prgMgr.xml", mp);
			if(cnt > 0) {
				message = "저장되었습니다.";
			} else {
				code = "-1";
