...
//public Map queryMap = null; //이변수는 다른스레드에서도 참조할 수 있으므로 절대 스크립트릿 안에서 사용하지 말것.(서블릿 특성상 오동작할 수 있음)
//xml 파서를 이용한 방법;
...
//권한그룹코드 검색
public List selectGrpCdList(String path, Map paramMap) throws Exception {
...
//권한그룹프로그램 검색
public List selectAthGrpMenuMgrList(String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
	//파라메터 복사.
	Map pm =  StringUtil.getParamMapData(paramMap);
	List list = null;
...
//권한그룹프로그램 팝업 검색
public List selectAthGrpMenuMgrRegPopList(String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
	//파라메터 복사.
	Map pm =  StringUtil.getParamMapData(paramMap);
	List list = null;
...
//권한그룹프로그램 팝업 저장.
public int saveAthGrpMenuMgrRegPop(String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
...
//프로그램 팝업 검색
public List selectPrgMgrPopList(String path, Map paramMap) throws Exception {
	Map queryMap = XmlQueryParser.getQueryMap(path);
	//파라메터 복사.
	Map pm =  StringUtil.getParamMapData(paramMap);
	List list = null;
...
<%
	//쿼리 맵 셋팅
	//setQueryMap(xmlPath+"/auth/athGrpMenuMgr.xml");
	...
	if("selectGrpCdList".equals(cmd)) {
		...
		try {
			listData = selectGrpCdList(xmlPath+"/auth/athGrpMenuMgr.xml", mp);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			listData = selectAthGrpMenuMgrList(xmlPath+"/auth/athGrpMenuMgr.xml", mp);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
		...
		try {
			listData = selectAthGrpMenuMgrRegPopList(xmlPath+"/auth/athGrpMenuMgr.xml", mp);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
		...
		try {
			int cnt = saveAthGrpMenuMgrRegPop(xmlPath+"/auth/athGrpMenuMgr.xml", mp);
		...
		try {
			listData = selectPrgMgrPopList(xmlPath+"/auth/athGrpMenuMgr.xml", mp);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
