...
//개인연금등 내역 조회
public List selectPenHisMgrList(Map paramMap, String locPath) throws Exception {
...
<%
	...
	if("selectPenHisMgrList".equals(cmd)) {
		...
		try {
			listData = selectPenHisMgrList(mp, locPath);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			...
			for(int i = 0; i < listMap.size(); i++) {
				...
				String[] param = new String[]{"","",sStatus,ssnEnterCd,workYy
						,adjustType,sabun,savingDeductType,retPenType,financeOrgCd
						,accountNo,applMon,adjInputType,feedbackType,ssnSabun,regDt,docSeq,docSeqDetail};
				String[] rstStr = DBConn.executeProcedure("PKG_CPN_YEA_"+workYy+"_SYNC.PENSION_INS",type,param);
