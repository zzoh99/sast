...
//보험료 자료 조회 (현근무지)
public Map selectYeaDataInsCurMap(Map paramMap, String locPath) throws Exception {
...
//보험료 자료 조회 (종전근무지)
public Map selectYeaDataInsBefMap(Map paramMap, String locPath) throws Exception {
...
<%
	...
	if("selectYeaDataInsList".equals(cmd)) {
		...
		try {
			mapData = selectYeaDataInsCurMap(mp, locPath);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
		...
		try {
			mapData = selectYeaDataInsBefMap(mp, locPath);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
