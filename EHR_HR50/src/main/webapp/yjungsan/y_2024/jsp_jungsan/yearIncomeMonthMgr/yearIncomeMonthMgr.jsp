...
<!DOCTYPE html> <html class="hidden"><head> <title>연간소득관리_월별</title>
...
<script type="text/javascript">
	$(function() {
		...
		initdata1.Cols = [
			...
			{Header:"연월",				Type:"Date",     	Hidden:0,  	Width:60,   Align:"Center",	ColMerge:0,	  SaveName:"ym",          		KeyField:0,   CalcLogic:"",   Format:"Ym",		PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:100 },
			...
			{Header:"사립학교교직원연금",		    Type:"AutoSum",     Hidden:0,  	Width:80,	Align:"Right",	ColMerge:0,   SaveName:"etc_mon1",   		KeyField:0,   CalcLogic:"",   Format:"Integer",	PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:35 },
			{Header:"별정우체국연금",		    Type:"AutoSum",     Hidden:0,  	Width:80,	Align:"Right",	ColMerge:0,   SaveName:"etc_mon4",   		KeyField:0,   CalcLogic:"",   Format:"Integer",	PointCount:0,   UpdateEdit:1,   InsertEdit:1,   EditLen:35 },
	...
	$(function() {
		$("#srchYm").mask("1111-11");
	    $("#srchYm").datepicker2({ymonly:true
	    	                    , onReturn:function() { chkValidSrchYm(); }
	    	                    });
		$("#srchYm").bind("keyup",function(event){
			readonly="readonly"
			if( event.keyCode == 13){
				chkValidSrchYm();				
			}
    ...
    function chkValidSrchYm() {
		var srchYm = $("#srchYm").val();

		if(srchYm != "" && srchYm.length >= 4) {
			srchYm = srchYm.substring(0,4);
			if (srchYm != "<%=yeaYear%>") {
				alert('귀속년도(<%=yeaYear%>)에 해당하는 월별 자료만 조회할 수 있습니다.');
				$("#srchYm").val("<%=yeaYear%>-01");
        	}
		}
		/* doAction1("Search")에서도 호출되기 때문에 주석처리
		if($("#srchYm").val().length > 6) {
			doAction1("Search");
		}*/
    }
    ...
    function doAction1(sAction) {
        ...
        case "Search":
			...
			var param = "cmd=selectYearIncomeMonthMgrList&"+$("#srchFrm").serialize();
			sheet1.DoSearch( "<%=jspPath%>/yearIncomeMonthMgr/yearIncomeMonthMgrRst.jsp", param );
        	break;
        case "Down2Excel":
	...
	function getCprBtnChk(){
        var params = "&cmbMode=all"
                   + "&searchWorkYy=" + $("#srchYm").val().replace("-","").substr(0,4)
                   + "&searchAdjustType="
                   + "&searchSabun=" ;
...
<div class="wrapper">
    ...
    <div class="sheet_search outer">
        ...
        <tr>
			<td>
				<span>대상년월 </span>
				<input id="srchYm" name="srchYm" class="date2" value="<%=yeaYear%>-01"/>
			</td>
			<td>
				<span>정산구분</span>
