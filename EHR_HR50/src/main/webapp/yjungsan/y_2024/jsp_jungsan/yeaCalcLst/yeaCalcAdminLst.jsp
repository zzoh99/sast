...
<%-- 본 페이지가 재정산의 계산내역으로 호출된 경우 처리 --%>
<% String reCalc = (request.getParameter("reCalc") ==null) ? "" : (String)request.getParameter("reCalc"); %>
<%-- 본 페이지가 재정산의 계산내역 팝업으로 호출된 경우 처리 --%>
<% String reCalcAdjustType = (request.getParameter("reCalcAdjustType") ==null) ? "" : (String)request.getParameter("reCalcAdjustType"); %>
<% String reCalcSabun = (request.getParameter("reCalcSabun") ==null) ? "" : (String)request.getParameter("reCalcSabun"); %>

<style type="text/css">
	#infoDiv{ padding:0px; border:2px solid #cacdcf; background:#fff; }
	div.explain { border:0px solid #cacdcf; background:#fff;padding:14px 14px 19px 14px;margin: 0px;}
	div.explain div.title {font-weight:700; color:#333; border-bottom:1px solid #dedede; padding:0 0 10px 10px; margin-bottom:10px}
...
// 외국납부 상세 엑셀 다운로드
function downForeignPay(){
	var downcol = makeHiddenSkipCol(sheet6);
	var param  = {DownCols:downcol,SheetDesign:1,Merge:1,CheckBoxOnValue:"Y",CheckBoxOffValue:"N",menuNm:"외납세 명세"};
	sheet6.Down2Excel(param);
}
function searchForeignPayData(){
	sheet6.DoSearch( "<%=jspPath%>/yeaCalcLst/yeaCalcLstRst.jsp?cmd=selectForeignPayDetailList", $("#sheetForm").serialize() );
}
...
<script type="text/javascript">
	...
	$(function(){
		...
		<%-- 본 페이지가 재정산의 계산내역 팝업으로 호출된 경우 처리 --%>
		<% if (reCalcAdjustType.contains("R")) { %>
		 	$("#searchUserId").val("<%=reCalcSabun%>");
		<% } %>
		...
		var initdata4 = {};
  		...
  		initdata4.Cols = [
  			{Header:"기부금코드",				Type:"Text",	Hidden:1,	Width:5,	Align:"Center",	ColMerge:0,	SaveName:"contribution_cd",		KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:500 },
  			{Header:"기부금종류",				Type:"Text",	Hidden:0,	Width:190,	Align:"Center",	ColMerge:0,	SaveName:"contribution_nm",		KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:500 },
  			{Header:"기부연도",				Type:"Text",	Hidden:0,	Width:50,	Align:"Center",	ColMerge:0,	SaveName:"donation_yy",			KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:500 },
  			{Header:"기부금액(A)",				Type:"Int",		Hidden:0,	Width:95,	Align:"Right",	ColMerge:0,	SaveName:"donation_mon",		KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:500 },
  			...
  			{Header:"공제대상금액(A-B)",		Type:"Int",		Hidden:0,	Width:105,	Align:"Right",	ColMerge:0,	SaveName:"ded_mon_obj",			KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:500 },
  			...
  			{Header:"소멸금액",				Type:"Int",		Hidden:0,	Width:80,	Align:"Right",	ColMerge:0,	SaveName:"extinction_mon",		KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:500 },
  			{Header:"이월금액",				Type:"Int",		Hidden:0,	Width:80,	Align:"Right",	ColMerge:0,	SaveName:"carried_mon",			KeyField:0,	Format:"",	PointCount:0,	UpdateEdit:0,	InsertEdit:0,	EditLen:500 }
		...
		initdata6.Cols = [
			...
			{Header:"납부정보|외납세액\n(a)",	    Type:"Int", 	Hidden:0, 	Width:80, 	Align:"Right", 		ColMerge:0, 	SaveName:"pay_tax_mon", 	KeyField:0, 	Format:"", 	PointCount:0, 	UpdateEdit:0, 	InsertEdit:0, EditLen:500 },
			{Header:"세액공제|공제한도\n(b)",		Type:"Int", 	Hidden:0, 	Width:80, 	Align:"Right", 		ColMerge:0, 	SaveName:"limit_mon", 		KeyField:0, 	Format:"", 	PointCount:0, 	UpdateEdit:0, 	InsertEdit:0, EditLen:500 },
			{Header:"세액공제|한도초과\n(c=a-b)",	Type:"Int", 	Hidden:1, 	Width:80, 	Align:"Right", 		ColMerge:0, 	SaveName:"limit_ov_mon", 	KeyField:0, 	Format:"", 	PointCount:0, 	UpdateEdit:0, 	InsertEdit:0, EditLen:500 },
			{Header:"세액공제|전기이월액\n(d)",		Type:"Int", 	Hidden:0, 	Width:80, 	Align:"Right", 		ColMerge:0, 	SaveName:"prev_carried_mon",KeyField:0, 	Format:"", 	PointCount:0, 	UpdateEdit:0, 	InsertEdit:0, EditLen:500 },
			{Header:"세액공제|공제대상액\n(e=a+d)",	Type:"Int", 	Hidden:0, 	Width:80, 	Align:"Right", 		ColMerge:0, 	SaveName:"cur_ded_mon", 	KeyField:0, 	Format:"", 	PointCount:0, 	UpdateEdit:0, 	InsertEdit:0, EditLen:500 },
			{Header:"이월배제|배제확정(소멸)",        Type:"Int", 	Hidden:1, 	Width:80, 	Align:"Right", 		ColMerge:0, 	SaveName:"no_carried_mon", 	KeyField:0, 	Format:"", 	PointCount:0, 	UpdateEdit:0, 	InsertEdit:0, EditLen:500 },
			{Header:"소멸금액\n(이월배제)\n(기간만료)\n(f)|소멸금액\n(이월배제)\n(기간만료)\n(f)",		Type:"Int", 	Hidden:0, 	Width:80, 	Align:"Right", 		ColMerge:0, 	SaveName:"extinction_mon", 	KeyField:0, 	Format:"", 	PointCount:0, 	UpdateEdit:0, 	InsertEdit:0, EditLen:500 },
			{Header:"<%--국가별 --%>이월금액|한도초과분\n(g=e-b-배제)",	Type:"Int", 	Hidden:0, 	Width:80, 	Align:"Right", 		ColMerge:0, 	SaveName:"carried_mon", 	KeyField:0, 	Format:"", 	PointCount:0, 	UpdateEdit:0, 	InsertEdit:0, EditLen:500 },
			{Header:"<%--통합 --%>이월금액|미공제분\n(h)",	    Type:"Int", 	Hidden:0, 	Width:80, 	Align:"Right", 		ColMerge:0, 	SaveName:"carried_mon_841", KeyField:0, 	Format:"", 	PointCount:0, 	UpdateEdit:0, 	InsertEdit:0, EditLen:500 },			
			{Header:"실공제액\n(i=e-f-g-h)|실공제액\n(i=e-f-g-h)",		Type:"Int", 	Hidden:0, 	Width:80, 	Align:"Right", 		ColMerge:0, 	SaveName:"ded_mon", 		KeyField:0, 	Format:"", 	PointCount:0, 	UpdateEdit:0, 	InsertEdit:0, EditLen:500 }
	...
	$(function(){
		...
		//본 페이지가 재정산 수정(이력)의 계산내역 팝업으로 호출된 경우 처리
		if ("" != "<%=reCalcAdjustType%>") {
			$("#spnEmpHeader").addClass("hide");
			
	        var params = "&cmbMode=onlyH"
			...
			$("#searchAdjustType").html(searchReCalcSeq[2]);
			$("#searchAdjustType").val("<%=reCalcAdjustType%>");
		}
	});
	...
	function sht2ToCtl(){
		if(sheet2.RowCount() > 0){
			...
			$("#sht2_CurrNotaxBabyMon").val( sheet2.GetCellText(1, "curr_notax_baby_mon"));//현근무지보육비과세
			$("#sht2_CurrNotaxBirthMon").val( sheet2.GetCellText(1, "curr_notax_child_birth_mon"));//현근무지출산지원금과세
			...
			$("#sht2_PreNotaxBabyMon").val( sheet2.GetCellText(1, "pre_notax_baby_mon"));//종전근무지보육비과세
			$("#sht2_PreNotaxBirthMon").val( sheet2.GetCellText(1, "pre_notax_child_birth_mon"));//종전근무지출산지원금비과세
			...
			$("#sht2_PreNotaxBirthMon").val("0");//종전근무지출산지원금비과세
	...
	function finalCloseYnValueSet(){
		...
		$("#sht2_PreNotaxBabyMon").val("");//종전근무지보육비과세
		$("#sht2_PreNotaxBirthMon").val("");//종전근무지출산지원금비과세
		...
		$("#sht2_CurrNotaxBabyMon").val("");//현근무지보육비과세
		$("#sht2_CurrNotaxBirthMon").val("");//현근무지출산지원금과세
		...
		$("#sht5_A070_15").val("");//임차차입금 이자금공제   -- 2011년 이전(15년이상)
		...
		$("#sht5_A070_15_INP").val("");//이자상환액(차입금상환기간 2011년 이전(15년이상)                )
	...
	function print(printGbn){
		...
		var adjType = $("#searchAdjustType").val() ;
		if(printGbn == "printBk") {
			if ("2024" >= $("#searchWorkYy").val()) {
				adjType = $('#searchReCalcSeq').val() ;
			} else {
		...
		args["rdParam"] = "[<%=session.getAttribute("ssnEnterCd")%>]"             // 1.ssnEnterCd
			            + " ["+$("#searchWorkYy").val()+"]"			              // 2.searchWorkYY
			            + " ["+ adjType +"]"                                      // 3.searchAdjustType
			            ...
			            + " ["+ imgFile +"]"						              // 14.imgFile
			            + " [1]"									              // 15.stampChk
                        + "["+$("#searchOption").val()+"]" //사대보험출력여부
                        + "["+adjType+"]";  //일괄다운로드 adjust_type리스트
						//rd파라매터
	...
	function cprYeedAdjm(){
        var args = [];
        args["reCalcAdjustType"]   = $("#searchReCalcSeq").val() ;
        args["reCalcSabun"]        = $("#searchUserId").val() ;
        
        if(!isPopup()) {return;}
        var rv = openPopup("<%=jspPath%>/yeaCalcLst/yeaCalcAdminLst.jsp",args,"1000","750");
		...
		<% if ("Y".equals(reCalc)) { %>
			...
			if(searchReCalcSeq == null || searchReCalcSeq == "" || searchReCalcSeq[0] == "" ) {
	            ...
	            $("#cmpBtn").addClass("hide");
	   			...
	   			$("#searchReCalcSeq").html(searchReCalcSeq[2]);
	...
	/* function f_ageChkYn($this) {
		if($this.prop("checked")) {
			alert("대상자에 해당하면 자료등록(관리자) 메뉴 연금계좌 탭에서\n50세이상 체크박스를 확인 해주세요.\n\n체크박스를 변경한 경우 세금계산을 재계산해야 합니다.");
		}
		else {
			alert("대상자에 해당하지 않으면 자료등록(관리자) 메뉴 연금계좌 탭에서\n50세이상 체크박스를 확인 해주세요.\n\n체크박스를 변경한 경우 세금계산을 재계산해야 합니다.");
		}
		return;
	} */
					...
					<div class="sheet_title">
						<ul>
							...
							<li class="btn">
								...
								<a href="javascript:downForeignPay();" class="basic">엑셀다운</a>
							</li>
						</ul>
					</div>
...
<div class="wrapper">
	<span id="spnEmpHeader" name="spnEmpHeader">
	...
	<input type="hidden" id="calcLstMd" name="calcLstMd" value="A" />
    ...
    <!-- 관리자 조회 조건 -->
        ...
        <tr>
            <td>
				...
				--%>
					<input type="text" id="searchWorkYy" name="searchWorkYy" class="text w35 center readonly" maxlength="4" size="4" readonly/>
				...
				<div class="outer">
					<ul>
						...
						<li class="btn">
                            ...
                            <a href="javascript:print('print');" class="basic btn-white ico-print">원천징수영수증</a>
						    ...
						    <span id="cmpBtn" class="hide">
	                            ...
	                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								수정(이력)
	                            <select id="searchReCalcSeq" name="searchReCalcSeq"></select>
	                            
	                            <a href="javascript:cprYeedAdjm();" class="cute" >계산내역</a>
	                            <a href="javascript:print('printBk');" class="cute">원천징수영수증</a>
                            </span>
						</li>
					</ul>
				</div>
		        ...
		        <div class="explain">
			        ...
			        <div class="txt">
							<ul>
							    ...
							    <li>2. 계산내역 화면에 금액이 조회 되려면 담당자마감, 계산결과개인오픈이 체크되어 있어야 합니다. (소득공제자료관리>자료입력/마감현황)</li>
	...
	<div class="outer" >
	    ...
	    <table border="0" cellpadding="0" cellspacing="0" class="default line inner" id="wkp_table" name="wkp_table">
		<colgroup>
	        <col width="14%" />
	        <col width="14%" />
	        <col width="18%" />
		...
		<tr>
			<th class="center" rowspan="24">그 밖의<br>소득공제</th>
		...
		<tr>
			<th class="center" colspan="4">표준세액공제&nbsp;&nbsp;&nbsp;<font id="sht5_A099_01_font" class="blue">(표준세액공제 적용시 특별세액공제 계산되지 않음)</font></th>
			<td class="right">
