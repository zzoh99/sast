...
<%!
//외납세이월자료 조회
public List selectAbroadTaxUploadList(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		listData  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectAbroadTaxUploadList",pm);
...
//외납세이월자료 저장.
public int saveAbroadTaxUpload(List paramList, String locPath, String ssnYeaLogYn) throws Exception {
	...
	if(paramList != null && paramList.size() > 0 && conn != null) {
		...
		try{
			for(int i = 0; i < paramList.size(); i++ ) {
                ...
                if("I".equals(sStatus)) {
					rstCnt += DBConn.executeUpdate(conn, queryMap, "insertAbroadTaxUpload", mp);
				} else if("U".equals(sStatus)) {
					rstCnt += DBConn.executeUpdate(conn, queryMap, "updateAbroadTaxUpload", mp);
				} else if("D".equals(sStatus)) {
					rstCnt += DBConn.executeUpdate(conn, queryMap, "deleteAbroadTaxUpload", mp);
	            }
...
<%
	...
	String locPath = xmlPath+"/abroadTax/abroadTaxUpload.xml";
	...
	if("selectAbroadTaxUploadList".equals(cmd)) {
		//외납세이월자료 조회
		...
		try {
			listData = selectAbroadTaxUploadList(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
	...
	} else if("saveAbroadTaxUpload".equals(cmd)) {
		//저장
		...
		try {
			int cnt = saveAbroadTaxUpload(listMap, locPath, ssnYeaLogYn);
			if(cnt > 0) {
				message = "저장되었습니다.";
			} else {
				code = "-1";
