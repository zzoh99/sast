...
<script type="text/javascript">
	...
	var report_type = p.popDialogArgument("report_type");
	...
	$(function() {
		...
		var rtnval = ajaxCall("<%=jspPath%>/incomeDeductionSta/incomeInvoker.jsp?cmd=onload&rdMrd="+rdMrd, $("#srchFrm").serialize(), false);
	...
	function doAction(sAction) {
		...
		switch (sAction) {
	        case "chk":
	        	chkDisk(ajaxCall("<%=jspPath%>/incomeDeductionSta/incomeInvoker.jsp?cmd=CheckDisk&report_type="+report_type, $("#srchFrm").serialize(), false));
			break;
	        ...
	        case "cre":
	        	...
	        	var rtnData = ajaxCall("<%=jspPath%>/incomeDeductionSta/incomeInvoker.jsp?cmd=creFile", $("#srchFrm").serialize(), true
	        			,function(){
	        		/* $("#progressCover").show(); */
                    workingYn = "Y";
    				timer_start();
		                ...
		                ,function(rtnData){
			        		...
			        		if(rtnData.downFileName.length > 0) {
			        			$("iframe").attr("src",'<%=jspPath%>/incomeDeductionSta/incomeInvoker.jsp?cmd=downFile&downfileName='+$("#fileName").val()+'&searchWorkYy='+searchWorkYy+'&report_type='+report_type);	
			        		}
			        		workingYn = "N";
	...
	function Timer2(){
		...
		fileCountChk(ajaxCall("<%=jspPath%>/incomeDeductionSta/incomeInvoker.jsp?cmd=cntChk&searchWorkYy="+searchWorkYy+"&report_type="+report_type, "", false));
	}
