...
%>
		...
		initdata2.Cols = [
			...
			{Header:"교육대상 구분|교육대상 구분",			Type:"Combo",		Hidden:0,	Width:60,	Align:"Center",	ColMerge:1,	SaveName:"work_type",		KeyField:1,	Format:"",		PointCount:0,	UpdateEdit:0,	InsertEdit:1,	EditLen:10 },
...
<%-- 			{Header:"금액자료|담당자용",				Type:"AutoSum",			Hidden:0,	Width:60,	Align:"Right",	ColMerge:1,	SaveName:"appl_mon",		KeyField:1,	Format:"#,###",		PointCount:0,	UpdateEdit:<%=applEdit%>,	InsertEdit:<%=applEdit%>,	EditLen:35, MinimumValue: 0 }, --%>
		...
		var workTypeList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonCodeList&searchYear=<%=yeaYear%>","C00313"), "");
...
<%--         sheet2.SetRangeBackColor(0,0,1, sheet2.LastCol(),"<%=headerColorA%>") ; --%>
	...
	function doAction2(sAction) {
		...
		case "Save":
			...
			for( var i = 2; i < sheet2.LastRow()+2; i++) {
				...
				if( sheet2.GetCellValue(i, "sStatus") == "U" ||
					...
					if( sheet2.GetCellValue(i, "work_type") == "40" ) { // 40(대학교_본인외)
						if( parseInt( sheet2.GetCellValue(i, "input_mon") , 10) > 9000000 ) { 
							alert("교육대상구분 대학교 입력금액한도는 개인별 최대 900만원까지 가능합니다.") ;
							sheet2.SelectCell(i, "input_mon") ;
							return ;
						}
					}
		...
		case "Copy":
			...
			sumData = 0;
			for(var i=1; i < sheet2.LastRow()+1; i++){
                if((sheet2.GetCellValue(newRow,"famres") == sheet2.GetCellValue(i,"famres")) && (sheet2.GetCellValue(newRow,"work_type") == sheet2.GetCellValue(i,"work_type"))
                        && sheet2.GetCellValue(newRow,"work_type") == "40"){ // 40(대학교_본인외)
                    sumData = sumData + parseInt(sheet2.GetCellValue(i,"appl_mon"), 10);
                }
            }
			if(sumData > 9000000){
                alert("대학교 한도금액이 초과되었습니다.");
	...
	function sheet2_OnSearchEnd(Code, Msg, StCode, StMsg) {
		try {
			...
			var work_type20 = []; // 취학전 대상자 리스트
			var work_type30 = []; // 초중고 대상자 리스트
			var work_type40 = []; // 대학교 대상자 리스트
			
			var dup         = []; // 취학전+초중고) 대상자명 중복체크용 리스트
			var dupVal      = []; // 취학전+초중고) 대상자 value 중복체크용 리스트
			var dup40       = []; // 초중고+대학교) 대상자명 중복체크용 리스트
			var dupVal40    = []; // 초중고+대학교) 대상자 value 중복체크용 리스트
			
			if (Code == 1 && sheet2.RowCount() > 0) {
				var sText = sheet2.GetComboInfo(sheet2.HeaderRows(),"famres", "Text");
				var sCode = sheet2.GetComboInfo(sheet2.HeaderRows(),"famres", "Code") ;
				var arrText = sText.split("|");
				var arrCode = sCode.split("|");
				
				for(var i = sheet2.HeaderRows(); i <= sheet2.LastRow(); i++){

					if(sheet2.GetCellValue(i, "work_type") == "20"){
						var fam = sheet2.GetCellValue(i, "famres");
						work_type20.push(fam);
						
						if(jQuery.inArray(fam,work_type30) > -1) {
							dupVal.push(fam);
						...
						sheet2.SetCellEditable(i, "restrict_cd", 0) ;
					} else if(sheet2.GetCellValue(i, "work_type") == "30"){
							sheet2.CellComboItem(i,	"restrict_cd",	{ComboText:"|"+contributionCdList1[0], ComboCode:"|"+contributionCdList1[1]} );
							sheet2.SetCellEditable(i, "restrict_cd", 1) ;
							
							var fam = sheet2.GetCellValue(i, "famres");
							work_type30.push(fam);
							
							if(jQuery.inArray(fam, work_type20) > -1) {
								dupVal.push(fam);
								for(j=0; j<arrCode.length; j++){
									if(fam == arrCode[j]){
										dup.push(arrText[j]);		
							...
							if(jQuery.inArray(fam, work_type40) > -1) {
								dupVal40.push(fam);
								for(j=0; j<arrCode.length; j++){
									if(fam == arrCode[j]){
										dup40.push(arrText[j]);		
									}
								}
							}		
				...
				if(dup.length > 0){
					alert("대상자 중 '"+ dup[0]+ "'의 교육비가 취학전, 초중고 두 건 존재합니다.\n[교육비 취학전 + 초중고 <= 300(만원) 한도]")
					
					for(var i = sheet2.HeaderRows(); i <= sheet2.LastRow(); i++){
						if(jQuery.inArray(sheet2.GetCellValue(i, "famres"), dupVal) > -1){
							sheet2.SetRowBackColor(i,"#C0C0C0");
						}
					}
				}

				if(dup40.length > 0){
					alert("대상자 중 '"+ dup40[0]+ "'의 교육비가 초중고, 대학교 두 건 존재합니다.\n[교육비 초중고 + 대학교 <= 900(만원) 한도]")
					
					for(var i = sheet2.HeaderRows(); i <= sheet2.LastRow(); i++){
						if(jQuery.inArray(sheet2.GetCellValue(i, "famres"), dupVal40) > -1){
							sheet2.SetRowBackColor(i,"#D3D3D3");
						}
					}
				}
			}
	...
	function sheet2_OnChange(Row, Col, Value) {
		try{
			if(sheet2.ColSaveName(Col) == "famres" || sheet2.ColSaveName(Col) == "work_type"){
				...
				if(sht1FamresIdx == "-1"){
					if(sheet2.GetCellValue(Row, "work_type") != ""){
						alert("대상자 먼저 선택 후 입력 바랍니다.");
				...
				*/
				...
				/*
					25.02.07
					장애인 특수재활 교육비 관련 내용 추가
					장애인 복지법상 장애인으로 등록되지 않은 만 6세 미만의 아동의 경우에도 발달재활서비스를 지원받은 경우 세법상 장애인으로 인정되므로,
					해당 아동을 위해 발달재활서비스 제공기간에 지출한 교육비도 장애인재활특수교육비 공제가 가능.
				*/
				var age = 0;
	            if(sheet1.GetCellValue(sht1FamresIdx,"famres").substring(6,7) == "3" || sheet1.GetCellValue(sht1FamresIdx,"famres").substring(6,7) == "4"
	               || sheet1.GetCellValue(sht1FamresIdx,"famres").substring(6,7) == "7" || sheet1.GetCellValue(sht1FamresIdx,"famres").substring(6,7) == "8") {
	                age = systemYY - parseInt("20" + sheet1.GetCellValue(sht1FamresIdx, "famres").substring(0, 2), 10);
	            }else {
	                age = systemYY - parseInt("19" + sheet1.GetCellValue(sht1FamresIdx, "famres").substring(0, 2), 10);
	            }
			...
			if(sheet2.ColSaveName(Col) == "work_type"){
				if(sheet2.GetCellValue(Row,"work_type") == "30"){
					...
					// 2023-12-28. 수능응시료, 대학입학전형료가 추가됨에 따라 교육대상구분이 '대학교_본인외'일 경우에도 상세 구분을 입력하는 경우가 생겨 활성화되도록 수정
					sheet2.CellComboItem(Row,	"restrict_cd",	{ComboText:"|"+contributionCdList1[0], ComboCode:"|"+contributionCdList1[1]} );
					sheet2.SetCellEditable(Row,"restrict_cd", 1) ;
...
--%>
					...
					if(sheet2.GetCellValue(Row,"work_type") == "40"){ // 40(대학교_본인외)
                        var sumData = 0;

                        for(var i=1; i < sheet2.LastRow()+1; i++){
                            if((sheet2.GetCellValue(Row,"famres") == sheet2.GetCellValue(i,"famres")) && (sheet2.GetCellValue(Row,"work_type") == sheet2.GetCellValue(i,"work_type"))){
                                sumData = sumData + parseInt(sheet2.GetCellValue(i,"appl_mon"), 10);
                            }
                        }
...
<div class="wrapper">
	...
	<div class="outer">
		...
		<ul>
			<li class="txt">교육비
				...
				<a href="#layerPopup" class="basic btn-white ico-question" id="InfoPlus"><b>교육비 안내+</b></a>
	            <a href="#layerPopup" class="basic btn-white ico-question" id="InfoMinus" style="display:none"><b>교육비 안내-</b></a>
			</li>
		</ul>
		</div>
	</div>
	...
	<div class="outer">
	    <div>
	    	<font size='2'>
			    <font class="blue">※ 교복구입비(개인별 50만원한도)</font> : 교육대상 구분 "초중고 > 교복구입비" 로 선택,
				<font class="blue">체험학습비(개인별 30만원한도)</font> : 교육대상 구분 "초중고 > 체험학습비" 로 선택<br>
			    <font class="blue">※ 해당연도 중 취학전 아동이 초등학생이 된 경우</font> 합산 300만원 이하로 수정 or 하나는 삭제하고 그외 항목에 합쳐서 기입 (개별 한도체크는 동작)<br>
			    <font class="blue">※ 해당연도 중 고등학생이 대학생이 된 경우</font> 합산 900만원 초과시 각 구분별 한도내에 금액으로 수정(합산은 900만원이하)<br>
		    </font>
	    </div>
