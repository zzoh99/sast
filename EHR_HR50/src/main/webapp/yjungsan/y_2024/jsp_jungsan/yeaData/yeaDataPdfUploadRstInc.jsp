...
//                          if(!"B".equals(docType)) {
                        ...
                        for(int manIndex = 0; manIndex < childList.size(); manIndex++ ) {
                            ...
                            if("A102Y".equals(formCd)) {
                                ...
                                for(int dataIndex = 0; dataIndex < childList2.size(); dataIndex++ ) {
                                    ...
                                    for(int insuIndex = 0; insuIndex < childList3.size(); insuIndex++) {
                                        ...
                                        else if("tot_pre_year_sum".equals(dataEtcName)) {
                                            dataMap.put("A3","");
                                            dataMap.put("A4","");
                                            dataMap.put("A5","");
                                            dataMap.put("A6","");

                                            dataMap.put("A8",dataEtcValue); //2023년 사용금액 합계
                                        }
                                        else if("tot_curr_year_sum".equals(dataEtcName)) {
                                            dataMap.put("A9",dataEtcValue); //2024년 사용금액 합계
                                        }
                                    }
                                ...
                                for(int dataIndex = 0; dataIndex < childList2.size(); dataIndex++ ) {
                                    ...
                                    for(int insuIndex = 0; insuIndex < childList3.size(); insuIndex++) {
                                        ...
                                        if("sum".equals(dataEtcName)) {
                                            ...
                                            dataMap.put("A6",dataEtcValue); //2023년 사용금액 합계
                                        }
                                        else if("tot_curr_year_sum".equals(dataEtcName)) {
                                            dataMap.put("A7",dataEtcValue); //2024년 사용금액 합계
                                        }
                                    }
                            ...
                            // 2024 소득기준초과 부양가족 추가
                            else if("T101M".equals(formCd)) {
                                for(int dataIndex = 0; dataIndex < childList2.size(); dataIndex++ ) {
                                    dataMap = getDefaultDataMap(ssnEnterCd,ssnSabun,paramYear,paramAdjustType,paramSabun,docType,docSeq,formCd,manResId,manName);

                                    dataMap.put("A3",((Element)childList2.get(dataIndex)).getAttributeValue("dat_cd")); // 자료코드
                                    dataMap.put("A4",((Element)childList2.get(dataIndex)).getAttributeValue("rlt_cd")); // 관계코드
                                    dataMap.put("A5",((Element)childList2.get(dataIndex)).getAttributeValue("sup_resid")); // 대상자 주민번호
                                    dataMap.put("A6",((Element)childList2.get(dataIndex)).getAttributeValue("sup_name"));  // 대상자 이름
                                    dataMap.put("A7",((Element)childList2.get(dataIndex)).getAttributeValue("inc_exc"));   // 소득기준 초과 Y/N여부
