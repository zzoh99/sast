...
//연말정산 항목 프로세스 검색
public List selectYearEndItemMgrProcess(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
//연말정산 항목 검색
public List selectYearEndItemMgr(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
...
//연말정산 프로세스 저장.
public int saveYearEndItemMgrProcess(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
		...
		try{
			for(int i = 0; i < list.size(); i++ ) {
				...
				if("D".equals(sStatus)) {
					...
					//입력 중복체크
					Map dupMap = DBConn.executeQueryMap(conn, queryMap,"selectYearEndItemMgrProcessCnt",mp);
...
//연말정산 항목 저장.
public int saveYearEndItemMgr(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
		...
		try{
			for(int i = 0; i < list.size(); i++ ) {
				...
				if("D".equals(sStatus)) {
					...
					//입력 중복체크
					Map dupMap = DBConn.executeQueryMap(conn, queryMap,"selectYearEndItemMgrCnt",mp);
...
//도움말(SUB) 조회
public List selectYearEndItemMgrPopupSub(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	...
	try{
		//쿼리 실행및 결과 받기.
		list  = (queryMap == null) ? null : DBConn.executeQueryList(queryMap,"selectYearEndItemMgrPopupSub",pm);
		saveLog(null, pm, ssnYeaLogYn);
	} catch (Exception e) {
		Log.Error("[Exception] " + e);
		throw new Exception("조회에 실패하였습니다.");
...
//도움말(SUB) 저장.
public int saveYearEndItemMgrPopupSub(Map paramMap, String locPath, String ssnYeaLogYn) throws Exception {
	//파라메터 맵안에는 배열형태로 저장되어 있으니 다시 리스트형태로 뽑아서 루프를 돈다.
	List list = StringUtil.getParamListData(paramMap);
	//xml 파서를 이용한 방법;
	Map queryMap = XmlQueryParser.getQueryMap(locPath);
	...
	if(list != null && list.size() > 0 && conn != null) {
		...
		try{
			for(int i = 0; i < list.size(); i++ ) {
				...
				if("D".equals(sStatus)) {
					//삭제
					rstCnt += DBConn.executeUpdate(conn, queryMap, "deleteYearEndItemMgrPopupSub", mp);
				} else if("U".equals(sStatus)) {
					//수정
					rstCnt += DBConn.executeUpdate(conn, queryMap, "updateYearEndItemMgrPopupSub", mp);
				} else if("I".equals(sStatus)) {
					//입력 중복체크
					Map dupMap = DBConn.executeQueryMap(conn, queryMap,"selectYearEndItemMgrPopupSubCnt",mp);

					if(dupMap != null && Integer.parseInt((String)dupMap.get("cnt")) > 0 ) {
						throw new UserException("중복되어 저장할 수 없습니다.");
					}
					//입력
					rstCnt += DBConn.executeUpdate(conn, queryMap, "insertYearEndItemMgrPopupSub", mp);
				}saveLog(conn, mp, ssnYeaLogYn);
			}
...
<%
	...
	if("selectYearEndItemMgrProcess".equals(cmd)) {
		...
		try {
			listData = selectYearEndItemMgrProcess(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
		...
		try {
			listData = selectYearEndItemMgr(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
		...
		try {
			int cnt = saveYearEndItemMgrProcess(mp, locPath, ssnYeaLogYn);
		...
		try {
			...
			int cnt = saveYearEndItemMgr(mp, locPath, ssnYeaLogYn);
	...
	} else if("selectYearEndItemMgrPopupSub".equals(cmd)) {
		//도움말(SUB) 조회
		...
		try {
			listData = selectYearEndItemMgrPopupSub(mp, locPath, ssnYeaLogYn);
		} catch(Exception e) {
			code = "-1";
			message = e.getMessage();
		}
	...
	}  else if("saveYearEndItemMgrPopupSub".equals(cmd)) {
		//도움말(SUB) 저장
		...
		try {
			int cnt = saveYearEndItemMgrPopupSub(mp, locPath, ssnYeaLogYn);
