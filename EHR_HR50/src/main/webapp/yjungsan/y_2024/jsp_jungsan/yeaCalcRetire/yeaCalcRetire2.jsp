...
<script type="text/javascript">
	...
	function getYeaPayActionInfo() {
		...
		var paymentInfo = ajaxCall("<%=jspPath%>/yeaCalcRetire/yeaCalcRetireRst.jsp?cmd=selectYeaPayActionInfo2", "searchYear="+$("#searchWorkYy").val(), false);

		if(paymentInfo.Data.pay_action_cd != undefined){
			$("#searchPayActionCd").val(nvl(paymentInfo.Data.pay_action_cd,""));
			$("#searchPayActionNm").val(nvl(paymentInfo.Data.pay_action_nm,""));
			$("#searchPayActionCdNm").val(nvl(paymentInfo.Data.pay_action_nm,""));
	...
	function payActionSearchPopup() {
		...
		if(!isPopup()) {return;}
		pGubun = "yeaCalcRetirePayDayViewPopup2";

		var args = new Array();
		var result = openPopup("<%=jspPath%>/yeaCalcRetire/yeaCalcRetirePayDayViewPopup2.jsp?authPg=<%=authPg%>",args,"900","580");
		/*
		if (result) {
	...
	function getReturnValue(returnValue) {
		...
		if ( pGubun == "yeaCalcRetirePayDayViewPopup2" ){
			$("#searchPayActionCd").val(result["payActionCd"]);
			$("#searchPayActionNm").val(result["payActionNm"]);
			$("#searchPayActionCdNm").val(result["payActionNm"]);
			$("#searchWorkYy").val(result["payYm"].substring(0,4)) ;
			$("#payYM").val(result["payYm"].substring(0,4) + "-" + result["payYm"].substring(4,6)) ;
			$("#ordYmd").val(result["ordSymd"].substring(0,4) +"-"+result["ordSymd"].substring(4,6)+"-"+result["ordSymd"].substring(6,8) + " ~ " + result["ordEymd"].substring(0,4) +"-"+result["ordEymd"].substring(4,6)+"-"+result["ordEymd"].substring(6,8)) ;
			sheetSearch() ;
...
<div class="wrapper">
	<form id="sheetForm" name="sheetForm">
		...
		<input type="hidden" id="searchRePayActionCd" name="searchRePayActionCd" value="">
		<input type="hidden" id="searchPayActionCdNm" name="searchPayActionCdNm" value="">
		...
		<tr>
			<td class="top center">
				<div class="sheet_title">
					<ul>
						<li id="txt" class="txt">작업진행순서
							<span>&nbsp;<font class='blue'>[퇴직금 계산한 인원들로 퇴직금 급여일자로 자동 세팅됨. (퇴직자정산으로 해도 무방)]</font></span>
						</li>
					</ul>
				</div>
				<table id="sheet_main_table" border="0" cellpadding="0" cellspacing="0" class="default">
					<colgroup>
						<col width="20%" />
						<col width="25%" />
						<col width="20%" />
					...
					<tr>
						...
						<td class="left" colspan="3">
							...
							<a onclick="javascript:payActionSearchPopup();" href="javascript:void(0);" class="basic btn-white ico-search no-text"></a>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="4" class="center">
							...
							<a href="javascript:cancelJob();"	class="basic authA">작업취소</a></td>
					</tr>
					<tr>
						<td colspan="4" class="center">
				...
				<div class="outer">
					<ul>
						...
						<li class="btn">
							<a href="javascript:openPeopleSet();"		class="basic authA">대상자기준</a>
							<a href="javascript:openYEACalRetry();"		class="basic authA">재계산대상자작업</a>
							<a href="javascript:optPop();"				class="basic authA">연말정산옵션</a>
						</li>
					</ul>
				</div>
				<br>
			...
			<td class="top">
				...
				<div class="explain">
					...
					<div class="txt">
						<ul>
					        <li>1. 대상자기준에서 대상자를 등록</li>
					        <li style="padding-left:13px;">작업할 대상자를 선택 저장합니다.</li>

					        <li>2. 총급여합산, 세금계산을 체크하여 [작업]을 실행합니다.</li>
					        ...
					        <li>3. 작업이 종료되면 [마감] 버튼을 클릭하여 마감처리</li>
					        <li>4.  마감후 재작업을 하시려면 [마감취소] 버튼을 눌러</li>
