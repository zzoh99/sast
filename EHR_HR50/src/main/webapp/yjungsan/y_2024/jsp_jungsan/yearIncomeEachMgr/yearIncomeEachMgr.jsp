...
<script type="text/javascript">
	...
	var adjTypeCmbList = null;
		...
		initdata1.Cols = [
            ...
            {Header:"갱신일시",          Type:"Text",     Hidden:0,   Width:150,   Align:"Center",  ColMerge:0,   SaveName:"chkdate",          KeyField:0,   CalcLogic:"",   Format:"YmdHms", PointCount:0,   UpdateEdit:0,   InsertEdit:0,   EditLen:35 }
	    ...
	    adjTypeCmbList = stfConvCode( codeList("<%=jspPath%>/common/commonCode.jsp?cmd=getCommonNSCodeList&srchWorkYy="+$("#srchYear").val(),"getYearEndItemList") , "");
		...
		templeteTitle1 += "No, 갱신일시 : 기재할 필요 없음 \n";
	});
    ...
    function doAction1(sAction) {
        ...
        case "Down2Template":
			...
			var param  = {DownCols:downcol,SheetDesign:1,Merge:1,DownRows:'0',ExcelFontSize:"9"
				,TitleText:templeteTitle1,UserMerge :"0,0,1,35",menuNm:$(document).find("title").text()};
			sheet1.Down2Excel(param);
			break;
        case "LoadExcel":
        	if(chkRqr()){
    ...
    function getPayTotal(){
	    ...
	    //건강보험 합계(sheet1)
        bTotal = sheet1.GetSumText("31");
        bTotal = bTotal.replace(/,/gi,"");

        //고용보험 합계(sheet1)
        cTotal = sheet1.GetSumText("32");
   ...
   function getReduceYmd(){
       ...
       if(data.Result.Code == 1) {
    	   // 24.11.07 시작일/종료일 중 미입력된 일자가 있으면 시작일/종료일 모두 미표기하는 부분 수정 => 시작일/종료일 각각 표기되도록
           if((data.Data.s_ymd != "" && typeof data.Data.s_ymd != "undefined") || (data.Data.e_ymd != "" && typeof data.Data.e_ymd != "undefined")){
               // 감면기간 값 초기화
               $("#reduce_s_ymd").val("");
               $("#reduce_e_ymd").val("");

               // 감면기간 시작일 세팅
               if(data.Data.s_ymd != "" && typeof data.Data.s_ymd != "undefined"){
                   $("#reduce_s_ymd").val(data.Data.s_ymd);
                   $("#reduce_s_ymd").mask("1111-11-11") ;
               }

               // 감면기간 종료일 세팅
               if(data.Data.e_ymd != "" && typeof data.Data.e_ymd != "undefined") {
 ...
 //수정(이력) 관련 세팅
	function getCprBtnChk(){
       var params = "&cmbMode=all"
                  ...
                  + "&searchAdjustType="
                  + "&searchSabun="+$("#searchUserId").val(); //사번이 확정되는 [개별] 화면은 대상자별로 조회 가능
...
<div class="wrapper">
    ...
    <div class="outer">
        ...
        <ul>
            ...
            <li class="btn">
                <span id="reduceDiv"><b>감면기간&nbsp;&nbsp;&nbsp;</b><input id="reduce_s_ymd" name="reduce_s_ymd" class="date2" onchange="reduceChg(1)"/> ~ <input id="reduce_e_ymd" name="reduce_e_ymd" class="date2" onchange="reduceChg(2)"/></span>
            	<span id="btn_2">
