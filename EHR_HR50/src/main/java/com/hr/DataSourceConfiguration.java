...
public class DataSourceConfiguration {
    ...
    @Value("${spring.datasource.hr.jndi-name}")
    private String jndiName;

    private final String JNDI_NAME = "jdbc/ehr";
    ...
    public DataSourceConfiguration(@Value("${embedded.tomcat.useYn:#{false}}") boolean bEmbeddedTomcatUseYn) {
        this.bEmbeddedTomcatUseYn = bEmbeddedTomcatUseYn;
        if(bEmbeddedTomcatUseYn) {
            jndiContextUtil = new JndiContextUtil(JNDI_NAME);
            Log.Debug("=========== 내장 톰캣 사용 ===========");
        } else {
            Log.Debug("=========== 외장 WAS 사용 ===========");
        }
    ...
    @Bean(name = "recDataSource")
    public DataSource recDataSource() {
        return getDataSource(jndiName);
    }

    @Primary
    @Bean(name = "phrTransactionManager")
    PlatformTransactionManager phrTransactionManager() {
        DataSourceTransactionManager jndiTransactionManager = new DataSourceTransactionManager();
        jndiTransactionManager.setDataSource(phrDataSource());

        return jndiTransactionManager;
    }

    @Bean
    PlatformTransactionManager proTransactionManager() {
        DataSourceTransactionManager jndiTransactionManager = new DataSourceTransactionManager();
        jndiTransactionManager.setDataSource(proDataSource());

        return jndiTransactionManager;
    }

    @Bean
    PlatformTransactionManager recTransactionManager() {
        DataSourceTransactionManager jndiTransactionManager = new DataSourceTransactionManager();
        jndiTransactionManager.setDataSource(recDataSource());

        return jndiTransactionManager;
    }

    @Bean
    public TransactionInterceptor phrTxAdvice(){
        ...
        interceptor.setTransactionManager(phrTransactionManager());
    ...
    @Bean
    public TransactionInterceptor proTxAdvice(){
        List<RollbackRuleAttribute> rollbackRules = Collections.singletonList(new RollbackRuleAttribute(Exception.class));
        ...
        TransactionInterceptor interceptor = new TransactionInterceptor();
        interceptor.setTransactionManager(proTransactionManager());
        interceptor.setTransactionAttributeSource(attributeSource);
    ...
    @Bean
    public TransactionInterceptor recTxAdvice(){
        List<RollbackRuleAttribute> rollbackRules = Collections.singletonList(new RollbackRuleAttribute(Exception.class));
        ...
        TransactionInterceptor interceptor = new TransactionInterceptor();
        interceptor.setTransactionManager(recTransactionManager());
        interceptor.setTransactionAttributeSource(attributeSource);
    ...
    @Order(1)
    @Bean
    public Advisor phrTxAdvisor(){
        AspectJExpressionPointcut pointcut = new AspectJExpressionPointcut();
        pointcut.setExpression(EXECUTION);
        return new DefaultPointcutAdvisor(pointcut, phrTxAdvice());
    }
    @Order(2)
    @Bean
    public Advisor proTxAdvisor(){
        AspectJExpressionPointcut pointcut = new AspectJExpressionPointcut();
        pointcut.setExpression(EXECUTION);
        return new DefaultPointcutAdvisor(pointcut, proTxAdvice());
    }
    @Order(3)
    @Bean
    public Advisor recTxAdvisor(){
        AspectJExpressionPointcut pointcut = new AspectJExpressionPointcut();
        pointcut.setExpression(EXECUTION);
        return new DefaultPointcutAdvisor(pointcut, recTxAdvice());
    }
