 ...
 * @지급기준 입사일에 첫 회계일까지 근무기간 대비 연차 선부여 케이스
 * @지급여부판단 생성일자가 입사일인 경우 지급.
 * @생성기준 입사일자 ~ 입사일 이후 첫 번째 회계일자 1일 전까지 개근으로 인한 생성.
 * @사용기간 입사일 부터 입사일 이후 첫 번째 회계일자 + 1년 까지 사용 가능
 * @생성개수 입사년 재직일 / 총 일수
 */
public class WtmAnnualLeaveWithinOneYearC implements WtmLeaveCreLogic, WtmAnnualLeaveWithinOneYearCreLogic {
    ...
    public WtmAnnualLeaveWithinOneYearC(LocalDate empDate, LocalDate stdDate, LocalDate finDate) {
    ...
    public boolean isTarget() {
        return this.stdDate.isEqual(this.empDate);
    }
    ...
    public Double getAnnualWithinOneYearCnt() {
        ...
        long empDaysOfYear = this.empDate.until(this.finDate.minusDays(1), ChronoUnit.DAYS);
        // 회계일자 속한 년도의 총 일수 조회.
        long totDaysOfYear = this.finDate.minusYears(1).until(this.finDate.minusDays(1), ChronoUnit.DAYS);
