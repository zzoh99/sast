 ...
 *
 */
@Service("MagamGrpCdMgrService")
public class MagamGrpCdMgrService {
	...
	public int saveGrpCdMgrDetail(Map<String, Object> convertMap) throws Exception {
		...
		if( ((List<?>)convertMap.get("deleteRows")).size() > 0){
			...
			List<Map<String, Object>> deleteRows2 = deleteRows.stream().filter(map -> "B10230".equals(map.get("grcodeCd"))).collect(Collectors.toList());
			if (!deleteRows2.isEmpty()) {
				convertMap.put("deleteRows2", deleteRows2);
				dao.delete("deleteMagamGrpCdMgrDetailBen005", convertMap);
			}
			cnt += dao.delete("deleteMagamGrpCdMgrDetail", convertMap);
		}
