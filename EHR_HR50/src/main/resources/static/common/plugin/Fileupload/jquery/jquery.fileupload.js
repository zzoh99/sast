...
/****************************************************************************************************************************************
 * [20210311] IBUpload 사용으로 인한 Override 
 ****************************************************************************************************************************************/
	$.filedownload = function(uploadType, sendData) {
		try {
			/* 모바일 다운로드 허용 확인 로직 추가(24.09.26) */
	...
	$.filedownloadPdf = function(filePath, fileName) {
		try {
			/* 모바일 다운로드 허용 확인 로직 추가(24.09.26) */
			if (!isDown) {
				// 모바일 접속이며, 모바일 다운로드 미허용인 경우,
			...
			var newForm = $('<form>', {
				'id' : "filedownform",
				'method' : "POST",
				'action' : "/fileuploadJFileUpload.do?cmd=downloadPdf",
				'target' : 'myIFrame'
			});
...
/****************************************************************************************************************************************
 * [20240617] temp 폴더 사용으로 인한 함수 추가
 ****************************************************************************************************************************************/

	$.ibFileDelete = function(uploadType, sendData, callback) {
		var params = function(u, p) {
			var rtn = "&uploadType=" + u;
		...
		$.ajax({
			url: "/fileuploadJFileUpload.do?cmd=ibFileDelete",
			type: "post",
			dataType: "json",
			async: true,
			data: params,
