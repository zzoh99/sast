	            ...
	            onClick : function(evt) {
	            	...
	            	if (evt.isNode || evt.isField || evt.isMember) {
	            		...
	            		if(currKey !=  null  && pkey == currKey) {
	            	        ...
	            	        if(ObjType == "Job"){
								// rk 생성
								var rv = "jobCd="+orgObj.GetNodeDBData(nodeID, "deptcd")
									+"&sdate="+orgObj.GetNodeDBData(nodeID, "sdate");
								var rkData = ajaxCall("/JobSchemeIBOrgSrch.do?cmd=getJobPrtRk", rv, false);

								if ( rkData != null && rkData.DATA != null ){
									const rdData = {
										rk : rkData.DATA.rk
									};
									window.top.showRdLayer('/JobSchemeIBOrgSrch.do?cmd=getJobPrtEncryptRd', rdData, null, "직무기술서");
								}
	            	        } else {
								var rv = "enterCd="+pEnterCd
									+"&sabun="+pSabun;
								var data = ajaxCall("/JobSchemeIBOrgSrch.do?cmd=getEmpCardPrtRk", rv, false);
								if ( data != null && data.DATA != null ){
									const rdData = {
										rk : data.DATA.rk
									};
									window.top.showRdLayer('/JobSchemeIBOrgSrch.do?cmd=getEmpCardPrtEncryptRd', rdData, null, "인사카드");
								}
	            	        }
	            		} else {
	            			currKey = pkey;
...
//		myOrg.saveAsJson();
	...
	showTreeInfo : function(pkey) {
    	...
    	if($("#ObjType") != undefined && ObjType == "Job") {
...
//			orgJson.fields['email'] = email;
			...
			if(ObjType == "Job"){
				var sdate = sheetObj.GetCellValue(i, "sdate");
				orgJson.fields['sdate'] = sdate;
			}
