...
var sheetObj = {
	 ...
	 * IBSheet 초기화
	 */
	init : function(sheetHeight) {
		var initdata = {};
		this.create(sheetHeight);
	    ...
	    initdata.Cols = [
	    	...
	    	{Header:"조직트리", 		Type:"Popup",	Width:232,	SaveName:"deptnm",		Align:"Left", TreeCol:1},
	    	...
	    	{Header:"하위부서세로정렬여부",	Type:"Text",	Width:0,	SaveName:"vrtclOrderYn", 	Hidden:1, Align:"Center", Edit:0}
	    ];
...
//	    doAction("Org");
	...
	create : function(height) {
		...
		if(typeof height != "undefined") {
			createIBSheet2(document.getElementById("sheetDiv"), "mySheet", "100%", height, "kr");
		} else {
			createIBSheet2(document.getElementById("sheetDiv"), "mySheet", "100%", "100%", "kr");
		}
	},
	/**
	 * 시트 데이터 로드
		...
		addEvents(mySheet.id, {
			...
			OnSearchEnd : function(Code, Msg, StCode, StMsg) {
				...
				if (Code != 0) {
					...
					sheetObj.memberList = mySheet.GetEtcData("memberList");
					sheetObj.orgList = mySheet.GetEtcData("orgList");
					
					mySheet.FitColWidth();

					// 시트 조회후, 조직도를 생성하도록 한다.
					
					// searchType 최초 로드시 clear하기 위해 searchType = 1 을 넘김  
					if(!searchType){
						orgObj.sheetToOrg(1);
					}
					searchType = "";
					...
					if( typeof setUseLevel !== "undefined") setUseLevel();
				}
			},
			...
			OnClick : function(Row, Col, Value, CellX, CellY, CellW, CellH) {
				...
				*/
				if(Row >= mySheet.HeaderRows()) {
					loadObj.showBlockUI();
					setTimeout(function() {
						orgObj.sheetToOrg(Row);
					}, 200);
				}
			},
			/**
