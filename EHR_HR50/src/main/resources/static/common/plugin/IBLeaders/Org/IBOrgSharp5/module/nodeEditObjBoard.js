/**
 * nodeEditObjBoard : 화상조직도에 선택된 노드의 정보를 표시하기 위한 창
 */
var nodeEditObj = {
	config: {
	...
	create: function () {
		var
			...
			imgW = 180,		// 정보창 이미지 가로 사이즈
			imgH = 270		// 정보창 이미지 세로 사이즈
		;

		// 정보 영역 필드 정보 지우기
		$("#editPanelArea").html("");

		table = $("<table />", {"style": "width:96%; margin:16px 8px 16px 8px;"});
		tbody = $("<tbody />");
		...
		fields.forEach(function(fld) {
			...
			// 명칭
			td = $("<td />", {"style": "min-width:50px;width:50px;font-weight:500; font-weight:bold;"});
			td.append(fld.name);
			tr.append(td);
			...
			if (fld.type === "picture") {
				...
				// 값을 입력 받을 input 영역
				input = $("<input />", { type: "text", "class": "editNodeCol form-input", id: "txt_" + fld.code, "readonly": readonly, "style": "width:85%;padding:5px;"  });
			}
		...
		fields.forEach(function(fld) {
			...
			if (fld.type === "picture") {
				...
				// 값을 입력 받을 input 영역
				input = $("<input />", { type: "text", "class": "editNodeCol form-input", id: "txt_" + fld.code + "2", "readonly": readonly });
			}
	...
	setEditData: function (NodeID) {
		...
		var
			...
			template
		;
		$("#editPanelArea").children().remove(".member");
		var arrExcept = []; // 동적으로 생성하는 조직원 중 제외할 대상자
		...
		// 조직원 동적 생성 후 보여줘야함.
		var memberList = JSON.parse(orgObj.GetNodeDBData(NodeID, "memberlist"));
		memberList.filter(member => {
			if (arrExcept.indexOf(member["sabun"]) == -1) {
				return member;
			}})
			.forEach(member => {
				var
				...
				var _member = member;
				// 화면에 동적 생성
				table = $("<table />", {"style": "width:96%; margin:16px 8px 16px 8px;", "class":"member"});
				tbody = $("<tbody />");
				for (var i=0; i<fields.length; i++) {
					...
					if (fld.type === "picture") {
						input = $("<img />", {
							...
							src: _member[fld.code]
						});
					} else {
						// 값을 입력 받을 input 영역
						input = $("<input />", {
							type: "text",
							"class": "editNodeCol form-input",
							id: "txt_" + fld.code,
							"readonly": readonly,
							"style": "width:85%;padding:5px;",
							value: _member[fld.code]
						});
					}
