...
$(document).click(function(e){
	if( !$(e.target).hasClass("GMCell") && !$(e.target).hasClass("cp-Div") && !$(e.target).hasClass("evo-pop") ){
		$("#divColorPicker").hide();
    }
});
...
/*
   ColorPicker 생성
*/
function init_ColorPicker(returnColor){
	$('<div></div>', {
        id: "divColorPicker"
       , "class":"cp-Div" 
	...
	$("#divColorPicker").colorpicker().on("change.color", returnColor);
}

/*
 	시트 컬럼 하단에 ColorPicker 이동
 */
function fnSheetDivPopup(sheet, Row, Col){

	var saveName = sheet.ColSaveName(Col);
	//팝업위치
	var itop = $("#DIV_"+sheet.id).offset().top + sheet.RowTop(Row) + sheet.GetRowHeight(Row);
	var ileft = sheet.ColLeft(saveName)-2;
	var iwidth = sheet.GetColWidth(saveName)-2;
	
	/* 박스 크기가 브라우저에 넘치는지 체크 해서 위치 다시 잡기*/
	var ibottom = itop + $("#divColorPicker").outerHeight();
	if( ibottom > $("body").height() ){
		itop = $("#DIV_"+sheet.id).offset().top + sheet.RowTop(Row) - $("#divColorPicker").outerHeight() - 2;
	}

	var iright = ileft + $("#divColorPicker").outerWidth();
	if( iright > $("body").outerWidth() ){
		ileft = ileft - ( iright - $("body").outerWidth() );
	}

	var color = sheet.GetCellValue(Row, Col);
	if( color != "" ){
		$("#divColorPicker").colorpicker("val",color);
	}
	
	$("#divColorPicker").css("top", itop).css("left", ileft).show();
