$(document).ready(function(){
    ...
    var rsa = new RSAKey();
    rsa.setPublic(""
        , "10001");
    console.log(rsa.encrypt("10"));
    setIframeHeight();
});
...
function ajaxGetCall(url, params, async) {
    var obj = new Object();
    //headerReSetTimer();
    $.ajax({
        url: url,
...
function setTopButton() {
    ...
    $("#btnTabPrev").click(function(e) {
        var scr = $("#tabGroup").scrollLeft();
        if(!$("#tabGroup").is(":animated")) {
            $("#tabGroup").animate({
                scrollLeft: (scr - 150)
            }, 200);
        }
    ...
    */
    ...
    if( duplication > -1) {
        if( confirm(getMsgLanguage({"msgid": "msg.confirmChkTab", "defaultMsg":"이미 해당탭이 존재합니다.\n새로고침하시겠습니까?"}))) {
            ...
            tabs.tabs( "option", "active", data.duplication);
        }
        return;
    }
...
function tabRefresh(a) {
    ...
    if(a.url) {
        ...
        $("#surl").val(a.surl);
        //프로세스 맵 추가 2023.10.16 송은선
        submitCall($("#subForm"),tabs.find("iframe:eq("+a.duplication+")").attr("name"),"post",a.url,null,a.procMapLinkBarInfo);
    ...
    //main menu one depth refresh 기능 추가
        ...
        $("#surl").val(a.surl);
        $("#murl").val(a.surl);
        submitCall($("#subForm"), tabs.find("iframe:eq("+a.duplication+")").attr("name"), "post", '/getSubMenuContents.do', null, a.procMapLinkBarInfo);
    }
}
...
function setIframeHeight() {
    ...
    //.sub_wrap의 크기를 전체
    var subWrapH = $('#container').outerHeight(true) - $('#header').outerHeight(true);
    $('.sub_wrap').css('height', subWrapH);
    var conHeight = $(".sub_wrap").outerHeight(true) - $("#tabGroup").height();
...
function subResize() {
    ...
    var tabsHeight = $("#tabs").parent().height() - $("#header").height();
    $("#tabs").css("height", tabsHeight);

    setIframeHeight();

//	var tabMaxWidth = $("#tabs").width() - $("ul.btn_sub_tab").width() - $("div.top_btn_group").width(); // 231017 김기용: 우측 버튼들 없어졌으므로 사이즈 계산에서 삭제
//	var tabMaxWidth = $("#tabs").width();
...
function majorMenuAction(s, p){
    ...
    $("#majorTop, #tabGroup").on("mousewheel DOMMouseScroll", function(event) {
...
// 탭 새로고침 클릭시 이벤트
function tabReload(index) {
    if(isLog) console.log("tabReload()-->");
    var tabSurl = tabs.find("ul.tab_group").find("li:eq("+index+")").attr("surl");
    $("#surl").val(tabSurl);
    var data = ajaxCall("/getDecryptUrl.do",$("#subForm").serialize(),false).map;
    data.surl = tabSurl;
...
function setTabContextMenu(id) {
    ...
    $.contextMenu({
        ...
        callback: function(key, options) {
            //console.log(options);
            if(key == "quit") {
                var this_index = $("div[id='tabs'] ul.tab_group li a.ui-tabs-anchor").index($(options.selector));
                ...
                var index = $("div[id='tabs'] ul.tab_group li a.ui-tabs-anchor").index($(options.selector));
                tabReload(index);
...
function enableTop() {
    if(isLog) console.log("enableTop()-->");
...
function disableTop() {
    if(isLog) console.log("disableTop()-->");
